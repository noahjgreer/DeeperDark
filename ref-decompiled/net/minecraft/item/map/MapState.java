...
public class MapState extends PersistentState {
   ...
   public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(World.CODEC.fieldOf("dimension").forGetter((mapState) -> {
         return mapState.dimension;
      }), Codec.INT.fieldOf("xCenter").forGetter((mapState) -> {
         return mapState.centerX;
      }), Codec.INT.fieldOf("zCenter").forGetter((mapState) -> {
         return mapState.centerZ;
      }), Codec.BYTE.optionalFieldOf("scale", (byte)0).forGetter((mapState) -> {
         return mapState.scale;
      }), Codec.BYTE_BUFFER.fieldOf("colors").forGetter((mapState) -> {
         return ByteBuffer.wrap(mapState.colors);
      }), Codec.BOOL.optionalFieldOf("trackingPosition", true).forGetter((mapState) -> {
         return mapState.showDecorations;
      }), Codec.BOOL.optionalFieldOf("unlimitedTracking", false).forGetter((mapState) -> {
         return mapState.unlimitedTracking;
      }), Codec.BOOL.optionalFieldOf("locked", false).forGetter((mapState) -> {
         return mapState.locked;
      }), MapBannerMarker.CODEC.listOf().optionalFieldOf("banners", List.of()).forGetter((mapState) -> {
         return List.copyOf(mapState.banners.values());
      }), MapFrameMarker.CODEC.listOf().optionalFieldOf("frames", List.of()).forGetter((mapState) -> {
         return List.copyOf(mapState.frames.values());
   ...
   private final List updateTrackers;
   private final Map updateTrackersByPlayer;
   private final Map banners;
   final Map decorations;
   private final Map frames;
   private int decorationCount;

   public static PersistentStateType createStateType(MapIdComponent mapId) {
      return new PersistentStateType(mapId.asString(), () -> {
         throw new IllegalStateException("Should never create an empty map saved data");
      }, CODEC, DataFixTypes.SAVED_DATA_MAP_DATA);
   }

   private MapState(int centerX, int centerZ, byte scale, boolean showDecorations, boolean unlimitedTracking, boolean locked, RegistryKey dimension) {
   ...
   private MapState(RegistryKey dimension, int centerX, int centerZ, byte scale, ByteBuffer colors, boolean showDecorations, boolean unlimitedTracking, boolean locked, List banners, List frames) {
      ...
      Iterator var11 = banners.iterator();

      while(var11.hasNext()) {
         MapBannerMarker mapBannerMarker = (MapBannerMarker)var11.next();
         this.banners.put(mapBannerMarker.getKey(), mapBannerMarker);
         this.addDecoration(mapBannerMarker.getDecorationType(), (WorldAccess)null, mapBannerMarker.getKey(), (double)mapBannerMarker.pos().getX(), (double)mapBannerMarker.pos().getZ(), 180.0, (Text)mapBannerMarker.name().orElse((Object)null));
      }

      var11 = frames.iterator();

      while(var11.hasNext()) {
         MapFrameMarker mapFrameMarker = (MapFrameMarker)var11.next();
   ...
   public static MapState of(double centerX, double centerZ, byte scale, boolean showDecorations, boolean unlimitedTracking, RegistryKey dimension) {
      int i = 128 * (1 << scale);
   ...
   private static Predicate getEqualPredicate(ItemStack stack) {
   ...
   public void update(PlayerEntity player, ItemStack stack) {
      ...
      Predicate predicate = getEqualPredicate(stack);
      ...
      for(int i = 0; i < this.updateTrackers.size(); ++i) {
         ...
         } else if (!stack.isInFrame() && playerEntity.getWorld().getRegistryKey() == this.dimension && this.showDecorations) {
   ...
   public static void addDecorationsNbt(ItemStack stack, BlockPos pos, String id, RegistryEntry decorationType) {
   ...
   private void addDecoration(RegistryEntry type, @Nullable WorldAccess world, String key, double x, double z, double rotation, @Nullable Text text) {
      int i = 1 << this.scale;
   ...
   @Nullable
   private Marker getMarker(RegistryEntry type, @Nullable WorldAccess world, double rotation, float dx, float dz) {
      byte b = offsetToMarkerPosition(dx);
      byte c = offsetToMarkerPosition(dz);
      if (type.matches(MapDecorationTypes.PLAYER)) {
         Pair pair = this.getPlayerMarkerAndRotation(type, world, rotation, dx, dz);
   ...
   @Nullable
   private Pair getPlayerMarkerAndRotation(RegistryEntry type, @Nullable WorldAccess world, double rotation, float dx, float dz) {
      if (isInBounds(dx, dz)) {
         return Pair.of(type, this.getPlayerMarkerRotation(world, rotation));
      } else {
         RegistryEntry registryEntry = this.getPlayerMarker(dx, dz);
   ...
   @Nullable
   private RegistryEntry getPlayerMarker(float dx, float dz) {
      int i = true;
   ...
   private void markDirty(int x, int z) {
      ...
      while(var3.hasNext()) {
         PlayerUpdateTracker playerUpdateTracker = (PlayerUpdateTracker)var3.next();
   ...
   public boolean addBanner(WorldAccess world, BlockPos pos) {
      ...
      if (f >= -63.0 && g >= -63.0 && f <= 63.0 && g <= 63.0) {
         ...
         if (!this.decorationCountNotLessThan(256)) {
            this.banners.put(mapBannerMarker.getKey(), mapBannerMarker);
            this.addDecoration(mapBannerMarker.getDecorationType(), world, mapBannerMarker.getKey(), d, e, 180.0, (Text)mapBannerMarker.name().orElse((Object)null));
            this.markDirty();
            return true;
         }
      }
   ...
   public boolean hasExplorationMapDecoration() {
      ...
      do {
         ...
         mapDecoration = (MapDecoration)var1.next();
      } while(!((MapDecorationType)mapDecoration.type().value()).explorationMapElement());

      return true;
   }

   public void replaceDecorations(List decorations) {
   ...
   static record Marker(RegistryEntry type, byte x, byte y, byte rot) {
      Marker(RegistryEntry registryEntry, byte b, byte c, byte d) {
   ...
   public static record UpdateData(int startX, int startZ, int width, int height, byte[] colors) {
      public static final PacketCodec CODEC = PacketCodec.ofStatic(UpdateData::encode, UpdateData::decode);
      ...
      private static void encode(ByteBuf buf, Optional updateData) {
      ...
      private static Optional decode(ByteBuf buf) {
