...
public class BoatItem extends Item {
   private final EntityType boatEntityType;

   public BoatItem(EntityType boatEntityType, Item.Settings settings) {
      super(settings);
   ...
   public ActionResult use(World world, PlayerEntity user, Hand hand) {
      ...
      if (hitResult.getType() == HitResult.Type.MISS) {
         ...
         double d = 5.0;
         List list = world.getOtherEntities(user, user.getBoundingBox().stretch(vec3d.multiply(5.0)).expand(1.0), EntityPredicates.CAN_HIT);
         if (!list.isEmpty()) {
   ...
   private AbstractBoatEntity createEntity(World world, HitResult hitResult, ItemStack stack, PlayerEntity player) {
      AbstractBoatEntity abstractBoatEntity = (AbstractBoatEntity)this.boatEntityType.create(world, SpawnReason.SPAWN_ITEM_USE);
      if (abstractBoatEntity != null) {
