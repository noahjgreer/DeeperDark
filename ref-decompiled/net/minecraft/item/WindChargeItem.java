...
public class WindChargeItem extends Item implements ProjectileItem {
   ...
   public ActionResult use(World world, PlayerEntity user, Hand hand) {
      ...
      if (world instanceof ServerWorld serverWorld) {
         ProjectileEntity.spawnWithVelocity((world2, shooter, stack) -> {
            return new WindChargeEntity(user, world, user.getPos().getX(), user.getEyePos().getY(), user.getPos().getZ());
         }, serverWorld, itemStack, user, 0.0F, POWER, 1.0F);
      }

      world.playSound((Entity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)SoundEvents.ENTITY_WIND_CHARGE_THROW, SoundCategory.NEUTRAL, 0.5F, 0.4F / (world.getRandom().nextFloat() * 0.4F + 0.8F));
