...
public class GlassBottleItem extends Item {
   ...
   public ActionResult use(World world, PlayerEntity user, Hand hand) {
      ...
      if (!list.isEmpty()) {
         ...
         world.playSound((Entity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)SoundEvents.ITEM_BOTTLE_FILL_DRAGONBREATH, SoundCategory.NEUTRAL, 1.0F, 1.0F);
         ...
         if (blockHitResult.getType() == HitResult.Type.MISS) {
            ...
            if (blockHitResult.getType() == HitResult.Type.BLOCK) {
               ...
               if (world.getFluidState(blockPos).isIn(FluidTags.WATER)) {
                  world.playSound(user, user.getX(), user.getY(), user.getZ(), (SoundEvent)SoundEvents.ITEM_BOTTLE_FILL, SoundCategory.NEUTRAL, 1.0F, 1.0F);
