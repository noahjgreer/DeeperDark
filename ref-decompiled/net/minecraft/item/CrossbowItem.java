...
public class CrossbowItem extends RangedWeaponItem {
   ...
   private static boolean loadProjectiles(LivingEntity shooter, ItemStack crossbow) {
      List list = load(crossbow, shooter.getProjectileType(crossbow), shooter);
      if (!list.isEmpty()) {
   ...
   protected void shoot(LivingEntity shooter, ProjectileEntity projectile, int index, float speed, float divergence, float yaw, @Nullable LivingEntity target) {
      ...
      shooter.getWorld().playSound((Entity)null, shooter.getX(), shooter.getY(), shooter.getZ(), (SoundEvent)SoundEvents.ITEM_CROSSBOW_SHOOT, shooter.getSoundCategory(), 1.0F, h);
   }
      ...
      if (!world.isClient) {
         ...
         if (f >= 0.2F && !this.charged) {
            ...
            loadingSounds.start().ifPresent((sound) -> {
               world.playSound((Entity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)((SoundEvent)sound.value()), SoundCategory.PLAYERS, 0.5F, 1.0F);
            });
         }
         ...
         if (f >= 1.0F && !isCharged(stack) && loadProjectiles(user, stack)) {
            loadingSounds.end().ifPresent((sound) -> {
               world.playSound((Entity)null, user.getX(), user.getY(), user.getZ(), (SoundEvent)((SoundEvent)sound.value()), user.getSoundCategory(), 1.0F, 1.0F / (world.getRandom().nextFloat() * 0.5F + 1.0F) + 0.2F);
            });
         }
      }
   ...
   public static record LoadingSounds(Optional start, Optional mid, Optional end) {
      public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
         return instance.group(SoundEvent.ENTRY_CODEC.optionalFieldOf("start").forGetter(LoadingSounds::start), SoundEvent.ENTRY_CODEC.optionalFieldOf("mid").forGetter(LoadingSounds::mid), SoundEvent.ENTRY_CODEC.optionalFieldOf("end").forGetter(LoadingSounds::end)).apply(instance, LoadingSounds::new);
      });

      public LoadingSounds(Optional optional, Optional optional2, Optional optional3) {
         this.start = optional;
         this.mid = optional2;
         this.end = optional3;
      }
   ...
   public static enum ChargeType implements StringIdentifiable {
      ...
      public static final Codec CODEC = StringIdentifiable.createCodec(ChargeType::values);
      ...
      // $FF: synthetic method
      private static ChargeType[] method_65354() {
