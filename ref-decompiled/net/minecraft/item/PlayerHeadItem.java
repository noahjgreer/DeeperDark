...
public class PlayerHeadItem extends VerticallyAttachableBlockItem {
   ...
   public Text getName(ItemStack stack) {
      ...
      return (Text)(profileComponent != null && profileComponent.name().isPresent() ? Text.translatable(this.translationKey + ".named", profileComponent.name().get()) : super.getName(stack));
   }

   public void postProcessComponents(ItemStack stack) {
      ProfileComponent profileComponent = (ProfileComponent)stack.get(DataComponentTypes.PROFILE);
      if (profileComponent != null && !profileComponent.isCompleted()) {
         profileComponent.getFuture().thenAcceptAsync((profile) -> {
