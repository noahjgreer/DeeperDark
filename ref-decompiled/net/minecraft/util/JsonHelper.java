...
public class JsonHelper {
   ...
   public static RegistryEntry asItem(JsonElement element, String name) {
      if (element.isJsonPrimitive()) {
         String string = element.getAsString();
         return (RegistryEntry)Registries.ITEM.getEntry(Identifier.of(string)).orElseThrow(() -> {
   ...
   public static RegistryEntry getItem(JsonObject object, String key) {
      if (object.has(key)) {
   ...
   @Nullable
   @Contract("_,_,!null->!null;_,_,null->_")
   public static RegistryEntry getItem(JsonObject object, String key, @Nullable RegistryEntry defaultValue) {
   ...
   @Nullable
   public static Object deserializeNullable(Gson gson, Reader reader, TypeToken typeToken) {
      try {
         JsonReader jsonReader = new JsonReader(reader);
         jsonReader.setStrictness(Strictness.STRICT);
   ...
   public static Object deserialize(Gson gson, Reader reader, TypeToken typeToken) {
      Object object = deserializeNullable(gson, reader, typeToken);
      if (object == null) {
         throw new JsonParseException("JSON data was null or empty");
      } else {
         return object;
   ...
   public static Object deserialize(Gson gson, String content, Class type) {
      return deserialize(gson, (Reader)(new StringReader(content)), (Class)type);
   }

   public static JsonObject deserialize(String content) {
      return deserialize((Reader)(new StringReader(content)));
   }

   public static JsonObject deserialize(Reader reader) {
      return (JsonObject)deserialize(GSON, reader, JsonObject.class);
   }
   ...
   public static JsonArray deserializeArray(Reader reader) {
      return (JsonArray)deserialize(GSON, reader, JsonArray.class);
   }
   ...
   public static boolean isTooLarge(JsonElement json, int maxLength) {
      try {
         Streams.write(json, new JsonWriter(Streams.writerForAppendable(new CharacterCounter(maxLength))));
         return false;
