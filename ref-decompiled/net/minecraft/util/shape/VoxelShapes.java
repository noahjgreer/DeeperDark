...
public final class VoxelShapes {
   ...
   public static VoxelShape combine(VoxelShape one, VoxelShape two, BooleanBiFunction function) {
      if (function.apply(false, false)) {
         ...
         if (one.isEmpty()) {
            ...
            PairList pairList3 = createListPair((pairList.size() - 1) * (pairList2.size() - 1), one.getPointPositions(Direction.Axis.Z), two.getPointPositions(Direction.Axis.Z), bl, bl2);
            BitSetVoxelSet bitSetVoxelSet = BitSetVoxelSet.combine(one.voxels, two.voxels, pairList, pairList2, pairList3, function);
            return (VoxelShape)(pairList instanceof FractionalPairList && pairList2 instanceof FractionalPairList && pairList3 instanceof FractionalPairList ? new SimpleVoxelShape(bitSetVoxelSet) : new ArrayVoxelShape(bitSetVoxelSet, pairList.getPairs(), pairList2.getPairs(), pairList3.getPairs()));
         }
      }
   }
      ...
      if (predicate.apply(false, false)) {
         ...
         if (!bl && !bl2) {
            if (shape1 == shape2) {
               ...
               PairList pairList3 = createListPair((pairList.size() - 1) * (pairList2.size() - 1), shape1.getPointPositions(Direction.Axis.Z), shape2.getPointPositions(Direction.Axis.Z), bl3, bl4);
   ...
   public static double calculateMaxOffset(Direction.Axis axis, Box box, Iterable shapes, double maxDist) {
      ...
      for(Iterator var5 = shapes.iterator(); var5.hasNext(); maxDist = voxelShape.calculateMaxDistance(axis, box, maxDist)) {
         voxelShape = (VoxelShape)var5.next();
   ...
   public static boolean isSideCovered(VoxelShape shape, VoxelShape neighbor, Direction direction) {
      if (shape == fullCube() && neighbor == fullCube()) {
         ...
         return DoubleMath.fuzzyEquals(voxelShape.getMax(axis), 1.0, 1.0E-7) && DoubleMath.fuzzyEquals(voxelShape2.getMin(axis), 0.0, 1.0E-7) && !matchesAnywhere(new SlicedVoxelShape(voxelShape, axis, voxelShape.voxels.getSize(axis) - 1), new SlicedVoxelShape(voxelShape2, axis, 0), booleanBiFunction);
      }
   }
   ...
   public static boolean adjacentSidesCoverSquare(VoxelShape one, VoxelShape two, Direction direction) {
      if (one != fullCube() && two != fullCube()) {
         ...
         return !matchesAnywhere(fullCube(), combine(new SlicedVoxelShape(voxelShape, axis, voxelShape.voxels.getSize(axis) - 1), new SlicedVoxelShape(voxelShape2, axis, 0), BooleanBiFunction.OR), BooleanBiFunction.ONLY_FIRST);
      } else {
         return true;
      }
   }
   ...
   public static VoxelShape transform(VoxelShape shape, DirectionTransformation transformation, Vec3d anchor) {
      if (transformation == DirectionTransformation.IDENTITY) {
         ...
         if (shape instanceof SimpleVoxelShape && BLOCK_CENTER.equals(anchor)) {
            ...
            return new ArrayVoxelShape(voxelSet, transform(doubleList, bl4, anchor.getComponentAlongAxis(axis), anchor.x), transform(doubleList2, bl5, anchor.getComponentAlongAxis(axis2), anchor.y), transform(doubleList3, bl6, anchor.getComponentAlongAxis(axis3), anchor.z));
         }
      }
   }
   ...
   static DoubleList transform(DoubleList pointPositions, boolean flip, double component, double anchor) {
      if (!flip && component == anchor) {
         ...
         for(int k = flip ? i - 1 : 0; k >= 0 && k < i; k += j) {
            doubleList.add(anchor + (double)j * (pointPositions.getDouble(k) - component));
         }
   ...
   public static Map createHorizontalAxisShapeMap(VoxelShape shape) {
      return createHorizontalAxisShapeMap(shape, BLOCK_CENTER);
   }

   public static Map createHorizontalAxisShapeMap(VoxelShape shape, Vec3d anchor) {
      return Maps.newEnumMap(Map.of(Direction.Axis.Z, shape, Direction.Axis.X, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90), anchor)));
   }

   public static Map createAxisShapeMap(VoxelShape shape) {
      return createAxisShapeMap(shape, BLOCK_CENTER);
   }

   public static Map createAxisShapeMap(VoxelShape shape, Vec3d anchor) {
      return Maps.newEnumMap(Map.of(Direction.Axis.Z, shape, Direction.Axis.X, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90), anchor), Direction.Axis.Y, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R90, AxisRotation.R0), anchor)));
   }

   public static Map createHorizontalFacingShapeMap(VoxelShape shape) {
      return createHorizontalFacingShapeMap(shape, BLOCK_CENTER);
   }

   public static Map createHorizontalFacingShapeMap(VoxelShape shape, Vec3d anchor) {
      return Maps.newEnumMap(Map.of(Direction.NORTH, shape, Direction.EAST, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90), anchor), Direction.SOUTH, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R180), anchor), Direction.WEST, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R270), anchor)));
   }

   public static Map createFacingShapeMap(VoxelShape shape) {
      return createFacingShapeMap(shape, BLOCK_CENTER);
   }

   public static Map createFacingShapeMap(VoxelShape shape, Vec3d anchor) {
      return Maps.newEnumMap(Map.of(Direction.NORTH, shape, Direction.EAST, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90), anchor), Direction.SOUTH, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R180), anchor), Direction.WEST, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R270), anchor), Direction.UP, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R270, AxisRotation.R0), anchor), Direction.DOWN, transform(shape, DirectionTransformation.fromRotations(AxisRotation.R90, AxisRotation.R0), anchor)));
   }

   public static Map createBlockFaceHorizontalFacingShapeMap(VoxelShape shape) {
      return Map.of(BlockFace.WALL, createHorizontalFacingShapeMap(shape), BlockFace.FLOOR, createHorizontalFacingShapeMap(transform(shape, DirectionTransformation.fromRotations(AxisRotation.R270, AxisRotation.R0))), BlockFace.CEILING, createHorizontalFacingShapeMap(transform(shape, DirectionTransformation.fromRotations(AxisRotation.R90, AxisRotation.R180))));
   }
