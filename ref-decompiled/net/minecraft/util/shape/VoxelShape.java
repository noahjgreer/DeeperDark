...
public abstract class VoxelShape {
   ...
   public VoxelShape asCuboid() {
      return this.isEmpty() ? VoxelShapes.empty() : VoxelShapes.cuboid(this.getMin(Direction.Axis.X), this.getMin(Direction.Axis.Y), this.getMin(Direction.Axis.Z), this.getMax(Direction.Axis.X), this.getMax(Direction.Axis.Y), this.getMax(Direction.Axis.Z));
   }
   ...
   public VoxelShape offset(Vec3i vec) {
      return this.offset((double)vec.getX(), (double)vec.getY(), (double)vec.getZ());
   }
   ...
   public BlockHitResult raycast(Vec3d start, Vec3d end, BlockPos pos) {
      if (this.isEmpty()) {
         ...
         if (vec3d.lengthSquared() < 1.0E-7) {
            ...
            return this.voxels.inBoundsAndContains(this.getCoordIndex(Direction.Axis.X, vec3d2.x - (double)pos.getX()), this.getCoordIndex(Direction.Axis.Y, vec3d2.y - (double)pos.getY()), this.getCoordIndex(Direction.Axis.Z, vec3d2.z - (double)pos.getZ())) ? new BlockHitResult(vec3d2, Direction.getFacing(vec3d.x, vec3d.y, vec3d.z).getOpposite(), pos, true) : Box.raycast(this.getBoundingBoxes(), start, end, pos);
         }
      }
   }
         ...
         if (maxDist > 0.0) {
               ...
               for(q = k; q < l; ++q) {
                  for(r = m; r < n; ++r) {
                     if (this.voxels.inBoundsAndContains(axisCycleDirection, p, q, r)) {
                        f = this.getPointPosition(axis, p) - d;
                        if (f >= -1.0E-7) {
            ...
            for(p = i - 1; p >= 0; --p) {
               for(q = k; q < l; ++q) {
                  for(r = m; r < n; ++r) {
                     if (this.voxels.inBoundsAndContains(axisCycleDirection, p, q, r)) {
                        f = this.getPointPosition(axis, p + 1) - e;
                        if (f <= 1.0E-7) {
