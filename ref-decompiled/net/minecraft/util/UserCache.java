...
public class UserCache {
   ...
   private static Optional findProfileByName(GameProfileRepository repository, String name) {
      if (!StringHelper.isValidPlayerName(name)) {
         ...
         Optional optional = repository.findProfileByName(name);
         return optional.isEmpty() ? getOfflinePlayerProfile(name) : optional;
      }
   }

   private static Optional getOfflinePlayerProfile(String name) {
   ...
   public CompletableFuture findByNameAsync(String username) {
      if (this.executor == null) {
         ...
         if (completableFuture != null) {
            ...
            }, Util.getMainWorkerExecutor().named("getProfile")).whenCompleteAsync((profile, throwable) -> {
