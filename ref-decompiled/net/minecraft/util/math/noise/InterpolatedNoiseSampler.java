...
public class InterpolatedNoiseSampler implements DensityFunction.Base {
   private static final Codec SCALE_AND_FACTOR_RANGE = Codec.doubleRange(0.001, 1000.0);
   private static final MapCodec MAP_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(SCALE_AND_FACTOR_RANGE.fieldOf("xz_scale").forGetter((interpolatedNoiseSampler) -> {
         return interpolatedNoiseSampler.xzScale;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("y_scale").forGetter((interpolatedNoiseSampler) -> {
         return interpolatedNoiseSampler.yScale;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("xz_factor").forGetter((interpolatedNoiseSampler) -> {
         return interpolatedNoiseSampler.xzFactor;
      }), SCALE_AND_FACTOR_RANGE.fieldOf("y_factor").forGetter((interpolatedNoiseSampler) -> {
         return interpolatedNoiseSampler.yFactor;
      }), Codec.doubleRange(1.0, 8.0).fieldOf("smear_scale_multiplier").forGetter((interpolatedNoiseSampler) -> {
         return interpolatedNoiseSampler.smearScaleMultiplier;
      })).apply(instance, InterpolatedNoiseSampler::createBase3dNoiseFunction);
   });
   public static final CodecHolder CODEC;
   ...
   public double sample(DensityFunction.NoisePos pos) {
      ...
      for(int r = 0; r < 16; ++r) {
         ...
         if (!bl2) {
            perlinNoiseSampler2 = this.lowerInterpolatedNoise.getOctave(r);
         ...
         if (!bl3) {
            perlinNoiseSampler2 = this.upperInterpolatedNoise.getOctave(r);
            if (perlinNoiseSampler2 != null) {
   ...
   static {
      CODEC = CodecHolder.of(MAP_CODEC);
   }
}
