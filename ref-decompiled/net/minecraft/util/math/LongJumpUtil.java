...
public final class LongJumpUtil {
   public static Optional getJumpingVelocity(MobEntity entity, Vec3d jumpTarget, float maxVelocity, int angle, boolean requireClearPath) {
      ...
      float f = (float)angle * 3.1415927F / 180.0F;
      ...
      if (p < 0.0) {
         ...
         if (q > (double)maxVelocity) {
            ...
            return Optional.of((new Vec3d(r * o, s, r * n)).multiply(0.949999988079071));
         }
      }
   }
   ...
   private static boolean isPathClear(MobEntity entity, EntityDimensions dimensions, Vec3d lastPos, Vec3d nextPos) {
      ...
      for(int j = 0; j < i; ++j) {
         vec3d3 = j == i - 1 ? nextPos : vec3d3.add(vec3d2.multiply(d * 0.8999999761581421));
