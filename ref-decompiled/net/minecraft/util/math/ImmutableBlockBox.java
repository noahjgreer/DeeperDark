...
public record ImmutableBlockBox(BlockPos min, BlockPos max) implements Iterable {
   ...
   public ImmutableBlockBox(final BlockPos first, final BlockPos second) {
   ...
   public ImmutableBlockBox expand(Direction direction, int offset) {
      if (offset == 0) {
         ...
         return direction.getDirection() == Direction.AxisDirection.POSITIVE ? of(this.min, BlockPos.max(this.min, this.max.offset(direction, offset))) : of(BlockPos.min(this.min.offset(direction, offset), this.max), this.max);
      }
   }
