...
public class BlockBox {
    ...
    public static final Codec<BlockBox> CODEC = Codec.INT_STREAM.comapFlatMap(values -> Util.decodeFixedLengthArray(values, 6).map(array -> new BlockBox(array[0], array[1], array[2], array[3], array[4], array[5])), box -> IntStream.of(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ)).stable();
    public static final PacketCodec<ByteBuf, BlockBox> PACKET_CODEC = PacketCodec.tuple(BlockPos.PACKET_CODEC, box -> new BlockPos(box.minX, box.minY, box.minZ), BlockPos.PACKET_CODEC, box -> new BlockPos(box.maxX, box.maxY, box.maxZ), (min, max) -> new BlockBox(min.getX(), min.getY(), min.getZ(), max.getX(), max.getY(), max.getZ()));
    private int minX;
    private int minY;
    private int minZ;
    private int maxX;
    ...
    public BlockBox(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        ...
        if (maxX < minX || maxY < minY || maxZ < minZ) {
            Util.logErrorOrPause("Invalid bounding box data, inverted bounds for: " + String.valueOf(this));
    ...
    public static BlockBox createEncompassing(BlockBox box1, BlockBox box2) {
        return new BlockBox(Math.min(box1.minX, box2.minX), Math.min(box1.minY, box2.minY), Math.min(box1.minZ, box2.minZ), Math.max(box1.maxX, box2.maxX), Math.max(box1.maxY, box2.maxY), Math.max(box1.maxZ, box2.maxZ));
    }
