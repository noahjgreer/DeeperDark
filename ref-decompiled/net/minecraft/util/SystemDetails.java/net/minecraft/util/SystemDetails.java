...
public class SystemDetails {
    ...
    public SystemDetails() {
        ...
        this.addSection("JVM Flags", () -> SystemDetails.collectJvmArguments(flag -> flag.startsWith("-X")));
        this.addSection("Debug Flags", () -> SystemDetails.collectJvmArguments(flag -> flag.startsWith("-DMC_DEBUG_")));
    }
    ...
    private static String collectJvmArguments(Predicate<String> predicate) {
        ...
        List<String> list2 = list.stream().filter(predicate).toList();
    ...
    private void addProcessorGroup(CentralProcessor centralProcessor) {
        ...
        this.addSection("Processor Vendor", () -> ((CentralProcessor.ProcessorIdentifier)processorIdentifier).getVendor());
    ...
    private void addStorageSection(String name, Supplier<@Nullable String> pathSupplier) {
        ...
        try {
            ...
            FileStore fileStore = Files.getFileStore(Path.of(string2, new String[0]));
            this.addSection(string, String.format(Locale.ROOT, "available: %.2f, total: %.2f", Float.valueOf(SystemDetails.toMebibytes(fileStore.getUsableSpace())), Float.valueOf(SystemDetails.toMebibytes(fileStore.getTotalSpace()))));
        }
