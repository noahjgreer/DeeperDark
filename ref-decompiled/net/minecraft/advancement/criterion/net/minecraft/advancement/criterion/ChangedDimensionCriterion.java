...
extends AbstractCriterion<Conditions> {
    ...
    {
        public static final Codec<Conditions> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)EntityPredicate.LOOT_CONTEXT_PREDICATE_CODEC.optionalFieldOf("player").forGetter(Conditions::player), (App)RegistryKey.createCodec(RegistryKeys.WORLD).optionalFieldOf("from").forGetter(Conditions::from), (App)RegistryKey.createCodec(RegistryKeys.WORLD).optionalFieldOf("to").forGetter(Conditions::to)).apply((Applicative)instance, Conditions::new));
