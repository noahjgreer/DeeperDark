...
extends AbstractCriterion<Conditions> {
    ...
    {
        public static final Codec<Conditions> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)EntityPredicate.LOOT_CONTEXT_PREDICATE_CODEC.optionalFieldOf("player").forGetter(Conditions::player)).apply((Applicative)instance, Conditions::new));
        ...
        public static AdvancementCriterion<Conditions> createLocation(RegistryEntryLookup<Block> blockRegistry, RegistryEntryLookup<Item> itemRegistry, Block steppingOn, Item boots) {
            return Conditions.createLocation(EntityPredicate.Builder.create().equipment(EntityEquipmentPredicate.Builder.create().feet(ItemPredicate.Builder.create().items(itemRegistry, boots))).steppingOn(LocationPredicate.Builder.create().block(BlockPredicate.Builder.create().blocks(blockRegistry, steppingOn))));
        }
    }
}
