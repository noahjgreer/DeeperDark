...
extends AbstractCriterion<Conditions> {
    ...
    {
        public static final Codec<Conditions> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)EntityPredicate.LOOT_CONTEXT_PREDICATE_CODEC.optionalFieldOf("player").forGetter(Conditions::player), (App)Registries.BLOCK.getEntryCodec().optionalFieldOf("block").forGetter(Conditions::block), (App)StatePredicate.CODEC.optionalFieldOf("state").forGetter(Conditions::state)).apply((Applicative)instance, Conditions::new)).validate(Conditions::validate);
        ...
        public static AdvancementCriterion<Conditions> block(Block block) {
            return Criteria.ENTER_BLOCK.create(new Conditions(Optional.empty(), Optional.of(block.getRegistryEntry()), Optional.empty()));
        }
