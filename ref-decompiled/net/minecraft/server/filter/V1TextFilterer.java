...
public class V1TextFilterer extends AbstractTextFilterer {
   ...
   private V1TextFilterer(URL url, AbstractTextFilterer.MessageEncoder messageEncoder, AbstractTextFilterer.HashIgnorer hashIgnorer, ExecutorService threadPool, ConfidentialClientApplication application, ClientCredentialParameters credentialParameters, Set fullyFilteredEvents, int readTimeout) {
   ...
   public static AbstractTextFilterer load(String response) {
      ...
      jsonArray.forEach((json) -> {
         set.add(JsonHelper.asString(json, "filteredEvent"));
      });
      ...
      try {
         confidentialClientApplication = ((ConfidentialClientApplication.Builder)((ConfidentialClientApplication.Builder)ConfidentialClientApplication.builder(string4, iClientCertificate).sendX5c(true).executorService(executorService)).authority(String.format(Locale.ROOT, "https://login.microsoftonline.com/%s/", string5))).build();
   ...
   protected FilteredMessage filter(String raw, AbstractTextFilterer.HashIgnorer hashIgnorer, JsonObject response) {
      JsonObject jsonObject = JsonHelper.getObject(response, "result", (JsonObject)null);
      if (jsonObject == null) {
         ...
         if (!bl) {
            ...
            do {
               ...
               string = JsonHelper.getString(jsonObject2, "id", "");
            } while(!this.fullyFilteredEvents.contains(string));
