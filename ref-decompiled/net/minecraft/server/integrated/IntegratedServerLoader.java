...
public class IntegratedServerLoader {
   ...
   public void createAndStart(String levelName, LevelInfo levelInfo, GeneratorOptions dynamicRegistryManager, Function dimensionsRegistrySupplier, Screen screen) {
      ...
      if (session != null) {
         ...
         try {
            ...
            SaveLoader saveLoader = (SaveLoader)this.load(dataPacks, (context) -> {
               DimensionOptionsRegistryHolder.DimensionsConfig dimensionsConfig = ((DimensionOptionsRegistryHolder)dimensionsRegistrySupplier.apply(context.worldGenRegistryManager())).toConfig(context.dimensionsRegistryManager().getOrThrow(RegistryKeys.DIMENSION));
               return new SaveLoading.LoadContext(new LevelProperties(levelInfo, dynamicRegistryManager, dimensionsConfig.specialWorldProperty(), dimensionsConfig.getLifecycle()), dimensionsConfig.toDynamicRegistryManager());
            }, SaveLoader::new);
   ...
   public void startNewWorld(LevelStorage.Session session, DataPackContents dataPackContents, CombinedDynamicRegistries dynamicRegistryManager, SaveProperties saveProperties) {
      ResourcePackManager resourcePackManager = VanillaDataPackProvider.createManager(session);
   ...
   public SaveLoader load(Dynamic levelProperties, boolean safeMode, ResourcePackManager dataPackManager) throws Exception {
      ...
      return (SaveLoader)this.load(dataPacks, (context) -> {
         Registry registry = context.dimensionsRegistryManager().getOrThrow(RegistryKeys.DIMENSION);
         ParsedSaveProperties parsedSaveProperties = LevelStorage.parseSaveProperties(levelProperties, context.dataConfiguration(), registry, context.worldGenRegistryManager());
   ...
   public Pair loadForRecreation(LevelStorage.Session session) throws Exception {
      ...
      return (Pair)this.load(dataPacks, (context) -> {
         ...
         record CurrentSettings(LevelInfo levelInfo, GeneratorOptions options, Registry existingDimensionRegistry) {
            ...
            CurrentSettings(LevelInfo levelInfo, GeneratorOptions generatorOptions, Registry registry) {
         ...
         return new SaveLoading.LoadContext(new CurrentSettings(parsedSaveProperties.properties().getLevelInfo(), parsedSaveProperties.properties().getGeneratorOptions(), parsedSaveProperties.dimensions().dimensions()), context.dimensionsRegistryManager());
      }, (resourceManager, dataPackContents, combinedRegistryManager, currentSettings) -> {
         resourceManager.close();
         InitialWorldOptions initialWorldOptions = new InitialWorldOptions(WorldCreator.Mode.SURVIVAL, Set.of(), (RegistryKey)null);
         return Pair.of(currentSettings.levelInfo, new GeneratorOptionsHolder(currentSettings.options, new DimensionOptionsRegistryHolder(currentSettings.existingDimensionRegistry), combinedRegistryManager, dataPackContents, currentSettings.levelInfo.getDataConfiguration(), initialWorldOptions));
      });
   }
   ...
   private Object load(SaveLoading.DataPacks dataPacks, SaveLoading.LoadContextSupplier loadContextSupplier, SaveLoading.SaveApplierFactory saveApplierFactory) throws Exception {
      ...
      CompletableFuture completableFuture = SaveLoading.load(serverConfig, loadContextSupplier, saveApplierFactory, Util.getMainWorkerExecutor(), this.client);
   ...
   private void start(LevelStorage.Session session, Runnable onCancel) {
      ...
      try {
         ...
         crashReportSection.add("World folder", (Object)session.getDirectoryName());
   ...
   private void start(LevelStorage.Session session, LevelSummary summary, Dynamic levelProperties, Runnable onCancel) {
      if (!summary.isVersionAvailable()) {
         session.tryClose();
         this.client.setScreen(new NoticeScreen(onCancel, Text.translatable("selectWorld.incompatible.title").withColor(-65536), Text.translatable("selectWorld.incompatible.description", summary.getVersion())));
      } else {
         ...
         if (conversionWarning.promptsBackup()) {
            ...
            Text text = Text.translatable(string2, summary.getVersion(), SharedConstants.getGameVersion().name());
   ...
   private void start(LevelStorage.Session session, Dynamic levelProperties, boolean safeMode, Runnable onCancel) {
      ...
      try {
         ...
         Iterator var7 = saveLoader.combinedDynamicRegistries().getCombinedRegistryManager().getOrThrow(RegistryKeys.DIMENSION).iterator();

         while(var7.hasNext()) {
            DimensionOptions dimensionOptions = (DimensionOptions)var7.next();
   ...
   private void checkBackupAndStart(LevelStorage.Session session, SaveLoader saveLoader, ResourcePackManager dataPackManager, Runnable onCancel) {
      ...
      if (!bl && !bl2) {
         ...
         this.showBackupPromptScreen(session, bl, () -> {
            this.start(session, saveLoader, dataPackManager, onCancel);
         }, () -> {
            saveLoader.close();
            session.tryClose();
   ...
   private void start(LevelStorage.Session session, SaveLoader saveLoader, ResourcePackManager dataPackManager, Runnable onCancel) {
      ...
      this.applyWorldPack(serverResourcePackLoader, session).thenApply((v) -> {
         return true;
   ...
   private CompletableFuture applyWorldPack(ServerResourcePackLoader loader, LevelStorage.Session session) {
      Path path = session.getDirectory(WorldSavePath.RESOURCES_ZIP);
      if (Files.exists(path, new LinkOption[0]) && !Files.isDirectory(path, new LinkOption[0])) {
         loader.initWorldPack();
         CompletableFuture completableFuture = loader.getPackLoadFuture(WORLD_PACK_ID);
