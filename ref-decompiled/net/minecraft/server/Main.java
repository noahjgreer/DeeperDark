...
public class Main {
   ...
   public static void main(String[] args) {
      ...
      OptionSpec optionSpec7 = optionParser.accepts("recreateRegionFiles");
      OptionSpec optionSpec8 = optionParser.accepts("safeMode", "Loads level with vanilla datapack only");
      OptionSpec optionSpec9 = optionParser.accepts("help").forHelp();
      ...
      try {
         ...
         if (session.levelDatExists()) {
            ...
            try {
               ...
               try {
                  ...
                  LOGGER.error("Failed to load world data from {} and {}. World files may be corrupted. Shutting down.", levelSave.getLevelDatPath(), levelSave.getLevelDatOldPath());
                  return;
               }
         ...
         try {
            ...
            saveLoader = (SaveLoader)Util.waitAndApply((applyExecutor) -> {
               return SaveLoading.load(serverConfig, (context) -> {
                  ...
                  if (dynamic2 != null) {
                     ...
                     if (optionSet.has(optionSpec3)) {
                        ...
                        levelInfo = new LevelInfo(serverPropertiesHandler.levelName, serverPropertiesHandler.gameMode, serverPropertiesHandler.hardcore, serverPropertiesHandler.difficulty, false, new GameRules(context.dataConfiguration().enabledFeatures()), context.dataConfiguration());
                     ...
                     return new SaveLoading.LoadContext(new LevelProperties(levelInfo, generatorOptions, dimensionsConfig.specialWorldProperty(), lifecycle), dimensionsConfig.toDynamicRegistryManager());
                  }
         ...
         if (optionSet.has(optionSpec5) || bl2) {
            forceUpgradeWorld(session, saveProperties, Schemas.getFixer(), optionSet.has(optionSpec6), () -> {
               return true;
            }, immutable, bl2);
         }

         session.backupLevelDataFile(immutable, saveProperties);
         final MinecraftDedicatedServer minecraftDedicatedServer = (MinecraftDedicatedServer)MinecraftServer.startServer((threadx) -> {
            MinecraftDedicatedServer minecraftDedicatedServer = new MinecraftDedicatedServer(threadx, session, resourcePackManager, saveLoader, serverPropertiesLoader, Schemas.getFixer(), apiServices, WorldGenerationProgressLogger::create);
   ...
   private static void forceUpgradeWorld(LevelStorage.Session session, SaveProperties saveProperties, DataFixer dataFixer, boolean eraseCache, BooleanSupplier continueCheck, DynamicRegistryManager registries, boolean recreateRegionFiles) {
      ...
      WorldUpdater worldUpdater = new WorldUpdater(session, dataFixer, saveProperties, registries, eraseCache, recreateRegionFiles);
