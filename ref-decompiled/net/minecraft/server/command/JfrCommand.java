...
public class JfrCommand {
   ...
   private static final DynamicCommandExceptionType JFR_DUMP_FAILED_EXCEPTION = new DynamicCommandExceptionType((message) -> {
      return Text.stringifiedTranslatable("commands.jfr.dump.failed", message);
   });
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("jfr").requires(CommandManager.requirePermissionLevel(4))).then(CommandManager.literal("start").executes((context) -> {
   ...
   private static int executeStop(ServerCommandSource source) throws CommandSyntaxException {
      try {
         ...
         Text text = Text.literal(path.toString()).formatted(Formatting.UNDERLINE).styled((style) -> {
            return style.withClickEvent(new ClickEvent.CopyToClipboard(path2.toString())).withHoverEvent(new HoverEvent.ShowText(Text.translatable("chat.copy.click")));
         });
         source.sendFeedback(() -> {
            return Text.translatable("commands.jfr.stopped", text);
         }, false);
         return 1;
