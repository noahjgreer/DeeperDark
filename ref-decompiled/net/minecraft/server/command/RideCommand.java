...
public class RideCommand {
   private static final DynamicCommandExceptionType NOT_RIDING_EXCEPTION = new DynamicCommandExceptionType((entity) -> {
      return Text.stringifiedTranslatable("commands.ride.not_riding", entity);
   });
   private static final Dynamic2CommandExceptionType ALREADY_RIDING_EXCEPTION = new Dynamic2CommandExceptionType((rider, vehicle) -> {
      return Text.stringifiedTranslatable("commands.ride.already_riding", rider, vehicle);
   });
   private static final Dynamic2CommandExceptionType GENERIC_FAILURE_EXCEPTION = new Dynamic2CommandExceptionType((rider, vehicle) -> {
      return Text.stringifiedTranslatable("commands.ride.mount.failure.generic", rider, vehicle);
   });
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("ride").requires(CommandManager.requirePermissionLevel(2))).then(((RequiredArgumentBuilder)CommandManager.argument("target", EntityArgumentType.entity()).then(CommandManager.literal("mount").then(CommandManager.argument("vehicle", EntityArgumentType.entity()).executes((context) -> {
   ...
   private static int executeDismount(ServerCommandSource source, Entity rider) throws CommandSyntaxException {
      ...
      if (entity == null) {
         ...
         rider.stopRiding();
         source.sendFeedback(() -> {
            return Text.translatable("commands.ride.dismount.success", rider.getDisplayName(), entity.getDisplayName());
         }, true);
         return 1;
      }
   }
