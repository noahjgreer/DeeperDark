...
public class SpreadPlayersCommand {
   ...
   private static final Dynamic4CommandExceptionType FAILED_TEAMS_EXCEPTION = new Dynamic4CommandExceptionType((pilesCount, x, z, maxSpreadDistance) -> {
      return Text.stringifiedTranslatable("commands.spreadplayers.failed.teams", pilesCount, x, z, maxSpreadDistance);
   });
   private static final Dynamic4CommandExceptionType FAILED_ENTITIES_EXCEPTION = new Dynamic4CommandExceptionType((pilesCount, x, z, maxSpreadDistance) -> {
      return Text.stringifiedTranslatable("commands.spreadplayers.failed.entities", pilesCount, x, z, maxSpreadDistance);
   });
   private static final Dynamic2CommandExceptionType INVALID_HEIGHT_EXCEPTION = new Dynamic2CommandExceptionType((maxY, worldBottomY) -> {
      return Text.stringifiedTranslatable("commands.spreadplayers.failed.invalid.height", maxY, worldBottomY);
   });

   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("spreadplayers").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.argument("center", Vec2ArgumentType.vec2()).then(CommandManager.argument("spreadDistance", FloatArgumentType.floatArg(0.0F)).then(((RequiredArgumentBuilder)CommandManager.argument("maxRange", FloatArgumentType.floatArg(1.0F)).then(CommandManager.argument("respectTeams", BoolArgumentType.bool()).then(CommandManager.argument("targets", EntityArgumentType.entities()).executes((context) -> {
         return execute((ServerCommandSource)context.getSource(), Vec2ArgumentType.getVec2(context, "center"), FloatArgumentType.getFloat(context, "spreadDistance"), FloatArgumentType.getFloat(context, "maxRange"), ((ServerCommandSource)context.getSource()).getWorld().getTopYInclusive() + 1, BoolArgumentType.getBool(context, "respectTeams"), EntityArgumentType.getEntities(context, "targets"));
   ...
   private static int execute(ServerCommandSource source, Vec2f center, float spreadDistance, float maxRange, int maxY, boolean respectTeams, Collection targets) throws CommandSyntaxException {
      ...
      if (maxY < i) {
         ...
         Pile[] piles = makePiles(random, respectTeams ? getPileCountRespectingTeams(targets) : targets.size(), d, e, f, g);
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.spreadplayers.success." + (respectTeams ? "teams" : "entities"), piles.length, center.x, center.y, String.format(Locale.ROOT, "%.2f", h));
         }, true);
         return piles.length;
      }
   }
   ...
   private static double teleport(Collection entities, ServerWorld world, Pile[] piles, int maxY, boolean respectTeams) {
      double d = 0.0;
      int i = 0;
      ...
      for(Iterator var9 = entities.iterator(); var9.hasNext(); d += e) {
         ...
         entity.teleport(world, (double)MathHelper.floor(pile.x) + 0.5, (double)pile.getY(world, maxY), (double)MathHelper.floor(pile.z) + 0.5, Set.of(), entity.getYaw(), entity.getPitch(), true);
         e = Double.MAX_VALUE;
         Pile[] var14 = piles;
