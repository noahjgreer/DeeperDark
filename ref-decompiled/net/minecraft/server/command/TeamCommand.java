...
public class TeamCommand {
   ...
   public static void register(CommandDispatcher dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("team").requires(CommandManager.requirePermissionLevel(2))).then(((LiteralArgumentBuilder)CommandManager.literal("list").executes((context) -> {
         ...
         return executeAdd((ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "team"), TextArgumentType.parseTextArgument(context, "displayName"));
         ...
         return executeJoin((ServerCommandSource)context.getSource(), TeamArgumentType.getTeam(context, "team"), Collections.singleton(((ServerCommandSource)context.getSource()).getEntityOrThrow()));
         ...
         return executeModifyDisplayName((ServerCommandSource)context.getSource(), TeamArgumentType.getTeam(context, "team"), TextArgumentType.parseTextArgument(context, "displayName"));
      ...
      })))).then(CommandManager.literal("prefix").then(CommandManager.argument("prefix", TextArgumentType.text(registryAccess)).executes((context) -> {
         return executeModifyPrefix((ServerCommandSource)context.getSource(), TeamArgumentType.getTeam(context, "team"), TextArgumentType.parseTextArgument(context, "prefix"));
      })))).then(CommandManager.literal("suffix").then(CommandManager.argument("suffix", TextArgumentType.text(registryAccess)).executes((context) -> {
         return executeModifySuffix((ServerCommandSource)context.getSource(), TeamArgumentType.getTeam(context, "team"), TextArgumentType.parseTextArgument(context, "suffix"));
      }))))));
   }

   private static Text getMemberName(Collection members) {
   ...
   private static int executeLeave(ServerCommandSource source, Collection members) {
      ...
      if (members.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.leave.success.single", getMemberName(members));
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.leave.success.multiple", members.size());
         }, true);
      }
   ...
   private static int executeJoin(ServerCommandSource source, Team team, Collection members) {
      ...
      if (members.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.join.success.single", getMemberName(members), team.getFormattedName());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.join.success.multiple", members.size(), team.getFormattedName());
         }, true);
      }
   ...
   private static int executeModifyNametagVisibility(ServerCommandSource source, Team team, AbstractTeam.VisibilityRule visibility) throws CommandSyntaxException {
      if (team.getNameTagVisibilityRule() == visibility) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.option.nametagVisibility.success", team.getFormattedName(), visibility.getDisplayName());
         }, true);
         return 0;
      }
   }
   ...
   private static int executeModifyDeathMessageVisibility(ServerCommandSource source, Team team, AbstractTeam.VisibilityRule visibility) throws CommandSyntaxException {
      if (team.getDeathMessageVisibilityRule() == visibility) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.option.deathMessageVisibility.success", team.getFormattedName(), visibility.getDisplayName());
         }, true);
         return 0;
      }
   }
   ...
   private static int executeModifySeeFriendlyInvisibles(ServerCommandSource source, Team team, boolean allowed) throws CommandSyntaxException {
      if (team.shouldShowFriendlyInvisibles() == allowed) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.option.seeFriendlyInvisibles." + (allowed ? "enabled" : "disabled"), team.getFormattedName());
         }, true);
         return 0;
      }
   }
   ...
   private static int executeModifyFriendlyFire(ServerCommandSource source, Team team, boolean allowed) throws CommandSyntaxException {
      if (team.isFriendlyFireAllowed() == allowed) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.option.friendlyfire." + (allowed ? "enabled" : "disabled"), team.getFormattedName());
         }, true);
         return 0;
      }
   }
   ...
   private static int executeModifyColor(ServerCommandSource source, Team team, Formatting color) throws CommandSyntaxException {
      if (team.getColor() == color) {
         ...
         team.setColor(color);
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.option.color.success", team.getFormattedName(), color.getName());
         }, true);
         return 0;
      }
   }
   ...
   private static int executeEmpty(ServerCommandSource source, Team team) throws CommandSyntaxException {
      ...
      if (collection.isEmpty()) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.empty.success", collection.size(), team.getFormattedName());
         }, true);
   ...
   private static int executeAdd(ServerCommandSource source, String team, Text displayName) throws CommandSyntaxException {
      ...
      if (scoreboard.getTeam(team) != null) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.add.success", team2.getFormattedName());
         }, true);
         return scoreboard.getTeams().size();
      }
   }
   ...
   private static int executeListMembers(ServerCommandSource source, Team team) {
      ...
      if (collection.isEmpty()) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.list.members.success", team.getFormattedName(), collection.size(), Texts.joinOrdered(collection));
         }, false);
      }
   ...
   private static int executeListTeams(ServerCommandSource source) {
      ...
      if (collection.isEmpty()) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.team.list.teams.success", collection.size(), Texts.join(collection, Team::getFormattedName));
         }, false);
      }
   ...
   private static int executeModifyPrefix(ServerCommandSource source, Team team, Text prefix) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.team.option.prefix.success", prefix);
      }, false);
      return 1;
   }
   ...
   private static int executeModifySuffix(ServerCommandSource source, Team team, Text suffix) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.team.option.suffix.success", suffix);
      }, false);
      return 1;
   }
}
