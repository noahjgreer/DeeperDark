...
public class WorldBorderCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("worldborder").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("add").then(((RequiredArgumentBuilder)CommandManager.argument("distance", DoubleArgumentType.doubleArg(-5.9999968E7, 5.9999968E7)).executes((context) -> {
   ...
   private static int executeBuffer(ServerCommandSource source, float distance) throws CommandSyntaxException {
      ...
      if (worldBorder.getSafeZone() == (double)distance) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.worldborder.damage.buffer.success", String.format(Locale.ROOT, "%.2f", distance));
         }, true);
         return (int)distance;
      }
   }
   ...
   private static int executeDamage(ServerCommandSource source, float damagePerBlock) throws CommandSyntaxException {
      ...
      if (worldBorder.getDamagePerBlock() == (double)damagePerBlock) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.worldborder.damage.amount.success", String.format(Locale.ROOT, "%.2f", damagePerBlock));
         }, true);
   ...
   private static int executeWarningTime(ServerCommandSource source, int time) throws CommandSyntaxException {
      ...
      if (worldBorder.getWarningTime() == time) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.worldborder.warning.time.success", time);
         }, true);
         return time;
      }
   }
   ...
   private static int executeWarningDistance(ServerCommandSource source, int distance) throws CommandSyntaxException {
      ...
      if (worldBorder.getWarningBlocks() == distance) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.worldborder.warning.distance.success", distance);
         }, true);
         return distance;
      }
   }
   ...
   private static int executeGet(ServerCommandSource source) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.worldborder.get", String.format(Locale.ROOT, "%.0f", d));
      }, false);
   ...
   private static int executeCenter(ServerCommandSource source, Vec2f pos) throws CommandSyntaxException {
      ...
      if (worldBorder.getCenterX() == (double)pos.x && worldBorder.getCenterZ() == (double)pos.y) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.worldborder.center.success", String.format(Locale.ROOT, "%.2f", pos.x), String.format(Locale.ROOT, "%.2f", pos.y));
         }, true);
         return 0;
      } else {
   ...
   private static int executeSet(ServerCommandSource source, double distance, long time) throws CommandSyntaxException {
      ...
      if (d == distance) {
         ...
         if (time > 0L) {
            ...
            if (distance > d) {
               source.sendFeedback(() -> {
                  return Text.translatable("commands.worldborder.set.grow", String.format(Locale.ROOT, "%.1f", distance), Long.toString(time / 1000L));
               }, true);
            } else {
               source.sendFeedback(() -> {
                  return Text.translatable("commands.worldborder.set.shrink", String.format(Locale.ROOT, "%.1f", distance), Long.toString(time / 1000L));
               }, true);
            }
         } else {
            worldBorder.setSize(distance);
            source.sendFeedback(() -> {
               return Text.translatable("commands.worldborder.set.immediate", String.format(Locale.ROOT, "%.1f", distance));
            }, true);
         }
