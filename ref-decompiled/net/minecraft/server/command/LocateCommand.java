...
public class LocateCommand {
   ...
   public static void register(CommandDispatcher dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("locate").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("structure").then(CommandManager.argument("structure", RegistryPredicateArgumentType.registryPredicate(RegistryKeys.STRUCTURE)).executes((context) -> {
   ...
   private static Optional getStructureListForPredicate(RegistryPredicateArgumentType.RegistryPredicate predicate, Registry structureRegistry) {
      ...
      Function var10001 = (key) -> {
         return structureRegistry.getOptional(key).map((entry) -> {
            return RegistryEntryList.of(entry);
         });
      };
      Objects.requireNonNull(structureRegistry);
      return (Optional)var10000.map(var10001, structureRegistry::getOptional);
   }
   ...
   private static int executeLocateStructure(ServerCommandSource source, RegistryPredicateArgumentType.RegistryPredicate predicate) throws CommandSyntaxException {
      Registry registry = source.getWorld().getRegistryManager().getOrThrow(RegistryKeys.STRUCTURE);
      RegistryEntryList registryEntryList = (RegistryEntryList)getStructureListForPredicate(predicate, registry).orElseThrow(() -> {
      ...
      Pair pair = serverWorld.getChunkManager().getChunkGenerator().locateStructure(serverWorld, registryEntryList, blockPos, 100, false);
      stopwatch.stop();
      if (pair == null) {
   ...
   private static int executeLocateBiome(ServerCommandSource source, RegistryEntryPredicateArgumentType.EntryPredicate predicate) throws CommandSyntaxException {
      BlockPos blockPos = BlockPos.ofFloored(source.getPosition());
      Stopwatch stopwatch = Stopwatch.createStarted(Util.TICKER);
      Pair pair = source.getWorld().locateBiome(predicate, blockPos, 6400, 32, 64);
      stopwatch.stop();
      if (pair == null) {
   ...
   private static int executeLocatePoi(ServerCommandSource source, RegistryEntryPredicateArgumentType.EntryPredicate predicate) throws CommandSyntaxException {
      ...
      ServerWorld serverWorld = source.getWorld();
      Stopwatch stopwatch = Stopwatch.createStarted(Util.TICKER);
      Optional optional = serverWorld.getPointOfInterestStorage().getNearestTypeAndPosition(predicate, blockPos, 256, PointOfInterestStorage.OccupationStatus.ANY);
      stopwatch.stop();
      if (optional.isEmpty()) {
   ...
   public static int sendCoordinates(ServerCommandSource source, RegistryEntryPredicateArgumentType.EntryPredicate predicate, BlockPos currentPos, Pair result, String successMessage, boolean includeY, Duration timeTaken) {
      String string = (String)predicate.getEntry().map((entry) -> {
         ...
         return var10000 + " (" + ((RegistryEntry)result.getSecond()).getIdAsString() + ")";
      });
   ...
   public static int sendCoordinates(ServerCommandSource source, RegistryPredicateArgumentType.RegistryPredicate structure, BlockPos currentPos, Pair result, String successMessage, boolean includeY, Duration timeTaken) {
      String string = (String)structure.getKey().map((key) -> {
         ...
         String var10000 = String.valueOf(key.id());
         return "#" + var10000 + " (" + ((RegistryEntry)result.getSecond()).getIdAsString() + ")";
      });
      return sendCoordinates(source, currentPos, result, successMessage, includeY, string, timeTaken);
   }
   ...
   private static int sendCoordinates(ServerCommandSource source, BlockPos currentPos, Pair result, String successMessage, boolean includeY, String entryString, Duration timeTaken) {
      ...
      Text text = Texts.bracketed(Text.translatable("chat.coordinates", blockPos.getX(), string, blockPos.getZ())).styled((style) -> {
         return style.withColor(Formatting.GREEN).withClickEvent(new ClickEvent.SuggestCommand("/tp @s " + blockPos.getX() + " " + string + " " + blockPos.getZ())).withHoverEvent(new HoverEvent.ShowText(Text.translatable("chat.coordinates.tooltip")));
      });
      source.sendFeedback(() -> {
         return Text.translatable(successMessage, entryString, text, i);
      }, false);
