...
public class ScoreboardCommand {
    private static final SimpleCommandExceptionType OBJECTIVES_ADD_DUPLICATE_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.scoreboard.objectives.add.duplicate"));
    private static final SimpleCommandExceptionType OBJECTIVES_DISPLAY_ALREADY_EMPTY_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.scoreboard.objectives.display.alreadyEmpty"));
    private static final SimpleCommandExceptionType OBJECTIVES_DISPLAY_ALREADY_SET_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.scoreboard.objectives.display.alreadySet"));
    private static final SimpleCommandExceptionType PLAYERS_ENABLE_FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.scoreboard.players.enable.failed"));
    private static final SimpleCommandExceptionType PLAYERS_ENABLE_INVALID_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.scoreboard.players.enable.invalid"));
    ...
    private static ArgumentBuilder<ServerCommandSource, ?> makeNumberFormatArguments(CommandRegistryAccess registryAccess, ArgumentBuilder<ServerCommandSource, ?> argumentBuilder, NumberFormatCommandExecutor executor) {
        return argumentBuilder.then(CommandManager.literal("blank").executes(context -> executor.run((CommandContext<ServerCommandSource>)context, BlankNumberFormat.INSTANCE))).then(CommandManager.literal("fixed").then(CommandManager.argument("contents", TextArgumentType.text(registryAccess)).executes(context -> {
            Text text = TextArgumentType.parseTextArgument((CommandContext<ServerCommandSource>)context, "contents");
            return executor.run((CommandContext<ServerCommandSource>)context, new FixedNumberFormat(text));
            ...
            Style style = StyleArgumentType.getStyle((CommandContext<ServerCommandSource>)context, "style");
            return executor.run((CommandContext<ServerCommandSource>)context, new StyledNumberFormat(style));
        }))).executes(context -> executor.run((CommandContext<ServerCommandSource>)context, null));
    }
    ...
    private static CompletableFuture<Suggestions> suggestDisabled(ServerCommandSource source, Collection<ScoreHolder> targets, SuggestionsBuilder builder) {
        ...
        for (ScoreboardObjective scoreboardObjective : scoreboard.getObjectives()) {
            ...
            for (ScoreHolder scoreHolder : targets) {
                ...
                if (readableScoreboardScore != null && !readableScoreboardScore.isLocked()) continue;
                bl = true;
                break;
            }
            if (!bl) continue;
    ...
    private static int executeGet(ServerCommandSource source, ScoreHolder scoreHolder, ScoreboardObjective objective) throws CommandSyntaxException {
        ...
        if (readableScoreboardScore == null) {
            throw PLAYERS_GET_NULL_EXCEPTION.create((Object)objective.getName(), (Object)scoreHolder.getStyledDisplayName());
        }
        source.sendFeedback(() -> Text.translatable("commands.scoreboard.players.get.success", scoreHolder.getStyledDisplayName(), readableScoreboardScore.getScore(), objective.toHoverableText()), false);
    ...
    private static int executeEnable(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective) throws CommandSyntaxException {
        ...
        for (ScoreHolder scoreHolder : targets) {
            ...
            if (!scoreAccess.isLocked()) continue;
            scoreAccess.unlock();
            ++i;
        }
        if (i == 0) {
    ...
    private static int executeAdd(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            int j = i;
            source.sendFeedback(() -> Text.translatable("commands.scoreboard.players.add.success.single", score, objective.toHoverableText(), ScoreboardCommand.getNextDisplayName(targets), j), true);
        } else {
    ...
    private static int executeRemove(ServerCommandSource source, Collection<ScoreHolder> targets, ScoreboardObjective objective, int score) {
        ...
        if (targets.size() == 1) {
            int j = i;
            source.sendFeedback(() -> Text.translatable("commands.scoreboard.players.remove.success.single", score, objective.toHoverableText(), ScoreboardCommand.getNextDisplayName(targets), j), true);
        } else {
