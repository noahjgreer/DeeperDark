...
public class RaidCommand {
   public static void register(CommandDispatcher dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("raid").requires(CommandManager.requirePermissionLevel(3))).then(CommandManager.literal("start").then(CommandManager.argument("omenlvl", IntegerArgumentType.integer(0)).executes((context) -> {
      ...
      }))).then(CommandManager.literal("sound").then(CommandManager.argument("type", TextArgumentType.text(registryAccess)).executes((context) -> {
         return executeSound((ServerCommandSource)context.getSource(), TextArgumentType.parseTextArgument(context, "type"));
   ...
   private static int executeSetOmen(ServerCommandSource source, int level) throws CommandSyntaxException {
      ...
      if (raid != null) {
         ...
         if (level > i) {
            ...
            source.sendFeedback(() -> {
               return Text.literal("Changed village's raid omen level from " + j + " to " + level);
            }, false);
         }
      } else {
   ...
   private static int executeSpawnLeader(ServerCommandSource source) {
      ...
      RaiderEntity raiderEntity = (RaiderEntity)EntityType.PILLAGER.create(source.getWorld(), SpawnReason.COMMAND);
      if (raiderEntity == null) {
         ...
         raiderEntity.initialize(source.getWorld(), source.getWorld().getLocalDifficulty(BlockPos.ofFloored(source.getPosition())), SpawnReason.COMMAND, (EntityData)null);
   ...
   private static int executeSound(ServerCommandSource source, @Nullable Text type) {
      if (type != null && type.getString().equals("local")) {
         ...
         serverWorld.playSound((Entity)null, vec3d.x, vec3d.y, vec3d.z, SoundEvents.EVENT_RAID_HORN, SoundCategory.NEUTRAL, 2.0F, 1.0F, serverWorld.random.nextLong());
      }
   ...
   private static int executeCheck(ServerCommandSource source) throws CommandSyntaxException {
      ...
      if (raid != null) {
         ...
         source.sendFeedback(() -> {
            return Text.literal(stringBuilder2.toString());
         }, false);
         return 1;
      } else {
