...
public class RaidCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("raid").requires(CommandManager.requirePermissionLevel(CommandManager.ADMINS_CHECK))).then(CommandManager.literal("start").then(CommandManager.argument("omenlvl", IntegerArgumentType.integer((int)0)).executes(context -> RaidCommand.executeStart((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger((CommandContext)context, (String)"omenlvl")))))).then(CommandManager.literal("stop").executes(context -> RaidCommand.executeStop((ServerCommandSource)context.getSource())))).then(CommandManager.literal("check").executes(context -> RaidCommand.executeCheck((ServerCommandSource)context.getSource())))).then(CommandManager.literal("sound").then(CommandManager.argument("type", TextArgumentType.text(registryAccess)).executes(context -> RaidCommand.executeSound((ServerCommandSource)context.getSource(), TextArgumentType.parseTextArgument((CommandContext<ServerCommandSource>)context, "type")))))).then(CommandManager.literal("spawnleader").executes(context -> RaidCommand.executeSpawnLeader((ServerCommandSource)context.getSource())))).then(CommandManager.literal("setomen").then(CommandManager.argument("level", IntegerArgumentType.integer((int)0)).executes(context -> RaidCommand.executeSetOmen((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger((CommandContext)context, (String)"level")))))).then(CommandManager.literal("glow").executes(context -> RaidCommand.executeGlow((ServerCommandSource)context.getSource()))));
    }
    ...
    private static int executeStart(ServerCommandSource source, int level) throws CommandSyntaxException {
        ...
        if (serverPlayerEntity.getEntityWorld().hasRaidAt(blockPos)) {
            source.sendError(Text.literal("Raid already started close by"));
            return -1;
        }
        RaidManager raidManager = serverPlayerEntity.getEntityWorld().getRaidManager();
    ...
    private static int executeStop(ServerCommandSource source) throws CommandSyntaxException {
        ServerPlayerEntity serverPlayerEntity = source.getPlayerOrThrow();
        BlockPos blockPos = serverPlayerEntity.getBlockPos();
        Raid raid = serverPlayerEntity.getEntityWorld().getRaidAt(blockPos);
        if (raid != null) {
            raid.invalidate();
    ...
    private static @Nullable Raid getRaid(ServerPlayerEntity player) {
        return player.getEntityWorld().getRaidAt(player.getBlockPos());
    }
}
