...
public class TickCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("tick").requires(CommandManager.requirePermissionLevel(3))).then(CommandManager.literal("query").executes((context) -> {
         return executeQuery((ServerCommandSource)context.getSource());
      }))).then(CommandManager.literal("rate").then(CommandManager.argument("rate", FloatArgumentType.floatArg(1.0F, 10000.0F)).suggests((context, suggestionsBuilder) -> {
         return CommandSource.suggestMatching(new String[]{DEFAULT_TICK_RATE_STRING}, suggestionsBuilder);
      }).executes((context) -> {
         return executeRate((ServerCommandSource)context.getSource(), FloatArgumentType.getFloat(context, "rate"));
      })))).then(((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("step").executes((context) -> {
         return executeStep((ServerCommandSource)context.getSource(), 1);
      })).then(CommandManager.literal("stop").executes((context) -> {
         return executeStopStep((ServerCommandSource)context.getSource());
      }))).then(CommandManager.argument("time", TimeArgumentType.time(1)).suggests((context, suggestionsBuilder) -> {
         return CommandSource.suggestMatching(new String[]{"1t", "1s"}, suggestionsBuilder);
      }).executes((context) -> {
         return executeStep((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "time"));
      })))).then(((LiteralArgumentBuilder)CommandManager.literal("sprint").then(CommandManager.literal("stop").executes((context) -> {
         return executeStopSprint((ServerCommandSource)context.getSource());
      }))).then(CommandManager.argument("time", TimeArgumentType.time(1)).suggests((context, suggestionsBuilder) -> {
         return CommandSource.suggestMatching(new String[]{"60s", "1d", "3d"}, suggestionsBuilder);
      }).executes((context) -> {
         return executeSprint((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "time"));
      })))).then(CommandManager.literal("unfreeze").executes((context) -> {
         return executeFreeze((ServerCommandSource)context.getSource(), false);
      }))).then(CommandManager.literal("freeze").executes((context) -> {
         return executeFreeze((ServerCommandSource)context.getSource(), true);
      })));
   }
   ...
   private static int executeRate(ServerCommandSource source, float rate) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.tick.rate.success", string);
      }, true);
      return (int)rate;
   }
   ...
   private static int executeQuery(ServerCommandSource source) {
      ...
      if (serverTickManager.isSprinting()) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.tick.query.rate.sprinting", string2, string);
         }, false);
      } else {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.tick.query.rate.running", string2, string, string3);
         }, false);
      }
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.tick.query.percentiles", string4, string5, string6, ls.length);
      }, false);
      return (int)f;
   }
   ...
   private static int executeStep(ServerCommandSource source, int steps) {
      ...
      if (bl) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.tick.step.success", steps);
         }, true);
      } else {
