...
public class TitleCommand {
   public static void register(CommandDispatcher dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("title").requires(CommandManager.requirePermissionLevel(2))).then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("targets", EntityArgumentType.players()).then(CommandManager.literal("clear").executes((context) -> {
      ...
      }))).then(CommandManager.literal("title").then(CommandManager.argument("title", TextArgumentType.text(registryAccess)).executes((context) -> {
      ...
      })))).then(CommandManager.literal("subtitle").then(CommandManager.argument("title", TextArgumentType.text(registryAccess)).executes((context) -> {
      ...
      })))).then(CommandManager.literal("actionbar").then(CommandManager.argument("title", TextArgumentType.text(registryAccess)).executes((context) -> {
   ...
   private static int executeClear(ServerCommandSource source, Collection targets) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.cleared.single", ((ServerPlayerEntity)targets.iterator().next()).getDisplayName());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.cleared.multiple", targets.size());
         }, true);
      }
   ...
   private static int executeReset(ServerCommandSource source, Collection targets) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.reset.single", ((ServerPlayerEntity)targets.iterator().next()).getDisplayName());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.reset.multiple", targets.size());
         }, true);
      }
   ...
   private static int executeTitle(ServerCommandSource source, Collection targets, Text title, String titleType, Function constructor) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.show." + titleType + ".single", ((ServerPlayerEntity)targets.iterator().next()).getDisplayName());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.show." + titleType + ".multiple", targets.size());
         }, true);
      }
   ...
   private static int executeTimes(ServerCommandSource source, Collection targets, int fadeIn, int stay, int fadeOut) {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.times.single", ((ServerPlayerEntity)targets.iterator().next()).getDisplayName());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.title.times.multiple", targets.size());
         }, true);
      }
