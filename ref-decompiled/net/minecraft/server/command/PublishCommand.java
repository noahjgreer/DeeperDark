...
public class PublishCommand {
   ...
   private static final DynamicCommandExceptionType ALREADY_PUBLISHED_EXCEPTION = new DynamicCommandExceptionType((port) -> {
      return Text.stringifiedTranslatable("commands.publish.alreadyPublished", port);
   });

   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("publish").requires(CommandManager.requirePermissionLevel(4))).executes((context) -> {
         return execute((ServerCommandSource)context.getSource(), NetworkUtils.findLocalPort(), false, (GameMode)null);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("allowCommands", BoolArgumentType.bool()).executes((context) -> {
         return execute((ServerCommandSource)context.getSource(), NetworkUtils.findLocalPort(), BoolArgumentType.getBool(context, "allowCommands"), (GameMode)null);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("gamemode", GameModeArgumentType.gameMode()).executes((context) -> {
         return execute((ServerCommandSource)context.getSource(), NetworkUtils.findLocalPort(), BoolArgumentType.getBool(context, "allowCommands"), GameModeArgumentType.getGameMode(context, "gamemode"));
