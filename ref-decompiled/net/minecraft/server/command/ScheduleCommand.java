...
public class ScheduleCommand {
   ...
   private static final DynamicCommandExceptionType CLEARED_FAILURE_EXCEPTION = new DynamicCommandExceptionType((eventName) -> {
      return Text.stringifiedTranslatable("commands.schedule.cleared.failure", eventName);
   });
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("schedule").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("function").then(CommandManager.argument("function", CommandFunctionArgumentType.commandFunction()).suggests(FunctionCommand.SUGGESTION_PROVIDER).then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("time", TimeArgumentType.time()).executes((context) -> {
   ...
   private static int execute(ServerCommandSource source, Pair function, int time, boolean replace) throws CommandSyntaxException {
      if (time == 0) {
         ...
         Timer timer = source.getServer().getSaveProperties().getMainWorldProperties().getScheduledEvents();
         ...
         if (optional.isPresent()) {
            ...
            source.sendFeedback(() -> {
               return Text.translatable("commands.schedule.created.function", Text.of(identifier), time, l);
            }, true);
         } else {
            ...
            source.sendFeedback(() -> {
               return Text.translatable("commands.schedule.created.tag", Text.of(identifier), time, l);
            }, true);
         }
   ...
   private static int clearEvent(ServerCommandSource source, String eventName) throws CommandSyntaxException {
      ...
      if (i == 0) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.schedule.cleared.success", i, eventName);
         }, true);
         return i;
      }
   }
