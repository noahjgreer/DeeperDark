...
public class ChaseCommand {
   ...
   private static int stop(ServerCommandSource source) {
      if (client != null) {
         client.stop();
         source.sendFeedback(() -> {
            return Text.literal("You have now stopped chasing");
         }, false);
         client = null;
      }
      ...
      if (server != null) {
         server.stop();
         source.sendFeedback(() -> {
            return Text.literal("You are no longer being chased");
         }, false);
         server = null;
      }
   ...
   private static int startServer(ServerCommandSource source, String ip, int port) {
      if (isRunning(source)) {
         ...
         try {
            server.start();
            source.sendFeedback(() -> {
               return Text.literal("Chase server is now running on port " + port + ". Clients can follow you using /chase follow <ip> <port>");
            }, false);
   ...
   private static int startClient(ServerCommandSource source, String ip, int port) {
      if (isRunning(source)) {
         ...
         client.start();
         source.sendFeedback(() -> {
            return Text.literal("You are now chasing " + ip + ":" + port + ". If that server does '/chase lead' then you will automatically go to the same position. Use '/chase stop' to stop chasing.");
         }, false);
         return 0;
      }
   }
