...
public class TriggerCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)CommandManager.literal("trigger").then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("objective", ScoreboardObjectiveArgumentType.scoreboardObjective()).suggests((context, builder) -> {
         ...
         return executeSimple((ServerCommandSource)context.getSource(), ((ServerCommandSource)context.getSource()).getPlayerOrThrow(), ScoreboardObjectiveArgumentType.getObjective(context, "objective"));
      })).then(CommandManager.literal("add").then(CommandManager.argument("value", IntegerArgumentType.integer()).executes((context) -> {
         return executeAdd((ServerCommandSource)context.getSource(), ((ServerCommandSource)context.getSource()).getPlayerOrThrow(), ScoreboardObjectiveArgumentType.getObjective(context, "objective"), IntegerArgumentType.getInteger(context, "value"));
      })))).then(CommandManager.literal("set").then(CommandManager.argument("value", IntegerArgumentType.integer()).executes((context) -> {
         return executeSet((ServerCommandSource)context.getSource(), ((ServerCommandSource)context.getSource()).getPlayerOrThrow(), ScoreboardObjectiveArgumentType.getObjective(context, "objective"), IntegerArgumentType.getInteger(context, "value"));
      })))));
   }
   ...
   public static CompletableFuture suggestObjectives(ServerCommandSource source, SuggestionsBuilder builder) {
      ...
      if (scoreHolder != null) {
         ...
         while(var5.hasNext()) {
            ScoreboardObjective scoreboardObjective = (ScoreboardObjective)var5.next();
   ...
   private static int executeAdd(ServerCommandSource source, ServerPlayerEntity player, ScoreboardObjective objective, int amount) throws CommandSyntaxException {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.trigger.add.success", objective.toHoverableText(), amount);
      }, true);
      return i;
   }
   ...
   private static int executeSet(ServerCommandSource source, ServerPlayerEntity player, ScoreboardObjective objective, int value) throws CommandSyntaxException {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.trigger.set.success", objective.toHoverableText(), value);
      }, true);
      return value;
   }
   ...
   private static int executeSimple(ServerCommandSource source, ServerPlayerEntity player, ScoreboardObjective objective) throws CommandSyntaxException {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.trigger.simple.success", objective.toHoverableText());
      }, true);
      return i;
   }
