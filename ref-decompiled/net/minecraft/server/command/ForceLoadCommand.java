...
public class ForceLoadCommand {
   ...
   private static final Dynamic2CommandExceptionType TOO_BIG_EXCEPTION = new Dynamic2CommandExceptionType((maxCount, count) -> {
      return Text.stringifiedTranslatable("commands.forceload.toobig", maxCount, count);
   });
   private static final Dynamic2CommandExceptionType QUERY_FAILURE_EXCEPTION = new Dynamic2CommandExceptionType((chunkPos, registryKey) -> {
      return Text.stringifiedTranslatable("commands.forceload.query.failure", chunkPos, registryKey);
   });
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("forceload").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("add").then(((RequiredArgumentBuilder)CommandManager.argument("from", ColumnPosArgumentType.columnPos()).executes((context) -> {
   ...
   private static int executeQuery(ServerCommandSource source, ColumnPos pos) throws CommandSyntaxException {
      ...
      if (bl) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.forceload.query.success", Text.of(chunkPos), Text.of(registryKey.getValue()));
         }, false);
         return 1;
      } else {
   ...
   private static int executeQuery(ServerCommandSource source) {
      ...
      if (i > 0) {
         ...
         if (i == 1) {
            source.sendFeedback(() -> {
               return Text.translatable("commands.forceload.list.single", Text.of(registryKey.getValue()), string);
            }, false);
         } else {
            source.sendFeedback(() -> {
               return Text.translatable("commands.forceload.list.multiple", i, Text.of(registryKey.getValue()), string);
            }, false);
         }
      } else {
   ...
   private static int executeRemoveAll(ServerCommandSource source) {
      ...
      longSet.forEach((chunkPos) -> {
         serverWorld.setChunkForced(ChunkPos.getPackedX(chunkPos), ChunkPos.getPackedZ(chunkPos), false);
      });
      source.sendFeedback(() -> {
         return Text.translatable("commands.forceload.removed.all", Text.of(registryKey.getValue()));
      }, true);
      return 0;
   }
      ...
      if (i >= -30000000 && j >= -30000000 && k < 30000000 && l < 30000000) {
         ...
         if (q > 256L) {
            ...
            if (r == 0) {
               ...
               if (r == 1) {
                  source.sendFeedback(() -> {
                     return Text.translatable("commands.forceload." + (forceLoaded ? "added" : "removed") + ".single", Text.of(chunkPos), Text.of(registryKey.getValue()));
                  }, true);
               } else {
                  ChunkPos chunkPos3 = new ChunkPos(m, n);
                  ChunkPos chunkPos4 = new ChunkPos(o, p);
                  source.sendFeedback(() -> {
                     return Text.translatable("commands.forceload." + (forceLoaded ? "added" : "removed") + ".multiple", r, Text.of(registryKey.getValue()), Text.of(chunkPos3), Text.of(chunkPos4));
                  }, true);
               }
