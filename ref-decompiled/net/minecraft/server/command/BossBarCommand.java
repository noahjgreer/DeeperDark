...
public class BossBarCommand {
   private static final DynamicCommandExceptionType CREATE_FAILED_EXCEPTION = new DynamicCommandExceptionType((name) -> {
      return Text.stringifiedTranslatable("commands.bossbar.create.failed", name);
   });
   private static final DynamicCommandExceptionType UNKNOWN_EXCEPTION = new DynamicCommandExceptionType((name) -> {
      return Text.stringifiedTranslatable("commands.bossbar.unknown", name);
   });
   ...
   public static void register(CommandDispatcher dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("bossbar").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("add").then(CommandManager.argument("id", IdentifierArgumentType.identifier()).then(CommandManager.argument("name", TextArgumentType.text(registryAccess)).executes((context) -> {
         return addBossBar((ServerCommandSource)context.getSource(), IdentifierArgumentType.getIdentifier(context, "id"), TextArgumentType.parseTextArgument(context, "name"));
      ...
      }))).then(CommandManager.literal("set").then(((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("id", IdentifierArgumentType.identifier()).suggests(SUGGESTION_PROVIDER).then(CommandManager.literal("name").then(CommandManager.argument("name", TextArgumentType.text(registryAccess)).executes((context) -> {
         return setName((ServerCommandSource)context.getSource(), getBossBar(context), TextArgumentType.parseTextArgument(context, "name"));
   ...
   private static int getValue(ServerCommandSource source, CommandBossBar bossBar) {
      source.sendFeedback(() -> {
         return Text.translatable("commands.bossbar.get.value", bossBar.toHoverableText(), bossBar.getValue());
      }, true);
   ...
   private static int getMaxValue(ServerCommandSource source, CommandBossBar bossBar) {
      source.sendFeedback(() -> {
         return Text.translatable("commands.bossbar.get.max", bossBar.toHoverableText(), bossBar.getMaxValue());
      }, true);
   ...
   private static int isVisible(ServerCommandSource source, CommandBossBar bossBar) {
      if (bossBar.isVisible()) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.get.visible.visible", bossBar.toHoverableText());
         }, true);
         return 1;
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.get.visible.hidden", bossBar.toHoverableText());
         }, true);
         return 0;
      }
   }
   ...
   private static int getPlayers(ServerCommandSource source, CommandBossBar bossBar) {
      if (bossBar.getPlayers().isEmpty()) {
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.get.players.none", bossBar.toHoverableText());
         }, true);
      } else {
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.get.players.some", bossBar.toHoverableText(), bossBar.getPlayers().size(), Texts.join(bossBar.getPlayers(), PlayerEntity::getDisplayName));
         }, true);
      }
   ...
   private static int setVisible(ServerCommandSource source, CommandBossBar bossBar, boolean visible) throws CommandSyntaxException {
      if (bossBar.isVisible() == visible) {
         ...
         if (visible) {
            source.sendFeedback(() -> {
               return Text.translatable("commands.bossbar.set.visible.success.visible", bossBar.toHoverableText());
            }, true);
         } else {
            source.sendFeedback(() -> {
               return Text.translatable("commands.bossbar.set.visible.success.hidden", bossBar.toHoverableText());
            }, true);
         }
   ...
   private static int setValue(ServerCommandSource source, CommandBossBar bossBar, int value) throws CommandSyntaxException {
      if (bossBar.getValue() == value) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.set.value.success", bossBar.toHoverableText(), value);
         }, true);
         return value;
      }
   }
   ...
   private static int setMaxValue(ServerCommandSource source, CommandBossBar bossBar, int value) throws CommandSyntaxException {
      if (bossBar.getMaxValue() == value) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.set.max.success", bossBar.toHoverableText(), value);
         }, true);
         return value;
      }
   }
   ...
   private static int setColor(ServerCommandSource source, CommandBossBar bossBar, BossBar.Color color) throws CommandSyntaxException {
      if (bossBar.getColor().equals(color)) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.set.color.success", bossBar.toHoverableText());
         }, true);
         return 0;
      }
   }
   ...
   private static int setStyle(ServerCommandSource source, CommandBossBar bossBar, BossBar.Style style) throws CommandSyntaxException {
      if (bossBar.getStyle().equals(style)) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.set.style.success", bossBar.toHoverableText());
         }, true);
         return 0;
      }
   }
   ...
   private static int setName(ServerCommandSource source, CommandBossBar bossBar, Text name) throws CommandSyntaxException {
      ...
      if (bossBar.getName().equals(text)) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.set.name.success", bossBar.toHoverableText());
         }, true);
         return 0;
      }
   }
   ...
   private static int setPlayers(ServerCommandSource source, CommandBossBar bossBar, Collection players) throws CommandSyntaxException {
      ...
      if (!bl) {
         ...
         if (bossBar.getPlayers().isEmpty()) {
            source.sendFeedback(() -> {
               return Text.translatable("commands.bossbar.set.players.success.none", bossBar.toHoverableText());
            }, true);
         } else {
            source.sendFeedback(() -> {
               return Text.translatable("commands.bossbar.set.players.success.some", bossBar.toHoverableText(), players.size(), Texts.join(players, PlayerEntity::getDisplayName));
            }, true);
         }
   ...
   private static int listBossBars(ServerCommandSource source) {
      ...
      if (collection.isEmpty()) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.list.bars.some", collection.size(), Texts.join(collection, CommandBossBar::toHoverableText));
         }, false);
      }
   ...
   private static int addBossBar(ServerCommandSource source, Identifier name, Text displayName) throws CommandSyntaxException {
      ...
      if (bossBarManager.get(name) != null) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.bossbar.create.success", commandBossBar.toHoverableText());
         }, true);
   ...
   private static int removeBossBar(ServerCommandSource source, CommandBossBar bossBar) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.bossbar.remove.success", bossBar.toHoverableText());
      }, true);
      return bossBarManager.getAll().size();
   }
