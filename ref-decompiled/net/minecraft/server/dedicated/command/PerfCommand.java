...
public class PerfCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("perf").requires(CommandManager.requirePermissionLevel(4))).then(CommandManager.literal("start").executes((context) -> {
   ...
   private static int executeStart(ServerCommandSource source) throws CommandSyntaxException {
      ...
      if (minecraftServer.isRecorderActive()) {
         ...
         Consumer consumer = (result) -> {
            sendProfilingStoppedMessage(source, result);
         };
         Consumer consumer2 = (dumpDirectory) -> {
            saveReport(source, dumpDirectory, minecraftServer);
         };
   ...
   private static void saveReport(ServerCommandSource source, Path tempProfilingDirectory, MinecraftServer server) {
      ...
      source.sendFeedback(() -> {
         return Text.translatable("commands.perf.reportSaved", string2);
      }, false);
   }
   ...
   private static void sendProfilingStoppedMessage(ServerCommandSource source, ProfileResult result) {
      if (result != EmptyProfileResult.INSTANCE) {
         ...
         source.sendFeedback(() -> {
            return Text.translatable("commands.perf.stopped", String.format(Locale.ROOT, "%.2f", d), i, String.format(Locale.ROOT, "%.2f", (double)i / d));
         }, false);
      }
   }
}
