...
public class DeOpCommand {
   ...
   public static void register(CommandDispatcher dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("deop").requires(CommandManager.requirePermissionLevel(3))).then(CommandManager.argument("targets", GameProfileArgumentType.gameProfile()).suggests((context, builder) -> {
   ...
   private static int deop(ServerCommandSource source, Collection targets) throws CommandSyntaxException {
      ...
      while(var4.hasNext()) {
         ...
         if (playerManager.isOperator(gameProfile)) {
            ...
            ++i;
            source.sendFeedback(() -> {
               return Text.translatable("commands.deop.success", ((GameProfile)targets.iterator().next()).getName());
            }, true);
         }
      }
