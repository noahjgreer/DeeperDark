...
public class DedicatedServerWatchdog implements Runnable {
   ...
   public void run() {
      while(this.server.isRunning()) {
         ...
         if (n > this.maxTickTime) {
            LOGGER.error(LogUtils.FATAL_MARKER, "A single server tick took {} seconds (should be max {})", String.format(Locale.ROOT, "%.2f", (float)n / (float)TimeHelper.SECOND_IN_NANOS), String.format(Locale.ROOT, "%.2f", this.server.getTickManager().getMillisPerTick() / (float)TimeHelper.SECOND_IN_MILLIS));
   ...
   public static CrashReport createCrashReport(String message, long threadId) {
      ...
      for(int var9 = 0; var9 < var8; ++var9) {
         ThreadInfo threadInfo = var7[var9];
