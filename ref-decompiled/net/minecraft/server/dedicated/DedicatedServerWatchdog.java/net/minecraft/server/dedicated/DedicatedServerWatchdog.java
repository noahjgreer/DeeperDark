    ...
    public void run() {
        while (this.server.isRunning()) {
            ...
            if (n > this.maxTickTime) {
                ...
                CrashReport crashReport = DedicatedServerWatchdog.createCrashReport("Watching Server", this.server.getThread().threadId());
                ...
                crashReportSection.add("Random tick rate", () -> this.server.getSaveProperties().getGameRules().getRuleValueName(GameRules.RANDOM_TICK_SPEED));
                crashReportSection.add("Level stats", () -> Streams.stream(this.server.getWorlds()).map(world -> String.valueOf(world.getRegistryKey().getValue()) + ": " + world.getDebugString()).collect(Collectors.joining(",\n")));
