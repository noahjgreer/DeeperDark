...
public abstract class ChunkLevelManager {
   ...
   protected final Set chunkHoldersWithPendingUpdates = new ReferenceOpenHashSet();
   ...
   protected ChunkLevelManager(ChunkTicketManager ticketManager, Executor executor, Executor mainThreadExecutor) {
      ...
      TaskExecutor taskExecutor = TaskExecutor.of("player ticket throttler", mainThreadExecutor);
   ...
   public boolean update(ServerChunkLoadingManager chunkLoadingManager) {
      ...
      if (!this.chunkHoldersWithPendingUpdates.isEmpty()) {
         Iterator var9 = this.chunkHoldersWithPendingUpdates.iterator();

         ChunkHolder chunkHolder;
         while(var9.hasNext()) {
            chunkHolder = (ChunkHolder)var9.next();
            chunkHolder.updateStatus(chunkLoadingManager);
         }

         var9 = this.chunkHoldersWithPendingUpdates.iterator();
         ...
         if (!this.freshPlayerTicketPositions.isEmpty()) {
            ...
            while(longIterator.hasNext()) {
               ...
               if (this.ticketManager.getTickets(l).stream().anyMatch((ticket) -> {
                  return ticket.getType() == ChunkTicketType.PLAYER_LOADING;
               })) {
                  ...
                  CompletableFuture completableFuture = chunkHolder2.getEntityTickingFuture();
                  completableFuture.thenAccept((optionalChunk) -> {
                     this.mainThreadExecutor.execute(() -> {
                        this.scheduler.remove(l, () -> {
                        }, false);
                     });
                  });
               }
   ...
   static {
      NEARBY_PLAYER_TICKET_LEVEL = ChunkLevels.getLevelFromType(ChunkLevelType.ENTITY_TICKING);
   }
      ...
      private void updateTicket(long pos, int distance, boolean oldWithinViewDistance, boolean withinViewDistance) {
         if (oldWithinViewDistance != withinViewDistance) {
            ...
            if (withinViewDistance) {
               ChunkLevelManager.this.scheduler.add(() -> {
                  ChunkLevelManager.this.mainThreadExecutor.execute(() -> {
                     if (this.isWithinViewDistance(this.getLevel(pos))) {
                        ...
                        ChunkLevelManager.this.scheduler.remove(pos, () -> {
                        }, false);
                     }
               ...
               ChunkLevelManager.this.scheduler.remove(pos, () -> {
                  ChunkLevelManager.this.mainThreadExecutor.execute(() -> {
                     ChunkLevelManager.this.ticketManager.removeTicket(pos, chunkTicket);
                  });
               }, true);
            }
         }
      ...
      public void updateLevels() {
         ...
         if (!this.positionsAffected.isEmpty()) {
            ...
            while(longIterator.hasNext()) {
               ...
               if (i != j) {
                  ChunkLevelManager.this.scheduler.updateLevel(new ChunkPos(l), () -> {
