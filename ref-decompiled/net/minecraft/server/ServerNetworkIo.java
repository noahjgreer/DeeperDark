...
public class ServerNetworkIo {
   ...
   public static final Supplier DEFAULT_CHANNEL = Suppliers.memoize(() -> {
      return new NioEventLoopGroup(0, (new ThreadFactoryBuilder()).setNameFormat("Netty Server IO #%d").setDaemon(true).build());
   });
   public static final Supplier EPOLL_CHANNEL = Suppliers.memoize(() -> {
      ...
      synchronized(this.channels) {
         ...
         this.channels.add(((ServerBootstrap)((ServerBootstrap)(new ServerBootstrap()).channel(class_)).childHandler(new ChannelInitializer() {
            protected void initChannel(Channel channel) {
               ...
               ClientConnection.addHandlers(channelPipeline, NetworkSide.SERVERBOUND, false, (PacketSizeLogger)null);
               ...
               ((ClientConnection)clientConnection).addFlowControlHandler(channelPipeline);
               ((ClientConnection)clientConnection).setInitialPacketListener(new ServerHandshakeNetworkHandler(ServerNetworkIo.this.server, (ClientConnection)clientConnection));
            }
      ...
      synchronized(this.connections) {
         ...
         while(true) {
            while(true) {
               ...
               } while(clientConnection.isChannelAbsent());
   ...
   private static class DelayingChannelInboundHandler extends ChannelInboundHandlerAdapter {
      ...
      private void forward(Timeout timeout) {
         Packet packet = (Packet)this.packets.remove(0);
