...
implements ServerPlayerConfigurationTask {
    ...
    private void syncRegistryAndTags(Consumer<Packet<?>> sender, Set<VersionedIdentifier> commonKnownPacks) {
        RegistryOps<NbtElement> dynamicOps = this.registries.getCombinedRegistryManager().getOps(NbtOps.INSTANCE);
        SerializableRegistries.forEachSyncedRegistry(dynamicOps, this.registries.getSucceedingRegistryManagers(ServerDynamicRegistryType.WORLDGEN), commonKnownPacks, (key, entries) -> sender.accept(new DynamicRegistriesS2CPacket((RegistryKey<? extends Registry<?>>)key, (List<SerializableRegistries.SerializedRegistryEntry>)entries)));
