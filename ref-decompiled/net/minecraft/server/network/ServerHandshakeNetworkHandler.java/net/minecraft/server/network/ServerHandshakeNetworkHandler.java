...
implements ServerHandshakePacketListener {
        ...
        switch (packet.intendedState()) {
            ...
            case TRANSFER: {
                if (!this.server.acceptsTransfers()) {
                    ...
                    MutableText text = Text.translatable("multiplayer.disconnect.transfers_disabled");
    ...
    private void login(HandshakeC2SPacket packet, boolean transfer) {
        ...
        if (packet.protocolVersion() != SharedConstants.getGameVersion().protocolVersion()) {
            MutableText text = packet.protocolVersion() < 754 ? Text.translatable("multiplayer.disconnect.outdated_client", SharedConstants.getGameVersion().name()) : Text.translatable("multiplayer.disconnect.incompatible", SharedConstants.getGameVersion().name());
            this.connection.send(new LoginDisconnectS2CPacket(text));
            this.connection.disconnect(text);
        } else {
