...
public class ChunkDataSender {
      ...
      if (this.unacknowledgedBatches < this.maxUnacknowledgedBatches) {
         ...
         if (!(this.pending < 1.0F)) {
            if (!this.chunks.isEmpty()) {
               ...
               List list = this.makeBatch(serverChunkLoadingManager, player.getChunkPos());
               if (!list.isEmpty()) {
                  ...
                  while(var7.hasNext()) {
                     WorldChunk worldChunk = (WorldChunk)var7.next();
   ...
   private static void sendChunkData(ServerPlayNetworkHandler handler, ServerWorld world, WorldChunk chunk) {
      handler.sendPacket(new ChunkDataS2CPacket(chunk, world.getLightingProvider(), (BitSet)null, (BitSet)null));
      ChunkPos chunkPos = chunk.getPos();
      DebugInfoSender.sendChunkWatchingChange(world, chunkPos);
   }

   private List makeBatch(ServerChunkLoadingManager chunkLoadingManager, ChunkPos playerPos) {
      int i = MathHelper.floor(this.pending);
      LongStream var10000;
      List list;
      if (!this.local && this.chunks.size() > i) {
         Stream var7 = this.chunks.stream();
         Objects.requireNonNull(playerPos);
         var10000 = ((List)var7.collect(Comparators.least(i, Comparator.comparingInt(playerPos::getSquaredDistance)))).stream().mapToLong(Long::longValue);
         Objects.requireNonNull(chunkLoadingManager);
         list = var10000.mapToObj(chunkLoadingManager::getPostProcessedChunk).filter(Objects::nonNull).toList();
      } else {
         var10000 = this.chunks.longStream();
         Objects.requireNonNull(chunkLoadingManager);
         list = var10000.mapToObj(chunkLoadingManager::getPostProcessedChunk).filter(Objects::nonNull).sorted(Comparator.comparingInt((chunk) -> {
            return playerPos.getSquaredDistance(chunk.getPos());
         })).toList();
      }
      ...
      while(var5.hasNext()) {
         WorldChunk worldChunk = (WorldChunk)var5.next();
