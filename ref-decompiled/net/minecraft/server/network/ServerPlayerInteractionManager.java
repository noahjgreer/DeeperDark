...
public class ServerPlayerInteractionManager {
   ...
   public void update() {
      ...
      if (this.failedToMine) {
         blockState = this.world.getBlockState(this.failedMiningPos);
         if (blockState.isAir()) {
         ...
         blockState = this.world.getBlockState(this.miningPos);
         if (blockState.isAir()) {
   ...
   public void processBlockBreakingAction(BlockPos pos, PlayerActionC2SPacket.Action action, Direction direction, int worldHeight, int sequence) {
      if (!this.player.canInteractWithBlockAt(pos, 1.0)) {
         ...
         if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
            ...
            if (!blockState.isAir()) {
               EnchantmentHelper.onHitBlock(this.world, this.player.getMainHandStack(), this.player, this.player, EquipmentSlot.MAINHAND, Vec3d.ofCenter(pos), blockState, (item) -> {
      ...
      if (!blockState.getBlock().isEnabled(world.getEnabledFeatures())) {
         ...
         if (!bl2) {
            ...
            if (actionResult instanceof ActionResult.PassToDefaultBlockAction && hand == Hand.MAIN_HAND) {
               actionResult2 = blockState.onUse(world, player, hitResult);
