...
public class ServerPlayerInteractionManager {
    ...
    public boolean changeGameMode(GameMode gameMode) {
        ...
        PlayerAbilities playerAbilities = this.player.getAbilities();
        this.setGameMode(gameMode, this.gameMode);
        if (playerAbilities.flying && gameMode != GameMode.SPECTATOR && this.isHoveringOnGround()) {
    ...
    protected void setGameMode(GameMode gameMode, @Nullable GameMode previousGameMode) {
        ...
        PlayerAbilities playerAbilities = this.player.getAbilities();
        gameMode.setAbilities(playerAbilities);
    }
    ...
    private float continueMining(BlockState state, BlockPos pos, int failedStartMiningTime) {
        ...
        float f = state.calcBlockBreakingDelta(this.player, this.player.getEntityWorld(), pos) * (float)(i + 1);
        int j = (int)(f * 10.0f);
    ...
    private void onBlockBreakingAction(BlockPos pos, boolean success, int sequence, String reason) {
        if (SharedConstants.BLOCK_BREAK) {
            LOGGER.debug("Server ACK {} {} {} {}", new Object[]{sequence, pos, success, reason});
        }
    }
    ...
    public void processBlockBreakingAction(BlockPos pos, PlayerActionC2SPacket.Action action, Direction direction, int worldHeight, int sequence) {
        ...
        if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
            ...
            if (!blockState.isAir()) {
                EnchantmentHelper.onHitBlock(this.world, this.player.getMainHandStack(), this.player, this.player, EquipmentSlot.MAINHAND, Vec3d.ofCenter(pos), blockState, item -> this.player.sendEquipmentBreakStatus((Item)item, EquipmentSlot.MAINHAND));
                blockState.onBlockBreakStart(this.world, pos, this.player);
                f = blockState.calcBlockBreakingDelta(this.player, this.player.getEntityWorld(), pos);
            }
            ...
            if (pos.equals(this.miningPos)) {
                ...
                BlockState blockState = this.world.getBlockState(pos);
                if (!blockState.isAir()) {
                    float g = blockState.calcBlockBreakingDelta(this.player, this.player.getEntityWorld(), pos) * (float)(j + 1);
                    if (g >= 0.7f) {
                        this.mining = false;
    ...
    public boolean tryBreakBlock(BlockPos pos) {
        ...
        if (SharedConstants.BLOCK_BREAK) {
            LOGGER.info("server broke {} {} -> {}", new Object[]{pos, blockState2, this.world.getBlockState(pos)});
        }
        if (bl) {
    ...
    public ActionResult interactBlock(ServerPlayerEntity player, World world, ItemStack stack, Hand hand, BlockHitResult hitResult) {
        ...
        if (!bl2) {
            ...
            if (actionResult instanceof ActionResult.PassToDefaultBlockAction && hand == Hand.MAIN_HAND && (actionResult2 = blockState.onUse(world, player, hitResult)).isAccepted()) {
