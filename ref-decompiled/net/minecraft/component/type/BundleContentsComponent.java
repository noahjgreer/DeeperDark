...
public final class BundleContentsComponent implements TooltipData {
   ...
   BundleContentsComponent(List stacks, Fraction occupancy, int selectedStackIndex) {
      this.stacks = stacks;
      this.occupancy = occupancy;
      this.selectedStackIndex = selectedStackIndex;
   }

   private static DataResult validateOccupancy(List stacks) {
      try {
   ...
   public BundleContentsComponent(List stacks) {
      this(stacks, calculateOccupancy(stacks), -1);
   }

   private static Fraction calculateOccupancy(List stacks) {
   ...
   static Fraction getOccupancy(ItemStack stack) {
      ...
      if (bundleContentsComponent != null) {
         ...
         List list = ((BeesComponent)stack.getOrDefault(DataComponentTypes.BEES, BeesComponent.DEFAULT)).bees();
   ...
   static {
      CODEC = ItemStack.CODEC.listOf().flatXmap(BundleContentsComponent::validateOccupancy, (component) -> {
