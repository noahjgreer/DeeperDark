...
public record LoreComponent(List lines, List styledLines) implements TooltipAppender {
   ...
   public LoreComponent(List lines) {
      this(lines, Lists.transform(lines, (style) -> {
         return Texts.setStyleIfAbsent(style.copy(), STYLE);
      }));
   }

   public LoreComponent(List lines, List styledLines) {
      if (lines.size() > 256) {
   ...
   public List styledLines() {
      return this.styledLines;
   }

   static {
      STYLE = Style.EMPTY.withColor(Formatting.DARK_PURPLE).withItalic(true);
      CODEC = TextCodecs.CODEC.sizeLimitedListOf(256).xmap(LoreComponent::new, LoreComponent::lines);
      PACKET_CODEC = TextCodecs.REGISTRY_PACKET_CODEC.collect(PacketCodecs.toList(256)).xmap(LoreComponent::new, LoreComponent::lines);
   }
}
