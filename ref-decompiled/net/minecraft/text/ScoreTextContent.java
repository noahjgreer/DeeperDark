...
public record ScoreTextContent(Either name, String objective) implements TextContent {
   public static final MapCodec INNER_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(Codec.either(ParsedSelector.CODEC, Codec.STRING).fieldOf("name").forGetter(ScoreTextContent::name), Codec.STRING.fieldOf("objective").forGetter(ScoreTextContent::objective)).apply(instance, ScoreTextContent::new);
   });
   public static final MapCodec CODEC;
   public static final TextContent.Type TYPE;

   public ScoreTextContent(Either either, String objective) {
      this.name = either;
   ...
   private ScoreHolder getScoreHolder(ServerCommandSource source) throws CommandSyntaxException {
      ...
      if (optional.isPresent()) {
         List list = ((ParsedSelector)optional.get()).comp_3068().getEntities(source);
         if (!list.isEmpty()) {
            if (list.size() != 1) {
   ...
   public MutableText parse(@Nullable ServerCommandSource source, @Nullable Entity sender, int depth) throws CommandSyntaxException {
      if (source == null) {
         ...
         return this.getScore((ScoreHolder)scoreHolder2, source);
      }
   }
   ...
   static {
      ...
      TYPE = new TextContent.Type(CODEC, "score");
   }
}
