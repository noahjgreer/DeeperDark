...
public class XmlReportingTestCompletionListener implements TestCompletionListener {
   ...
   private Element addTestCase(GameTestState test, String name) {
      ...
      element.setAttribute("classname", test.getStructure().toString());
   ...
   public void onTestFailed(GameTestState test) {
      String string = test.getId().toString();
      String string2 = test.getThrowable().getMessage();
      Element element = this.document.createElement(test.isRequired() ? "failure" : "skipped");
      String var10002 = test.getPos().toShortString();
      element.setAttribute("message", "(" + var10002 + ") " + string2);
