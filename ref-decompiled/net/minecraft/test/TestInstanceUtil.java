...
public class TestInstanceUtil {
   ...
   public static TestInstanceBlockEntity createTestInstanceBlockEntity(Identifier testInstanceId, BlockPos pos, Vec3i size, BlockRotation rotation, ServerWorld world) {
      ...
      RegistryKey registryKey = RegistryKey.of(RegistryKeys.TEST_INSTANCE, testInstanceId);
   ...
   public static Optional findContainingTestInstanceBlock(BlockPos pos, int radius, ServerWorld world) {
      return findTestInstanceBlocks(pos, radius, world).filter((blockPos) -> {
         return isInBounds(blockPos, pos, world);
      }).findFirst();
   }

   public static Optional findNearestTestInstanceBlock(BlockPos pos, int radius, ServerWorld world) {
      Comparator comparator = Comparator.comparingInt((blockPos) -> {
         return blockPos.getManhattanDistance(pos);
      });
      return findTestInstanceBlocks(pos, radius, world).min(comparator);
   }

   public static Stream findTestInstanceBlocks(BlockPos pos, int radius, ServerWorld world) {
      return world.getPointOfInterestStorage().getPositions((poiType) -> {
         return poiType.matchesKey(PointOfInterestTypes.TEST_INSTANCE);
      }, (poiPos) -> {
         return true;
      }, pos, radius, PointOfInterestStorage.OccupationStatus.ANY).map(BlockPos::toImmutable);
   }

   public static Stream findTargetedTestInstanceBlock(BlockPos pos, Entity entity, ServerWorld world) {
      int i = true;
      Vec3d vec3d = entity.getEyePos();
      Vec3d vec3d2 = vec3d.add(entity.getRotationVector().multiply(200.0));
      Stream var10000 = findTestInstanceBlocks(pos, 200, world).map((blockPos) -> {
         return world.getBlockEntity(blockPos, BlockEntityType.TEST_INSTANCE_BLOCK);
      }).flatMap(Optional::stream).filter((testInstanceBlockEntity) -> {
         return testInstanceBlockEntity.getBox().raycast(vec3d, vec3d2).isPresent();
      }).map(BlockEntity::getPos);
      Objects.requireNonNull(pos);
      return var10000.sorted(Comparator.comparing(pos::getSquaredDistance)).limit(1L);
   }
   ...
   private static boolean isInBounds(BlockPos testInstanceBlockPos, BlockPos pos, ServerWorld world) {
      BlockEntity var4 = world.getBlockEntity(testInstanceBlockPos);
      if (var4 instanceof TestInstanceBlockEntity testInstanceBlockEntity) {
