...
public class VanillaDataPackProvider extends VanillaResourcePackProvider {
   ...
   public static ResourcePackManager createManager(Path dataPacksPath, SymlinkFinder symlinkFinder) {
      return new ResourcePackManager(new ResourcePackProvider[]{new VanillaDataPackProvider(symlinkFinder), new FileResourcePackProvider(dataPacksPath, ResourceType.SERVER_DATA, ResourcePackSource.WORLD, symlinkFinder)});
   }

   public static ResourcePackManager createClientManager() {
      return new ResourcePackManager(new ResourcePackProvider[]{new VanillaDataPackProvider(new SymlinkFinder((path) -> {
         return true;
      }))});
   }
   ...
   static {
      METADATA = new PackResourceMetadata(Text.translatable("dataPack.vanilla.description"), SharedConstants.getGameVersion().packVersion(ResourceType.SERVER_DATA), Optional.empty());
      ...
      INFO = new ResourcePackInfo("vanilla", Text.translatable("dataPack.vanilla.name"), ResourcePackSource.BUILTIN, Optional.of(VANILLA_ID));
      BOTTOM_POSITION = new ResourcePackPosition(false, ResourcePackProfile.InsertionPosition.BOTTOM, false);
      TOP_POSITION = new ResourcePackPosition(false, ResourcePackProfile.InsertionPosition.TOP, false);
      ID = Identifier.ofVanilla("datapacks");
   }
}
