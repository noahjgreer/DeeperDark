...
public class DirectoryResourcePack extends AbstractFileResourcePack {
   ...
   public static void findResources(String namespace, Path path, List prefixSegments, ResourcePack.ResultConsumer consumer) {
      ...
      try {
         Stream stream = Files.find(path2, Integer.MAX_VALUE, DirectoryResourcePack::isRegularFile, new FileVisitOption[0]);
   ...
   private static boolean isRegularFile(Path path, BasicFileAttributes fileAttributes) {
      if (!SharedConstants.isDevelopment) {
         ...
         return fileAttributes.isRegularFile() && !StringUtils.equalsIgnoreCase(path.getFileName().toString(), ".ds_store");
      }
   }
