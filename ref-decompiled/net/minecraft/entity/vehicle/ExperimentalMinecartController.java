...
public class ExperimentalMinecartController extends MinecartController {
   ...
   public final List stagingLerpSteps = new LinkedList();
   ...
   public ExperimentalMinecartController(AbstractMinecartEntity abstractMinecartEntity) {
      ...
      this.initialStep = ExperimentalMinecartController.Step.ZERO;
   }
   ...
   private void tickClient() {
      if (--this.ticksToNextRefresh <= 0) {
         ...
         if (!this.stagingLerpSteps.isEmpty()) {
            ...
            Step step;
            for(Iterator var1 = this.currentLerpSteps.iterator(); var1.hasNext(); this.totalWeight += (double)step.weight) {
               step = (Step)var1.next();
            }
   ...
   private InterpolatedStep getLerpedStep(float tickProgress) {
      if (tickProgress == this.lastQueriedTickProgress && this.ticksToNextRefresh == this.lastQueriedTicksToNextRefresh && this.lastReturnedInterpolatedStep != null) {
         ...
         for(i = 0; i < this.currentLerpSteps.size(); ++i) {
            float j = ((Step)this.currentLerpSteps.get(i)).weight;
            if (!(j <= 0.0F)) {
               g += j;
         ...
         Step step = (Step)this.currentLerpSteps.get(i);
         Step step2 = i > 0 ? (Step)this.currentLerpSteps.get(i - 1) : this.initialStep;
   ...
   public void adjustToRail(BlockPos pos, BlockState blockState, boolean ignoreWeight) {
      if (AbstractRailBlock.isRail(blockState)) {
         ...
         if (this.getVelocity().length() > 9.999999747378752E-6 && this.getVelocity().dotProduct(vec3d3) < this.getVelocity().dotProduct(vec3d4) || this.ascends(vec3d4, railShape)) {
            Vec3d vec3d5 = vec3d3;
            vec3d3 = vec3d4;
            vec3d4 = vec3d5;
         }
   ...
   public void moveOnRail(ServerWorld world) {
      for(MoveIteration moveIteration = new MoveIteration(); moveIteration.shouldContinue() && this.minecart.isAlive(); moveIteration.initial = false) {
         ...
         if (d > 9.999999747378752E-6) {
            if (!(vec3d2.horizontalLengthSquared() > 9.999999747378752E-6)) {
         ...
         if (d > 9.999999747378752E-6 || moveIteration.initial) {
   ...
   private Vec3d calcNewHorizontalVelocity(ServerWorld world, Vec3d horizontalVelocity, MoveIteration iteration, BlockPos pos, BlockState railState, RailShape railShape) {
      ...
      if (!iteration.slopeVelocityApplied) {
         vec3d2 = this.applySlopeVelocity(horizontalVelocity, railShape);
      ...
      if (iteration.initial) {
         vec3d2 = this.applyInitialVelocity(vec3d);
      ...
      if (!iteration.decelerated) {
         vec3d2 = this.decelerateFromPoweredRail(vec3d, railState);
         if (vec3d2.horizontalLengthSquared() != vec3d.horizontalLengthSquared()) {
            iteration.decelerated = true;
            vec3d = vec3d2;
         }
      ...
      if (!iteration.accelerated) {
         vec3d2 = this.accelerateFromPoweredRail(vec3d, pos, railState);
         if (vec3d2.horizontalLengthSquared() != vec3d.horizontalLengthSquared()) {
   ...
   private Vec3d applySlopeVelocity(Vec3d horizontalVelocity, RailShape railShape) {
      ...
      switch (railShape) {
         case ASCENDING_EAST:
            var10000 = horizontalVelocity.add(-d, 0.0, 0.0);
            break;
         case ASCENDING_WEST:
            var10000 = horizontalVelocity.add(d, 0.0, 0.0);
            break;
         case ASCENDING_NORTH:
            var10000 = horizontalVelocity.add(0.0, 0.0, d);
            break;
         case ASCENDING_SOUTH:
            var10000 = horizontalVelocity.add(0.0, 0.0, -d);
            break;
         default:
            var10000 = horizontalVelocity;
      }
   ...
   public double moveAlongTrack(BlockPos blockPos, RailShape railShape, double remainingMovement) {
      if (remainingMovement < 9.999999747378752E-6) {
         return 0.0;
      } else {
         ...
         if (vec3d2.length() < 9.999999747378752E-6) {
            ...
            Vec3d vec3d5 = blockPos.toBottomCenterPos().add(vec3d4).add(0.0, 0.1, 0.0).add(vec3d4.normalize().multiply(9.999999747378752E-6));
            ...
            if (bl) {
               if (AbstractRailBlock.isRail(blockState)) {
                  RailShape railShape2 = (RailShape)blockState.get(((AbstractRailBlock)blockState.getBlock()).getShapeProperty());
            ...
            if (this.getPos().distanceTo(vec3d) < 9.999999747378752E-6 && vec3d7.distanceTo(vec3d) > 9.999999747378752E-6) {
               this.setVelocity(Vec3d.ZERO);
               return 0.0;
            } else {
               this.setVelocity(vec3d2);
   ...
   private boolean ascends(Vec3d velocity, RailShape railShape) {
      ...
      switch (railShape) {
         case ASCENDING_EAST:
            var10000 = velocity.x < 0.0;
            break;
         case ASCENDING_WEST:
            var10000 = velocity.x > 0.0;
            break;
         case ASCENDING_NORTH:
            var10000 = velocity.z > 0.0;
            break;
         case ASCENDING_SOUTH:
            var10000 = velocity.z < 0.0;
            break;
      ...
      if (this.minecart.isRideable()) {
         ...
         if (!list.isEmpty()) {
            ...
            while(true) {
               ...
               } while(!(entity instanceof PlayerEntity) && !(entity instanceof IronGolemEntity) && !(entity instanceof AbstractMinecartEntity) && !this.minecart.hasPassengers() && !entity.hasVehicle());
         ...
         Iterator var6 = this.getWorld().getOtherEntities(this.minecart, box).iterator();
   ...
   public static record Step(Vec3d position, Vec3d movement, float yRot, float xRot, float weight) {
      ...
      public static Step ZERO;

      public Step(Vec3d vec3d, Vec3d vec3d2, float f, float g, float h) {
         this.position = vec3d;
      ...
      static {
         ...
         ZERO = new Step(Vec3d.ZERO, Vec3d.ZERO, 0.0F, 0.0F, 0.0F);
      }
   }
   ...
   private static class MoveIteration {
      ...
      public boolean shouldContinue() {
         return this.initial || this.remainingMovement > 9.999999747378752E-6;
      }
   }
}
