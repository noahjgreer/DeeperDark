...
public class SpawnRestriction {
   private static final Map RESTRICTIONS = Maps.newHashMap();

   public static void register(EntityType type, SpawnLocation location, Heightmap.Type heightmapType, SpawnPredicate predicate) {
   ...
   public static SpawnLocation getLocation(EntityType type) {
      Entry entry = (Entry)RESTRICTIONS.get(type);
   ...
   static {
      ...
      register(EntityType.HAPPY_GHAST, SpawnLocationTypes.ON_GROUND, Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, AnimalEntity::isValidNaturalSpawn);
   ...
   static record Entry(Heightmap.Type heightmapType, SpawnLocation location, SpawnPredicate predicate) {
   ...
   public interface SpawnPredicate {
      boolean test(EntityType type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random);
   }
}
