...
implements FlyingItemEntity {
        ...
        if (f > 0.0f) {
            ...
            for (LivingEntity livingEntity : list2) {
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    BlockHitResult hitResult = this.getEntityWorld().raycast(new RaycastContext(vec3d, vec3d2, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this));
    ...
    public void handleStatus(byte status) {
        if (status == 17 && this.getEntityWorld().isClient()) {
            ...
            this.getEntityWorld().addFireworkParticle(this.getX(), this.getY(), this.getZ(), vec3d.x, vec3d.y, vec3d.z, this.getExplosions());
        }
