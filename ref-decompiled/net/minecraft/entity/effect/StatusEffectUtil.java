...
public final class StatusEffectUtil {
   ...
   public static List addEffectToPlayersWithinDistance(ServerWorld world, @Nullable Entity entity, Vec3d origin, double range, StatusEffectInstance statusEffectInstance, int duration) {
      ...
      List list = world.getPlayers((player) -> {
         return player.interactionManager.isSurvivalLike() && (entity == null || !entity.isTeammate(player)) && origin.isInRange(player.getPos(), range) && (!player.hasStatusEffect(registryEntry) || player.getStatusEffect(registryEntry).getAmplifier() < statusEffectInstance.getAmplifier() || player.getStatusEffect(registryEntry).isDurationBelow(duration - 1));
      });
      list.forEach((player) -> {
