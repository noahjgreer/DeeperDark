...
public final class StatusEffectUtil {
    ...
    public static boolean hasWaterBreathing(LivingEntity entity) {
        return entity.hasStatusEffect(StatusEffects.WATER_BREATHING) || entity.hasStatusEffect(StatusEffects.CONDUIT_POWER) || entity.hasStatusEffect(StatusEffects.BREATH_OF_THE_NAUTILUS);
    }

    public static boolean canIncreaseAirOnLand(LivingEntity entity) {
        return !entity.hasStatusEffect(StatusEffects.BREATH_OF_THE_NAUTILUS) || entity.hasStatusEffect(StatusEffects.WATER_BREATHING) || entity.hasStatusEffect(StatusEffects.CONDUIT_POWER);
    }
    ...
    public static List<ServerPlayerEntity> addEffectToPlayersWithinDistance(ServerWorld world, @Nullable Entity entity, Vec3d origin, double range, StatusEffectInstance statusEffectInstance, int duration) {
        ...
        List<ServerPlayerEntity> list = world.getPlayers(player -> !(!player.interactionManager.isSurvivalLike() || entity != null && entity.isTeammate((Entity)player) || !origin.isInRange(player.getEntityPos(), range) || player.hasStatusEffect(registryEntry) && player.getStatusEffect(registryEntry).getAmplifier() >= statusEffectInstance.getAmplifier() && !player.getStatusEffect(registryEntry).isDurationBelow(duration - 1)));
