...
class OozingStatusEffect extends StatusEffect {
   ...
   protected OozingStatusEffect(StatusEffectCategory category, int color, ToIntFunction slimeCountFunction) {
   ...
   private void spawnSlime(World world, double x, double y, double z) {
      SlimeEntity slimeEntity = (SlimeEntity)EntityType.SLIME.create(world, SpawnReason.TRIGGERED);
      if (slimeEntity != null) {
   ...
   protected interface SlimeCounter {
      ...
      static SlimeCounter around(LivingEntity entity) {
         return (limit) -> {
            ...
            entity.getWorld().collectEntitiesByType(EntityType.SLIME, entity.getBoundingBox().expand(2.0), (slime) -> {
               return slime != entity;
            }, list, limit);
            return list.size();
         };
