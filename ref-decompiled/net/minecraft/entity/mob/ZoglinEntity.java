...
public class ZoglinEntity extends HostileEntity implements Hoglin {
   ...
   public ZoglinEntity(EntityType entityType, World world) {
   ...
   private static void addIdleTasks(Brain brain) {
      brain.setTaskList(Activity.IDLE, 10, ImmutableList.of(UpdateAttackTargetTask.create((world, target) -> {
         return target.getHoglinTarget(world);
      }), LookAtMobWithIntervalTask.follow(8.0F, UniformIntProvider.create(30, 60)), new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(0.4F), 2), Pair.of(GoToLookTargetTask.create(0.4F, 3), 2), Pair.of(new WaitTask(30, 60), 1)))));
   }

   private static void addFightTasks(Brain brain) {
      brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(RangedApproachTask.create(1.0F), TaskTriggerer.runIf(ZoglinEntity::isAdult, MeleeAttackTask.create(40)), TaskTriggerer.runIf(ZoglinEntity::isBaby, MeleeAttackTask.create(15)), ForgetAttackTargetTask.create()), (MemoryModuleType)MemoryModuleType.ATTACK_TARGET);
   }

   private Optional getHoglinTarget(ServerWorld world) {
      return ((LivingTargetCache)this.getBrain().getOptionalRegisteredMemory(MemoryModuleType.VISIBLE_MOBS).orElse(LivingTargetCache.empty())).findFirst((target) -> {
         return this.shouldAttack(world, target);
      });
   }
