...
implements Vibrations {
    ...
    public boolean canSpawn(WorldView world) {
        return super.canSpawn(world) && world.isSpaceEmpty(this, this.getType().getDimensions().getBoxAt(this.getEntityPos()));
    }
    ...
    public boolean isInvulnerableTo(ServerWorld world, DamageSource source) {
        if (this.isDiggingOrEmerging() && !source.isIn(DamageTypeTags.BYPASSES_INVULNERABILITY)) {
            return true;
        }
    ...
    public static DefaultAttributeContainer.Builder addAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.MAX_HEALTH, 500.0).add(EntityAttributes.MOVEMENT_SPEED, 0.3f).add(EntityAttributes.KNOCKBACK_RESISTANCE, 1.0).add(EntityAttributes.ATTACK_KNOCKBACK, 1.5).add(EntityAttributes.ATTACK_DAMAGE, 30.0).add(EntityAttributes.FOLLOW_RANGE, 24.0);
    }
    ...
    public void tick() {
        ...
        if (this.getEntityWorld().isClient()) {
            if (this.age % this.getHeartRate() == 0) {
                ...
                if (!this.isSilent()) {
                    this.getEntityWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_WARDEN_HEARTBEAT, this.getSoundCategory(), 5.0f, this.getSoundPitch(), false);
                }
            }
    ...
    protected void mobTick(ServerWorld world) {
        ...
        if ((this.age + this.getId()) % 120 == 0) {
            WardenEntity.addDarknessToClosePlayers(world, this.getEntityPos(), this, 20);
        }
        if (this.age % 20 == 0) {
    ...
    public boolean isValidTarget(@Nullable Entity entity) {
        ...
        if (this.getEntityWorld() != entity.getEntityWorld()) return false;
        ...
        if (!this.getEntityWorld().getWorldBorder().contains(livingEntity.getBoundingBox())) return false;
        return true;
    }
