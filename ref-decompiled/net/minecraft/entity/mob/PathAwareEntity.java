...
public abstract class PathAwareEntity extends MobEntity {
   ...
   public boolean isPanicking() {
      if (this.brain.hasMemoryModule(MemoryModuleType.IS_PANICKING)) {
         ...
         Iterator var1 = this.goalSelector.getGoals().iterator();
         ...
         do {
            ...
            prioritizedGoal = (PrioritizedGoal)var1.next();
   ...
   public void onShortLeashTick(Entity entity) {
      ...
      if (this.shouldFollowLeash() && !this.isPanicking()) {
         ...
         Vec3d vec3d = (new Vec3d(entity.getX() - this.getX(), entity.getY() - this.getY(), entity.getZ() - this.getZ())).normalize().multiply((double)Math.max(g - 2.0F, 0.0F));
