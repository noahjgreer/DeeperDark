...
extends MobEntity {
    ...
    public boolean isPanicking() {
        ...
        for (PrioritizedGoal prioritizedGoal : this.goalSelector.getGoals()) {
            if (!prioritizedGoal.isRunning() || !(prioritizedGoal.getGoal() instanceof EscapeDangerGoal)) continue;
            return true;
        }
        return false;
    }
    ...
    public void onShortLeashTick(Entity entity) {
        ...
        if (this.shouldFollowLeash() && !this.isPanicking()) {
            ...
            Vec3d vec3d = new Vec3d(entity.getX() - this.getX(), entity.getY() - this.getY(), entity.getZ() - this.getZ()).normalize().multiply(Math.max(g - 2.0f, 0.0f));
