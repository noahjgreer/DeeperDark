...
public class GhastEntity extends MobEntity implements Monster {
   ...
   public GhastEntity(EntityType entityType, World world) {
   ...
   protected void initGoals() {
      ...
      this.targetSelector.add(1, new ActiveTargetGoal(this, PlayerEntity.class, 10, true, false, (entity, world) -> {
   ...
   public static void updateYaw(MobEntity ghast) {
      if (ghast.getTarget() == null) {
         ...
         ghast.setYaw(-((float)MathHelper.atan2(vec3d.x, vec3d.z)) * 57.295776F);
   ...
   public static class GhastMoveControl extends MoveControl {
      ...
      private boolean willCollide(Vec3d movement) {
         ...
         if (this.happy) {
            Iterator var4 = BlockPos.iterate(box2.expand(1.0)).iterator();

            while(var4.hasNext()) {
               BlockPos blockPos = (BlockPos)var4.next();
               if (!this.canPassThrough(this.ghast.getWorld(), (Vec3d)null, (Vec3d)null, blockPos, false, false)) {
                  return false;
               }
            }
         }
         ...
         Vec3d vec3d = this.ghast.getPos();
         Vec3d vec3d2 = vec3d.add(movement);
         return BlockView.collectCollisionsBetween(vec3d, vec3d2, box2, (pos, version) -> {
            return box.contains(pos) ? true : this.canPassThrough(this.ghast.getWorld(), vec3d, vec3d2, pos, bl, bl2);
         });
      }
      ...
      private boolean canPassThrough(BlockView world, @Nullable Vec3d oldPos, @Nullable Vec3d newPos, BlockPos blockPos, boolean waterAllowed, boolean lavaAllowed) {
         ...
         if (blockState.isAir()) {
            ...
            boolean bl2 = bl ? !this.ghast.collides(oldPos, newPos, blockState.getCollisionShape(world, blockPos).offset(new Vec3d(blockPos)).getBoundingBoxes()) : blockState.getCollisionShape(world, blockPos).isEmpty();
            if (!this.happy) {
               return bl2;
         ...
         if (livingEntity != null) {
            ...
            if (livingEntity.squaredDistanceTo(this.ghast) < 4096.0 && this.ghast.canSee(livingEntity)) {
               ...
               if (this.cooldown == 10 && !this.ghast.isSilent()) {
                  world.syncWorldEvent((Entity)null, 1015, this.ghast.getBlockPos(), 0);
               }
               ...
               if (this.cooldown == 20) {
                  ...
                  if (!this.ghast.isSilent()) {
                     world.syncWorldEvent((Entity)null, 1016, this.ghast.getBlockPos(), 0);
                  }
