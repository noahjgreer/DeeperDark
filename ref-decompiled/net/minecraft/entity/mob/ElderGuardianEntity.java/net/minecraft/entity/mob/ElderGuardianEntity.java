...
extends GuardianEntity {
    ...
    public static DefaultAttributeContainer.Builder createElderGuardianAttributes() {
        return GuardianEntity.createGuardianAttributes().add(EntityAttributes.MOVEMENT_SPEED, 0.3f).add(EntityAttributes.ATTACK_DAMAGE, 8.0).add(EntityAttributes.MAX_HEALTH, 80.0);
    }
    ...
    protected void mobTick(ServerWorld world) {
        ...
        if ((this.age + this.getId()) % 1200 == 0) {
            ...
            List<ServerPlayerEntity> list = StatusEffectUtil.addEffectToPlayersWithinDistance(world, this, this.getEntityPos(), 50.0, statusEffectInstance, 1200);
            list.forEach(player -> player.networkHandler.sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.ELDER_GUARDIAN_EFFECT, this.isSilent() ? 0.0f : 1.0f)));
        }
