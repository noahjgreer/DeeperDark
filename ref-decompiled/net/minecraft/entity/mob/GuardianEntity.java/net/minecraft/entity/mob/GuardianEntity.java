...
extends HostileEntity {
    ...
    public GuardianEntity(EntityType<? extends GuardianEntity> entityType, World world) {
        ...
        this.lastTailAngle = this.tailAngle = this.random.nextFloat();
    }
            ...
            if (this.getEntityWorld().isClient()) {
                ...
                if (!this.isTouchingWater()) {
                    ...
                    if (vec3d.y > 0.0 && this.flopping && !this.isSilent()) {
                        this.getEntityWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), this.getFlopSound(), this.getSoundCategory(), 1.0f, 1.0f, false);
                    }
                ...
                this.spikesExtension = !this.isTouchingWater() ? this.random.nextFloat() : (this.areSpikesRetracted() ? (this.spikesExtension += (0.0f - this.spikesExtension) * 0.25f) : (this.spikesExtension += (1.0f - this.spikesExtension) * 0.06f));
                if (this.areSpikesRetracted() && this.isTouchingWater()) {
                    vec3d = this.getRotationVec(0.0f);
                    for (int i = 0; i < 2; ++i) {
                        this.getEntityWorld().addParticleClient(ParticleTypes.BUBBLE, this.getParticleX(0.5) - vec3d.x * 1.5, this.getRandomBodyY() - vec3d.y * 1.5, this.getParticleZ(0.5) - vec3d.z * 1.5, 0.0, 0.0, 0.0);
                    }
                }
                if (this.hasBeamTarget()) {
                    ...
                    if ((livingEntity = this.getBeamTarget()) != null) {
                        ...
                        while (j < h) {
                            this.getEntityWorld().addParticleClient(ParticleTypes.BUBBLE, this.getX() + e * (j += 1.8 - d + this.random.nextDouble() * (1.7 - d)), this.getEyeY() + f * j, this.getZ() + g * j, 0.0, 0.0, 0.0);
                        }
                    }
                }
            }
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (this.beamTicks == 0) {
                ...
                ServerWorld serverWorld = FireBeamGoal.getServerWorld(this.guardian);
