...
public class PiglinBruteBrain {
   ...
   protected static void setCurrentPosAsHome(PiglinBruteEntity piglinBrute) {
      ...
      piglinBrute.getBrain().remember(MemoryModuleType.HOME, (Object)globalPos);
   }
   ...
   private static void addFightActivities(PiglinBruteEntity piglinBrute, Brain brain) {
      brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(ForgetAttackTargetTask.create((world, target) -> {
         return !isTarget(world, piglinBrute, target);
      }), RangedApproachTask.create(1.0F), MeleeAttackTask.create(20)), (MemoryModuleType)MemoryModuleType.ATTACK_TARGET);
   }
   ...
   private static RandomTask getIdleTasks() {
      return new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(0.6F), 2), Pair.of(FindEntityTask.create(EntityType.PIGLIN, 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(FindEntityTask.create(EntityType.PIGLIN_BRUTE, 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(GoToPosTask.create(MemoryModuleType.HOME, 0.6F, 2, 100), 2), Pair.of(GoAroundTask.create(MemoryModuleType.HOME, 0.6F, 5), 2), Pair.of(new WaitTask(30, 60), 1)));
   }
   ...
   protected static void tick(PiglinBruteEntity piglinBrute) {
      ...
      brain.resetPossibleActivities((List)ImmutableList.of(Activity.FIGHT, Activity.IDLE));
   ...
   private static boolean isTarget(ServerWorld world, AbstractPiglinEntity piglin, LivingEntity target) {
      return getTarget(world, piglin).filter((targetx) -> {
         return targetx == target;
      }).isPresent();
   }

   private static Optional getTarget(ServerWorld world, AbstractPiglinEntity piglin) {
      ...
      if (optional.isPresent() && Sensor.testAttackableTargetPredicateIgnoreVisibility(world, piglin, (LivingEntity)optional.get())) {
         ...
         Optional optional2 = piglin.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER);
