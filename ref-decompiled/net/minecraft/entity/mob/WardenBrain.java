...
public class WardenBrain {
   ...
   private static final List SENSORS;
   private static final List MEMORY_MODULES;
   private static final Task RESET_DIG_COOLDOWN_TASK;

   public static void updateActivities(WardenEntity warden) {
      warden.getBrain().resetPossibleActivities((List)ImmutableList.of(Activity.EMERGE, Activity.DIG, Activity.ROAR, Activity.FIGHT, Activity.INVESTIGATE, Activity.SNIFF, Activity.IDLE));
   }
   ...
   protected static Brain create(WardenEntity warden, Dynamic dynamic) {
      ...
      Brain brain = profile.deserialize(dynamic);
   ...
   private static void addEmergeActivities(Brain brain) {
      brain.setTaskList(Activity.EMERGE, 5, ImmutableList.of(new EmergeTask(EMERGE_DURATION)), (MemoryModuleType)MemoryModuleType.IS_EMERGING);
   }
   ...
   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, 10, ImmutableList.of(FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), StartSniffingTask.create(), new RandomTask(ImmutableMap.of(MemoryModuleType.IS_SNIFFING, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(StrollTask.create(0.5F), 2), Pair.of(new WaitTask(30, 60), 1)))));
   }

   private static void addInvestigateActivities(Brain brain) {
      brain.setTaskList(Activity.INVESTIGATE, 5, ImmutableList.of(FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), WalkTowardsFuzzyPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7F)), (MemoryModuleType)MemoryModuleType.DISTURBANCE_LOCATION);
   }

   private static void addSniffActivities(Brain brain) {
      brain.setTaskList(Activity.SNIFF, 5, ImmutableList.of(FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), new SniffTask(SNIFF_DURATION)), (MemoryModuleType)MemoryModuleType.IS_SNIFFING);
   }

   private static void addRoarActivities(Brain brain) {
      brain.setTaskList(Activity.ROAR, 10, ImmutableList.of(new RoarTask()), (MemoryModuleType)MemoryModuleType.ROAR_TARGET);
   }

   private static void addFightActivities(WardenEntity warden, Brain brain) {
      brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(RESET_DIG_COOLDOWN_TASK, ForgetAttackTargetTask.create((world, target) -> {
         return !warden.getAngriness().isAngry() || !warden.isValidTarget(target);
      }, WardenBrain::removeDeadSuspect, false), LookAtMobTask.create((entity) -> {
         return isTargeting(warden, entity);
      }, (float)warden.getAttributeValue(EntityAttributes.FOLLOW_RANGE)), RangedApproachTask.create(1.2F), new SonicBoomTask(), MeleeAttackTask.create(18)), (MemoryModuleType)MemoryModuleType.ATTACK_TARGET);
   }

   private static boolean isTargeting(WardenEntity warden, LivingEntity entity) {
      return warden.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).filter((entityx) -> {
   ...
   static {
      SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
