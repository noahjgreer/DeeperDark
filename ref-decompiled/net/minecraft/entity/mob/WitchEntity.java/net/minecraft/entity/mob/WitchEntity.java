...
implements RangedAttackMob {
            ...
            if (this.isDrinking()) {
                if (this.drinkTimeLeft-- <= 0) {
                    ...
                    if (itemStack.isOf(Items.POTION) && potionContentsComponent != null) {
                        potionContentsComponent.forEachEffect(this::addStatusEffect, itemStack.getOrDefault(DataComponentTypes.POTION_DURATION_SCALE, Float.valueOf(1.0f)).floatValue());
                    }
    ...
    public void shootAt(LivingEntity target, float pullProgress) {
        ...
        if (target instanceof RaiderEntity) {
            registryEntry = target.getHealth() <= 4.0f ? Potions.HEALING : Potions.REGENERATION;
            this.setTarget(null);
