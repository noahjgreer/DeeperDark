...
public class WitchEntity extends RaiderEntity implements RangedAttackMob {
   ...
   public WitchEntity(EntityType entityType, World world) {
   ...
   protected void initGoals() {
      ...
      this.raidGoal = new RaidGoal(this, RaiderEntity.class, true, (target, world) -> {
         return this.hasActiveRaid() && target.getType() != EntityType.WITCH;
      });
      this.attackPlayerGoal = new DisableableFollowTargetGoal(this, PlayerEntity.class, 10, true, false, (TargetPredicate.EntityPredicate)null);
         ...
         if (this.isDrinking()) {
            if (this.drinkTimeLeft-- <= 0) {
               ...
               if (itemStack.isOf(Items.POTION) && potionContentsComponent != null) {
                  potionContentsComponent.forEachEffect(this::addStatusEffect, (Float)itemStack.getOrDefault(DataComponentTypes.POTION_DURATION_SCALE, 1.0F));
               }
            ...
            if (registryEntry != null) {
               ...
               if (!this.isSilent()) {
                  this.getWorld().playSound((Entity)null, this.getX(), this.getY(), this.getZ(), (SoundEvent)SoundEvents.ENTITY_WITCH_DRINK, this.getSoundCategory(), 1.0F, 0.8F + this.random.nextFloat() * 0.4F);
               }
   ...
   public void handleStatus(byte status) {
      if (status == 15) {
         for(int i = 0; i < this.random.nextInt(35) + 10; ++i) {
            this.getWorld().addParticleClient(ParticleTypes.WITCH, this.getX() + this.random.nextGaussian() * 0.12999999523162842, this.getBoundingBox().maxY + 0.5 + this.random.nextGaussian() * 0.12999999523162842, this.getZ() + this.random.nextGaussian() * 0.12999999523162842, 0.0, 0.0, 0.0);
         }
      } else {
   ...
   public void shootAt(LivingEntity target, float pullProgress) {
      if (!this.isDrinking()) {
         ...
         RegistryEntry registryEntry = Potions.HARMING;
         ...
         if (!this.isSilent()) {
            this.getWorld().playSound((Entity)null, this.getX(), this.getY(), this.getZ(), (SoundEvent)SoundEvents.ENTITY_WITCH_THROW, this.getSoundCategory(), 1.0F, 0.8F + this.random.nextFloat() * 0.4F);
         }
   ...
   static {
      DRINKING_SPEED_PENALTY_MODIFIER = new EntityAttributeModifier(DRINKING_SPEED_PENALTY_MODIFIER_ID, -0.25, EntityAttributeModifier.Operation.ADD_VALUE);
