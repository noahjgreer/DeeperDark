...
public class EndermiteEntity extends HostileEntity {
   ...
   public EndermiteEntity(EntityType entityType, World world) {
   ...
   public static DefaultAttributeContainer.Builder createEndermiteAttributes() {
      return HostileEntity.createHostileAttributes().add(EntityAttributes.MAX_HEALTH, 8.0).add(EntityAttributes.MOVEMENT_SPEED, 0.25).add(EntityAttributes.ATTACK_DAMAGE, 2.0);
   }
   ...
   public void tickMovement() {
      ...
      if (this.getWorld().isClient) {
         for(int i = 0; i < 2; ++i) {
            this.getWorld().addParticleClient(ParticleTypes.PORTAL, this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), (this.random.nextDouble() - 0.5) * 2.0, -this.random.nextDouble(), (this.random.nextDouble() - 0.5) * 2.0);
         }
      } else {
