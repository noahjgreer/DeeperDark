...
extends HostileEntity {
    private @Nullable CallForHelpGoal callForHelpGoal;
    ...
    public static DefaultAttributeContainer.Builder createSilverfishAttributes() {
        return HostileEntity.createHostileAttributes().add(EntityAttributes.MAX_HEALTH, 8.0).add(EntityAttributes.MOVEMENT_SPEED, 0.25).add(EntityAttributes.ATTACK_DAMAGE, 1.0);
    }
                    ...
                    while (j <= 10 && j >= -10) {
                        ...
                        while (k <= 10 && k >= -10) {
                            ...
                            if (block instanceof InfestedBlock) {
                                if (CallForHelpGoal.castToServerWorld(world).getGameRules().getValue(GameRules.DO_MOB_GRIEFING).booleanValue()) {
    ...
    extends WanderAroundGoal {
        ...
        public boolean canStart() {
            ...
            if (WanderAndInfestGoal.getServerWorld(this.mob).getGameRules().getValue(GameRules.DO_MOB_GRIEFING).booleanValue() && random.nextInt(WanderAndInfestGoal.toGoalTicks(10)) == 0) {
