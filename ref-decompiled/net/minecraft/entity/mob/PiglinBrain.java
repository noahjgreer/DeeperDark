...
public class PiglinBrain {
   ...
   private static void addCoreActivities(Brain brain) {
      brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new UpdateLookControlTask(45, 90), new MoveToTargetTask(), OpenDoorsTask.create(), goToNemesisTask(), makeFleeFromZombifiedPiglinTask(), RemoveOffHandItemTask.create(), AdmireItemTask.create(119), DefeatTargetTask.create(300, PiglinBrain::isHuntingTarget), ForgetAngryAtTargetTask.create()));
   }

   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, 10, ImmutableList.of(LookAtMobTask.create(PiglinBrain::isGoldHoldingPlayer, 14.0F), UpdateAttackTargetTask.create((world, target) -> {
         return target.isAdult();
      }, PiglinBrain::getPreferredTarget), TaskTriggerer.runIf(PiglinEntity::canHunt, HuntHoglinTask.create()), makeGoToSoulFireTask(), makeRememberRideableHoglinTask(), makeRandomFollowTask(), makeRandomWanderTask(), FindInteractionTargetTask.create(EntityType.PLAYER, 4)));
   }

   private static void addFightActivities(PiglinEntity piglin, Brain brain) {
      brain.setTaskList(Activity.FIGHT, 10, ImmutableList.of(ForgetAttackTargetTask.create((world, target) -> {
         return !isPreferredAttackTarget(world, piglin, target);
      }), TaskTriggerer.runIf(PiglinBrain::isHoldingCrossbow, AttackTask.create(5, 0.75F)), RangedApproachTask.create(1.0F), MeleeAttackTask.create(20), new CrossbowAttackTask(), HuntFinishTask.create(), ForgetTask.create(PiglinBrain::getNearestZombifiedPiglin, MemoryModuleType.ATTACK_TARGET)), (MemoryModuleType)MemoryModuleType.ATTACK_TARGET);
   }

   private static void addCelebrateActivities(Brain brain) {
      brain.setTaskList(Activity.CELEBRATE, 10, ImmutableList.of(makeGoToSoulFireTask(), LookAtMobTask.create(PiglinBrain::isGoldHoldingPlayer, 14.0F), UpdateAttackTargetTask.create((world, target) -> {
         return target.isAdult();
      }, PiglinBrain::getPreferredTarget), TaskTriggerer.runIf((piglin) -> {
         return !piglin.isDancing();
      }, WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 2, 1.0F)), TaskTriggerer.runIf(PiglinEntity::isDancing, WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 4, 0.6F)), new RandomTask(ImmutableList.of(Pair.of(LookAtMobTask.create(EntityType.PIGLIN, 8.0F), 1), Pair.of(StrollTask.create(0.6F, 2, 1), 1), Pair.of(new WaitTask(10, 20), 1)))), (MemoryModuleType)MemoryModuleType.CELEBRATE_LOCATION);
   }

   private static void addAdmireItemActivities(Brain brain) {
      brain.setTaskList(Activity.ADMIRE_ITEM, 10, ImmutableList.of(WalkTowardsNearestVisibleWantedItemTask.create(PiglinBrain::doesNotHaveGoldInOffHand, 1.0F, true, 9), WantNewItemTask.create(9), AdmireItemTimeLimitTask.create(200, 200)), (MemoryModuleType)MemoryModuleType.ADMIRING_ITEM);
   }

   private static void addAvoidActivities(Brain brain) {
      brain.setTaskList(Activity.AVOID, 10, ImmutableList.of(GoToRememberedPositionTask.createEntityBased(MemoryModuleType.AVOID_TARGET, 1.0F, 12, true), makeRandomFollowTask(), makeRandomWanderTask(), ForgetTask.create(PiglinBrain::shouldRunAwayFromHoglins, MemoryModuleType.AVOID_TARGET)), (MemoryModuleType)MemoryModuleType.AVOID_TARGET);
   }

   private static void addRideActivities(Brain brain) {
      brain.setTaskList(Activity.RIDE, 10, ImmutableList.of(StartRidingTask.create(0.8F), LookAtMobTask.create(PiglinBrain::isGoldHoldingPlayer, 8.0F), TaskTriggerer.runIf((TaskRunnable)TaskTriggerer.predicate(Entity::hasVehicle), (TaskRunnable)Tasks.pickRandomly(ImmutableList.builder().addAll(makeFollowTasks()).add(Pair.of(TaskTriggerer.predicate((piglin) -> {
         return true;
      }), 1)).build())), RidingTask.create(8, PiglinBrain::canRide)), (MemoryModuleType)MemoryModuleType.RIDE_TARGET);
   }
   ...
   private static RandomTask makeRandomWanderTask() {
      return new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(0.6F), 2), Pair.of(FindEntityTask.create(EntityType.PIGLIN, 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(TaskTriggerer.runIf(PiglinBrain::canWander, GoToLookTargetTask.create(0.6F, 3)), 2), Pair.of(new WaitTask(30, 60), 1)));
   }
   ...
   private static Task makeFleeFromZombifiedPiglinTask() {
   ...
   protected static void consumeOffHandItem(ServerWorld world, PiglinEntity piglin, boolean barter) {
      ...
      if (piglin.isAdult()) {
         ...
         bl = !piglin.tryEquip(world, itemStack).isEmpty();
         if (!bl) {
   ...
   private static List getBarteredItem(PiglinEntity piglin) {
      ...
      List list = lootTable.generateLoot((new LootWorldContext.Builder((ServerWorld)piglin.getWorld())).add(LootContextParameters.THIS_ENTITY, piglin).build(LootContextTypes.BARTER));
      return list;
   }
   ...
   private static boolean isPreferredAttackTarget(ServerWorld world, PiglinEntity piglin, LivingEntity target) {
      return getPreferredTarget(world, piglin).filter((preferredTarget) -> {
   ...
   private static Optional getPreferredTarget(ServerWorld world, PiglinEntity piglin) {
      Brain brain = piglin.getBrain();
      if (getNearestZombifiedPiglin(piglin)) {
         return Optional.empty();
      } else {
         Optional optional = TargetUtil.getEntity(piglin, MemoryModuleType.ANGRY_AT);
         if (optional.isPresent() && Sensor.testAttackableTargetPredicateIgnoreVisibility(world, piglin, (LivingEntity)optional.get())) {
            ...
            if (brain.hasMemoryModule(MemoryModuleType.UNIVERSAL_ANGER)) {
               optional2 = brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER);
            ...
            if (optional2.isPresent()) {
               ...
               Optional optional3 = brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GOLD);
   ...
   public static void onGuardedBlockInteracted(ServerWorld world, PlayerEntity player, boolean blockOpen) {
      List list = player.getWorld().getNonSpectatingEntities(PiglinEntity.class, player.getBoundingBox().expand(16.0));
      list.stream().filter(PiglinBrain::hasIdleActivity).filter((piglin) -> {
         return !blockOpen || TargetUtil.isVisibleInMemory(piglin, player);
      }).forEach((nearbyPiglin) -> {
   ...
   private static boolean hasTargetToAvoid(PiglinEntity piglin) {
      Brain brain = piglin.getBrain();
      return !brain.hasMemoryModule(MemoryModuleType.AVOID_TARGET) ? false : ((LivingEntity)brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET).get()).isInRange(piglin, 12.0);
   }
   ...
   public static boolean isWearingPiglinSafeArmor(LivingEntity entity) {
      ...
      } while(!entity.getEquippedStack(equipmentSlot).isIn(ItemTags.PIGLIN_SAFE_ARMOR));
   ...
   private static Task makeRememberRideableHoglinTask() {
      ...
      return MemoryTransferTask.create((entity) -> {
         return entity.isBaby() && interval.shouldRun(entity.getWorld().random);
   ...
   private static void angerAtIfCloser(ServerWorld world, AbstractPiglinEntity piglin, LivingEntity target) {
      ...
      LivingEntity livingEntity = TargetUtil.getCloserEntity(piglin, (Optional)optional, target);
   ...
   private static void runAwayFromClosestTarget(PiglinEntity piglin, LivingEntity target) {
      Brain brain = piglin.getBrain();
      LivingEntity livingEntity = TargetUtil.getCloserEntity(piglin, (Optional)brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET), target);
      livingEntity = TargetUtil.getCloserEntity(piglin, (Optional)brain.getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET), livingEntity);
