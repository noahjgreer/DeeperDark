...
public class BreezeEntity extends HostileEntity {
   ...
   private static final ProjectileDeflection PROJECTILE_DEFLECTOR = (projectile, hitEntity, random) -> {
      hitEntity.getWorld().playSoundFromEntity((Entity)null, hitEntity, SoundEvents.ENTITY_BREEZE_DEFLECT, hitEntity.getSoundCategory(), 1.0F, 1.0F);
   ...
   public static DefaultAttributeContainer.Builder createBreezeAttributes() {
      return MobEntity.createMobAttributes().add(EntityAttributes.MOVEMENT_SPEED, 0.6299999952316284).add(EntityAttributes.MAX_HEALTH, 30.0).add(EntityAttributes.FOLLOW_RANGE, 24.0).add(EntityAttributes.ATTACK_DAMAGE, 3.0);
   }

   public BreezeEntity(EntityType entityType, World world) {
   ...
   public void addLongJumpingParticles() {
      if (++this.longJumpingParticleAddCount <= 5) {
         ...
         Vec3d vec3d2 = this.getPos().add(vec3d).add(0.0, 0.10000000149011612, 0.0);

         for(int i = 0; i < 3; ++i) {
            this.getWorld().addParticleClient(new BlockStateParticleEffect(ParticleTypes.BLOCK, blockState), vec3d2.x, vec3d2.y, vec3d2.z, 0.0, 0.0, 0.0);
         }
   ...
   public void playAmbientSound() {
      if (this.getTarget() == null || !this.isOnGround()) {
         this.getWorld().playSoundFromEntityClient(this, this.getAmbientSound(), this.getSoundCategory(), 1.0F, 1.0F);
      }
   }
   ...
   public void playWhirlSound() {
      ...
      this.getWorld().playSoundFromEntityClient(this, SoundEvents.ENTITY_BREEZE_WHIRL, this.getSoundCategory(), g, f);
   }
   ...
   public Optional getHurtBy() {
      return this.getBrain().getOptionalRegisteredMemory(MemoryModuleType.HURT_BY).map(DamageSource::getAttacker).filter((attacker) -> {
         return attacker instanceof LivingEntity;
      }).map((livingAttacker) -> {
         return (LivingEntity)livingAttacker;
      });
   }
   ...
   public double getChargeY() {
      return this.getY() + (double)(this.getHeight() / 2.0F) + 0.30000001192092896;
   }
