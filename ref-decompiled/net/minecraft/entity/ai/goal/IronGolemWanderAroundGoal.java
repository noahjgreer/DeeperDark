...
public class IronGolemWanderAroundGoal extends WanderAroundGoal {
   ...
   private Vec3d findVillagerPos() {
      ...
      List list = serverWorld.getEntitiesByType(EntityType.VILLAGER, this.mob.getBoundingBox().expand(32.0), this::canVillagerSummonGolem);
      if (list.isEmpty()) {
         return null;
      } else {
   ...
   private ChunkSectionPos findRandomChunkPos() {
      ...
      List list = (List)ChunkSectionPos.stream(ChunkSectionPos.from((EntityLike)this.mob), 2).filter((sectionPos) -> {
         return serverWorld.getOccupiedPointOfInterestDistance(sectionPos) == 0;
   ...
   private BlockPos findRandomPosInChunk(ChunkSectionPos pos) {
      ...
      List list = (List)pointOfInterestStorage.getInCircle((registryEntry) -> {
         return true;
