...
extends DiveJumpingGoal {
    ...
    private boolean isWater(BlockPos pos, int offsetX, int offsetZ, int multiplier) {
        ...
        return this.dolphin.getEntityWorld().getFluidState(blockPos).isIn(FluidTags.WATER) && !this.dolphin.getEntityWorld().getBlockState(blockPos).blocksMovement();
    }

    private boolean isAirAbove(BlockPos pos, int offsetX, int offsetZ, int multiplier) {
        return this.dolphin.getEntityWorld().getBlockState(pos.add(offsetX * multiplier, 1, offsetZ * multiplier)).isAir() && this.dolphin.getEntityWorld().getBlockState(pos.add(offsetX * multiplier, 2, offsetZ * multiplier)).isAir();
    }
