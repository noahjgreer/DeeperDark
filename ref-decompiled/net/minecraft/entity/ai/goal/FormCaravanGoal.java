...
public class FormCaravanGoal extends Goal {
   ...
   public boolean canStart() {
      if (!this.llama.isLeashed() && !this.llama.isFollowing()) {
         List list = this.llama.getWorld().getOtherEntities(this.llama, this.llama.getBoundingBox().expand(9.0, 4.0, 9.0), (entityx) -> {
         ...
         while(var5.hasNext()) {
            ...
            llamaEntity2 = (LlamaEntity)entity;
            if (llamaEntity2.isFollowing() && !llamaEntity2.hasFollower()) {
               e = this.llama.squaredDistanceTo(llamaEntity2);
               if (!(e > d)) {
                  d = e;
                  llamaEntity = llamaEntity2;
               }
