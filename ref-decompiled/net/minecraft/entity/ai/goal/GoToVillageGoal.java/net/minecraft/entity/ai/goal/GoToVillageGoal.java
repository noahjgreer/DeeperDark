...
extends Goal {
    ...
    public void tick() {
        ...
        if (entityNavigation.isIdle() && !this.targetPosition.isWithinDistance(this.mob.getEntityPos(), 10.0)) {
    ...
    private void findOtherWaypoint() {
        ...
        BlockPos blockPos = this.mob.getEntityWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, this.mob.getBlockPos().add(-8 + random.nextInt(16), 0, -8 + random.nextInt(16)));
