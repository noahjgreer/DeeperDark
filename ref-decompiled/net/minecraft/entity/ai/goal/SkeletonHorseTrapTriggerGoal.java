...
public class SkeletonHorseTrapTriggerGoal extends Goal {
   ...
   public void tick() {
      ...
      LightningEntity lightningEntity = (LightningEntity)EntityType.LIGHTNING_BOLT.create(serverWorld, SpawnReason.TRIGGERED);
   ...
   private AbstractHorseEntity getHorse(LocalDifficulty localDifficulty) {
      SkeletonHorseEntity skeletonHorseEntity = (SkeletonHorseEntity)EntityType.SKELETON_HORSE.create(this.skeletonHorse.getWorld(), SpawnReason.TRIGGERED);
      if (skeletonHorseEntity != null) {
         skeletonHorseEntity.initialize((ServerWorld)this.skeletonHorse.getWorld(), localDifficulty, SpawnReason.TRIGGERED, (EntityData)null);
   ...
   private SkeletonEntity getSkeleton(LocalDifficulty localDifficulty, AbstractHorseEntity vehicle) {
      SkeletonEntity skeletonEntity = (SkeletonEntity)EntityType.SKELETON.create(vehicle.getWorld(), SpawnReason.TRIGGERED);
      if (skeletonEntity != null) {
         skeletonEntity.initialize((ServerWorld)vehicle.getWorld(), localDifficulty, SpawnReason.TRIGGERED, (EntityData)null);
