...
public class ChaseBoatGoal extends Goal {
   ...
   public boolean canStart() {
      List list = this.mob.getWorld().getNonSpectatingEntities(AbstractBoatEntity.class, this.mob.getBoundingBox().expand(5.0));
      boolean bl = false;
      Iterator var3 = list.iterator();

      while(var3.hasNext()) {
         AbstractBoatEntity abstractBoatEntity = (AbstractBoatEntity)var3.next();
         Entity entity = abstractBoatEntity.getControllingPassenger();
         if (entity instanceof PlayerEntity playerEntity) {
            if (MathHelper.abs(playerEntity.sidewaysSpeed) > 0.0F || MathHelper.abs(playerEntity.forwardSpeed) > 0.0F) {
               bl = true;
               break;
            }
         }
   ...
   public void start() {
      ...
      while(var2.hasNext()) {
         AbstractBoatEntity abstractBoatEntity = (AbstractBoatEntity)var2.next();
         LivingEntity var5 = abstractBoatEntity.getControllingPassenger();
