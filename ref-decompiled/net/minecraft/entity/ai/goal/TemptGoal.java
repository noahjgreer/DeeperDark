...
public class TemptGoal extends Goal {
   ...
   private final Predicate temptItemPredicate;
   private final boolean canBeScared;
   private final double range;

   public TemptGoal(PathAwareEntity entity, double speed, Predicate temptItemPredicate, boolean canBeScared) {
      this((MobEntity)entity, speed, temptItemPredicate, canBeScared, 2.5);
   }

   public TemptGoal(PathAwareEntity entity, double speed, Predicate temptItemPredicate, boolean canBeScared, double range) {
      this((MobEntity)entity, speed, temptItemPredicate, canBeScared, range);
   }

   TemptGoal(MobEntity entity, double speed, Predicate temptItemPredicate, boolean canBeScared, double range) {
      this.mob = entity;
      this.speed = speed;
      ...
      this.predicate = TEMPTING_ENTITY_PREDICATE.copy().setPredicate((entityx, world) -> {
         return this.isTemptedBy(entityx);
      });
   }
   ...
   public static class HappyGhastTemptGoal extends TemptGoal {
      public HappyGhastTemptGoal(MobEntity mobEntity, double d, Predicate predicate, boolean bl, double e) {
      ...
      protected void startMovingTo(PlayerEntity player) {
         Vec3d vec3d = player.getEyePos().subtract(this.mob.getPos()).multiply(this.mob.getRandom().nextDouble()).add(this.mob.getPos());
