...
extends Goal {
    ...
    public boolean canStart() {
        return this.mob.isOnGround() && !this.mob.getEntityWorld().getFluidState(this.mob.getBlockPos()).isIn(FluidTags.WATER);
    }
    ...
    public void start() {
        ...
        for (BlockPos blockPos2 : iterable) {
            if (!this.mob.getEntityWorld().getFluidState(blockPos2).isIn(FluidTags.WATER)) continue;
            blockPos = blockPos2;
            break;
        }
        if (blockPos != null) {
