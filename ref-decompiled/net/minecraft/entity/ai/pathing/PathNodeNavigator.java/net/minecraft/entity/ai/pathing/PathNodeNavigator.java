...
public class PathNodeNavigator {
    ...
    private @Nullable Path findPathToAny(PathNode startNode, Map<TargetPathNode, BlockPos> positions, float followRange, int distance, float rangeMultiplier) {
        ...
        while (!this.minHeap.isEmpty() && ++i < j) {
            ...
            for (TargetPathNode targetPathNode : set) {
                if (!(pathNode.getManhattanDistance(targetPathNode) <= (float)distance)) continue;
        ...
        if (bl) {
            path.setDebugInfo(this.minHeap.getNodes(), (PathNode[])set2.toArray(PathNode[]::new), set);
        }
        return path;
    }
