...
extends LandPathNodeMaker {
    ...
    protected PathNodeType getNodeType(int x, int y, int z) {
        return (PathNodeType)((Object)this.pathNodes.computeIfAbsent(BlockPos.asLong(x, y, z), pos -> this.getNodeType(this.context, x, y, z, this.entity)));
    }
    ...
    public PathNodeType getDefaultNodeType(PathContext context, int x, int y, int z) {
        ...
        if (pathNodeType == PathNodeType.WALKABLE || pathNodeType == PathNodeType.OPEN) {
            pathNodeType = BirdPathNodeMaker.getNodeTypeFromNeighbors(context, x, y, z, pathNodeType);
        }
        return pathNodeType;
    }
    ...
    private Iterable<BlockPos> getPotentialEscapePositions(MobEntity entity) {
        ...
        double d = Math.max(0.0, (double)1.1f - box.getLengthZ());
        double e = Math.max(0.0, (double)1.1f - box.getLengthX());
        double f = Math.max(0.0, (double)1.1f - box.getLengthY());
