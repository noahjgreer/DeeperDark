...
public class BirdPathNodeMaker extends LandPathNodeMaker {
   ...
   private Iterable getPotentialEscapePositions(MobEntity entity) {
      ...
      if (!bl) {
         return List.of(BlockPos.ofFloored(box.minX, (double)entity.getBlockY(), box.minZ), BlockPos.ofFloored(box.minX, (double)entity.getBlockY(), box.maxZ), BlockPos.ofFloored(box.maxX, (double)entity.getBlockY(), box.minZ), BlockPos.ofFloored(box.maxX, (double)entity.getBlockY(), box.maxZ));
      } else {
         double d = Math.max(0.0, 1.100000023841858 - box.getLengthZ());
         double e = Math.max(0.0, 1.100000023841858 - box.getLengthX());
         double f = Math.max(0.0, 1.100000023841858 - box.getLengthY());
