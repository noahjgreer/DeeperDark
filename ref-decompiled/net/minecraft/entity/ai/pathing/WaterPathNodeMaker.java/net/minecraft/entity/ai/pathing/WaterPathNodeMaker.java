...
extends PathNodeMaker {
    ...
    public int getSuccessors(PathNode[] successors, PathNode node) {
        ...
        for (Direction direction2 : Direction.Type.HORIZONTAL) {
            ...
            if (!WaterPathNodeMaker.hasPenalty((PathNode)map.get(direction2)) || !WaterPathNodeMaker.hasPenalty((PathNode)map.get(direction3)) || !this.hasNotVisited(pathNode2 = this.getPassableNode(node.x + direction2.getOffsetX() + direction3.getOffsetX(), node.y, node.z + direction2.getOffsetZ() + direction3.getOffsetZ()))) continue;
    ...
    protected PathNodeType addPathNodePos(int x, int y, int z) {
        return (PathNodeType)((Object)this.nodePosToType.computeIfAbsent(BlockPos.asLong(x, y, z), pos -> this.getDefaultNodeType(this.context, x, y, z)));
    }
