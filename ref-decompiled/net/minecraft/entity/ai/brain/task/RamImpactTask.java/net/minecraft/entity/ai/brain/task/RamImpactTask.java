...
extends MultiTickTask<GoatEntity> {
    ...
    protected void keepRunning(ServerWorld serverWorld, GoatEntity goatEntity, long l) {
        ...
        if (!list.isEmpty()) {
            ...
            if (livingEntity.damage(serverWorld, damageSource = serverWorld.getDamageSources().mobAttackNoAggro(goatEntity), f = (float)goatEntity.getAttributeValue(EntityAttributes.ATTACK_DAMAGE))) {
