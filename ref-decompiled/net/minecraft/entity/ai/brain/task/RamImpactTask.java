...
public class RamImpactTask extends MultiTickTask {
   ...
   protected void run(ServerWorld serverWorld, GoatEntity goatEntity, long l) {
      ...
      brain.remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(vec3d, this.speed, 0)));
   }

   protected void keepRunning(ServerWorld serverWorld, GoatEntity goatEntity, long l) {
      List list = serverWorld.getTargets(LivingEntity.class, this.targetPredicate, goatEntity, goatEntity.getBoundingBox());
      Brain brain = goatEntity.getBrain();
      if (!list.isEmpty()) {
         ...
         livingEntity.takeKnockback((double)(m * h) * this.strengthMultiplierFactory.applyAsDouble(goatEntity), this.direction.getX(), this.direction.getZ());
         this.finishRam(serverWorld, goatEntity);
         serverWorld.playSoundFromEntity((Entity)null, goatEntity, (SoundEvent)this.impactSoundFactory.apply(goatEntity), SoundCategory.NEUTRAL, 1.0F, 1.0F);
      } else if (this.shouldSnapHorn(serverWorld, goatEntity)) {
         serverWorld.playSoundFromEntity((Entity)null, goatEntity, (SoundEvent)this.impactSoundFactory.apply(goatEntity), SoundCategory.NEUTRAL, 1.0F, 1.0F);
         boolean bl = goatEntity.dropHorn();
         if (bl) {
            serverWorld.playSoundFromEntity((Entity)null, goatEntity, (SoundEvent)this.hornBreakSoundFactory.apply(goatEntity), SoundCategory.NEUTRAL, 1.0F, 1.0F);
         }

         this.finishRam(serverWorld, goatEntity);
      } else {
         Optional optional = brain.getOptionalRegisteredMemory(MemoryModuleType.WALK_TARGET);
         Optional optional2 = brain.getOptionalRegisteredMemory(MemoryModuleType.RAM_TARGET);
