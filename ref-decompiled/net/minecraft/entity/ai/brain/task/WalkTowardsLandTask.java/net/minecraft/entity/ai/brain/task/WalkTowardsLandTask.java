    ...
    public static Task<PathAwareEntity> create(int range, float speed) {
        ...
        return TaskTriggerer.task(context -> context.group(context.queryMemoryAbsent(MemoryModuleType.ATTACK_TARGET), context.queryMemoryAbsent(MemoryModuleType.WALK_TARGET), context.queryMemoryOptional(MemoryModuleType.LOOK_TARGET)).apply((Applicative)context, (attackTarget, walkTarget, lookTarget) -> (world, entity, time) -> {
            ...
            for (BlockPos blockPos2 : BlockPos.iterateOutwards(blockPos, range, range, range)) {
                ...
                BlockState blockState2 = world.getBlockState(mutable.set((Vec3i)blockPos2, Direction.DOWN));
                if (blockState.isOf(Blocks.WATER) || !world.getFluidState(blockPos2).isEmpty() || !blockState.getCollisionShape(world, blockPos2, shapeContext).isEmpty() || !blockState2.isSideSolidFullSquare(world, mutable, Direction.UP)) continue;
