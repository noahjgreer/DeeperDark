...
public class BreezeMovementUtil {
   ...
   public static boolean canMoveTo(BreezeEntity breeze, Vec3d pos) {
      ...
      if (pos.distanceTo(vec3d) > getMaxMoveDistance(breeze)) {
         return false;
      } else {
         return breeze.getWorld().raycast(new RaycastContext(vec3d, pos, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, breeze)).getType() == HitResult.Type.MISS;
      }
   }
