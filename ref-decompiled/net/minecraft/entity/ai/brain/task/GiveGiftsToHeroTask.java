...
public class GiveGiftsToHeroTask extends MultiTickTask {
   ...
   protected void run(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      villagerEntity.getBrain().remember(MemoryModuleType.INTERACTION_TARGET, (Object)playerEntity);
   ...
   protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      if (this.isCloseEnough(villagerEntity, playerEntity)) {
         ...
         TargetUtil.walkTowards(villagerEntity, (Entity)playerEntity, 0.5F, 5);
      }
   ...
   private void giveGifts(ServerWorld world, VillagerEntity villager, LivingEntity recipient) {
      villager.forEachGiftedItem(world, getGiftLootTable(villager), (worldx, stack) -> {
         TargetUtil.give(villager, stack, recipient.getPos());
      });
   }

   private static RegistryKey getGiftLootTable(VillagerEntity villager) {
      if (villager.isBaby()) {
         return LootTables.HERO_OF_THE_VILLAGE_BABY_GIFT_GAMEPLAY;
      } else {
         Optional optional = villager.getVillagerData().profession().getKey();
         return optional.isEmpty() ? LootTables.HERO_OF_THE_VILLAGE_UNEMPLOYED_GIFT_GAMEPLAY : (RegistryKey)GIFTS.getOrDefault(optional.get(), LootTables.HERO_OF_THE_VILLAGE_UNEMPLOYED_GIFT_GAMEPLAY);
      }
   }
   ...
   static {
      GIFTS = ImmutableMap.builder().put(VillagerProfession.ARMORER, LootTables.HERO_OF_THE_VILLAGE_ARMORER_GIFT_GAMEPLAY).put(VillagerProfession.BUTCHER, LootTables.HERO_OF_THE_VILLAGE_BUTCHER_GIFT_GAMEPLAY).put(VillagerProfession.CARTOGRAPHER, LootTables.HERO_OF_THE_VILLAGE_CARTOGRAPHER_GIFT_GAMEPLAY).put(VillagerProfession.CLERIC, LootTables.HERO_OF_THE_VILLAGE_CLERIC_GIFT_GAMEPLAY).put(VillagerProfession.FARMER, LootTables.HERO_OF_THE_VILLAGE_FARMER_GIFT_GAMEPLAY).put(VillagerProfession.FISHERMAN, LootTables.HERO_OF_THE_VILLAGE_FISHERMAN_GIFT_GAMEPLAY).put(VillagerProfession.FLETCHER, LootTables.HERO_OF_THE_VILLAGE_FLETCHER_GIFT_GAMEPLAY).put(VillagerProfession.LEATHERWORKER, LootTables.HERO_OF_THE_VILLAGE_LEATHERWORKER_GIFT_GAMEPLAY).put(VillagerProfession.LIBRARIAN, LootTables.HERO_OF_THE_VILLAGE_LIBRARIAN_GIFT_GAMEPLAY).put(VillagerProfession.MASON, LootTables.HERO_OF_THE_VILLAGE_MASON_GIFT_GAMEPLAY).put(VillagerProfession.SHEPHERD, LootTables.HERO_OF_THE_VILLAGE_SHEPHERD_GIFT_GAMEPLAY).put(VillagerProfession.TOOLSMITH, LootTables.HERO_OF_THE_VILLAGE_TOOLSMITH_GIFT_GAMEPLAY).put(VillagerProfession.WEAPONSMITH, LootTables.HERO_OF_THE_VILLAGE_WEAPONSMITH_GIFT_GAMEPLAY).build();
   }
}
