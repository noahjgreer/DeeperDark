...
public class BreedTask extends MultiTickTask {
   ...
   public BreedTask(EntityType targetType) {
      this(targetType, 1.0F, 2);
   }

   public BreedTask(EntityType targetType, float speed, int approachDistance) {
   ...
   protected void run(ServerWorld serverWorld, AnimalEntity animalEntity, long l) {
      ...
      animalEntity.getBrain().remember(MemoryModuleType.BREED_TARGET, (Object)animalEntity2);
      animalEntity2.getBrain().remember(MemoryModuleType.BREED_TARGET, (Object)animalEntity);
   ...
   private Optional findBreedTarget(AnimalEntity animal) {
      Optional var10000 = ((LivingTargetCache)animal.getBrain().getOptionalRegisteredMemory(MemoryModuleType.VISIBLE_MOBS).get()).findFirst((entity) -> {
         boolean var10000;
         if (entity.getType() == this.targetType && entity instanceof AnimalEntity animalEntity2) {
            if (animal.canBreedWith(animalEntity2) && !animalEntity2.isPanicking()) {
               var10000 = true;
               return var10000;
            }
         }
