...
extends MultiTickTask<WardenEntity> {
    ...
    public SonicBoomTask() {
        super((Map<MemoryModuleType<?>, MemoryModuleState>)ImmutableMap.of(MemoryModuleType.ATTACK_TARGET, (Object)((Object)MemoryModuleState.VALUE_PRESENT), MemoryModuleType.SONIC_BOOM_COOLDOWN, (Object)((Object)MemoryModuleState.VALUE_ABSENT), MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN, (Object)((Object)MemoryModuleState.REGISTERED), MemoryModuleType.SONIC_BOOM_SOUND_DELAY, (Object)((Object)MemoryModuleState.REGISTERED)), RUN_TIME);
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, WardenEntity wardenEntity, long l) {
        wardenEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(target -> wardenEntity.getLookControl().lookAt(target.getEntityPos()));
        ...
        wardenEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).filter(wardenEntity::isValidTarget).filter(target -> wardenEntity.isInRange((Entity)target, 15.0, 20.0)).ifPresent(target -> {
            Vec3d vec3d = wardenEntity.getEntityPos().add(wardenEntity.getAttachments().getPoint(EntityAttachmentType.WARDEN_CHEST, 0, wardenEntity.getYaw()));
