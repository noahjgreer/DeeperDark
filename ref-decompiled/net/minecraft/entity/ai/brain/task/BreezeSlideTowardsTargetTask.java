...
public class BreezeSlideTowardsTargetTask extends MultiTickTask {
   ...
   protected void run(ServerWorld serverWorld, BreezeEntity breezeEntity, long l) {
      LivingEntity livingEntity = (LivingEntity)breezeEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).orElse((Object)null);
      if (livingEntity != null) {
         ...
         if (bl) {
            ...
            if (vec3d2 != null && BreezeMovementUtil.canMoveTo(breezeEntity, vec3d2) && livingEntity.squaredDistanceTo(vec3d2.x, vec3d2.y, vec3d2.z) > livingEntity.squaredDistanceTo(breezeEntity)) {
               vec3d = vec3d2;
            }
         }
         ...
         breezeEntity.getBrain().remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(BlockPos.ofFloored(vec3d), 0.6F, 1)));
      }
   }
