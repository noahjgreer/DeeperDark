...
extends MultiTickTask<VillagerEntity> {
    ...
    protected void finishRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
        this.raid = null;
        villagerEntity.getBrain().refreshActivities(serverWorld.getEnvironmentAttributes(), serverWorld.getTime(), villagerEntity.getEntityPos());
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
        ...
        if (random.nextInt(200) == 0 && SeekSkyTask.isSkyVisible(serverWorld, villagerEntity, villagerEntity.getBlockPos())) {
            ...
            ProjectileEntity.spawn(new FireworkRocketEntity(villagerEntity.getEntityWorld(), villagerEntity, villagerEntity.getX(), villagerEntity.getEyeY(), villagerEntity.getZ(), itemStack), serverWorld, itemStack);
        }
    }

    private ItemStack createFirework(DyeColor color, int flight) {
        ItemStack itemStack = new ItemStack(Items.FIREWORK_ROCKET);
        itemStack.set(DataComponentTypes.FIREWORKS, new FireworksComponent((byte)flight, List.of(new FireworkExplosionComponent(FireworkExplosionComponent.Type.BURST, IntList.of((int)color.getFireworkColor()), IntList.of(), false, false))));
        return itemStack;
    }
}
