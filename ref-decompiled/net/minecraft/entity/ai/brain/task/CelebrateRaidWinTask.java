...
public class CelebrateRaidWinTask extends MultiTickTask {
   @Nullable
   private Raid raid;
   ...
   protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      if (random.nextInt(200) == 0 && SeekSkyTask.isSkyVisible(serverWorld, villagerEntity, villagerEntity.getBlockPos())) {
         DyeColor dyeColor = (DyeColor)Util.getRandom((Object[])DyeColor.values(), random);
         int i = random.nextInt(3);
         ItemStack itemStack = this.createFirework(dyeColor, i);
         ProjectileEntity.spawn(new FireworkRocketEntity(villagerEntity.getWorld(), villagerEntity, villagerEntity.getX(), villagerEntity.getEyeY(), villagerEntity.getZ(), itemStack), serverWorld, itemStack);
      }
