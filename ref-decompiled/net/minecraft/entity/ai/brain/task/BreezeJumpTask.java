...
public class BreezeJumpTask extends MultiTickTask {
   ...
   private static final ObjectArrayList POSSIBLE_JUMP_ANGLES = new ObjectArrayList(Lists.newArrayList(new Integer[]{40, 55, 60, 75, 80}));
   ...
   public static boolean shouldJump(ServerWorld world, BreezeEntity breeze) {
      if (!breeze.isOnGround() && !breeze.isTouchingWater()) {
         ...
         LivingEntity livingEntity = (LivingEntity)breeze.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).orElse((Object)null);
         if (livingEntity == null) {
            ...
            if (blockPos == null) {
               ...
               if (breeze.getType().isInvalidSpawn(blockState)) {
                  ...
                  breeze.getBrain().remember(MemoryModuleType.BREEZE_JUMP_TARGET, (Object)blockPos);
                  return true;
               }
            }
         }
   ...
   protected void run(ServerWorld serverWorld, BreezeEntity breezeEntity, long l) {
      ...
      serverWorld.playSoundFromEntity((Entity)null, breezeEntity, SoundEvents.ENTITY_BREEZE_CHARGE, SoundCategory.HOSTILE, 1.0F, 1.0F);
      breezeEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.BREEZE_JUMP_TARGET).ifPresent((jumpTarget) -> {
         breezeEntity.lookAt(EntityAnchorArgumentType.EntityAnchor.EYES, jumpTarget.toCenterPos());
      });
   }
   ...
   protected void keepRunning(ServerWorld serverWorld, BreezeEntity breezeEntity, long l) {
      ...
      if (shouldStopInhalingPose(breezeEntity)) {
         Vec3d vec3d = (Vec3d)breezeEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.BREEZE_JUMP_TARGET).flatMap((jumpTarget) -> {
            return getJumpingVelocity(breezeEntity, breezeEntity.getRandom(), Vec3d.ofBottomCenter(jumpTarget));
         }).orElse((Object)null);
         if (vec3d == null) {
         ...
         if (bl) {
            breezeEntity.getBrain().remember(MemoryModuleType.BREEZE_LEAVING_WATER, (Object)Unit.INSTANCE);
         }
   ...
   private static Optional getJumpingVelocity(BreezeEntity breeze, Random random, Vec3d jumpTarget) {
      List list = Util.copyShuffled(POSSIBLE_JUMP_ANGLES, random);
      ...
      do {
         ...
         optional = LongJumpUtil.getJumpingVelocity(breeze, jumpTarget, f, i, false);
