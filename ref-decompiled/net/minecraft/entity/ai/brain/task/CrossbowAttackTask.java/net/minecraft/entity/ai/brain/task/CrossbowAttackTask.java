...
extends MultiTickTask<E> {
    ...
    protected boolean shouldRun(ServerWorld serverWorld, E mobEntity) {
        ...
        return ((LivingEntity)mobEntity).isHolding(Items.CROSSBOW) && TargetUtil.isVisibleInMemory(mobEntity, livingEntity) && TargetUtil.isTargetWithinAttackRange(mobEntity, livingEntity, 0);
    }
    ...
    protected void finishRunning(ServerWorld serverWorld, E mobEntity, long l) {
        ...
        if (((LivingEntity)mobEntity).isHolding(Items.CROSSBOW)) {
            ...
            ((LivingEntity)mobEntity).getActiveItem().set(DataComponentTypes.CHARGED_PROJECTILES, ChargedProjectilesComponent.DEFAULT);
        }
    }
    ...
    private void tickState(E entity, LivingEntity target) {
        if (this.state == CrossbowState.UNCHARGED) {
            ...
            if ((i = ((LivingEntity)entity).getItemUseTime()) >= CrossbowItem.getPullTime(itemStack = ((LivingEntity)entity).getActiveItem(), entity)) {
                ((LivingEntity)entity).stopUsingItem();
                this.state = CrossbowState.CHARGED;
                this.chargingCooldown = 20 + ((Entity)entity).getRandom().nextInt(20);
