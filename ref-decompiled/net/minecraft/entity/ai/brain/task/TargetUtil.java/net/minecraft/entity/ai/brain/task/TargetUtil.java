...
public class TargetUtil {
    ...
    public static void lookAtAndWalkTowardsEachOther(LivingEntity first, LivingEntity second, float speed, int walkCompletionRange) {
        TargetUtil.lookAtEachOther(first, second);
        TargetUtil.walkTowardsEachOther(first, second, speed, walkCompletionRange);
    }
    ...
    public static boolean canSee(Brain<?> brain, MemoryModuleType<? extends LivingEntity> memoryModuleType, EntityType<?> entityType) {
        return TargetUtil.canSee(brain, memoryModuleType, (LivingEntity entity) -> entity.getType() == entityType);
    }

    private static boolean canSee(Brain<?> brain, MemoryModuleType<? extends LivingEntity> memoryType, Predicate<LivingEntity> filter) {
        return brain.getOptionalRegisteredMemory(memoryType).filter(filter).filter(LivingEntity::isAlive).filter(target -> TargetUtil.canSee(brain, target)).isPresent();
    }

    private static void lookAtEachOther(LivingEntity first, LivingEntity second) {
        TargetUtil.lookAt(first, second);
        TargetUtil.lookAt(second, first);
    }
    ...
    private static void walkTowardsEachOther(LivingEntity first, LivingEntity second, float speed, int completionRange) {
        TargetUtil.walkTowards(first, second, speed, completionRange);
        TargetUtil.walkTowards(second, first, speed, completionRange);
    }

    public static void walkTowards(LivingEntity entity, Entity target, float speed, int completionRange) {
        TargetUtil.walkTowards(entity, new EntityLookTarget(target, true), speed, completionRange);
    }

    public static void walkTowards(LivingEntity entity, BlockPos target, float speed, int completionRange) {
        TargetUtil.walkTowards(entity, new BlockPosLookTarget(target), speed, completionRange);
    }
    ...
    public static void give(LivingEntity entity, ItemStack stack, Vec3d targetLocation) {
        ...
        TargetUtil.give(entity, stack, targetLocation, vec3d, 0.3f);
    }
    ...
    public static boolean isTargetWithinAttackRange(MobEntity mob, LivingEntity target, int rangedWeaponReachReduction) {
        ...
        if (item instanceof RangedWeaponItem) {
            ...
            if (mob.canUseRangedWeapon(mob.getMainHandStack())) {
    ...
    public static LivingEntity getCloserEntity(LivingEntity source, Optional<LivingEntity> first, LivingEntity second) {
        ...
        return TargetUtil.getCloserEntity(source, first.get(), second);
    }
    ...
    public static Optional<LivingEntity> getEntity(LivingEntity entity, MemoryModuleType<UUID> uuidMemoryModule) {
        ...
        return optional.map(uuid -> entity.getEntityWorld().getEntity((UUID)uuid)).map(target -> {
    ...
    public static @Nullable Vec3d find(PathAwareEntity entity, int horizontalRange, int verticalRange) {
        ...
        int i = 0;
        while (vec3d != null && !entity.getEntityWorld().getBlockState(BlockPos.ofFloored(vec3d)).canPathfindThrough(NavigationType.WATER) && i++ < 10) {
