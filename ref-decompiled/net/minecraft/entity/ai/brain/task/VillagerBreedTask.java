...
public class VillagerBreedTask extends MultiTickTask {
   ...
   private boolean isReadyToBreed(VillagerEntity villager) {
      ...
      Optional optional = brain.getOptionalRegisteredMemory(MemoryModuleType.BREED_TARGET).filter((passiveEntity) -> {
         return passiveEntity.getType() == EntityType.VILLAGER;
      });
      if (optional.isEmpty()) {
         return false;
      } else {
         return TargetUtil.canSee(brain, MemoryModuleType.BREED_TARGET, EntityType.VILLAGER) && villager.isReadyToBreed() && ((PassiveEntity)optional.get()).isReadyToBreed();
      }
   }
   ...
   private boolean canReachHome(VillagerEntity villager, BlockPos pos, RegistryEntry poiType) {
      Path path = villager.getNavigation().findPathTo(pos, ((PointOfInterestType)poiType.value()).searchDistance());
   ...
   private void setChildHome(ServerWorld world, VillagerEntity child, BlockPos pos) {
      ...
      child.getBrain().remember(MemoryModuleType.HOME, (Object)globalPos);
   }
