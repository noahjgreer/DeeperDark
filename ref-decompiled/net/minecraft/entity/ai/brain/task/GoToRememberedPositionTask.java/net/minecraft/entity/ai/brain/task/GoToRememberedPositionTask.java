...
public class GoToRememberedPositionTask {
    ...
    public static SingleTickTask<PathAwareEntity> createEntityBased(MemoryModuleType<? extends Entity> entityModule, float speed, int range, boolean requiresWalkTarget) {
        return GoToRememberedPositionTask.create(entityModule, speed, range, requiresWalkTarget, Entity::getEntityPos);
    }
    ...
    private static <T> SingleTickTask<PathAwareEntity> create(MemoryModuleType<T> posSource, float speed, int range, boolean requiresWalkTarget, Function<T, Vec3d> posGetter) {
        return TaskTriggerer.task(context -> context.group(context.queryMemoryOptional(MemoryModuleType.WALK_TARGET), context.queryMemoryValue(posSource)).apply((Applicative)context, (walkTarget, posSourcex) -> (world, entity, time) -> {
            Vec3d vec3d4;
            Vec3d vec3d3;
            Vec3d vec3d2;
            ...
            if (!vec3d.isInRange(vec3d2 = (Vec3d)posGetter.apply(context.getValue(posSourcex)), range)) {
                return false;
            }
