...
public class GiveInventoryToLookTargetTask extends MultiTickTask {
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      ((Optional)this.lookTargetFunction.apply(entity)).ifPresent((target) -> {
         TargetUtil.walkTowards(entity, (LookTarget)target, this.speed, 3);
      });
   }
   ...
   public static void playThrowSound(LivingEntity entity, ItemStack stack, Vec3d target) {
      Vec3d vec3d = new Vec3d(0.20000000298023224, 0.30000001192092896, 0.20000000298023224);
      TargetUtil.give(entity, stack, target, vec3d, 0.2F);
      World world = entity.getWorld();
      if (world.getTime() % 7L == 0L && world.random.nextDouble() < 0.9) {
         float f = (Float)Util.getRandom((List)AllayEntity.THROW_SOUND_PITCHES, world.getRandom());
         world.playSoundFromEntity((Entity)null, entity, SoundEvents.ENTITY_ALLAY_ITEM_THROWN, SoundCategory.NEUTRAL, 1.0F, f);
      }
