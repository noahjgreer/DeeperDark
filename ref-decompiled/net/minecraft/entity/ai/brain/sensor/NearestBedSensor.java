...
public class NearestBedSensor extends Sensor {
   ...
   protected void sense(ServerWorld serverWorld, MobEntity mobEntity) {
      if (mobEntity.isBaby()) {
         ...
         Set set = (Set)pointOfInterestStorage.getTypesAndPositions((registryEntry) -> {
            return registryEntry.matchesKey(PointOfInterestTypes.HOME);
         }, predicate, mobEntity.getBlockPos(), 48, PointOfInterestStorage.OccupationStatus.ANY).collect(Collectors.toSet());
         Path path = FindPointOfInterestTask.findPathToPoi(mobEntity, set);
         if (path != null && path.reachesTarget()) {
            BlockPos blockPos = path.getTarget();
            Optional optional = pointOfInterestStorage.getType(blockPos);
            if (optional.isPresent()) {
               mobEntity.getBrain().remember(MemoryModuleType.NEAREST_BED, (Object)blockPos);
            }
         } else if (this.tries < 5) {
