...
public abstract class Sensor {
   ...
   private void updateRange(LivingEntity entity) {
   ...
   public static boolean testTargetPredicate(ServerWorld world, LivingEntity entity, LivingEntity target) {
      return entity.getBrain().hasMemoryModuleWithValue(MemoryModuleType.ATTACK_TARGET, target) ? TARGET_PREDICATE_IGNORE_DISTANCE_SCALING.test(world, entity, target) : TARGET_PREDICATE.test(world, entity, target);
   }

   public static boolean testAttackableTargetPredicate(ServerWorld world, LivingEntity entity, LivingEntity target) {
      return entity.getBrain().hasMemoryModuleWithValue(MemoryModuleType.ATTACK_TARGET, target) ? ATTACKABLE_TARGET_PREDICATE_IGNORE_DISTANCE_SCALING.test(world, entity, target) : ATTACKABLE_TARGET_PREDICATE.test(world, entity, target);
   }

   public static BiPredicate hasTargetBeenAttackableRecently(LivingEntity entity, int ticks) {
      return hasPredicatePassedRecently(ticks, (world, target) -> {
         return testAttackableTargetPredicate(world, entity, target);
      });
   }
   ...
   public static boolean testAttackableTargetPredicateIgnoreVisibility(ServerWorld world, LivingEntity entity, LivingEntity target) {
      return entity.getBrain().hasMemoryModuleWithValue(MemoryModuleType.ATTACK_TARGET, target) ? ATTACKABLE_TARGET_PREDICATE_IGNORE_VISIBILITY_OR_DISTANCE_SCALING.test(world, entity, target) : ATTACKABLE_TARGET_PREDICATE_IGNORE_VISIBILITY.test(world, entity, target);
   }

   static BiPredicate hasPredicatePassedRecently(int times, BiPredicate predicate) {
