...
public class NearestPlayersSensor extends Sensor {
   ...
   protected void sense(ServerWorld world, LivingEntity entity) {
      Stream var10000 = world.getPlayers().stream().filter(EntityPredicates.EXCEPT_SPECTATOR).filter((player) -> {
         return entity.isInRange(player, this.getFollowRange(entity));
      });
      ...
      List list2 = (List)list.stream().filter((player) -> {
         return testTargetPredicate(world, entity, player);
      }).collect(Collectors.toList());
      brain.remember(MemoryModuleType.NEAREST_VISIBLE_PLAYER, (Object)(list2.isEmpty() ? null : (PlayerEntity)list2.get(0)));
      List list3 = list2.stream().filter((player) -> {
         return testAttackableTargetPredicate(world, entity, player);
      }).toList();
      brain.remember(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYERS, (Object)list3);
      brain.remember(MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, (Object)(list3.isEmpty() ? null : (PlayerEntity)list3.get(0)));
   }
