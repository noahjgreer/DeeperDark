...
public class SecondaryPointsOfInterestSensor extends Sensor {
   ...
   protected void sense(ServerWorld serverWorld, VillagerEntity villagerEntity) {
      ...
      for(int j = -4; j <= 4; ++j) {
         for(int k = -2; k <= 2; ++k) {
            for(int l = -4; l <= 4; ++l) {
               ...
               if (((VillagerProfession)villagerEntity.getVillagerData().profession().value()).secondaryJobSites().contains(serverWorld.getBlockState(blockPos2).getBlock())) {
      ...
      if (!list.isEmpty()) {
         brain.remember(MemoryModuleType.SECONDARY_JOB_SITE, (Object)list);
      } else {
