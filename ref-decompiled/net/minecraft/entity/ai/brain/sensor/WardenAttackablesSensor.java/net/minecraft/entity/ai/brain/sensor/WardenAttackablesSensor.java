...
extends NearestLivingEntitiesSensor<WardenEntity> {
    ...
    protected void sense(ServerWorld serverWorld, WardenEntity wardenEntity) {
        ...
        WardenAttackablesSensor.findNearestTarget(wardenEntity, entityx -> entityx.getType() == EntityType.PLAYER).or(() -> WardenAttackablesSensor.findNearestTarget(wardenEntity, entityx -> entityx.getType() != EntityType.PLAYER)).ifPresentOrElse(entityx -> wardenEntity.getBrain().remember(MemoryModuleType.NEAREST_ATTACKABLE, entityx), () -> wardenEntity.getBrain().forget(MemoryModuleType.NEAREST_ATTACKABLE));
    }
