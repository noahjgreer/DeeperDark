...
extends Sensor<MobEntity> {
    ...
    protected void sense(ServerWorld serverWorld, MobEntity mobEntity) {
        ...
        Optional<ItemEntity> optional = list.stream().filter(itemEntity -> mobEntity.canGather(serverWorld, itemEntity.getStack())).filter(itemEntityx -> itemEntityx.isInRange(mobEntity, 32.0)).filter(mobEntity::canSee).findFirst();
