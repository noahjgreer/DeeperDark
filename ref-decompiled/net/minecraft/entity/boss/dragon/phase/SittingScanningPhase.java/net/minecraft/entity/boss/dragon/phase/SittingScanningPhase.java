...
extends AbstractSittingPhase {
    ...
    private final TargetPredicate CLOSE_PLAYER_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(20.0).setPredicate((player, world) -> Math.abs(player.getY() - enderDragonEntity.getY()) <= 10.0);
    private int ticks;
    ...
    public void serverTick(ServerWorld world) {
        ++this.ticks;
        PlayerEntity livingEntity = world.getClosestPlayer(this.CLOSE_PLAYER_PREDICATE, this.dragon, this.dragon.getX(), this.dragon.getY(), this.dragon.getZ());
