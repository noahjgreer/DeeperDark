...
public abstract class DisplayEntity extends Entity {
   ...
   private static final TrackedData TELEPORT_DURATION;
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.setBillboardMode((BillboardMode)view.read("billboard", DisplayEntity.BillboardMode.CODEC).orElse(DisplayEntity.BillboardMode.FIXED));
      ...
      this.setBrightness((Brightness)view.read("brightness", Brightness.CODEC).orElse((Object)null));
   }
   ...
   public final int getTeleportDuration() {
      return (Integer)this.dataTracker.get(TELEPORT_DURATION);
   }
   ...
   private RenderState getLerpedRenderState(RenderState state, float lerpProgress) {
      AffineTransformation affineTransformation = (AffineTransformation)state.transformation.interpolate(lerpProgress);
      float f = state.shadowRadius.lerp(lerpProgress);
      float g = state.shadowStrength.lerp(lerpProgress);
      return new RenderState(new AffineTransformationInterpolator(affineTransformation, getTransformation(this.dataTracker)), this.getBillboardMode(), this.getBrightness(), new FloatLerperImpl(f, this.getShadowRadius()), new FloatLerperImpl(g, this.getShadowStrength()), this.getGlowColorOverride());
   }
   ...
   static {
      ...
      TELEPORT_DURATION = DataTracker.registerData(DisplayEntity.class, TrackedDataHandlerRegistry.INTEGER);
      TRANSLATION = DataTracker.registerData(DisplayEntity.class, TrackedDataHandlerRegistry.VECTOR_3F);
      SCALE = DataTracker.registerData(DisplayEntity.class, TrackedDataHandlerRegistry.VECTOR_3F);
      LEFT_ROTATION = DataTracker.registerData(DisplayEntity.class, TrackedDataHandlerRegistry.QUATERNION_F);
      RIGHT_ROTATION = DataTracker.registerData(DisplayEntity.class, TrackedDataHandlerRegistry.QUATERNION_F);
      ...
      RENDERING_DATA_IDS = IntSet.of(new int[]{TRANSLATION.id(), SCALE.id(), LEFT_ROTATION.id(), RIGHT_ROTATION.id(), BILLBOARD.id(), BRIGHTNESS.id(), SHADOW_RADIUS.id(), SHADOW_STRENGTH.id()});
   }
   ...
   public static record RenderState(AbstractInterpolator transformation, BillboardMode billboardConstraints, int brightnessOverride, FloatLerper shadowRadius, FloatLerper shadowStrength, int glowColorOverride) {
      ...
      public RenderState(AbstractInterpolator abstractInterpolator, BillboardMode billboardMode, int i, FloatLerper floatLerper, FloatLerper floatLerper2, int j) {
   ...
   public static enum BillboardMode implements StringIdentifiable {
      ...
      public static final IntFunction FROM_INDEX = ValueLists.createIndexToValueFunction(BillboardMode::getIndex, values(), (ValueLists.OutOfBoundsHandling)ValueLists.OutOfBoundsHandling.ZERO);
   ...
   static record AffineTransformationInterpolator(AffineTransformation previous, AffineTransformation current) implements AbstractInterpolator {
      ...
      // $FF: synthetic method
      public Object interpolate(final float delta) {
   ...
   public static class TextDisplayEntity extends DisplayEntity {
      ...
      protected void readCustomData(ReadView view) {
         ...
         Optional optional = view.read("alignment", DisplayEntity.TextDisplayEntity.TextAlignment.CODEC);
         if (optional.isPresent()) {
            byte var10000;
            switch (((TextAlignment)optional.get()).ordinal()) {
               case 0:
                  var10000 = b;
                  break;
               case 1:
         ...
         Optional optional2 = view.read("text", TextCodecs.CODEC);
         if (optional2.isPresent()) {
            try {
               ...
               if (var6 instanceof ServerWorld) {
                  ...
                  Text text = Texts.parse(serverCommandSource, (Text)((Text)optional2.get()), this, 0);
                  this.setText(text);
               } else {
      ...
      static {
         ...
         TEXT_RENDERING_DATA_IDS = IntSet.of(new int[]{TEXT.id(), LINE_WIDTH.id(), BACKGROUND.id(), TEXT_OPACITY.id(), TEXT_DISPLAY_FLAGS.id()});
      }
   ...
   public static class ItemDisplayEntity extends DisplayEntity {
      ...
      public final ItemDisplayContext getItemDisplayContext() {
         return (ItemDisplayContext)ItemDisplayContext.FROM_INDEX.apply((Byte)this.dataTracker.get(ITEM_DISPLAY));
      }
      ...
      public static record Data(ItemStack itemStack, ItemDisplayContext itemTransform) {
         public Data(ItemStack itemStack, ItemDisplayContext itemDisplayContext) {
