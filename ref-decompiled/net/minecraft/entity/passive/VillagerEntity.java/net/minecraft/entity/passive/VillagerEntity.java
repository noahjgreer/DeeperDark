...
VillagerDataContainer {
    ...
    public static final Map<MemoryModuleType<GlobalPos>, BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>> POINTS_OF_INTEREST = ImmutableMap.of(MemoryModuleType.HOME, (villager, poi) -> poi.matchesKey(PointOfInterestTypes.HOME), MemoryModuleType.JOB_SITE, (villager, poi) -> villager.getVillagerData().profession().value().heldWorkstation().test((RegistryEntry<PointOfInterestType>)poi), MemoryModuleType.POTENTIAL_JOB_SITE, (villager, poi) -> VillagerProfession.IS_ACQUIRABLE_JOB_SITE.test((RegistryEntry<PointOfInterestType>)poi), MemoryModuleType.MEETING_POINT, (villager, poi) -> poi.matchesKey(PointOfInterestTypes.MEETING));
    ...
    private void initBrain(Brain<VillagerEntity> brain) {
        ...
        if (this.isBaby()) {
            ...
            brain.setTaskList(Activity.WORK, (ImmutableList<Pair<Integer, Task<VillagerEntity>>>)VillagerTaskListProvider.createWorkTasks(registryEntry, 0.5f), (Set<Pair<MemoryModuleType<?>, MemoryModuleState>>)ImmutableSet.of((Object)Pair.of(MemoryModuleType.JOB_SITE, (Object)((Object)MemoryModuleState.VALUE_PRESENT))));
        }
        brain.setTaskList(Activity.CORE, VillagerTaskListProvider.createCoreTasks(registryEntry, 0.5f));
        brain.setTaskList(Activity.MEET, (ImmutableList<Pair<Integer, Task<VillagerEntity>>>)VillagerTaskListProvider.createMeetTasks(registryEntry, 0.5f), (Set<Pair<MemoryModuleType<?>, MemoryModuleState>>)ImmutableSet.of((Object)Pair.of(MemoryModuleType.MEETING_POINT, (Object)((Object)MemoryModuleState.VALUE_PRESENT))));
    ...
    protected void mobTick(ServerWorld world) {
        ...
        if (!this.isAiDisabled() && this.random.nextInt(100) == 0 && (raid = world.getRaidAt(this.getBlockPos())) != null && raid.isActive() && !raid.isFinished()) {
    ...
    public boolean shouldRestock(ServerWorld world) {
        ...
        boolean bl = m > l;
        long n = world.getDay();
        boolean bl2 = this.lastRestockCheckTime > 0L && n > this.lastRestockCheckTime;
    ...
    public @Nullable VillagerEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        RegistryEntry<VillagerType> registryEntry = d < 0.5 ? serverWorld.getRegistryManager().getEntryOrThrow(VillagerType.forBiome(serverWorld.getBiome(this.getBlockPos()))) : (d < 0.75 ? this.getVillagerData().type() : ((VillagerEntity)passiveEntity).getVillagerData().type());
        VillagerEntity villagerEntity = new VillagerEntity(EntityType.VILLAGER, (World)serverWorld, registryEntry);
    ...
    public boolean canGather(ServerWorld world, ItemStack stack) {
        ...
        return (stack.isIn(ItemTags.VILLAGER_PICKS_UP) || this.getVillagerData().profession().value().gatherableItems().contains((Object)item)) && this.getInventory().canInsert(stack);
    }
    ...
    protected void fillRecipes(ServerWorld world) {
        ...
        RegistryKey registryKey = villagerData.profession().getKey().orElse(null);
        if (registryKey == null) {
            return;
        }
        Int2ObjectMap<TradeOffers.Factory[]> int2ObjectMap2 = this.getEntityWorld().getEnabledFeatures().contains(FeatureFlags.TRADE_REBALANCE) ? ((int2ObjectMap = TradeOffers.REBALANCED_PROFESSION_TO_LEVELED_TRADE.get(registryKey)) != null ? int2ObjectMap : TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(registryKey)) : TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(registryKey);
    ...
    protected <T> boolean setApplicableComponent(ComponentType<T> type, T value) {
        if (type == DataComponentTypes.VILLAGER_VARIANT) {
            RegistryEntry<VillagerType> registryEntry = VillagerEntity.castComponentValue(DataComponentTypes.VILLAGER_VARIANT, value);
