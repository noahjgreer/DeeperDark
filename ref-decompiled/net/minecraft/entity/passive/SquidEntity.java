...
public class SquidEntity extends WaterAnimalEntity {
   ...
   public SquidEntity(EntityType entityType, World world) {
   ...
   public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
      return (PassiveEntity)EntityType.SQUID.create(world, SpawnReason.BREEDING);
   }
   ...
   private Vec3d applyBodyRotations(Vec3d shootVector) {
      Vec3d vec3d = shootVector.rotateX(this.lastTiltAngle * 0.017453292F);
      vec3d = vec3d.rotateY(-this.lastBodyYaw * 0.017453292F);
      return vec3d;
   }
   ...
   private void squirt() {
      ...
      for(int i = 0; i < 30; ++i) {
         ...
         ((ServerWorld)this.getWorld()).spawnParticles(this.getInkParticle(), vec3d.x, vec3d.y + 0.5, vec3d.z, 0, vec3d3.x, vec3d3.y, vec3d3.z, 0.10000000149011612);
      }
   ...
   public boolean hasSwimmingVector() {
      return this.swimVec.lengthSquared() > 9.999999747378752E-6;
   }
   ...
   static class SwimGoal extends Goal {
      ...
      public void tick() {
         ...
         if (i > 100) {
            ...
            this.squid.swimVec = new Vec3d((double)(MathHelper.cos(f) * 0.2F), (double)(-0.1F + this.squid.getRandom().nextFloat() * 0.2F), (double)(MathHelper.sin(f) * 0.2F));
         }
   ...
   class EscapeAttackerGoal extends Goal {
      ...
      public void tick() {
         ...
         if (livingEntity != null) {
            ...
            if (this.timer % 10 == 5) {
               SquidEntity.this.getWorld().addParticleClient(ParticleTypes.BUBBLE, SquidEntity.this.getX(), SquidEntity.this.getY(), SquidEntity.this.getZ(), 0.0, 0.0, 0.0);
            }
