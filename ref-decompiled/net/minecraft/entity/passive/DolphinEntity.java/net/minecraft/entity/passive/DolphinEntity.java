...
extends WaterAnimalEntity {
    ...
    public static DefaultAttributeContainer.Builder createDolphinAttributes() {
        return MobEntity.createMobAttributes().add(EntityAttributes.MAX_HEALTH, 10.0).add(EntityAttributes.MOVEMENT_SPEED, 1.2f).add(EntityAttributes.ATTACK_DAMAGE, 3.0);
    }
    ...
    public void tick() {
        ...
        if (this.getEntityWorld().isClient() && this.isTouchingWater() && this.getVelocity().lengthSquared() > 0.03) {
            ...
            for (int i = 0; i < 2; ++i) {
                this.getEntityWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * (double)h + (double)f, this.getY() - vec3d.y, this.getZ() - vec3d.z * (double)h + (double)g, 0.0, 0.0, 0.0);
                this.getEntityWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * (double)h - (double)f, this.getY() - vec3d.y, this.getZ() - vec3d.z * (double)h - (double)g, 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    private void spawnParticlesAround(ParticleEffect parameters) {
        for (int i = 0; i < 7; ++i) {
            ...
            this.getEntityWorld().addParticleClient(parameters, this.getParticleX(1.0), this.getRandomBodyY() + 0.2, this.getParticleZ(1.0), d, e, f);
        }
    }
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (!itemStack.isEmpty() && itemStack.isIn(ItemTags.FISHES)) {
            ...
            if (this.isBaby()) {
                ...
                this.growUp(DolphinEntity.toGrowUpAge(-this.breedingAge), true);
            } else {
    ...
    protected boolean isNearTarget() {
        ...
        if (blockPos != null) {
            return blockPos.isWithinDistance(this.getEntityPos(), 12.0);
        }
        return false;
    }
    ...
    extends Goal {
        ...
        @Override
        public boolean canStart() {
            this.closestPlayer = SwimWithPlayerGoal.getServerWorld(this.dolphin).getClosestPlayer(CLOSE_PLAYER_PREDICATE, this.dolphin);
