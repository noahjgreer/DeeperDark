...
public class WolfEntity extends TameableEntity implements Angerable {
   ...
   private static final TrackedData SOUND_VARIANT;
   public static final TargetPredicate.EntityPredicate FOLLOW_TAMED_PREDICATE;
   ...
   public WolfEntity(EntityType entityType, World world) {
   ...
   private RegistryEntry getVariant() {
   ...
   private RegistryEntry getSoundVariant() {
      return (RegistryEntry)this.dataTracker.get(SOUND_VARIANT);
   }

   private void setSoundVariant(RegistryEntry soundVariant) {
   ...
   public Object get(ComponentType type) {
      if (type == DataComponentTypes.WOLF_VARIANT) {
         ...
         return type == DataComponentTypes.WOLF_COLLAR ? castComponentValue(type, this.getCollarColor()) : super.get(type);
      }
   }
   ...
   protected boolean setApplicableComponent(ComponentType type, Object value) {
      if (type == DataComponentTypes.WOLF_VARIANT) {
         this.setVariant((RegistryEntry)castComponentValue(DataComponentTypes.WOLF_VARIANT, value));
         return true;
      } else if (type == DataComponentTypes.WOLF_SOUND_VARIANT) {
         this.setSoundVariant((RegistryEntry)castComponentValue(DataComponentTypes.WOLF_SOUND_VARIANT, value));
         return true;
      } else if (type == DataComponentTypes.WOLF_COLLAR) {
         this.setCollarColor((DyeColor)castComponentValue(DataComponentTypes.WOLF_COLLAR, value));
         return true;
      } else {
   ...
   public static DefaultAttributeContainer.Builder createWolfAttributes() {
      return AnimalEntity.createAnimalAttributes().add(EntityAttributes.MOVEMENT_SPEED, 0.30000001192092896).add(EntityAttributes.MAX_HEALTH, 8.0).add(EntityAttributes.ATTACK_DAMAGE, 4.0);
   }

   protected void initDataTracker(DataTracker.Builder builder) {
      super.initDataTracker(builder);
      Registry registry = this.getRegistryManager().getOrThrow(RegistryKeys.WOLF_SOUND_VARIANT);
      builder.add(VARIANT, Variants.getOrDefaultOrThrow(this.getRegistryManager(), WolfVariants.DEFAULT));
      TrackedData var10001 = SOUND_VARIANT;
      Optional var10002 = registry.getOptional(WolfSoundVariants.CLASSIC);
   ...
   protected void writeCustomData(WriteView view) {
      ...
      this.getSoundVariant().getKey().ifPresent((soundVariant) -> {
         view.put("sound_variant", RegistryKey.createCodec(RegistryKeys.WOLF_SOUND_VARIANT), soundVariant);
      });
   }
   ...
   protected void readCustomData(ReadView view) {
      ...
      view.read("sound_variant", RegistryKey.createCodec(RegistryKeys.WOLF_SOUND_VARIANT)).flatMap((soundVariantKey) -> {
         return this.getRegistryManager().getOrThrow(RegistryKeys.WOLF_SOUND_VARIANT).getOptional(soundVariantKey);
   ...
   public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
      if (entityData instanceof WolfData wolfData) {
         ...
         if (optional.isPresent()) {
            ...
            entityData = new WolfData((RegistryEntry)optional.get());
         }
      }
   ...
   protected SoundEvent getAmbientSound() {
      if (this.hasAngerTime()) {
         ...
         return this.isTamed() && this.getHealth() < 20.0F ? (SoundEvent)((WolfSoundVariant)this.getSoundVariant().value()).whineSound().value() : (SoundEvent)((WolfSoundVariant)this.getSoundVariant().value()).pantSound().value();
      } else {
   ...
   protected SoundEvent getHurtSound(DamageSource source) {
      return this.shouldArmorAbsorbDamage(source) ? SoundEvents.ITEM_WOLF_ARMOR_DAMAGE : (SoundEvent)((WolfSoundVariant)this.getSoundVariant().value()).hurtSound().value();
   }
         ...
         if (this.isTouchingWaterOrRain()) {
            ...
            if (this.shakeProgress > 0.4F) {
               ...
               for(int j = 0; j < i; ++j) {
                  ...
                  this.getWorld().addParticleClient(ParticleTypes.SPLASH, this.getX() + (double)g, (double)(f + 0.8F), this.getZ() + (double)h, vec3d.x, vec3d.y, vec3d.z);
               }
            }
         }
   ...
   public float getBegAnimationProgress(float tickProgress) {
      return MathHelper.lerp(tickProgress, this.lastBegAnimationProgress, this.begAnimationProgress) * 0.15F * 3.1415927F;
   }
   ...
   protected void applyDamage(ServerWorld world, DamageSource source, float amount) {
      if (!this.shouldArmorAbsorbDamage(source)) {
         ...
         itemStack.damage(MathHelper.ceil(amount), this, (EquipmentSlot)EquipmentSlot.BODY);
      ...
      if (this.isTamed()) {
         if (this.isBreedingItem(itemStack) && this.getHealth() < this.getMaxHealth()) {
            ...
            if (this.canEquip(itemStack, EquipmentSlot.BODY) && !this.isWearingBodyArmor() && this.isOwner(player) && !this.isBaby()) {
               ...
               itemStack.decrementUnlessCreative(1, player);
               return ActionResult.SUCCESS;
            } else if (this.isInSittingPose() && this.isWearingBodyArmor() && this.isOwner(player) && this.getBodyArmor().isDamaged() && this.getBodyArmor().canRepairWith(itemStack)) {
   ...
   public WolfEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
      WolfEntity wolfEntity = (WolfEntity)EntityType.WOLF.create(serverWorld, SpawnReason.BREEDING);
   ...
   static {
      ...
      SOUND_VARIANT = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.WOLF_SOUND_VARIANT);
      FOLLOW_TAMED_PREDICATE = (entity, world) -> {
   ...
   class AvoidLlamaGoal extends FleeEntityGoal {
      ...
      public AvoidLlamaGoal(final WolfEntity wolf, final Class fleeFromType, final float distance, final double slowSpeed, final double fastSpeed) {
   ...
   public static class WolfData extends PassiveEntity.PassiveData {
      public final RegistryEntry variant;

      public WolfData(RegistryEntry variant) {
         super(false);
