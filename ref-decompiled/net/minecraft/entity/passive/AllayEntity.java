...
public class AllayEntity extends PathAwareEntity implements InventoryOwner, Vibrations {
   ...
   public AllayEntity(EntityType entityType, World world) {
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.setDuplicationCooldown((long)view.getInt("DuplicationCooldown", 0));
   }
   ...
   private void duplicate() {
      AllayEntity allayEntity = (AllayEntity)EntityType.ALLAY.create(this.getWorld(), SpawnReason.BREEDING);
      if (allayEntity != null) {
   ...
   class VibrationCallback implements Vibrations.Callback {
      ...
      public void accept(ServerWorld world, BlockPos pos, RegistryEntry event, @Nullable Entity sourceEntity, @Nullable Entity entity, float distance) {
         if (event.matches((RegistryEntry)GameEvent.NOTE_BLOCK_PLAY)) {
   ...
   private class JukeboxEventListener implements GameEventListener {
      ...
      public boolean listen(ServerWorld world, RegistryEntry event, GameEvent.Emitter emitter, Vec3d emitterPos) {
         if (event.matches((RegistryEntry)GameEvent.JUKEBOX_PLAY)) {
            AllayEntity.this.updateJukeboxPos(BlockPos.ofFloored(emitterPos), true);
            return true;
         } else if (event.matches((RegistryEntry)GameEvent.JUKEBOX_STOP_PLAY)) {
