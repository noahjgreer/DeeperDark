...
implements ItemSteerable {
    ...
    public @Nullable LivingEntity getControllingPassenger() {
        ...
        Entity entity;
        if (this.hasSaddleEquipped() && (entity = this.getFirstPassenger()) instanceof PlayerEntity && (playerEntity = (PlayerEntity)entity).isHolding(Items.WARPED_FUNGUS_ON_A_STICK)) {
            return playerEntity;
        }
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        for (BlockPos blockPos : set) {
            double g;
            if (this.getEntityWorld().getFluidState(blockPos).isIn(FluidTags.LAVA) || !Dismounting.canDismountInBlock(g = this.getEntityWorld().getDismountHeight(blockPos))) continue;
            Vec3d vec3d2 = Vec3d.ofCenter(blockPos, g);
            for (EntityPose entityPose : passenger.getPoses()) {
                Box box = passenger.getBoundingBox(entityPose);
                if (!Dismounting.canPlaceEntityAt(this.getEntityWorld(), passenger, box.offset(vec3d2))) continue;
    ...
    public void tick() {
        ...
        if (!this.isAiDisabled()) {
            ...
            boolean bl2 = entity instanceof StriderEntity && (striderEntity = (StriderEntity)entity).isCold();
            this.setCold(!bl || bl2);
        }
        super.tick();
        this.updateFloating();
    ...
    private void updateFloating() {
        if (this.isInLava()) {
            ...
            if (!shapeContext.isAbove(FluidBlock.COLLISION_SHAPE, this.getBlockPos(), true) || this.getEntityWorld().getFluidState(this.getBlockPos().up()).isIn(FluidTags.LAVA)) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (bl && !this.isSilent()) {
            this.getEntityWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_STRIDER_EAT, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
        }
        return actionResult;
    }
