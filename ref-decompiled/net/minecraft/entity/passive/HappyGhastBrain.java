...
public class HappyGhastBrain {
   ...
   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(1, new TemptTask((entity) -> {
         return 1.25F;
      }, (entity) -> {
         return 3.0;
      }, true)), Pair.of(2, WalkTowardsEntityTask.create(MOVE_TOWARDS_FRIENDLY_ENTITY_RANGE, (player) -> {
         return 1.1F;
      }, MemoryModuleType.NEAREST_VISIBLE_PLAYER, true)), Pair.of(3, WalkTowardsEntityTask.create(MOVE_TOWARDS_FRIENDLY_ENTITY_RANGE, (adult) -> {
         return 1.1F;
      }, MemoryModuleType.NEAREST_VISIBLE_ADULT, true)), Pair.of(4, new RandomTask(ImmutableList.of(Pair.of(StrollTask.createSolidTargeting(1.0F), 1), Pair.of(GoToLookTargetTask.create(1.0F, 3), 1))))));
   }
   ...
   public static void updateActivities(HappyGhastEntity happyGhast) {
      happyGhast.getBrain().resetPossibleActivities((List)ImmutableList.of(Activity.PANIC, Activity.IDLE));
   }

   static {
      SENSORS = ImmutableList.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.HURT_BY, SensorType.HAPPY_GHAST_TEMPTATIONS, SensorType.NEAREST_ADULT_ANY_TYPE, SensorType.NEAREST_PLAYERS);
      MEMORY_MODULES = ImmutableList.of(MemoryModuleType.WALK_TARGET, MemoryModuleType.LOOK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.TEMPTING_PLAYER, MemoryModuleType.TEMPTATION_COOLDOWN_TICKS, MemoryModuleType.IS_TEMPTED, MemoryModuleType.BREED_TARGET, MemoryModuleType.IS_PANICKING, MemoryModuleType.HURT_BY, MemoryModuleType.NEAREST_VISIBLE_ADULT, new MemoryModuleType[]{MemoryModuleType.NEAREST_PLAYERS, MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYERS});
   }
}
