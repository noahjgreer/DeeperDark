...
public class SnifferBrain {
   ...
   private static void addCoreActivities(Brain brain) {
      brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8F), new FleeTask(2.0F) {
         protected void run(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
            ...
            super.run(serverWorld, (PathAwareEntity)snifferEntity, l);
         }
   ...
   static class SearchingTask extends MultiTickTask {
      ...
      protected boolean shouldKeepRunning(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
         if (!snifferEntity.canTryToDig()) {
            ...
            Optional optional = snifferEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.WALK_TARGET).map(WalkTarget::getLookTarget).map(LookTarget::getBlockPos);
            Optional optional2 = snifferEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.SNIFFER_SNIFFING_TARGET);
      ...
      protected void finishRunning(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
         if (snifferEntity.canDig() && snifferEntity.canTryToDig()) {
            snifferEntity.getBrain().remember(MemoryModuleType.SNIFFER_DIGGING, (Object)true);
         }
   ...
   static class FinishDiggingTask extends MultiTickTask {
      ...
      protected void finishRunning(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
         ...
         snifferEntity.getBrain().remember(MemoryModuleType.SNIFFER_HAPPY, (Object)true);
      }
      ...
      protected void finishRunning(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
         ...
         if (bl) {
            snifferEntity.findSniffingTargetPos().ifPresent((pos) -> {
               snifferEntity.getBrain().remember(MemoryModuleType.SNIFFER_SNIFFING_TARGET, (Object)pos);
               snifferEntity.getBrain().remember(MemoryModuleType.WALK_TARGET, (Object)(new WalkTarget(pos, 1.25F, 0)));
            });
         }
