...
public class FrogBrain {
   ...
   protected static void coolDownLongJump(FrogEntity frog, Random random) {
      frog.getBrain().remember(MemoryModuleType.LONG_JUMP_COOLING_DOWN, (Object)LONG_JUMP_COOLDOWN_RANGE.get(random));
   }
   ...
   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(0, new BreedTask(EntityType.FROG)), Pair.of(1, new TemptTask((frog) -> {
         return 1.25F;
      })), Pair.of(2, UpdateAttackTargetTask.create((world, frog) -> {
         return isNotBreeding(frog);
      }, (world, frog) -> {
         return frog.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
      })), Pair.of(3, WalkTowardsLandTask.create(6, 1.0F)), Pair.of(4, new RandomTask(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableList.of(Pair.of(StrollTask.create(1.0F), 1), Pair.of(GoToLookTargetTask.create(1.0F, 3), 1), Pair.of(new CroakTask(), 3), Pair.of(TaskTriggerer.predicate(Entity::isOnGround), 2))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_IN_WATER, MemoryModuleState.VALUE_ABSENT)));
   }

   private static void addSwimActivities(Brain brain) {
      brain.setTaskList(Activity.SWIM, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(1, new TemptTask((frog) -> {
         return 1.25F;
      })), Pair.of(2, UpdateAttackTargetTask.create((world, frog) -> {
         return isNotBreeding(frog);
      }, (world, frog) -> {
         return frog.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
      })), Pair.of(3, WalkTowardsLandTask.create(8, 1.5F)), Pair.of(5, new CompositeTask(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableSet.of(), CompositeTask.Order.ORDERED, CompositeTask.RunMode.TRY_ALL, ImmutableList.of(Pair.of(StrollTask.createDynamicRadius(0.75F), 1), Pair.of(StrollTask.create(1.0F, true), 1), Pair.of(GoToLookTargetTask.create(1.0F, 3), 1), Pair.of(TaskTriggerer.predicate(Entity::isTouchingWater), 5))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_IN_WATER, MemoryModuleState.VALUE_PRESENT)));
   }

   private static void addLaySpawnActivities(Brain brain) {
      brain.setTaskList(Activity.LAY_SPAWN, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(1, UpdateAttackTargetTask.create((world, frog) -> {
         return isNotBreeding(frog);
      }, (world, frog) -> {
         return frog.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_ATTACKABLE);
      })), Pair.of(2, WalkTowardsWaterTask.create(8, 1.0F)), Pair.of(3, LayFrogSpawnTask.create(Blocks.FROGSPAWN)), Pair.of(4, new RandomTask(ImmutableList.of(Pair.of(StrollTask.create(1.0F), 2), Pair.of(GoToLookTargetTask.create(1.0F, 3), 1), Pair.of(new CroakTask(), 2), Pair.of(TaskTriggerer.predicate(Entity::isOnGround), 1))))), ImmutableSet.of(Pair.of(MemoryModuleType.LONG_JUMP_MID_JUMP, MemoryModuleState.VALUE_ABSENT), Pair.of(MemoryModuleType.IS_PREGNANT, MemoryModuleState.VALUE_PRESENT)));
   }

   private static void addLongJumpActivities(Brain brain) {
      brain.setTaskList(Activity.LONG_JUMP, ImmutableList.of(Pair.of(0, new LeapingChargeTask(LONG_JUMP_COOLDOWN_RANGE, SoundEvents.ENTITY_FROG_STEP)), Pair.of(1, new BiasedLongJumpTask(LONG_JUMP_COOLDOWN_RANGE, 2, 4, 3.5714288F, (frog) -> {
   ...
   private static void addTongueActivities(Brain brain) {
      brain.setTaskList(Activity.TONGUE, 0, ImmutableList.of(ForgetAttackTargetTask.create(), new FrogEatEntityTask(SoundEvents.ENTITY_FROG_TONGUE, SoundEvents.ENTITY_FROG_EAT)), (MemoryModuleType)MemoryModuleType.ATTACK_TARGET);
   }

   private static boolean shouldJumpTo(MobEntity frog, BlockPos pos) {
      ...
      if (world.getFluidState(pos).isEmpty() && world.getFluidState(blockPos).isEmpty() && world.getFluidState(pos.up()).isEmpty()) {
         ...
         if (!blockState.isIn(BlockTags.FROG_PREFER_JUMP_TO) && !blockState2.isIn(BlockTags.FROG_PREFER_JUMP_TO)) {
            ...
            return pathNodeType != PathNodeType.TRAPDOOR && (!blockState.isAir() || pathNodeType2 != PathNodeType.TRAPDOOR) ? LongJumpTask.shouldJumpTo(frog, pos) : true;
         } else {
            return true;
         }
      } else {
   ...
   public static void updateActivities(FrogEntity frog) {
      frog.getBrain().resetPossibleActivities((List)ImmutableList.of(Activity.TONGUE, Activity.LAY_SPAWN, Activity.LONG_JUMP, Activity.SWIM, Activity.IDLE));
   }
