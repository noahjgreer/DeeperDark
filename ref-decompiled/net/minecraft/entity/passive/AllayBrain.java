...
public class AllayBrain {
   ...
   private static void addCoreActivities(Brain brain) {
      brain.setTaskList(Activity.CORE, 0, ImmutableList.of(new StayAboveWaterTask(0.8F), new FleeTask(2.5F), new UpdateLookControlTask(45, 90), new MoveToTargetTask(), new TickCooldownTask(MemoryModuleType.LIKED_NOTEBLOCK_COOLDOWN_TICKS), new TickCooldownTask(MemoryModuleType.ITEM_PICKUP_COOLDOWN_TICKS)));
   }

   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, WalkTowardsNearestVisibleWantedItemTask.create((allay) -> {
         return true;
   ...
   public static void rememberNoteBlock(LivingEntity allay, BlockPos pos) {
      ...
      Optional optional = brain.getOptionalRegisteredMemory(MemoryModuleType.LIKED_NOTEBLOCK);
      if (optional.isEmpty()) {
         brain.remember(MemoryModuleType.LIKED_NOTEBLOCK, (Object)globalPos);
         brain.remember(MemoryModuleType.LIKED_NOTEBLOCK_COOLDOWN_TICKS, (int)600);
