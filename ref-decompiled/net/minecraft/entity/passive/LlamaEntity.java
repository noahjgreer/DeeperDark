...
public class LlamaEntity extends AbstractDonkeyEntity implements RangedAttackMob {
   ...
   public LlamaEntity(EntityType entityType, World world) {
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.setVariant((Variant)view.read("Variant", LlamaEntity.Variant.INDEX_CODEC).orElse(LlamaEntity.Variant.DEFAULT));
   }
   ...
   protected void initGoals() {
      ...
      this.goalSelector.add(5, new TemptGoal(this, 1.25, (stack) -> {
         return stack.isIn(ItemTags.LLAMA_TEMPT_ITEMS);
      }, false));
   ...
   public Variant getVariant() {
      return LlamaEntity.Variant.byIndex((Integer)this.dataTracker.get(VARIANT));
   }
   ...
   public Object get(ComponentType type) {
      return type == DataComponentTypes.LLAMA_VARIANT ? castComponentValue(type, this.getVariant()) : super.get(type);
   }
   ...
   protected boolean setApplicableComponent(ComponentType type, Object value) {
      if (type == DataComponentTypes.LLAMA_VARIANT) {
         this.setVariant((Variant)castComponentValue(DataComponentTypes.LLAMA_VARIANT, value));
         return true;
      } else {
   ...
   protected boolean receiveFood(PlayerEntity player, ItemStack item) {
      ...
      if (bl && !this.isSilent()) {
         ...
         if (soundEvent != null) {
            this.getWorld().playSound((Entity)null, this.getX(), this.getY(), this.getZ(), (SoundEvent)this.getEatSound(), this.getSoundCategory(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
         }
      }
   ...
   public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
      ...
      if (entityData instanceof LlamaData) {
         ...
         variant = (Variant)Util.getRandom((Object[])LlamaEntity.Variant.values(), random);
   ...
   protected LlamaEntity createChild() {
      return (LlamaEntity)EntityType.LLAMA.create(this.getWorld(), SpawnReason.BREEDING);
   }
   ...
   private void spitAt(LivingEntity target) {
      ...
      if (!this.isSilent()) {
         this.getWorld().playSound((Entity)null, this.getX(), this.getY(), this.getZ(), (SoundEvent)SoundEvents.ENTITY_LLAMA_SPIT, this.getSoundCategory(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
      }
   ...
   static {
      ...
      BABY_BASE_DIMENSIONS = EntityType.LLAMA.getDimensions().withAttachments(EntityAttachments.builder().add(EntityAttachmentType.PASSENGER, 0.0F, EntityType.LLAMA.getHeight() - 0.8125F, -0.3F)).scaled(0.5F);
   }
   ...
   static class SpitRevengeGoal extends RevengeGoal {
      ...
      public boolean shouldContinue() {
         ...
         if (var2 instanceof LlamaEntity llamaEntity) {
            if (llamaEntity.spit) {
   ...
   private static class ChaseWolvesGoal extends ActiveTargetGoal {
      public ChaseWolvesGoal(LlamaEntity llama) {
         super(llama, WolfEntity.class, 16, false, true, (wolf, world) -> {
