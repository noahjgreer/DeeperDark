...
extends FishEntity {
    ...
    public void tick() {
        List<?> list;
        super.tick();
        if (this.hasOtherFishInGroup() && this.getEntityWorld().random.nextInt(200) == 1 && (list = this.getEntityWorld().getNonSpectatingEntities(this.getClass(), this.getBoundingBox().expand(8.0, 8.0, 8.0))).size() <= 1) {
            this.groupSize = 1;
        }
    }
