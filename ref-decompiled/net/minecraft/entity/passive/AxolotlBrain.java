...
public class AxolotlBrain {
   ...
   private static void addIdleActivities(Brain brain) {
      brain.setTaskList(Activity.IDLE, ImmutableList.of(Pair.of(0, LookAtMobWithIntervalTask.follow(EntityType.PLAYER, 6.0F, UniformIntProvider.create(30, 60))), Pair.of(1, new BreedTask(EntityType.AXOLOTL, 0.2F, 2)), Pair.of(2, new RandomTask(ImmutableList.of(Pair.of(new TemptTask(AxolotlBrain::getTemptedSpeed), 1), Pair.of(WalkTowardsEntityTask.create(WALK_TOWARD_ADULT_RANGE, AxolotlBrain::getAdultFollowingSpeed, MemoryModuleType.NEAREST_VISIBLE_ADULT, false), 1)))), Pair.of(3, UpdateAttackTargetTask.create(AxolotlBrain::getAttackTarget)), Pair.of(3, SeekWaterTask.create(6, 0.15F)), Pair.of(4, new CompositeTask(ImmutableMap.of(MemoryModuleType.WALK_TARGET, MemoryModuleState.VALUE_ABSENT), ImmutableSet.of(), CompositeTask.Order.ORDERED, CompositeTask.RunMode.TRY_ALL, ImmutableList.of(Pair.of(StrollTask.createDynamicRadius(0.5F), 2), Pair.of(StrollTask.create(0.15F, false), 2), Pair.of(GoToLookTargetTask.create(AxolotlBrain::canGoToLookTarget, AxolotlBrain::getTemptedSpeed, 3), 3), Pair.of(TaskTriggerer.predicate(Entity::isTouchingWater), 5), Pair.of(TaskTriggerer.predicate(Entity::isOnGround), 5))))));
   }
   ...
   private static Optional getAttackTarget(ServerWorld world, AxolotlEntity axolotl) {
