...
public abstract class AbstractHorseEntity extends AnimalEntity implements RideableInventory, Tameable, JumpingMount {
   ...
   @Nullable
   private LazyEntityReference ownerReference;
   ...
   protected void initCustomGoals() {
      ...
      this.goalSelector.add(3, new TemptGoal(this, 1.25, (stack) -> {
         return stack.isIn(ItemTags.HORSE_TEMPT_ITEMS);
      }, false));
   }
   ...
   protected RegistryEntry getEquipSound(EquipmentSlot slot, ItemStack stack, EquippableComponent equippableComponent) {
      return (RegistryEntry)(slot == EquipmentSlot.SADDLE ? SoundEvents.ENTITY_HORSE_SADDLE : super.getEquipSound(slot, stack, equippableComponent));
   }
   ...
   protected void walkToParent(ServerWorld world) {
      if (this.isBred() && this.isBaby() && !this.isEatingGrass()) {
         LivingEntity livingEntity = world.getClosestEntity(AbstractHorseEntity.class, PARENT_HORSE_PREDICATE, this, this.getX(), this.getY(), this.getZ(), this.getBoundingBox().expand(16.0));
   ...
   public void updateAnger() {
      if (this.shouldAmbientStand() && (this.canActVoluntarily() || !this.getWorld().isClient)) {
         this.setAngry(20);
      }
   ...
   public void playAngrySound() {
      if (!this.isAngry() && !this.getWorld().isClient) {
         this.updateAnger();
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.ownerReference = LazyEntityReference.fromDataOrPlayerName(view, "Owner", this.getWorld());
   }
   ...
   private void setChildAttribute(PassiveEntity other, AbstractHorseEntity child, RegistryEntry attribute, double min, double max) {
   ...
   protected Vec3d getPassengerAttachmentPos(Entity passenger, EntityDimensions dimensions, float scaleFactor) {
      return super.getPassengerAttachmentPos(passenger, dimensions, scaleFactor).add((new Vec3d(0.0, 0.15 * (double)this.lastAngryAnimationProgress * (double)scaleFactor, -0.7 * (double)this.lastAngryAnimationProgress * (double)scaleFactor)).rotateY(-this.getYaw() * 0.017453292F));
   }
