...
public class DamageTracker {
   ...
   private Text getAttackedFallDeathMessage(Entity attacker, Text attackerDisplayName, String itemDeathTranslationKey, String deathTranslationKey) {
      ...
      return !itemStack.isEmpty() && itemStack.contains(DataComponentTypes.CUSTOM_NAME) ? Text.translatable(itemDeathTranslationKey, this.entity.getDisplayName(), attackerDisplayName, itemStack.toHoverableText()) : Text.translatable(deathTranslationKey, this.entity.getDisplayName(), attackerDisplayName);
   }
   ...
   private Text getFallDeathMessage(DamageRecord damageRecord, @Nullable Entity attacker) {
      ...
      if (!damageSource.isIn(DamageTypeTags.IS_FALL) && !damageSource.isIn(DamageTypeTags.ALWAYS_MOST_SIGNIFICANT_FALL)) {
         ...
         if (text2 != null && !text2.equals(text)) {
            ...
            return (Text)(text != null ? this.getAttackedFallDeathMessage(attacker, text, "death.fell.finish.item", "death.fell.finish") : Text.translatable("death.fell.killer", this.entity.getDisplayName()));
         }
      } else {
   ...
   static {
      INTENTIONAL_GAME_DESIGN_ISSUE_LINK_STYLE = Style.EMPTY.withClickEvent(new ClickEvent.OpenUrl(Urls.INTENTIONAL_GAME_DESIGN_ISSUE)).withHoverEvent(new HoverEvent.ShowText(Text.literal("MCPE-28723")));
   }
}
