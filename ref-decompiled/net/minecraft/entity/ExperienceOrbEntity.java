...
public class ExperienceOrbEntity extends Entity {
   protected static final TrackedData VALUE;
   ...
   public ExperienceOrbEntity(EntityType entityType, World world) {
   ...
   protected void tryMoveToOpenSpace(double boundingBoxLength) {
      Vec3d vec3d = this.getPos().add(0.0, (double)this.getHeight() / 2.0, 0.0);
      VoxelShape voxelShape = VoxelShapes.cuboid(Box.of(vec3d, boundingBoxLength, boundingBoxLength, boundingBoxLength));
      this.getWorld().findClosestCollision(this, voxelShape, vec3d, (double)this.getWidth(), (double)this.getHeight(), (double)this.getWidth()).ifPresent((pos) -> {
         this.setPosition(pos.add(0.0, (double)(-this.getHeight()) / 2.0, 0.0));
      });
   }
   ...
   public void tick() {
      ...
      if (this.firstUpdate && this.getWorld().isClient) {
         ...
         if (this.target == null && !this.getWorld().isClient && bl) {
            boolean bl2 = !this.getWorld().isSpaceEmpty(this.getBoundingBox().offset(this.getVelocity()));
            if (bl2) {
   ...
   private void expensiveUpdate() {
      if (this.getWorld() instanceof ServerWorld) {
         List list = this.getWorld().getEntitiesByType(TypeFilter.instanceOf(ExperienceOrbEntity.class), this.getBoundingBox().expand(0.5), this::isMergeable);
   ...
   private static boolean wasMergedIntoExistingOrb(ServerWorld world, Vec3d pos, int amount) {
      ...
      List list = world.getEntitiesByType(TypeFilter.instanceOf(ExperienceOrbEntity.class), box, (orb) -> {
   ...
   private void applyWaterMovement() {
      ...
      this.setVelocity(vec3d.x * 0.9900000095367432, Math.min(vec3d.y + 5.000000237487257E-4, 0.05999999865889549), vec3d.z * 0.9900000095367432);
   }
   ...
   private int repairPlayerGears(ServerPlayerEntity player, int amount) {
      Optional optional = EnchantmentHelper.chooseEquipmentWith(EnchantmentEffectComponentTypes.REPAIR_WITH_XP, player, ItemStack::isDamaged);
   ...
   public int getValue() {
      return (Integer)this.dataTracker.get(VALUE);
   }
   ...
   static {
      VALUE = DataTracker.registerData(ExperienceOrbEntity.class, TrackedDataHandlerRegistry.INTEGER);
   }
}
