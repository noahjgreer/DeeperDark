...
public record TagPredicate<T>(TagKey<T> tag, boolean expected) {
    public static <T> Codec<TagPredicate<T>> createCodec(RegistryKey<? extends Registry<T>> registryRef) {
        return RecordCodecBuilder.create(instance -> instance.group((App)TagKey.unprefixedCodec(registryRef).fieldOf("id").forGetter(TagPredicate::tag), (App)Codec.BOOL.fieldOf("expected").forGetter(TagPredicate::expected)).apply((Applicative)instance, TagPredicate::new));
    }
