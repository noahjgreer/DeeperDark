...
public record DistancePredicate(NumberRange.DoubleRange x, NumberRange.DoubleRange y, NumberRange.DoubleRange z, NumberRange.DoubleRange horizontal, NumberRange.DoubleRange absolute) {
   public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(NumberRange.DoubleRange.CODEC.optionalFieldOf("x", NumberRange.DoubleRange.ANY).forGetter(DistancePredicate::x), NumberRange.DoubleRange.CODEC.optionalFieldOf("y", NumberRange.DoubleRange.ANY).forGetter(DistancePredicate::y), NumberRange.DoubleRange.CODEC.optionalFieldOf("z", NumberRange.DoubleRange.ANY).forGetter(DistancePredicate::z), NumberRange.DoubleRange.CODEC.optionalFieldOf("horizontal", NumberRange.DoubleRange.ANY).forGetter(DistancePredicate::horizontal), NumberRange.DoubleRange.CODEC.optionalFieldOf("absolute", NumberRange.DoubleRange.ANY).forGetter(DistancePredicate::absolute)).apply(instance, DistancePredicate::new);
   });

   public DistancePredicate(NumberRange.DoubleRange x, NumberRange.DoubleRange y, NumberRange.DoubleRange z, NumberRange.DoubleRange horizontal, NumberRange.DoubleRange absolute) {
      this.x = x;
      this.y = y;
      this.z = z;
   ...
   public boolean test(double x0, double y0, double z0, double x1, double y1, double z1) {
      ...
      if (this.x.test((double)MathHelper.abs(f)) && this.y.test((double)MathHelper.abs(g)) && this.z.test((double)MathHelper.abs(h))) {
         if (!this.horizontal.testSqrt((double)(f * f + h * h))) {
            return false;
         } else {
