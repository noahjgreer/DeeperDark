...
public class LootContextPredicateValidator {
   ...
   public void validateEntityPredicate(Optional predicate, String path) {
      predicate.ifPresent((p) -> {
         this.validateEntityPredicate(p, path);
      });
   }

   public void validateEntityPredicates(List predicates, String path) {
   ...
   public void validate(List predicates, ContextType type, String path) {
      for(int i = 0; i < predicates.size(); ++i) {
         LootContextPredicate lootContextPredicate = (LootContextPredicate)predicates.get(i);
         lootContextPredicate.validateConditions(new LootTableReporter(this.errorReporter.makeChild(new ErrorReporter.NamedListElementContext(path, i)), type, this.conditionsLookup));
      }
