...
public class RuinedPortalStructurePiece extends SimpleStructurePiece {
   ...
   public RuinedPortalStructurePiece(StructureTemplateManager manager, NbtCompound nbt) {
      ...
      this.verticalPlacement = (VerticalPlacement)nbt.get("VerticalPlacement", RuinedPortalStructurePiece.VerticalPlacement.CODEC).orElseThrow();
      this.properties = (Properties)nbt.get("Properties", RuinedPortalStructurePiece.Properties.CODEC).orElseThrow();
   }

   protected void writeNbt(StructureContext context, NbtCompound nbt) {
      super.writeNbt(context, nbt);
      nbt.put("Rotation", BlockRotation.ENUM_NAME_CODEC, this.placementData.getRotation());
      nbt.put("Mirror", BlockMirror.ENUM_NAME_CODEC, this.placementData.getMirror());
      nbt.put("VerticalPlacement", RuinedPortalStructurePiece.VerticalPlacement.CODEC, this.verticalPlacement);
      nbt.put("Properties", RuinedPortalStructurePiece.Properties.CODEC, this.properties);
   }
   ...
   private static StructurePlacementData createPlacementData(StructureTemplateManager manager, NbtCompound nbt, Identifier id) {
      ...
      return createPlacementData((BlockMirror)nbt.get("Mirror", BlockMirror.ENUM_NAME_CODEC).orElseThrow(), (BlockRotation)nbt.get("Rotation", BlockRotation.ENUM_NAME_CODEC).orElseThrow(), (VerticalPlacement)nbt.get("VerticalPlacement", RuinedPortalStructurePiece.VerticalPlacement.CODEC).orElseThrow(), blockPos, (Properties)RuinedPortalStructurePiece.Properties.CODEC.parse(new Dynamic(NbtOps.INSTANCE, nbt.get("Properties"))).getPartialOrThrow());
   }
   ...
   private boolean canFillNetherrack(WorldAccess world, BlockPos pos) {
      BlockState blockState = world.getBlockState(pos);
      return !blockState.isOf(Blocks.AIR) && !blockState.isOf(Blocks.OBSIDIAN) && !blockState.isIn(BlockTags.FEATURES_CANNOT_REPLACE) && (this.verticalPlacement == RuinedPortalStructurePiece.VerticalPlacement.IN_NETHER || !blockState.isOf(Blocks.LAVA));
   }
   ...
   public static enum VerticalPlacement implements StringIdentifiable {
      ...
      public static final Codec CODEC = StringIdentifiable.createCodec(VerticalPlacement::values);
