...
public static class StrongholdGenerator.RightTurn
extends StrongholdGenerator.Turn {
    public StrongholdGenerator.RightTurn(int chainLength, Random random, BlockBox boundingBox, Direction orientation) {
        super(StructurePieceType.STRONGHOLD_RIGHT_TURN, chainLength, boundingBox);
        this.setOrientation(orientation);
        this.entryDoor = this.getRandomEntrance(random);
    }

    public StrongholdGenerator.RightTurn(NbtCompound nbt) {
    ...
    public static @Nullable StrongholdGenerator.RightTurn create(StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength) {
        BlockBox blockBox = BlockBox.rotated(x, y, z, -1, -1, 0, 5, 5, 5, orientation);
        if (!StrongholdGenerator.RightTurn.isInBounds(blockBox) || holder.getIntersecting(blockBox) != null) {
            return null;
        }
        return new StrongholdGenerator.RightTurn(chainLength, random, blockBox, orientation);
    }
