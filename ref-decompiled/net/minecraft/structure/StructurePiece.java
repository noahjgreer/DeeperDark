...
public abstract class StructurePiece {
   ...
   protected boolean isUnderSeaLevel(WorldView world, int x, int z, int y, BlockBox box) {
      ...
      if (!box.contains(blockPos)) {
         ...
         return blockPos.getY() < world.getTopY(Heightmap.Type.OCEAN_FLOOR_WG, blockPos.getX(), blockPos.getZ());
      }
   }
   ...
   protected boolean addChest(StructureWorldAccess world, BlockBox boundingBox, Random random, int x, int y, int z, RegistryKey lootTable) {
      return this.addChest(world, boundingBox, random, this.offsetPos(x, y, z), lootTable, (BlockState)null);
   }
   ...
   protected boolean addChest(ServerWorldAccess world, BlockBox boundingBox, Random random, BlockPos pos, RegistryKey lootTable, @Nullable BlockState block) {
   ...
   protected boolean addDispenser(StructureWorldAccess world, BlockBox boundingBox, Random random, int x, int y, int z, Direction facing, RegistryKey lootTable) {
      BlockPos blockPos = this.offsetPos(x, y, z);
