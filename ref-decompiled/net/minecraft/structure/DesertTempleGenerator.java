...
public class DesertTempleGenerator extends ShiftableStructurePiece {
   ...
   private final List potentialSuspiciousSandPositions = new ArrayList();
   private BlockPos basementMarkerPos;
   ...
   public DesertTempleGenerator(Random random, int x, int z) {
      ...
      this.basementMarkerPos = BlockPos.ORIGIN;
   }

   public DesertTempleGenerator(NbtCompound nbt) {
      super(StructurePieceType.DESERT_TEMPLE, nbt);
      this.basementMarkerPos = BlockPos.ORIGIN;
      this.hasPlacedChest[0] = nbt.getBoolean("hasPlacedChest0", false);
      this.hasPlacedChest[1] = nbt.getBoolean("hasPlacedChest1", false);
      this.hasPlacedChest[2] = nbt.getBoolean("hasPlacedChest2", false);
      this.hasPlacedChest[3] = nbt.getBoolean("hasPlacedChest3", false);
   }
      ...
      if (this.adjustToMinHeight(world, -random.nextInt(3))) {
         ...
         while(var19.hasNext()) {
            ...
            if (!this.hasPlacedChest[direction.getHorizontalQuarterTurns()]) {
               ...
               this.hasPlacedChest[direction.getHorizontalQuarterTurns()] = this.addChest(world, chunkBox, random, 10 + m, -11, 10 + n, LootTables.DESERT_PYRAMID_CHEST);
            }
         }
   ...
   private void generateBasementRoof(StructureWorldAccess world, BlockBox chunkBox, int startX, int y, int startZ, int endX, int endZ) {
      ...
      Random random = Random.create(world.getSeed()).nextSplitter().split((BlockPos)this.offsetPos(startX, y, startZ));
      j = random.nextBetween(startX, endX);
