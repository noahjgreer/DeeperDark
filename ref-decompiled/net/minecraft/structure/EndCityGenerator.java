...
public class EndCityGenerator {
   ...
   static {
      ...
      SMALL_TOWER = new Part() {
         ...
         public boolean create(StructureTemplateManager manager, int depth, Piece root, BlockPos pos, List pieces, Random random) {
            BlockRotation blockRotation = root.getPlacementData().getRotation();
            Piece piece = EndCityGenerator.addPiece(pieces, EndCityGenerator.createPiece(manager, root, new BlockPos(3 + random.nextInt(2), -3, 3 + random.nextInt(2)), "tower_base", blockRotation, true));
            ...
            if (piece2 != null) {
               ...
               while(var14.hasNext()) {
                  ...
                  if (random.nextBoolean()) {
                     Piece piece3 = EndCityGenerator.addPiece(pieces, EndCityGenerator.createPiece(manager, piece2, (BlockPos)pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)pair.getLeft()), true));
      ...
      FAT_TOWER = new Part() {
         ...
         public boolean create(StructureTemplateManager manager, int depth, Piece root, BlockPos pos, List pieces, Random random) {
            ...
            for(int i = 0; i < 2 && random.nextInt(3) != 0; ++i) {
               ...
               while(var10.hasNext()) {
                  Pair pair = (Pair)var10.next();
                  if (random.nextBoolean()) {
                     Piece piece2 = EndCityGenerator.addPiece(pieces, EndCityGenerator.createPiece(manager, piece, (BlockPos)pair.getRight(), "bridge_end", blockRotation.rotate((BlockRotation)pair.getLeft()), true));
   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      public Piece(StructureTemplateManager manager, NbtCompound nbt) {
         super(StructurePieceType.END_CITY, nbt, manager, (id) -> {
            return createPlacementData(nbt.getBoolean("OW", false), (BlockRotation)nbt.get("Rot", BlockRotation.ENUM_NAME_CODEC).orElseThrow());
         });
      }
      ...
      protected void handleMetadata(String metadata, BlockPos pos, ServerWorldAccess world, Random random, BlockBox boundingBox) {
         if (metadata.startsWith("Chest")) {
            ...
            if (metadata.startsWith("Sentry")) {
               ShulkerEntity shulkerEntity = (ShulkerEntity)EntityType.SHULKER.create(world.toServerWorld(), SpawnReason.STRUCTURE);
