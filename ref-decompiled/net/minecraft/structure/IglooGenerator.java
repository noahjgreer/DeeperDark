   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      public Piece(StructureTemplateManager manager, NbtCompound nbt) {
         super(StructurePieceType.IGLOO, nbt, manager, (identifier) -> {
            return createPlacementData((BlockRotation)nbt.get("Rot", BlockRotation.ENUM_NAME_CODEC).orElseThrow(), identifier);
         });
      }

      private static StructurePlacementData createPlacementData(BlockRotation rotation, Identifier identifier) {
         return (new StructurePlacementData()).setRotation(rotation).setMirror(BlockMirror.NONE).setPosition((BlockPos)IglooGenerator.OFFSETS.get(identifier)).addProcessor(BlockIgnoreStructureProcessor.IGNORE_STRUCTURE_BLOCKS).setLiquidSettings(StructureLiquidSettings.IGNORE_WATERLOGGING);
      }
