...
public class ShipwreckGenerator {
   ...
   public static Piece addParts(StructureTemplateManager structureTemplateManager, BlockPos pos, BlockRotation rotation, StructurePiecesHolder holder, Random random, boolean beached) {
      Identifier identifier = (Identifier)Util.getRandom((Object[])(beached ? BEACHED_TEMPLATES : REGULAR_TEMPLATES), random);
      Piece piece = new Piece(structureTemplateManager, identifier, pos, rotation, beached);
   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      public Piece(StructureTemplateManager manager, NbtCompound nbt) {
         super(StructurePieceType.SHIPWRECK, nbt, manager, (id) -> {
            return createPlacementData((BlockRotation)nbt.get("Rot", BlockRotation.ENUM_NAME_CODEC).orElseThrow());
         });
      ...
      protected void handleMetadata(String metadata, BlockPos pos, ServerWorldAccess world, Random random, BlockBox boundingBox) {
         RegistryKey registryKey = (RegistryKey)ShipwreckGenerator.LOOT_TABLES.get(metadata);
         if (registryKey != null) {
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         if (this.isTooLargeForNormalGeneration()) {
            ...
            if (k == 0) {
               ...
               int l;
               for(Iterator var14 = BlockPos.iterate(this.pos, blockPos).iterator(); var14.hasNext(); i = Math.min(i, l)) {
                  BlockPos blockPos2 = (BlockPos)var14.next();
                  l = world.getTopY(type, blockPos2.getX(), blockPos2.getZ());
                  j += l;
               }
