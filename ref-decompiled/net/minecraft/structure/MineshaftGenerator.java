...
public class MineshaftGenerator {
   ...
   private static MineshaftPart pickPiece(StructurePiecesHolder holder, Random random, int x, int y, int z, @Nullable Direction orientation, int chainLength, MineshaftStructure.Type type) {
      ...
      BlockBox blockBox;
      if (i >= 80) {
         blockBox = MineshaftGenerator.MineshaftCrossing.getBoundingBox(holder, random, x, y, z, orientation);
         if (blockBox != null) {
            return new MineshaftCrossing(chainLength, blockBox, orientation, type);
         }
      } else if (i >= 70) {
         blockBox = MineshaftGenerator.MineshaftStairs.getBoundingBox(holder, random, x, y, z, orientation);
         if (blockBox != null) {
            return new MineshaftStairs(chainLength, blockBox, orientation, type);
         }
      } else {
         blockBox = MineshaftGenerator.MineshaftCorridor.getBoundingBox(holder, random, x, y, z, orientation);
         if (blockBox != null) {
   ...
   public static class MineshaftCorridor extends MineshaftPart {
      ...
      protected boolean addChest(StructureWorldAccess world, BlockBox boundingBox, Random random, int x, int y, int z, RegistryKey lootTable) {
         ...
         if (boundingBox.contains(blockPos) && world.getBlockState(blockPos).isAir() && !world.getBlockState(blockPos.down()).isAir()) {
            ...
            ChestMinecartEntity chestMinecartEntity = (ChestMinecartEntity)EntityType.CHEST_MINECART.create(world.toServerWorld(), SpawnReason.CHUNK_GENERATION);
   ...
   public static class MineshaftRoom extends MineshaftPart {
      ...
      public void fillOpenings(StructurePiece start, StructurePiecesHolder holder, Random random) {
         ...
         for(k = 0; k < this.boundingBox.getBlockCountX(); k += 4) {
            ...
            structurePiece = MineshaftGenerator.pieceGenerator(start, holder, random, this.boundingBox.getMinX() + k, this.boundingBox.getMinY() + random.nextInt(j) + 1, this.boundingBox.getMinZ() - 1, Direction.NORTH, i);
         ...
         for(k = 0; k < this.boundingBox.getBlockCountX(); k += 4) {
            ...
            structurePiece = MineshaftGenerator.pieceGenerator(start, holder, random, this.boundingBox.getMinX() + k, this.boundingBox.getMinY() + random.nextInt(j) + 1, this.boundingBox.getMaxZ() + 1, Direction.SOUTH, i);
            if (structurePiece != null) {
               blockBox = structurePiece.getBoundingBox();
         ...
         for(k = 0; k < this.boundingBox.getBlockCountZ(); k += 4) {
            ...
            structurePiece = MineshaftGenerator.pieceGenerator(start, holder, random, this.boundingBox.getMinX() - 1, this.boundingBox.getMinY() + random.nextInt(j) + 1, this.boundingBox.getMinZ() + k, Direction.WEST, i);
            if (structurePiece != null) {
               blockBox = structurePiece.getBoundingBox();
         ...
         for(k = 0; k < this.boundingBox.getBlockCountZ(); k += 4) {
            ...
            structurePiece = MineshaftGenerator.pieceGenerator(start, holder, random, this.boundingBox.getMaxX() + 1, this.boundingBox.getMinY() + random.nextInt(j) + 1, this.boundingBox.getMinZ() + k, Direction.EAST, i);
            if (structurePiece != null) {
               blockBox = structurePiece.getBoundingBox();
