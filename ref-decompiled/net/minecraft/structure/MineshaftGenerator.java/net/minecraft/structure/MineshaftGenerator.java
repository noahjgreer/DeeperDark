...
public class MineshaftGenerator {
    ...
    private static @Nullable MineshaftPart pickPiece(StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength, MineshaftStructure.Type type) {
    ...
    static @Nullable MineshaftPart pieceGenerator(StructurePiece start, StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength) {
        if (chainLength > 8) {
            return null;
        }
            ...
            while (bl || bl2) {
                ...
                if (bl2) {
                    ...
                    if (!bl3 && this.sideCoversSmallSquare(world, mutable, blockState)) {
                        ...
                        MineshaftCorridor.fillColumn(world, Blocks.IRON_CHAIN.getDefaultState(), mutable, i + 2, i + j);
                        return;
                    }
