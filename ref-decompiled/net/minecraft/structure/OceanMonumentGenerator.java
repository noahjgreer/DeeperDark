...
public class OceanMonumentGenerator {
   ...
   private static class DoubleYZRoomFactory implements PieceFactory {
      DoubleYZRoomFactory() {
      }
      ...
      public boolean canGenerate(PieceSetting setting) {
         if (setting.neighborPresences[Direction.NORTH.getIndex()] && !setting.neighbors[Direction.NORTH.getIndex()].used && setting.neighborPresences[Direction.UP.getIndex()] && !setting.neighbors[Direction.UP.getIndex()].used) {
            PieceSetting pieceSetting = setting.neighbors[Direction.NORTH.getIndex()];
   ...
   private static class DoubleXYRoomFactory implements PieceFactory {
      DoubleXYRoomFactory() {
      }
      ...
      public boolean canGenerate(PieceSetting setting) {
         if (setting.neighborPresences[Direction.EAST.getIndex()] && !setting.neighbors[Direction.EAST.getIndex()].used && setting.neighborPresences[Direction.UP.getIndex()] && !setting.neighbors[Direction.UP.getIndex()].used) {
            PieceSetting pieceSetting = setting.neighbors[Direction.EAST.getIndex()];
            return pieceSetting.neighborPresences[Direction.UP.getIndex()] && !pieceSetting.neighbors[Direction.UP.getIndex()].used;
         } else {
            return false;
         }
   ...
   static class DoubleZRoomFactory implements PieceFactory {
      public boolean canGenerate(PieceSetting setting) {
   ...
   static class DoubleXRoomFactory implements PieceFactory {
      public boolean canGenerate(PieceSetting setting) {
   ...
   private static class DoubleYRoomFactory implements PieceFactory {
      DoubleYRoomFactory() {
      }
   ...
   static class SimpleRoomTopFactory implements PieceFactory {
      public boolean canGenerate(PieceSetting setting) {
   ...
   static class SimpleRoomFactory implements PieceFactory {
      public boolean canGenerate(PieceSetting setting) {
         return true;
      }
   ...
   private interface PieceFactory {
      boolean canGenerate(PieceSetting setting);

      Piece generate(Direction direction, PieceSetting setting, Random random);
   }

   private static class PieceSetting {
      final int roomIndex;
      ...
      public boolean hasEntranceConnection(int distance) {
         if (this.entrance) {
            ...
            for(int i = 0; i < 6; ++i) {
               if (this.neighbors[i] != null && this.neighborPresences[i] && this.neighbors[i].entranceDistance != distance && this.neighbors[i].hasEntranceConnection(distance)) {
                  return true;
               }
            }
   ...
   public static class CoreRoom extends Piece {
   ...
   public static class DoubleYZRoom extends Piece {
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         PieceSetting pieceSetting = this.setting.neighbors[Direction.NORTH.getIndex()];
         PieceSetting pieceSetting2 = this.setting;
         PieceSetting pieceSetting3 = pieceSetting.neighbors[Direction.UP.getIndex()];
         PieceSetting pieceSetting4 = pieceSetting2.neighbors[Direction.UP.getIndex()];
   ...
   public static class DoubleXYRoom extends Piece {
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         PieceSetting pieceSetting = this.setting.neighbors[Direction.EAST.getIndex()];
         PieceSetting pieceSetting2 = this.setting;
         PieceSetting pieceSetting3 = pieceSetting2.neighbors[Direction.UP.getIndex()];
         PieceSetting pieceSetting4 = pieceSetting.neighbors[Direction.UP.getIndex()];
         if (this.setting.roomIndex / 25 > 0) {
   ...
   public static class DoubleYRoom extends Piece {
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         ...
         PieceSetting pieceSetting = this.setting.neighbors[Direction.UP.getIndex()];
         if (pieceSetting.neighbors[Direction.UP.getIndex()] == null) {
   ...
   public static class SimpleRoomTop extends Piece {
   ...
   public static class SimpleRoom extends Piece {
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         ...
         boolean bl = this.roomType != 0 && random.nextBoolean() && !this.setting.neighborPresences[Direction.DOWN.getIndex()] && !this.setting.neighborPresences[Direction.UP.getIndex()] && this.setting.countNeighbors() > 1;
         if (this.roomType == 0) {
   ...
   public static class Entry extends Piece {
   ...
   public static class Base extends Piece {
      ...
      public Base(Random random, int x, int z, Direction orientation) {
         ...
         List list = this.setupPieces(random);
         ...
         while(true) {
            while(true) {
               ...
               do {
                  do {
                     if (!var7.hasNext()) {
                        ...
                        this.children.add(new WingRoom(orientation, blockBox, i++));
                        this.children.add(new WingRoom(orientation, blockBox2, i++));
                        this.children.add(new Penthouse(orientation, blockBox3));
                        return;
                     }

                     pieceSetting = (PieceSetting)var7.next();
               ...
               while(var9.hasNext()) {
                  PieceFactory pieceFactory = (PieceFactory)var9.next();
      ...
      private List setupPieces(Random random) {
         ...
         PieceSetting pieceSetting3 = new PieceSetting(1002);
         ...
         PieceSetting[] var20 = pieceSettings;
         var8 = pieceSettings.length;

         for(var9 = 0; var9 < var8; ++var9) {
            PieceSetting pieceSetting4 = var20[var9];
         ...
         int q = 1;
         ObjectListIterator var22 = objectArrayList.iterator();

         label95:
         while(var22.hasNext()) {
            PieceSetting pieceSetting5 = (PieceSetting)var22.next();
            int r = 0;
            m = 0;
            ...
            while(true) {
               while(true) {
                  do {
                     ...
                     ++m;
                     n = random.nextInt(6);
                  } while(!pieceSetting5.neighborPresences[n]);
      ...
      public void generate(StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, Random random, BlockBox chunkBox, ChunkPos chunkPos, BlockPos pivot) {
         ...
         Iterator var15 = this.children.iterator();

         while(var15.hasNext()) {
            Piece piece = (Piece)var15.next();
   ...
   protected abstract static class Piece extends StructurePiece {
      ...
      protected static final Set ICE_BLOCKS;
      ...
      protected Piece(StructurePieceType type, int length, Direction orientation, PieceSetting setting, int x, int y, int z) {
         super(type, length, createBox(orientation, setting, x, y, z));
         this.setOrientation(orientation);
         this.setting = setting;
      }

      private static BlockBox createBox(Direction orientation, PieceSetting setting, int x, int y, int z) {
      ...
      protected void spawnElderGuardian(StructureWorldAccess world, BlockBox box, int x, int y, int z) {
         ...
         if (box.contains(blockPos)) {
            ElderGuardianEntity elderGuardianEntity = (ElderGuardianEntity)EntityType.ELDER_GUARDIAN.create(world.toServerWorld(), SpawnReason.STRUCTURE);
            if (elderGuardianEntity != null) {
               ...
               elderGuardianEntity.initialize(world, world.getLocalDifficulty(elderGuardianEntity.getBlockPos()), SpawnReason.STRUCTURE, (EntityData)null);
      ...
      static {
         ...
         ALSO_PRISMARINE_BRICKS = PRISMARINE_BRICKS;
         SEA_LANTERN = Blocks.SEA_LANTERN.getDefaultState();
         WATER = Blocks.WATER.getDefaultState();
         ICE_BLOCKS = ImmutableSet.builder().add(Blocks.ICE).add(Blocks.PACKED_ICE).add(Blocks.BLUE_ICE).add(WATER.getBlock()).build();
