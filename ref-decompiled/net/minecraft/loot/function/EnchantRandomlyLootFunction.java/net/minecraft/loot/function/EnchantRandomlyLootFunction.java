...
extends ConditionalLootFunction {
    ...
    public static final MapCodec<EnchantRandomlyLootFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> EnchantRandomlyLootFunction.addConditionsField(instance).and(instance.group((App)RegistryCodecs.entryList(RegistryKeys.ENCHANTMENT).optionalFieldOf("options").forGetter(function -> function.options), (App)Codec.BOOL.optionalFieldOf("only_compatible", (Object)true).forGetter(function -> function.onlyCompatible))).apply((Applicative)instance, EnchantRandomlyLootFunction::new));
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        Stream<RegistryEntry> stream = this.options.map(RegistryEntryList::stream).orElseGet(() -> context.getWorld().getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().map(Function.identity())).filter(entry -> !bl2 || ((Enchantment)entry.value()).isAcceptableItem(stack));
        List<RegistryEntry> list = stream.toList();
        Optional<RegistryEntry> optional = Util.getRandomOrEmpty(list, random);
        if (optional.isEmpty()) {
            LOGGER.warn("Couldn't find a compatible enchantment for {}", (Object)stack);
            return stack;
        }
        return EnchantRandomlyLootFunction.addEnchantmentToStack(stack, optional.get(), random);
    }
    ...
    public static Builder builder(RegistryWrapper.WrapperLookup registries) {
        return EnchantRandomlyLootFunction.create().options(registries.getOrThrow(RegistryKeys.ENCHANTMENT).getOrThrow(EnchantmentTags.ON_RANDOM_LOOT));
    }
