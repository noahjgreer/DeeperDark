...
public record ScoreLootNumberProvider(LootScoreProvider target, String score, float scale) implements LootNumberProvider {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(LootScoreProviderTypes.CODEC.fieldOf("target").forGetter(ScoreLootNumberProvider::target), Codec.STRING.fieldOf("score").forGetter(ScoreLootNumberProvider::score), Codec.FLOAT.fieldOf("scale").orElse(1.0F).forGetter(ScoreLootNumberProvider::scale)).apply(instance, ScoreLootNumberProvider::new);
   });
