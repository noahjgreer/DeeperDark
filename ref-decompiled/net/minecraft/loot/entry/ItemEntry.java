package net.minecraft.loot.entry;

import com.mojang.serialization.MapCodec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import java.util.List;
import java.util.function.Consumer;
import net.minecraft.item.Item;
import net.minecraft.item.ItemConvertible;
import net.minecraft.item.ItemStack;
import net.minecraft.loot.context.LootContext;
import net.minecraft.registry.entry.RegistryEntry;

public class ItemEntry extends LeafEntry {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(Item.ENTRY_CODEC.fieldOf("name").forGetter((entry) -> {
         return entry.item;
      })).and(addLeafFields(instance)).apply(instance, ItemEntry::new);
   });
   private final RegistryEntry item;

   private ItemEntry(RegistryEntry item, int weight, int quality, List conditions, List functions) {
      super(weight, quality, conditions, functions);
      this.item = item;
   }

   public LootPoolEntryType getType() {
      return LootPoolEntryTypes.ITEM;
   }

   public void generateLoot(Consumer lootConsumer, LootContext context) {
      lootConsumer.accept(new ItemStack(this.item));
   }

   public static LeafEntry.Builder builder(ItemConvertible drop) {
      return builder((weight, quality, conditions, functions) -> {
         return new ItemEntry(drop.asItem().getRegistryEntry(), weight, quality, conditions, functions);
      });
   }
}
