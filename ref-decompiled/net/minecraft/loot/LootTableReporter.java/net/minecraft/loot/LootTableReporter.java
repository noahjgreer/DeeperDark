...
public class LootTableReporter {
    ...
    public LootTableReporter makeChild(ErrorReporter.Context context, RegistryKey<?> key) {
        ImmutableSet set = ImmutableSet.builder().addAll(this.referenceStack).add(key).build();
        return new LootTableReporter(this.errorReporter.makeChild(context), this.contextType, this.dataLookup, (Set<RegistryKey<?>>)set);
    }
    ...
    public void validateContext(LootContextAware contextAware) {
        ...
        Sets.SetView set2 = Sets.difference(set, this.contextType.getAllowed());
        if (!set2.isEmpty()) {
            this.errorReporter.report(new ParametersNotProvidedError((Set<ContextParameter<?>>)set2));
        }
    }
