...
extends ScreenHandler {
    ...
    private void addInputSlots(ForgingSlotsManager forgingSlotsManager) {
        for (final ForgingSlotsManager.ForgingSlot forgingSlot : forgingSlotsManager.getInputSlots()) {
            this.addSlot(new Slot(this, this.input, forgingSlot.slotId(), forgingSlot.x(), forgingSlot.y()){
    ...
    public ItemStack quickMove(PlayerEntity player, int slot) {
        ...
        if (slot2 != null && slot2.hasStack()) {
            ...
            } else if (slot >= 0 && slot < this.getResultSlotIndex() ? !this.insertItem(itemStack2, i, j, false) : (this.isValidIngredient(itemStack2) && slot >= this.getPlayerInventoryStartIndex() && slot < this.getPlayerHotbarEndIndex() ? !this.insertItem(itemStack2, 0, this.getResultSlotIndex(), false) : (slot >= this.getPlayerInventoryStartIndex() && slot < this.getPlayerInventoryEndIndex() ? !this.insertItem(itemStack2, this.getPlayerHotbarStartIndex(), this.getPlayerHotbarEndIndex(), false) : slot >= this.getPlayerHotbarStartIndex() && slot < this.getPlayerHotbarEndIndex() && !this.insertItem(itemStack2, this.getPlayerInventoryStartIndex(), this.getPlayerInventoryEndIndex(), false)))) {
                return ItemStack.EMPTY;
            }
