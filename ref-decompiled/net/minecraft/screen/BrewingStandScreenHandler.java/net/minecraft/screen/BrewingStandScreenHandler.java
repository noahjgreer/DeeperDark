...
extends ScreenHandler {
    ...
    public BrewingStandScreenHandler(int syncId, PlayerInventory playerInventory, Inventory inventory, PropertyDelegate propertyDelegate) {
        ...
        BrewingRecipeRegistry brewingRecipeRegistry = playerInventory.player.getEntityWorld().getBrewingRecipeRegistry();
    ...
    public ItemStack quickMove(PlayerEntity player, int slot) {
        ...
        if (slot2 != null && slot2.hasStack()) {
            ...
            } else if (FuelSlot.matches(itemStack) ? this.insertItem(itemStack2, 4, 5, false) || this.ingredientSlot.canInsert(itemStack2) && !this.insertItem(itemStack2, 3, 4, false) : (this.ingredientSlot.canInsert(itemStack2) ? !this.insertItem(itemStack2, 3, 4, false) : (PotionSlot.matches(itemStack) ? !this.insertItem(itemStack2, 0, 3, false) : (slot >= 5 && slot < 32 ? !this.insertItem(itemStack2, 32, 41, false) : (slot >= 32 && slot < 41 ? !this.insertItem(itemStack2, 5, 32, false) : !this.insertItem(itemStack2, 5, 41, false)))))) {
                return ItemStack.EMPTY;
            }
