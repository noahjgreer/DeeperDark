...
extends AbstractCraftingScreenHandler {
    ...
    public ItemStack quickMove(PlayerEntity player, int slot) {
        ...
        if (slot2.hasStack()) {
            ...
            } else if (slot >= 1 && slot < 5 ? !this.insertItem(itemStack2, 9, 45, false) : (slot >= 5 && slot < 9 ? !this.insertItem(itemStack2, 9, 45, false) : (equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR && !((Slot)this.slots.get(8 - equipmentSlot.getEntitySlotId())).hasStack() ? !this.insertItem(itemStack2, i = 8 - equipmentSlot.getEntitySlotId(), i + 1, false) : (equipmentSlot == EquipmentSlot.OFFHAND && !((Slot)this.slots.get(45)).hasStack() ? !this.insertItem(itemStack2, 45, 46, false) : (slot >= 9 && slot < 36 ? !this.insertItem(itemStack2, 36, 45, false) : (slot >= 36 && slot < 45 ? !this.insertItem(itemStack2, 9, 36, false) : !this.insertItem(itemStack2, 9, 45, false))))))) {
                return ItemStack.EMPTY;
            }
