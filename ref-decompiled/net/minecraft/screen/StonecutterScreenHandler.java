...
public class StonecutterScreenHandler extends ScreenHandler {
   ...
   private CuttingRecipeDisplay.Grouping availableRecipes;
   ...
   public CuttingRecipeDisplay.Grouping getAvailableRecipes() {
   ...
   void populateResult(int selectedId) {
      ...
      if (!this.availableRecipes.isEmpty() && this.isInBounds(selectedId)) {
         CuttingRecipeDisplay.GroupEntry groupEntry = (CuttingRecipeDisplay.GroupEntry)this.availableRecipes.entries().get(selectedId);
      ...
      optional.ifPresentOrElse((recipe) -> {
         ...
         this.outputSlot.setStackNoCallbacks(((StonecuttingRecipe)recipe.value()).craft(new SingleStackRecipeInput(this.input.getStack(0)), this.world.getRegistryManager()));
      }, () -> {
         this.outputSlot.setStackNoCallbacks(ItemStack.EMPTY);
