...
extends Slot {
    ...
    private DefaultedList<ItemStack> getRecipeRemainders(CraftingRecipeInput input, World world) {
        if (world instanceof ServerWorld) {
            ...
            return serverWorld.getRecipeManager().getFirstMatch(RecipeType.CRAFTING, input, serverWorld).map(recipe -> ((CraftingRecipe)recipe.value()).getRecipeRemainders(input)).orElseGet(() -> CraftingResultSlot.copyInput(input));
        }
