...
public class LoomScreenHandler extends ScreenHandler {
   ...
   private final RegistryEntryLookup bannerPatternLookup;
         ...
         public void onTakeItem(PlayerEntity player, ItemStack stack) {
            ...
            context.run((world, pos) -> {
               ...
               if (LoomScreenHandler.this.lastTakeResultTime != l) {
                  world.playSound((Entity)null, pos, SoundEvents.UI_LOOM_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
   ...
   private List getPatternsFor(ItemStack stack) {
      if (stack.isEmpty()) {
         ...
         TagKey tagKey = (TagKey)stack.get(DataComponentTypes.PROVIDES_BANNER_PATTERNS);
         return tagKey != null ? (List)this.bannerPatternLookup.getOptional(tagKey).map(ImmutableList::copyOf).orElse(ImmutableList.of()) : List.of();
      }
   }
   ...
   public void onContentChanged(Inventory inventory) {
      ...
      if (!itemStack.isEmpty() && !itemStack2.isEmpty()) {
         ...
         if (this.bannerPatterns.size() == 1) {
            ...
            RegistryEntry registryEntry2 = (RegistryEntry)list.get(i);
   ...
   private void updateOutputSlot(RegistryEntry pattern) {
      ...
      if (!itemStack.isEmpty() && !itemStack2.isEmpty()) {
         ...
         itemStack3.apply(DataComponentTypes.BANNER_PATTERNS, BannerPatternsComponent.DEFAULT, (component) -> {
            return (new BannerPatternsComponent.Builder()).addAll(component).add(pattern, dyeColor).build();
         });
      }
