...
public class TelemetryEventType {
    ...
    public static final TelemetryEventType WORLD_LOADED = TelemetryEventType.builder((String)"world_loaded", (String)"WorldLoaded").properties(REQUIRED_PROPERTIES).properties(TelemetryEventProperty.GAME_MODE).properties(TelemetryEventProperty.REALMS_MAP_CONTENT).build();
    public static final TelemetryEventType PERFORMANCE_METRICS = TelemetryEventType.builder((String)"performance_metrics", (String)"PerformanceMetrics").properties(REQUIRED_PROPERTIES).properties(TelemetryEventProperty.FRAME_RATE_SAMPLES).properties(TelemetryEventProperty.RENDER_TIME_SAMPLES).properties(TelemetryEventProperty.USED_MEMORY_SAMPLES).properties(TelemetryEventProperty.NUMBER_OF_SAMPLES).properties(TelemetryEventProperty.RENDER_DISTANCE).properties(TelemetryEventProperty.DEDICATED_MEMORY_KB).optional().build();
    public static final TelemetryEventType WORLD_LOAD_TIMES = TelemetryEventType.builder((String)"world_load_times", (String)"WorldLoadTimes").properties(REQUIRED_PROPERTIES).properties(TelemetryEventProperty.WORLD_LOAD_TIME_MS).properties(TelemetryEventProperty.NEW_WORLD).optional().build();
    public static final TelemetryEventType WORLD_UNLOADED = TelemetryEventType.builder((String)"world_unloaded", (String)"WorldUnloaded").properties(REQUIRED_PROPERTIES).properties(TelemetryEventProperty.SECONDS_SINCE_LOAD).properties(TelemetryEventProperty.TICKS_SINCE_LOAD).build();
    public static final TelemetryEventType ADVANCEMENT_MADE = TelemetryEventType.builder((String)"advancement_made", (String)"AdvancementMade").properties(REQUIRED_PROPERTIES).properties(TelemetryEventProperty.ADVANCEMENT_ID).properties(TelemetryEventProperty.ADVANCEMENT_GAME_TIME).optional().build();
    public static final TelemetryEventType GAME_LOAD_TIMES = TelemetryEventType.builder((String)"game_load_times", (String)"GameLoadTimes").properties(BASIC_PROPERTIES).properties(TelemetryEventProperty.LOAD_TIME_TOTAL_TIME_MS).properties(TelemetryEventProperty.LOAD_TIME_PRE_WINDOW_MS).properties(TelemetryEventProperty.LOAD_TIME_BOOTSTRAP_MS).properties(TelemetryEventProperty.LOAD_TIME_LOADING_OVERLAY_MS).optional().build();
    ...
    private MutableText getText(String key) {
        return Text.translatable((String)("telemetry.event." + this.id + "." + key));
    }
