...
implements ProfileKeys {
    ...
    public CompletableFuture<Optional<PlayerKeyPair>> fetchKeyPair() {
        ...
        this.keyFuture = this.keyFuture.thenCompose(arg_0 -> this.getKeyPair(arg_0));
        return this.keyFuture;
    }

    public boolean isExpired() {
        if (this.keyFuture.isDone() && Instant.now().isAfter(this.expiryCheckTime)) {
            return ((Optional)this.keyFuture.join()).map(PlayerKeyPair::needsRefreshing).orElse(true);
        }
        return false;
    }
    ...
    private Optional<PlayerKeyPair> loadKeyPairFromFile() {
        ...
        block9: {
            ...
            try {
                optional = PlayerKeyPair.CODEC.parse((DynamicOps)JsonOps.INSTANCE, (Object)StrictJsonParser.parse((Reader)bufferedReader)).result();
    ...
    private @Nullable PlayerKeyPair fetchKeyPair(UserApiService userApiService) throws NetworkEncryptionException, IOException {
        ...
        if (keyPairResponse != null) {
            PlayerPublicKey.PublicKeyData publicKeyData = ProfileKeysImpl.decodeKeyPairResponse((KeyPairResponse)keyPairResponse);
            return new PlayerKeyPair(NetworkEncryptionUtils.decodeRsaPrivateKeyPem((String)keyPairResponse.keyPair().privateKey()), new PlayerPublicKey(publicKeyData), Instant.parse(keyPairResponse.refreshedAfter()));
        }
        return null;
    }
    ...
    private static PlayerPublicKey.PublicKeyData decodeKeyPairResponse(KeyPairResponse keyPairResponse) throws NetworkEncryptionException {
        ...
        try {
            ...
            PublicKey publicKey = NetworkEncryptionUtils.decodeRsaPublicKeyPem((String)keyPair.publicKey());
            ...
            throw new NetworkEncryptionException((Throwable)runtimeException);
        }
    }
}
