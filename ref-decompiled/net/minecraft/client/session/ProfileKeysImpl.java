...
public class ProfileKeysImpl implements ProfileKeys {
   ...
   private CompletableFuture keyFuture = CompletableFuture.completedFuture(Optional.empty());
   ...
   public boolean isExpired() {
      return this.keyFuture.isDone() && Instant.now().isAfter(this.expiryCheckTime) ? (Boolean)((Optional)this.keyFuture.join()).map(PlayerKeyPair::needsRefreshing).orElse(true) : false;
   }
   ...
   private Optional loadKeyPairFromFile() {
      if (Files.notExists(this.jsonPath, new LinkOption[0])) {
         ...
         try {
            ...
            Optional var2;
            try {
               var2 = PlayerKeyPair.CODEC.parse(JsonOps.INSTANCE, StrictJsonParser.parse((Reader)bufferedReader)).result();
