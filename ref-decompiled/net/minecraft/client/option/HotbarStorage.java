...
public class HotbarStorage {
    ...
    private void load() {
        try {
            NbtCompound nbtCompound = NbtIo.read((Path)this.file);
            if (nbtCompound == null) {
                return;
            }
            int i = NbtHelper.getDataVersion((NbtCompound)nbtCompound, (int)1343);
    ...
    public void save() {
        try {
            NbtCompound nbtCompound = NbtHelper.putDataVersion((NbtCompound)new NbtCompound());
            for (int i = 0; i < 9; ++i) {
            ...
            NbtIo.write((NbtCompound)nbtCompound, (Path)this.file);
        }
        catch (Exception exception) {
