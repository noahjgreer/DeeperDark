...
extends RealmsScreen {
    ...
    private volatile @Nullable Text downloadError;
    private volatile Text status = Text.translatable((String)"mco.download.preparing");
    ...
    public RealmsDownloadLatestWorldScreen(Screen parent, WorldDownload worldDownload, String worldName, BooleanConsumer onBack) {
        ...
        this.downloadTitle = Text.translatable((String)"mco.download.title");
        this.narrationRateLimiter = RateLimiter.create((double)0.1f);
    }

    public void init() {
        this.cancelButton = (ButtonWidget)this.addDrawableChild((Element)ButtonWidget.builder((Text)ScreenTexts.CANCEL, button -> this.close()).dimensions((this.width - 200) / 2, this.height - 42, 200, 20).build());
    ...
    private void checkDownloadSize() {
        ...
        if (this.getContentLength(this.worldDownload.downloadLink()) >= 0x140000000L) {
            MutableText text = Text.translatable((String)"mco.download.confirmation.oversized", (Object[])new Object[]{SizeUnit.getUserFriendlyString((long)0x140000000L)});
            this.client.setScreen((Screen)RealmsPopups.createNonContinuableWarningPopup((Screen)this, (Text)text, popupScreen -> {
    ...
    private long getContentLength(String downloadLink) {
        return FileDownload.contentLength((String)downloadLink).orElse(0L);
    }
    ...
    private Text getNarration() {
        ...
        if (this.progress != null) {
            list.add(Text.translatable((String)"mco.download.percent", (Object[])new Object[]{this.progress}));
            list.add(Text.translatable((String)"mco.download.speed.narration", (Object[])new Object[]{SizeUnit.getUserFriendlyString((long)this.bytesPerSecond)}));
        }
    ...
    private void drawProgressBar(DrawContext context) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, (Text)Text.translatable((String)"mco.download.percent", (Object[])new Object[]{this.progress}), this.width / 2, 84, -1);
    }
    ...
    private void drawDownloadSpeed0(DrawContext context, long bytesPerSecond) {
        if (bytesPerSecond > 0L) {
            ...
            context.drawTextWithShadow(this.textRenderer, (Text)Text.translatable((String)"mco.download.speed", (Object[])new Object[]{SizeUnit.getUserFriendlyString((long)bytesPerSecond)}), this.width / 2 + i / 2 + 15, 84, -1);
        }
    }
        ...
        new Thread(() -> {
            try {
                if (!DOWNLOAD_LOCK.tryLock(1L, TimeUnit.SECONDS)) {
                    this.status = Text.translatable((String)"mco.download.failed");
                    return;
                }
                if (this.cancelled) {
                ...
                this.status = Text.translatable((String)"mco.download.downloading", (Object[])new Object[]{this.worldName});
                ...
                while (!fileDownload.isFinished()) {
                    if (fileDownload.isError()) {
                        fileDownload.cancel();
                        this.downloadError = Text.translatable((String)"mco.download.failed");
                        this.cancelButton.setMessage(ScreenTexts.DONE);
                        return;
                    }
                    if (fileDownload.isExtracting()) {
                        if (!this.extracting) {
                            this.status = Text.translatable((String)"mco.download.extracting");
                        }
                ...
                this.finished = true;
                this.status = Text.translatable((String)"mco.download.done");
                this.cancelButton.setMessage(ScreenTexts.DONE);
            }
    ...
    private void downloadCancelled() {
        this.status = Text.translatable((String)"mco.download.cancelled");
    }
}
