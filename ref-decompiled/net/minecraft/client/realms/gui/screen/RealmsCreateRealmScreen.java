...
public class RealmsCreateRealmScreen extends RealmsScreen {
   ...
   public RealmsCreateRealmScreen(RealmsMainScreen parent, RealmsServer server, boolean prerelease) {
      ...
      this.parent = parent;
      this.worldCreator = () -> {
         this.createWorld(server, prerelease);
      };
   }
   ...
   public void init() {
      ...
      ButtonWidget buttonWidget = ButtonWidget.builder(ScreenTexts.CONTINUE, (button) -> {
      ...
      directionalLayoutWidget2.add(ButtonWidget.builder(ScreenTexts.BACK, (button) -> {
         this.close();
      }).build());
   ...
   private void createWorld(RealmsServer realmsServer, boolean prerelease) {
      if (!realmsServer.isPrerelease() && prerelease) {
         ...
         CompletableFuture.supplyAsync(() -> {
         ...
         }, Util.getMainWorkerExecutor()).thenAcceptAsync((prereleaseServer) -> {
            ...
            Object text;
            if (throwable2 instanceof RealmsServiceException realmsServiceException) {
               text = realmsServiceException.error.getText();
            } else {
               text = Text.translatable("mco.errorMessage.initialize.failed");
            }

            this.client.setScreen(new RealmsGenericErrorScreen((Text)text, this.parent));
            return null;
         }, this.client);
      } else {
   ...
   private void createWorld(RealmsServer server) {
      ...
      RealmsCreateWorldScreen realmsCreateWorldScreen = RealmsCreateWorldScreen.newRealm(this, server, worldCreationTask, () -> {
         this.client.execute(() -> {
