...
implements UploadProgressTracker {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (this.showDots) {
            context.drawTextWithShadow(this.textRenderer, DOTS[this.animTick / 10 % DOTS.length], this.width / 2 + this.textRenderer.getWidth(this.status) / 2 + 5, 50, -1);
        }
    ...
    private void drawProgressBar(DrawContext context) {
        ...
        context.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("mco.upload.percent", this.progress), this.width / 2, 84, -1);
    }
    ...
    private void upload() {
        ...
        RealmsUploader realmsUploader = new RealmsUploader(path, realmsSlot = new RealmsSlot(this.slotId, realmsWorldOptions = RealmsWorldOptions.create(this.selectedLevel.getLevelInfo(), this.selectedLevel.getVersionInfo().getVersionName()), List.of(RealmsSettingDto.ofHardcore(this.selectedLevel.getLevelInfo().isHardcore()))), this.client.getSession(), this.worldId, this);
