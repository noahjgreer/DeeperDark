...
public class RealmsSettingsTab extends GridScreenTab implements RealmsUpdatableTab {
   ...
   private final Map availableRegions;
   ...
   RealmsSettingsTab(RealmsConfigureWorldScreen screen, MinecraftClient minecraftClient, RealmsServer realmsServer, Map availableRegions) {
      super(TITLE_TEXT);
      this.screen = screen;
      ...
      Objects.requireNonNull(screen.getTextRenderer());
      this.regionText = (TextWidget)axisGridWidget.add((new TextWidget(192, 9, Text.empty(), screen.getTextRenderer())).alignLeft());
      this.serviceQualityIcon = (IconWidget)axisGridWidget.add(IconWidget.create(10, 8, ServiceQuality.UNKNOWN.getIcon()));
      adder.add(axisGridWidget);
      adder.add(ButtonWidget.builder(Text.translatable("mco.configure.world.buttons.region_preference"), (button) -> {
         this.showRegionPreferenceScreen();
      }).dimensions(0, 0, 212, 20).build());
      adder.add(EmptyWidget.ofHeight(2));
      this.switchStateButton = (ButtonWidget)adder.add(ButtonWidget.builder(Text.empty(), (buttonWidget) -> {
   ...
   private static MutableText getRegionText(Region region) {
      return (region.preference().equals(RegionSelectionMethod.MANUAL) && region.region() != null ? Text.translatable(region.region().translationKey) : Text.translatable(region.preference().translationKey)).formatted(Formatting.GRAY);
   }

   private static Identifier getQualityIcon(Region region, Map qualityByRegion) {
   ...
   public void update(RealmsServer server) {
      ...
      if (server.regionSelectionPreference.selectionMethod == RegionSelectionMethod.MANUAL && server.regionSelectionPreference.preferredRegion == null) {
         ...
         optional.ifPresent((region) -> {
            server.regionSelectionPreference.preferredRegion = region;
         });
      }
   ...
   @Environment(EnvType.CLIENT)
   public static record Region(RegionSelectionMethod preference, @Nullable RealmsRegion region) {
