...
class RealmsWorldsTab extends GridScreenTab implements RealmsUpdatableTab {
   ...
   RealmsWorldsTab(RealmsConfigureWorldScreen screen, MinecraftClient client, RealmsServer server) {
      ...
      this.optionsButton = (ButtonWidget)adder3.add(ButtonWidget.builder(Text.translatable("mco.configure.world.buttons.options"), (button) -> {
         client.setScreen(new RealmsSlotOptionsScreen(screen, ((RealmsSlot)server.slots.get(server.activeSlot)).clone(), server.worldType, server.activeSlot));
      }).dimensions(0, 0, 150, 20).build());
      this.backupButton = (ButtonWidget)adder3.add(ButtonWidget.builder(Text.translatable("mco.configure.world.backup"), (button) -> {
         client.setScreen(new RealmsBackupScreen(screen, server.clone(), server.activeSlot));
      }).dimensions(0, 0, 150, 20).build());
      this.resetButton = (ButtonWidget)adder3.add(ButtonWidget.builder(Text.empty(), (button) -> {
         this.reset();
      }).dimensions(0, 0, 150, 20).build());
   ...
   private void reset() {
      if (this.isMinigame()) {
         ...
         this.client.setScreen(RealmsCreateWorldScreen.resetWorld(this.screen, this.server.clone(), () -> {
            this.client.execute(() -> {
               this.client.setScreen(this.screen.getNewScreen());
            });
         }));
      }
   ...
   private void switchMinigame(@Nullable WorldTemplate template) {
      if (template != null && WorldTemplate.WorldTemplateType.MINIGAME == template.type) {
         ...
         this.client.setScreen(new RealmsLongRunningMcoTaskScreen(realmsConfigureWorldScreen, new LongRunningTask[]{new SwitchMinigameTask(this.server.id, template, realmsConfigureWorldScreen)}));
      } else {
   ...
   public void update(RealmsServer server) {
      ...
      Iterator var5 = this.slotButtons.iterator();

      while(var5.hasNext()) {
         RealmsWorldSlotButton realmsWorldSlotButton = (RealmsWorldSlotButton)var5.next();
   ...
   private void switchWorld(int slotId, RealmsServer server) {
      this.client.setScreen(RealmsPopups.createInfoPopup(this.screen, Text.translatable("mco.configure.world.slot.switch.question.line1"), (popup) -> {
         RealmsConfigureWorldScreen realmsConfigureWorldScreen = this.screen.getNewScreen();
         this.screen.stateChanged();
         this.client.setScreen(new RealmsLongRunningMcoTaskScreen(realmsConfigureWorldScreen, new LongRunningTask[]{new SwitchSlotTask(server.id, slotId, () -> {
            this.client.execute(() -> {
               this.client.setScreen(realmsConfigureWorldScreen);
            });
         })}));
      }));
   }
   ...
   private void createWorld(int slotId, RealmsServer server) {
      this.client.setScreen(RealmsPopups.createInfoPopup(this.screen, Text.translatable("mco.configure.world.slot.switch.question.line1"), (popup) -> {
         this.screen.stateChanged();
         RealmsCreateWorldScreen realmsCreateWorldScreen = RealmsCreateWorldScreen.newWorld(this.screen, slotId, server, () -> {
            this.client.execute(() -> {
               this.client.setScreen(this.screen.getNewScreen());
            });
         });
