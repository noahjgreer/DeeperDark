...
class RealmsSubscriptionInfoTab extends GridScreenTab implements RealmsUpdatableTab {
   ...
   RealmsSubscriptionInfoTab(RealmsConfigureWorldScreen screen, MinecraftClient client, RealmsServer server) {
      ...
      adder.add((new TextWidget(200, 9, SUBSCRIPTION_START_LABEL_TEXT, textRenderer)).alignLeft());
      Objects.requireNonNull(textRenderer);
      this.startDateTextWidget = (TextWidget)adder.add((new TextWidget(200, 9, this.startDate, textRenderer)).alignLeft());
      adder.add(EmptyWidget.ofHeight(2));
      Objects.requireNonNull(textRenderer);
      this.timeLeftLabelTextWidget = (TextWidget)adder.add((new TextWidget(200, 9, TIME_LEFT_LABEL_TEXT, textRenderer)).alignLeft());
      Objects.requireNonNull(textRenderer);
      this.daysLeftTextWidget = (TextWidget)adder.add((new TextWidget(200, 9, this.daysLeft, textRenderer)).alignLeft());
      adder.add(EmptyWidget.ofHeight(2));
      adder.add(ButtonWidget.builder(Text.translatable("mco.configure.world.subscription.extend"), (button) -> {
         ConfirmLinkScreen.open(screen, (String)Urls.getExtendJavaRealmsUrl(server.remoteSubscriptionId, client.getSession().getUuidOrNull()));
      }).dimensions(0, 0, 200, 20).build());
      adder.add(EmptyWidget.ofHeight(2));
      this.deleteWorldButton = (ButtonWidget)adder.add(ButtonWidget.builder(Text.translatable("mco.configure.world.delete.button"), (button) -> {
         client.setScreen(RealmsPopups.createContinuableWarningPopup(screen, Text.translatable("mco.configure.world.delete.question.line1"), (popupScreen) -> {
            this.onDeletionConfirmed();
         }));
      }).dimensions(0, 0, 200, 20).build());
      adder.add(EmptyWidget.ofHeight(2));
      this.subscriptionInfoTextWidget = (NarratedMultilineTextWidget)adder.add(new NarratedMultilineTextWidget(200, Text.empty(), textRenderer, true, true, 4), Positioner.create().alignHorizontalCenter());
   ...
   private void onDeletionConfirmed() {
      RealmsUtil.class_11538 var10000 = (realmsClient) -> {
         realmsClient.deleteWorld(this.serverData.id);
      };
      RealmsConfigureWorldScreen var10001 = this.screen;
      Objects.requireNonNull(var10001);
      RealmsUtil.method_72216(var10000, RealmsUtil.method_72221(var10001::method_72205, "Couldn't delete world")).thenRunAsync(() -> {
         this.client.setScreen(this.screen.getParent());
      }, this.client);
   ...
   private void getSubscription(long worldId) {
      ...
      try {
         ...
         this.client.setScreen(this.screen.method_72205(var5));
      }
   ...
   static {
      EXPIRED_TEXT = Text.translatable("mco.configure.world.subscription.expired").formatted(Formatting.GRAY);
      EXPIRES_IN_LESS_THAN_A_DAY_TEXT = Text.translatable("mco.configure.world.subscription.less_than_a_day").formatted(Formatting.GRAY);
      UNKNOWN_TEXT = Text.translatable("mco.configure.world.subscription.unknown");
      RECURRING_INFO_TEXT = Text.translatable("mco.configure.world.subscription.recurring.info");
   }
}
