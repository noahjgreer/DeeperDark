        ...
        new Thread("Realms-connect-task"){
            ...
            public void run() {
                ...
                try {
                    ...
                    RealmsConnection.this.connection = ClientConnection.connect(inetSocketAddress, NetworkingBackend.remote(minecraftClient.options.shouldUseNativeTransport()), minecraftClient.getDebugHud().getPacketSizeLog());
                    if (RealmsConnection.this.aborted) {
                        return;
                    }
                    ClientLoginNetworkHandler clientLoginNetworkHandler = new ClientLoginNetworkHandler(RealmsConnection.this.connection, minecraftClient, server.createServerInfo(string), RealmsConnection.this.onlineScreen, false, null, status -> {}, new ClientChunkLoadProgress(), null);
                    ...
                    minecraftClient.getQuickPlayLogger().setWorld(QuickPlayLogger.WorldType.REALMS, String.valueOf(server.id), Objects.requireNonNullElse(server.name, "unknown"));
                    ...
                    minecraftClient.execute(() -> minecraftClient.setScreen(disconnectedScreen));
                }
            }
        }.start();
    }
