...
implements GuiElementRenderState {
    ...
    private @Nullable ScreenRect createOversizedBounds() {
        ...
        int i = MathHelper.ceil((double)(box.getLengthX() * 16.0));
        int j = MathHelper.ceil((double)(box.getLengthY() * 16.0));
        if (i > 16 || j > 16) {
            float f = (float)(box.minX * 16.0);
            float g = (float)(box.maxY * 16.0);
            int k = MathHelper.floor((float)f);
            int l = MathHelper.floor((float)g);
            int m = this.x + k + 8;
            int n = this.y - l + 8;
