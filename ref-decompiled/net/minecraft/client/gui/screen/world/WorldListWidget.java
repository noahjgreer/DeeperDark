...
public class WorldListWidget extends AlwaysSelectedEntryListWidget {
   ...
   private CompletableFuture loadLevels() {
      ...
      if (levelList.isEmpty()) {
         CreateWorldScreen.show(this.client, (Screen)null);
         return CompletableFuture.completedFuture(List.of());
      } else {
   ...
   public static class LoadingEntry extends Entry {
      ...
      public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickProgress) {
         ...
         context.drawTextWithShadow(this.client.textRenderer, (Text)LOADING_LIST_TEXT, i, j, -1);
   ...
   public final class WorldEntry extends Entry {
         ...
         if (this.iconPath != null) {
            try {
               ...
               if (basicFileAttributes.isSymbolicLink()) {
                  List list = this.client.getSymlinkFinder().validate(this.iconPath);
                  if (!list.isEmpty()) {
      ...
      public Text getNarration() {
         Text text = Text.translatable("narrator.select.world_info", this.level.getDisplayName(), Text.of(new Date(this.level.getLastPlayed())), this.level.getDetails());
      ...
      public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickProgress) {
         ...
         context.drawTextWithShadow(this.client.textRenderer, (String)string, x + 32 + 3, y + 1, -1);
         TextRenderer var10001 = this.client.textRenderer;
         int var10003 = x + 32 + 3;
         Objects.requireNonNull(this.client.textRenderer);
         context.drawTextWithShadow(var10001, string2, var10003, y + 9 + 3, -8355712);
         var10001 = this.client.textRenderer;
         var10003 = x + 32 + 3;
         Objects.requireNonNull(this.client.textRenderer);
         int var10004 = y + 9;
         Objects.requireNonNull(this.client.textRenderer);
         context.drawTextWithShadow(var10001, text, var10003, var10004 + 9 + 3, -8355712);
         ...
         if ((Boolean)this.client.options.getTouchscreen().getValue() || hovered) {
            ...
            if (this.level.isLocked()) {
               ...
               if (this.level.wouldBeDowngraded()) {
                  context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, identifier3, x, y, 32, 32);
                  if (bl) {
                     context.drawTooltip((List)ImmutableList.of(WorldListWidget.FROM_NEWER_VERSION_FIRST_LINE.asOrderedText(), WorldListWidget.FROM_NEWER_VERSION_SECOND_LINE.asOrderedText()), mouseX, mouseY);
                  }
               } else if (!SharedConstants.getGameVersion().stable()) {
                  context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, identifier2, x, y, 32, 32);
                  if (bl) {
                     context.drawTooltip((List)ImmutableList.of(WorldListWidget.SNAPSHOT_FIRST_LINE.asOrderedText(), WorldListWidget.SNAPSHOT_SECOND_LINE.asOrderedText()), mouseX, mouseY);
                  }
               }
            } else {
      ...
      public void recreate() {
         ...
         try {
            ...
            try {
               Pair pair = this.client.createIntegratedServerLoader().loadForRecreation(session);
               ...
               if (generatorOptionsHolder.generatorOptions().isLegacyCustomizedType()) {
                  this.client.setScreen(new ConfirmScreen((confirmed) -> {
                     this.client.setScreen((Screen)(confirmed ? CreateWorldScreen.create(this.client, this.screen, levelInfo, generatorOptionsHolder, path) : this.screen));
