...
public class RecoverWorldScreen extends Screen {
   ...
   public RecoverWorldScreen(MinecraftClient client, BooleanConsumer callback, LevelStorage.Session session) {
      ...
      directionalLayoutWidget.add(ButtonWidget.builder(BUG_TRACKER_TEXT, ConfirmLinkScreen.opening(this, (URI)Urls.SNAPSHOT_BUGS)).size(120, 20).build());
      ((ButtonWidget)directionalLayoutWidget.add(ButtonWidget.builder(RESTORE_TEXT, (buttonWidget) -> {
         this.tryRestore(client);
      }).size(120, 20).tooltip(bl ? null : Tooltip.of(NO_FALLBACK_TEXT)).build())).active = bl;
      this.layout.add(directionalLayoutWidget);
      this.layout.add(ButtonWidget.builder(ScreenTexts.BACK, (button) -> {
         this.close();
      }).size(120, 20).build());
   ...
   private void tryRestore(MinecraftClient client) {
      ...
      if (exception != null && exception2 == null) {
         ...
         LOGGER.error("Failed to recover world, files not as expected. level.dat: {}, level.dat_old: {}", exception != null ? exception.getMessage() : "no issues", exception2 != null ? exception2.getMessage() : "no issues");
         client.setScreen(new NoticeScreen(() -> {
            this.callback.accept(false);
         }, DONE_TITLE_TEXT, DONE_FAILED_TEXT));
      }
   ...
   private Text toText(LevelStorage.Session session, boolean old, @Nullable Exception exception) {
      if (old && exception instanceof FileNotFoundException) {
         ...
         MutableText mutableText2 = instant != null ? Text.literal(WorldListWidget.DATE_FORMAT.format(instant)) : Text.translatable("recover_world.state_entry.unknown");
         mutableText.append((Text)Text.translatable("recover_world.state_entry", mutableText2.formatted(Formatting.GRAY)));
         if (exception == null) {
            ...
            mutableText.append((Text)Text.literal(exception.getCause().toString()).formatted(Formatting.RED));
         } else {
            mutableText.append((Text)Text.literal(exception.toString()).formatted(Formatting.RED));
         }
   ...
   static {
      TITLE_TEXT = Text.translatable("recover_world.title").formatted(Formatting.BOLD);
      BUG_TRACKER_TEXT = Text.translatable("recover_world.bug_tracker");
      RESTORE_TEXT = Text.translatable("recover_world.restore");
      NO_FALLBACK_TEXT = Text.translatable("recover_world.no_fallback");
      DONE_TITLE_TEXT = Text.translatable("recover_world.done.title");
      DONE_SUCCESS_TEXT = Text.translatable("recover_world.done.success");
      DONE_FAILED_TEXT = Text.translatable("recover_world.done.failed");
      ISSUE_NONE_TEXT = Text.translatable("recover_world.issue.none").formatted(Formatting.GREEN);
      MISSING_FILE_TEXT = Text.translatable("recover_world.issue.missing_file").formatted(Formatting.RED);
   }
}
