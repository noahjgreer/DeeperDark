...
public class CreateWorldScreen extends Screen {
   ...
   public static void show(MinecraftClient client, @Nullable Screen parent) {
      show(client, parent, (screen, combinedDynamicRegistries, levelProperties, dataPackTempDir) -> {
         return screen.startServer(combinedDynamicRegistries, levelProperties);
      });
   }
   ...
   public static void show(MinecraftClient client, @Nullable Screen parent, CreateWorldCallback callback) {
      GeneratorOptionsFactory generatorOptionsFactory = (dataPackContents, dynamicRegistries, settings) -> {
         return new GeneratorOptionsHolder(settings.worldGenSettings(), dynamicRegistries, dataPackContents, settings.dataConfiguration());
      };
      Function function = (context) -> {
         return new WorldGenSettings(GeneratorOptions.createRandom(), WorldPresets.createDemoOptions(context.worldGenRegistryManager()));
      };
   ...
   public static void showTestWorld(MinecraftClient client, @Nullable Screen parent) {
      GeneratorOptionsFactory generatorOptionsFactory = (dataPackContents, dynamicRegistries, settings) -> {
         return new GeneratorOptionsHolder(settings.worldGenSettings().generatorOptions(), settings.worldGenSettings().dimensionOptionsRegistryHolder(), dynamicRegistries, dataPackContents, settings.dataConfiguration(), new InitialWorldOptions(WorldCreator.Mode.CREATIVE, Set.of(GameRules.DO_DAYLIGHT_CYCLE, GameRules.DO_WEATHER_CYCLE, GameRules.DO_MOB_SPAWNING), FlatLevelGeneratorPresets.REDSTONE_READY));
      };
      Function function = (context) -> {
         return new WorldGenSettings(GeneratorOptions.createTestWorld(), WorldPresets.createTestOptions(context.worldGenRegistryManager()));
      };
      show(client, parent, function, generatorOptionsFactory, WorldPresets.FLAT, (screen, combinedDynamicRegistries, levelProperties, dataPackTempDir) -> {
         return screen.startServer(combinedDynamicRegistries, levelProperties);
      });
   }

   private static void show(MinecraftClient client, @Nullable Screen parent, Function settingsSupplier, GeneratorOptionsFactory generatorOptionsFactory, RegistryKey presetKey, CreateWorldCallback callback) {
      ...
      CompletableFuture completableFuture = SaveLoading.load(serverConfig, (context) -> {
         return new SaveLoading.LoadContext(new WorldCreationSettings((WorldGenSettings)settingsSupplier.apply(context), context.dataConfiguration()), context.dimensionsRegistryManager());
      ...
      client.setScreen(new CreateWorldScreen(client, parent, (GeneratorOptionsHolder)completableFuture.join(), Optional.of(presetKey), OptionalLong.empty(), callback));
   }
   ...
   public static CreateWorldScreen create(MinecraftClient client, @Nullable Screen parent, LevelInfo levelInfo, GeneratorOptionsHolder generatorOptionsHolder, @Nullable Path dataPackTempDir) {
      CreateWorldScreen createWorldScreen = new CreateWorldScreen(client, parent, generatorOptionsHolder, WorldPresets.getWorldPreset(generatorOptionsHolder.selectedDimensions()), OptionalLong.of(generatorOptionsHolder.generatorOptions().getSeed()), (screen, combinedDynamicRegistries, levelProperties, dataPackTempDirx) -> {
         return screen.startServer(combinedDynamicRegistries, levelProperties);
      });
      ...
      createWorldScreen.worldCreator.getGameRules().setAllValues(levelInfo.getGameRules(), (MinecraftServer)null);
   ...
   private CreateWorldScreen(MinecraftClient client, @Nullable Screen parent, GeneratorOptionsHolder generatorOptionsHolder, Optional defaultWorldType, OptionalLong seed, CreateWorldCallback callback) {
   ...
   protected void init() {
      ...
      directionalLayoutWidget.add(ButtonWidget.builder(Text.translatable("selectWorld.create"), (button) -> {
         this.createLevel();
      }).build());
   ...
   private void createLevel() {
      ...
      CombinedDynamicRegistries combinedDynamicRegistries = generatorOptionsHolder.combinedDynamicRegistries().with(ServerDynamicRegistryType.DIMENSIONS, (DynamicRegistryManager.Immutable[])(dimensionsConfig.toDynamicRegistryManager()));
   ...
   private void createAndClearTempDir(CombinedDynamicRegistries combinedDynamicRegistries, LevelProperties levelProperties) {
   ...
   private boolean startServer(CombinedDynamicRegistries combinedDynamicRegistries, SaveProperties saveProperties) {
      String string = this.worldCreator.getWorldDirectoryName();
      GeneratorOptionsHolder generatorOptionsHolder = this.worldCreator.getGeneratorOptionsHolder();
      showMessage(this.client, PREPARING_TEXT);
      Optional optional = createSession(this.client, string, this.dataPackTempDir);
      if (optional.isEmpty()) {
   ...
   private void applyDataPacks(ResourcePackManager dataPackManager, boolean fromPackScreen, Consumer configurationSetter) {
      List list = ImmutableList.copyOf(dataPackManager.getEnabledIds());
      List list2 = (List)dataPackManager.getIds().stream().filter((name) -> {
         return !list.contains(name);
      }).collect(ImmutableList.toImmutableList());
      DataConfiguration dataConfiguration = new DataConfiguration(new DataPackSettings(list, list2), this.worldCreator.getGeneratorOptionsHolder().dataConfiguration().enabledFeatures());
   ...
   private void validateDataPacks(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration, Consumer configurationSetter) {
      ...
      CompletableFuture var10000 = SaveLoading.load(serverConfig, (context) -> {
         if (context.worldGenRegistryManager().getOrThrow(RegistryKeys.WORLD_PRESET).streamEntries().findAny().isEmpty()) {
            ...
            GeneratorOptionsHolder generatorOptionsHolder = this.worldCreator.getGeneratorOptionsHolder();
            DynamicOps dynamicOps = generatorOptionsHolder.getCombinedRegistryManager().getOps(JsonOps.INSTANCE);
            DataResult dataResult = WorldGenSettings.encode(dynamicOps, generatorOptionsHolder.generatorOptions(), (DimensionOptionsRegistryHolder)generatorOptionsHolder.selectedDimensions()).setLifecycle(Lifecycle.stable());
            DynamicOps dynamicOps2 = context.worldGenRegistryManager().getOps(JsonOps.INSTANCE);
            WorldGenSettings worldGenSettings = (WorldGenSettings)dataResult.flatMap((json) -> {
               return WorldGenSettings.CODEC.parse(dynamicOps2, json);
            }).getOrThrow((error) -> {
               return new IllegalStateException("Error parsing worldgen settings after loading data packs: " + error);
            });
      ...
      var10000.thenAcceptAsync(var10001::setGeneratorOptionsHolder, this.client).handleAsync((void_, throwable) -> {
         if (throwable != null) {
   ...
   private static Optional createSession(MinecraftClient client, String worldDirectoryName, @Nullable Path dataPackTempDir) {
      try {
         ...
         try {
            Stream stream = Files.walk(dataPackTempDir);
            ...
            try {
               ...
               stream.filter((pathx) -> {
                  return !pathx.equals(dataPackTempDir);
               }).forEach((pathx) -> {
                  copyDataPack(dataPackTempDir, path, pathx);
               });
   ...
   class GameTab extends GridScreenTab {
      ...
      GameTab() {
         ...
         adder.add(LayoutWidgets.createLabeledWidget(CreateWorldScreen.this.textRenderer, this.worldNameField, CreateWorldScreen.ENTER_NAME_TEXT), adder.copyPositioner().alignHorizontalCenter());
         CyclingButtonWidget cyclingButtonWidget = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder((value) -> {
            return value.name;
         ...
         CyclingButtonWidget cyclingButtonWidget2 = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder(Difficulty::getTranslatableName).values((Object[])Difficulty.values()).build(0, 0, 210, 20, Text.translatable("options.difficulty"), (button, value) -> {
         ...
         CyclingButtonWidget cyclingButtonWidget3 = (CyclingButtonWidget)adder.add(CyclingButtonWidget.onOffBuilder().tooltip((value) -> {
   ...
   private class WorldTab extends GridScreenTab {
      ...
      WorldTab() {
         ...
         CyclingButtonWidget cyclingButtonWidget = (CyclingButtonWidget)adder.add(CyclingButtonWidget.builder(WorldCreator.WorldType::getName).values(this.getWorldTypes()).narration(WorldTab::getWorldTypeNarrationMessage).build(0, 0, 150, 20, Text.translatable("selectWorld.mapType"), (button, worldType) -> {
         ...
         BooleanSupplier var6 = var10002::shouldGenerateStructures;
         WorldCreator var10003 = CreateWorldScreen.this.worldCreator;
         Objects.requireNonNull(var10003);
         builder.add(var10001, var6, var10003::setGenerateStructures).toggleable(() -> {
         ...
         Objects.requireNonNull(var10002);
         var6 = var10002::isBonusChestEnabled;
         var10003 = CreateWorldScreen.this.worldCreator;
         Objects.requireNonNull(var10003);
         builder.add(var10001, var6, var10003::setBonusChestEnabled).toggleable(() -> {
   ...
   private class MoreTab extends GridScreenTab {
      ...
      MoreTab() {
         ...
         GridWidget.Adder adder = this.grid.setRowSpacing(8).createAdder(1);
         adder.add(ButtonWidget.builder(GAME_RULES_TEXT, (button) -> {
            this.openGameRulesScreen();
         }).width(210).build());
         adder.add(ButtonWidget.builder(CreateWorldScreen.EXPERIMENTS_TEXT, (button) -> {
            CreateWorldScreen.this.openExperimentsScreen(CreateWorldScreen.this.worldCreator.getGeneratorOptionsHolder().dataConfiguration());
         }).width(210).build());
         adder.add(ButtonWidget.builder(DATA_PACKS_TEXT, (button) -> {
