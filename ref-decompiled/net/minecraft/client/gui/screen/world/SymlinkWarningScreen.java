...
public class SymlinkWarningScreen extends Screen {
   private static final Text WORLD_TITLE;
   private static final Text WORLD_MESSAGE;
   private static final Text PACK_TITLE;
   private static final Text PACK_MESSAGE;
   ...
   protected void init() {
      ...
      adder2.add(ButtonWidget.builder(ScreenTexts.OPEN_LINK, (button) -> {
         Util.getOperatingSystem().open(this.link);
      }).size(120, 20).build());
      adder2.add(ButtonWidget.builder(ScreenTexts.COPY_LINK_TO_CLIPBOARD, (button) -> {
         this.client.keyboard.setClipboard(this.link.toString());
      }).size(120, 20).build());
      adder2.add(ButtonWidget.builder(ScreenTexts.BACK, (button) -> {
         this.close();
      }).size(120, 20).build());
   ...
   static {
      WORLD_TITLE = Text.translatable("symlink_warning.title.world").formatted(Formatting.BOLD);
      WORLD_MESSAGE = Text.translatable("symlink_warning.message.world", Text.of(Urls.MINECRAFT_SYMLINKS));
      PACK_TITLE = Text.translatable("symlink_warning.title.pack").formatted(Formatting.BOLD);
      PACK_MESSAGE = Text.translatable("symlink_warning.message.pack", Text.of(Urls.MINECRAFT_SYMLINKS));
   }
}
