...
public class AdvancementWidget {
   ...
   public AdvancementWidget(AdvancementTab tab, MinecraftClient client, PlacedAdvancement advancement, AdvancementDisplay display) {
      ...
      Stream var10000 = this.title.stream();
      TextRenderer var10001 = client.textRenderer;
      Objects.requireNonNull(var10001);
      int i = Math.max(var10000.mapToInt(var10001::getWidth).max().orElse(0), 80);
      ...
      for(Iterator var8 = this.description.iterator(); var8.hasNext(); k = Math.max(k, client.textRenderer.getWidth(orderedText))) {
   ...
   public void drawTooltip(DrawContext context, int originX, int originY, float alpha, int x, int y) {
      ...
      int n = text == null ? 0 : textRenderer.getWidth((StringVisitable)text);
      boolean bl2 = k + m >= 113;
      float f = this.progress == null ? 0.0F : this.progress.getProgressBarPercentage();
      ...
      if (bl) {
         ...
         if (text != null) {
            context.drawTextWithShadow(textRenderer, (Text)text, originX + this.x - n, j + 9, -1);
         }
      } else {
         this.drawText(context, this.title, originX + this.x + 32, j + 9, -1);
         if (text != null) {
            context.drawTextWithShadow(textRenderer, (Text)text, originX + this.x + this.width - n - 5, j + 9, -1);
         }
      }
   ...
   private void drawText(DrawContext context, List text, int x, int y, int color) {
      TextRenderer textRenderer = this.client.textRenderer;
