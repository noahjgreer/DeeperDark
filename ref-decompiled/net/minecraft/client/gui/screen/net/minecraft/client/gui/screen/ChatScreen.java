...
extends Screen {
    ...
    private boolean handleClickEvent(Style style, boolean insert) {
        ...
        if (insert) {
            ...
            if (clickEvent instanceof ClickEvent.Custom && (custom = (ClickEvent.Custom)clickEvent).id().equals(ChatHud.EXPAND_CHAT_QUEUE_ID)) {
                MessageHandler messageHandler = this.client.getMessageHandler();
                if (messageHandler.getUnprocessedMessageCount() != 0L) {
                    messageHandler.process();
                }
            } else {
                ChatScreen.handleClickEvent(clickEvent, this.client, this);
            }
            return true;
        }
        return false;
    ...
    @Environment(value=EnvType.CLIENT)
    protected static final class CloseReason
    extends Enum<CloseReason> {
        public static final /* enum */ CloseReason INTENTIONAL = new CloseReason();
        public static final /* enum */ CloseReason INTERRUPTED = new CloseReason();
        public static final /* enum */ CloseReason DONE = new CloseReason();
        private static final /* synthetic */ CloseReason[] field_62018;

        public static CloseReason[] values() {
            return (CloseReason[])field_62018.clone();
        }

        public static CloseReason valueOf(String string) {
            return Enum.valueOf(CloseReason.class, string);
        }

        private static /* synthetic */ CloseReason[] method_73219() {
            return new CloseReason[]{INTENTIONAL, INTERRUPTED, DONE};
        }

        static {
            field_62018 = CloseReason.method_73219();
        }
    }
