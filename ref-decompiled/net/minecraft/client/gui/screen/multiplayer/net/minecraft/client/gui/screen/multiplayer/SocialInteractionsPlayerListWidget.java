...
extends ElementListWidget<SocialInteractionsPlayerListEntry> {
    ...
    private void collectOfflinePlayers(Map<UUID, SocialInteractionsPlayerListEntry> entriesByUuids) {
        ...
        for (Map.Entry<UUID, PlayerListEntry> entry : map.entrySet()) {
            entriesByUuids.computeIfAbsent(entry.getKey(), uuid -> {
                SocialInteractionsPlayerListEntry socialInteractionsPlayerListEntry = this.createListEntry((UUID)uuid, (PlayerListEntry)entry.getValue());
    ...
    private void markOfflineMembers(Map<UUID, SocialInteractionsPlayerListEntry> entries, boolean includeOffline) {
        Map<UUID, GameProfile> map = SocialInteractionsPlayerListWidget.collectReportableProfiles(this.client.getAbuseReportContext().getChatLog());
        map.forEach((uuid2, profile) -> {
            ...
            if (includeOffline) {
                socialInteractionsPlayerListEntry = entries.computeIfAbsent((UUID)uuid2, uuid -> {
                    SocialInteractionsPlayerListEntry socialInteractionsPlayerListEntry = new SocialInteractionsPlayerListEntry(this.client, this.parent, profile.id(), profile.name(), this.client.getSkinProvider().supplySkinTextures((GameProfile)profile, true), true);
                    socialInteractionsPlayerListEntry.setOffline(true);
                    return socialInteractionsPlayerListEntry;
                });
            } else {
    ...
    public void setPlayerOnline(PlayerListEntry player, SocialInteractionsScreen.Tab tab) {
        ...
        if ((tab == SocialInteractionsScreen.Tab.ALL || this.client.getSocialInteractionsManager().isPlayerMuted(uUID)) && (Strings.isNullOrEmpty((String)this.currentSearch) || player.getProfile().name().toLowerCase(Locale.ROOT).contains(this.currentSearch))) {
            ...
            socialInteractionsPlayerListEntry = new SocialInteractionsPlayerListEntry(this.client, this.parent, player.getProfile().id(), player.getProfile().name(), player::getSkinTextures, bl);
