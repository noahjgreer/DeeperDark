...
extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(CyclingButtonWidget.builder(ServerInfo.ResourcePackPolicy::getName, this.server.getResourcePackPolicy()).values((ServerInfo.ResourcePackPolicy[])ServerInfo.ResourcePackPolicy.values()).build(this.width / 2 - 100, this.height / 4 + 72, 200, 20, Text.translatable("manageServer.resourcePack"), (button, resourcePackPolicy) -> this.server.setResourcePackPolicy((ServerInfo.ResourcePackPolicy)((Object)resourcePackPolicy))));
        this.addButton = this.addDrawableChild(ButtonWidget.builder(ScreenTexts.DONE, button -> this.addAndClose()).dimensions(this.width / 2 - 100, this.height / 4 + 96 + 18, 200, 20).build());
