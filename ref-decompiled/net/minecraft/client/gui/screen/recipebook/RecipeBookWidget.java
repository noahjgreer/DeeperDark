...
Element,
Selectable {
    public static final ButtonTextures BUTTON_TEXTURES = new ButtonTextures(Identifier.ofVanilla((String)"recipe_book/button"), Identifier.ofVanilla((String)"recipe_book/button_highlighted"));
    protected static final Identifier TEXTURE = Identifier.ofVanilla((String)"textures/gui/recipe_book.png");
    private static final int field_52839 = 256;
    private static final int field_52840 = 256;
    private static final Text SEARCH_HINT_TEXT = Text.translatable((String)"gui.recipebook.search_hint").fillStyle(TextFieldWidget.SEARCH_STYLE);
    ...
    private static final Text TOGGLE_ALL_RECIPES_TEXT = Text.translatable((String)"gui.recipebook.toggleRecipes.all");
    ...
    public RecipeBookWidget(T craftingScreenHandler, List<Tab> tabs) {
        ...
        this.tabs = tabs;
        CurrentIndexProvider currentIndexProvider = () -> MathHelper.floor((float)(this.displayTime / 30.0f));
    ...
    private void reset() {
        ...
        this.searchField = new TextFieldWidget(textRenderer, i + 25, j + 13, 81, 9 + 5, (Text)Text.translatable((String)"itemGroup.search"));
        this.searchField.setMaxLength(50);
        this.searchField.setVisible(true);
        this.searchField.setEditableColor(-1);
        this.searchField.setText(string);
        this.searchField.setPlaceholder(SEARCH_HINT_TEXT);
        this.searchFieldRect = ScreenRect.of((NavigationAxis)NavigationAxis.HORIZONTAL, (int)(i + 8), (int)this.searchField.getY(), (int)(this.searchField.getX() - this.getLeft()), (int)this.searchField.getHeight());
        this.recipesArea.initialize(this.client, i, j);
        this.toggleCraftableButton = CyclingButtonWidget.onOffBuilder((Text)this.getToggleCraftableButtonText(), (Text)TOGGLE_ALL_RECIPES_TEXT, (boolean)bl).tooltip(value -> value != false ? Tooltip.of((Text)this.getToggleCraftableButtonText()) : Tooltip.of((Text)TOGGLE_ALL_RECIPES_TEXT)).icon((button, value) -> this.getBookButtonTextures().get(value.booleanValue(), button.isSelected())).labelType(CyclingButtonWidget.LabelType.HIDE).build(i + 110, j + 12, 26, 16, ScreenTexts.EMPTY, (button, value) -> {
            this.toggleFilteringCraftable();
            this.sendBookDataPacket();
            this.refreshResults(false, value.booleanValue());
        });
        this.tabButtons.clear();
        for (Tab tab : this.tabs) {
            this.tabButtons.add(new RecipeGroupButtonWidget(0, 0, tab, arg_0 -> this.onTabSelected(arg_0)));
        }
    ...
    private void refreshResults(boolean resetCurrentPage, boolean filteringCraftable) {
        ...
        if (!string.isEmpty() && (clientPlayNetworkHandler = this.client.getNetworkHandler()) != null) {
            ObjectLinkedOpenHashSet objectSet = new ObjectLinkedOpenHashSet((Collection)clientPlayNetworkHandler.getSearchManager().getRecipeOutputReloadFuture().findAll(string.toLowerCase(Locale.ROOT)));
        ...
        this.recipesArea.setResults((List)list2, resetCurrentPage, filteringCraftable);
    }
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (this.searchField != null) {
            boolean bl;
            boolean bl2 = bl = this.searchFieldRect != null && this.searchFieldRect.contains(MathHelper.floor((double)click.x()), MathHelper.floor((double)click.y()));
    ...
    private boolean select(RecipeResultCollection results, NetworkRecipeId recipeId, boolean craftAll) {
        if (!results.isCraftable(recipeId) && recipeId.equals((Object)this.selectedRecipeId)) {
            return false;
        }
    ...
    public boolean keyPressed(KeyInput input) {
        ...
        if (input.isEnterOrSpace() && this.selectedRecipeResults != null && this.selectedRecipe != null) {
            ClickableWidget.playClickSound((SoundManager)MinecraftClient.getInstance().getSoundManager());
    ...
    public void onCraftFailed(RecipeDisplay display) {
        this.ghostRecipe.clear();
        ContextParameterMap contextParameterMap = SlotDisplayContexts.createParameters((World)((World)Objects.requireNonNull(this.client.world)));
    ...
    protected void sendBookDataPacket() {
        if (this.client.getNetworkHandler() != null) {
            ...
            this.client.getNetworkHandler().sendPacket((Packet)new RecipeCategoryOptionsC2SPacket(recipeBookType, bl, bl2));
        }
    }
    ...
    public void appendNarrations(NarrationMessageBuilder builder) {
        ...
        Screen.SelectedElementNarrationData selectedElementNarrationData = Screen.findSelectedElementData((List)list, null);
