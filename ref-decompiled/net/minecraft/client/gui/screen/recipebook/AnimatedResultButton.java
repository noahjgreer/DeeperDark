...
public class AnimatedResultButton extends ClickableWidget {
   ...
   public void showResultCollection(RecipeResultCollection resultCollection, boolean filteringCraftable, RecipeBookResults results, ContextParameterMap context) {
      ...
      this.results = list.stream().map((entry) -> {
         return new Result(entry.id(), entry.getStacks(context));
      }).toList();
      this.allResultsEqual = areAllResultsEqual(this.results);
      Stream var10000 = list.stream().map(RecipeDisplayEntry::id);
      ClientRecipeBook var10001 = results.getRecipeBook();
      Objects.requireNonNull(var10001);
      List list2 = var10000.filter(var10001::isHighlighted).toList();
      if (!list2.isEmpty()) {
   ...
   private static boolean areAllResultsEqual(List results) {
      Iterator iterator = results.stream().flatMap((result) -> {
         return result.displayItems().stream();
      }).iterator();
      if (!iterator.hasNext()) {
         return true;
      } else {
         ...
         } while(ItemStack.areItemsAndComponentsEqual(itemStack, itemStack2));
   ...
   public NetworkRecipeId getCurrentId() {
      ...
      return ((Result)this.results.get(i)).id;
   }
   ...
   public List getTooltip(ItemStack stack) {
      List list = new ArrayList(Screen.getTooltipFromItem(MinecraftClient.getInstance(), stack));
      if (this.hasMultipleResults()) {
   ...
   @Environment(EnvType.CLIENT)
   private static record Result(NetworkRecipeId id, List displayItems) {
      final NetworkRecipeId id;

      Result(NetworkRecipeId networkRecipeId, List list) {
