...
public class GhostRecipe {
   private final Reference2ObjectMap items = new Reference2ObjectArrayMap();
   ...
   public void drawTooltip(DrawContext context, MinecraftClient client, int x, int y, @Nullable Slot slot) {
      if (slot != null) {
         CyclingItem cyclingItem = (CyclingItem)this.items.get(slot);
         if (cyclingItem != null) {
            ItemStack itemStack = cyclingItem.get(this.currentIndexProvider.currentIndex());
            context.drawTooltip(client.textRenderer, Screen.getTooltipFromItem(client, itemStack), x, y, (Identifier)itemStack.get(DataComponentTypes.TOOLTIP_STYLE));
         }
   ...
   @Environment(EnvType.CLIENT)
   static record CyclingItem(List items, boolean isResultSlot) {
      final boolean isResultSlot;

      CyclingItem(List list, boolean bl) {
         this.items = list;
