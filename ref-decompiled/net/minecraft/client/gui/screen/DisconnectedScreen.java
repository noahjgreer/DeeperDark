...
public class DisconnectedScreen extends Screen {
   ...
   protected void init() {
      ...
      this.info.bugReportLink().ifPresent((uri) -> {
         this.grid.add(ButtonWidget.builder(REPORT_TO_SERVER_TEXT, ConfirmLinkScreen.opening(this, (URI)uri, false)).width(200).build());
      });
      this.info.report().ifPresent((path) -> {
         this.grid.add(ButtonWidget.builder(OPEN_REPORT_DIR_TEXT, (button) -> {
            Util.getOperatingSystem().open(path.getParent());
         }).width(200).build());
      });
      ButtonWidget buttonWidget;
      if (this.client.isMultiplayerEnabled()) {
         buttonWidget = ButtonWidget.builder(this.buttonLabel, (button) -> {
            this.client.setScreen(this.parent);
         }).width(200).build();
      } else {
         buttonWidget = ButtonWidget.builder(TO_TITLE_TEXT, (button) -> {
