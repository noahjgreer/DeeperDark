...
public class JigsawBlockScreen extends Screen {
   ...
   private void updateServer() {
      this.client.getNetworkHandler().sendPacket(new UpdateJigsawC2SPacket(this.jigsaw.getPos(), Identifier.of(this.nameField.getText()), Identifier.of(this.targetField.getText()), Identifier.of(this.poolField.getText()), this.finalStateField.getText(), this.joint, this.parseInt(this.selectionPriorityField.getText()), this.parseInt(this.placementPriorityField.getText())));
   }
   ...
   protected void init() {
      ...
      this.jointRotationButton = (CyclingButtonWidget)this.addDrawableChild(CyclingButtonWidget.builder(JigsawBlockEntity.Joint::asText).values((Object[])JigsawBlockEntity.Joint.values()).initially(this.joint).omitKeyText().build(this.width / 2 + 54, 160, 100, 20, JOINT_LABEL_TEXT, (button, joint) -> {
         this.joint = joint;
      }));
      ...
      this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.keepJigsaws).build(this.width / 2 - 50, 185, 100, 20, Text.translatable("jigsaw_block.keep_jigsaws"), (button, keepJigsaws) -> {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
      ...
      context.drawTextWithShadow(this.textRenderer, (Text)POOL_TEXT, this.width / 2 - 153, 10, -6250336);
      this.poolField.render(context, mouseX, mouseY, deltaTicks);
      context.drawTextWithShadow(this.textRenderer, (Text)NAME_TEXT, this.width / 2 - 153, 45, -6250336);
      this.nameField.render(context, mouseX, mouseY, deltaTicks);
      context.drawTextWithShadow(this.textRenderer, (Text)TARGET_TEXT, this.width / 2 - 153, 80, -6250336);
      this.targetField.render(context, mouseX, mouseY, deltaTicks);
      context.drawTextWithShadow(this.textRenderer, (Text)FINAL_STATE_TEXT, this.width / 2 - 153, 115, -6250336);
      this.finalStateField.render(context, mouseX, mouseY, deltaTicks);
      context.drawTextWithShadow(this.textRenderer, (Text)SELECTION_PRIORITY_TEXT, this.width / 2 - 153, 150, -6250336);
      this.placementPriorityField.render(context, mouseX, mouseY, deltaTicks);
      context.drawTextWithShadow(this.textRenderer, (Text)PLACEMENT_PRIORITY_TEXT, this.width / 2 - 50, 150, -6250336);
      ...
      if (JigsawBlock.getFacing(this.jigsaw.getCachedState()).getAxis().isVertical()) {
         context.drawTextWithShadow(this.textRenderer, (Text)JOINT_LABEL_TEXT, this.width / 2 + 53, 150, -6250336);
      }
