...
public class HandledScreens {
   ...
   public static void open(ScreenHandlerType type, MinecraftClient client, int id, Text title) {
   ...
   public static void register(ScreenHandlerType type, Provider provider) {
      Provider provider2 = (Provider)PROVIDERS.put(type, provider);
      if (provider2 != null) {
   ...
   public static boolean isMissingScreens() {
      ...
      while(var1.hasNext()) {
         ScreenHandlerType screenHandlerType = (ScreenHandlerType)var1.next();
   ...
   public interface Provider {
      default void open(Text name, ScreenHandlerType type, MinecraftClient client, int id) {
         Screen screen = this.create(type.create(id, client.player.getInventory()), client.player.getInventory(), name);
