...
extends AbstractCommandBlockScreen {
    ...
    protected void addAdditionalButtons() {
        ...
        }, this.mode).values((CommandBlockBlockEntity.Type[])CommandBlockBlockEntity.Type.values()).omitKeyText().build(this.width / 2 - 50 - 100 - 4, 165, 100, 20, Text.translatable("advMode.mode"), (button, mode) -> {
            this.mode = mode;
        }));
        this.conditionalModeButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.translatable("advMode.mode.conditional"), Text.translatable("advMode.mode.unconditional"), this.conditional).omitKeyText().build(this.width / 2 - 50, 165, 100, 20, Text.translatable("advMode.type"), (button, conditional) -> {
            this.conditional = conditional;
        }));
        this.redstoneTriggerButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(Text.translatable("advMode.mode.autoexec.bat"), Text.translatable("advMode.mode.redstoneTriggered"), this.autoActivate).omitKeyText().build(this.width / 2 + 50 + 4, 165, 100, 20, Text.translatable("advMode.triggering"), (button, autoActivate) -> {
    ...
    protected void syncSettingsToServer() {
        this.client.getNetworkHandler().sendPacket(new UpdateCommandBlockC2SPacket(this.blockEntity.getPos(), this.consoleCommandTextField.getText(), this.mode, this.blockEntity.getCommandExecutor().isTrackingOutput(), this.conditional, this.autoActivate));
    }
}
