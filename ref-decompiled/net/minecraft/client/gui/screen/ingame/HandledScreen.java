...
implements ScreenHandlerProvider<T> {
    public static final Identifier BACKGROUND_TEXTURE = Identifier.ofVanilla((String)"textures/gui/container/inventory.png");
    private static final Identifier SLOT_HIGHLIGHT_BACK_TEXTURE = Identifier.ofVanilla((String)"container/slot_highlight_back");
    private static final Identifier SLOT_HIGHLIGHT_FRONT_TEXTURE = Identifier.ofVanilla((String)"container/slot_highlight_front");
    ...
    @Nullable HandledScreen.LetGoTouchStack letGoTouchStack;
    protected int x;
    protected int y;
    ...
    protected void init() {
        ...
        this.addTooltipSubmenuHandler((TooltipSubmenuHandler)new BundleTooltipSubmenuHandler(this.client));
    }
    ...
    public void renderLetGoTouchStack(DrawContext context) {
        if (this.letGoTouchStack != null) {
            float f = MathHelper.clamp((float)((float)(Util.getMeasuringTimeMs() - this.letGoTouchStack.time) / 100.0f), (float)0.0f, (float)1.0f);
    ...
    protected List<Text> getTooltipFromItem(ItemStack stack) {
        return HandledScreen.getTooltipFromItem((MinecraftClient)this.client, (ItemStack)stack);
    }
    ...
    protected void drawSlot(DrawContext context, Slot slot, int mouseX, int mouseY) {
        ...
        if (slot == this.touchDragSlotStart && !this.touchDragStack.isEmpty() && this.touchIsRightClickDrag && !itemStack.isEmpty()) {
            ...
            if (ScreenHandler.canInsertItemIntoSlot((Slot)slot, (ItemStack)itemStack2, (boolean)true) && this.handler.canInsertIntoSlot(slot)) {
                ...
                int m = ScreenHandler.calculateStackSize((Set)this.cursorDragSlots, (int)this.heldButtonType, (ItemStack)itemStack2) + l;
                if (m > k) {
                    m = k;
    ...
    private void calculateOffset() {
        ...
        for (Slot slot : this.cursorDragSlots) {
            ...
            int k = Math.min(ScreenHandler.calculateStackSize((Set)this.cursorDragSlots, (int)this.heldButtonType, (ItemStack)itemStack) + i, j);
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (click.button() == 0 || click.button() == 1 || bl) {
            ...
            if (((Boolean)this.client.options.getTouchscreen().getValue()).booleanValue() && bl2 && this.handler.getCursorStack().isEmpty()) {
                this.close();
                return true;
            }
            if (k != -1) {
                if (((Boolean)this.client.options.getTouchscreen().getValue()).booleanValue()) {
    ...
    public boolean mouseDragged(Click click, double offsetX, double offsetY) {
        Slot slot = this.getSlotAt(click.x(), click.y());
        ItemStack itemStack = this.handler.getCursorStack();
        if (this.touchDragSlotStart != null && ((Boolean)this.client.options.getTouchscreen().getValue()).booleanValue()) {
        ...
        if (this.cursorDragging && slot != null && !itemStack.isEmpty() && (itemStack.getCount() > this.cursorDragSlots.size() || this.heldButtonType == 2) && ScreenHandler.canInsertItemIntoSlot((Slot)slot, (ItemStack)itemStack, (boolean)true) && slot.canInsert(itemStack) && this.handler.canInsertIntoSlot(slot)) {
            this.cursorDragSlots.add(slot);
            this.calculateOffset();
            return true;
        }
        if (slot == null && this.handler.getCursorStack().isEmpty()) {
    ...
    protected void onMouseClick(Slot slot, int slotId, int button, SlotActionType actionType) {
        ...
        this.client.interactionManager.clickSlot(this.handler.syncId, slotId, button, actionType, (PlayerEntity)this.client.player);
    }
