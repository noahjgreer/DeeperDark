...
extends HandledScreen<StonecutterScreenHandler> {
    private static final Identifier SCROLLER_TEXTURE = Identifier.ofVanilla((String)"container/stonecutter/scroller");
    private static final Identifier SCROLLER_DISABLED_TEXTURE = Identifier.ofVanilla((String)"container/stonecutter/scroller_disabled");
    private static final Identifier RECIPE_SELECTED_TEXTURE = Identifier.ofVanilla((String)"container/stonecutter/recipe_selected");
    private static final Identifier RECIPE_HIGHLIGHTED_TEXTURE = Identifier.ofVanilla((String)"container/stonecutter/recipe_highlighted");
    private static final Identifier RECIPE_TEXTURE = Identifier.ofVanilla((String)"container/stonecutter/recipe");
    private static final Identifier TEXTURE = Identifier.ofVanilla((String)"textures/gui/container/stonecutter.png");
    ...
    protected void drawMouseoverTooltip(DrawContext context, int x, int y) {
        ...
        if (this.canCraft) {
            ...
            CuttingRecipeDisplay.Grouping grouping = ((StonecutterScreenHandler)this.handler).getAvailableRecipes();
    ...
    private void renderRecipeBackground(DrawContext context, int mouseX, int mouseY, int x, int y, int scrollOffset) {
        for (int i = this.scrollOffset; i < scrollOffset && i < ((StonecutterScreenHandler)this.handler).getAvailableRecipeCount(); ++i) {
            ...
            int l = j / 4;
            int m = y + l * 18 + 2;
            Identifier identifier = i == ((StonecutterScreenHandler)this.handler).getSelectedRecipe() ? RECIPE_SELECTED_TEXTURE : (mouseX >= k && mouseY >= m && mouseX < k + 16 && mouseY < m + 18 ? RECIPE_HIGHLIGHTED_TEXTURE : RECIPE_TEXTURE);
            int n = m - 1;
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, identifier, k, n, 16, 18);
            if (mouseX < k || mouseY < n || mouseX >= k + 16 || mouseY >= n + 18) continue;
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        if (this.canCraft) {
            ...
            for (int l = this.scrollOffset; l < k; ++l) {
                ...
                if (!(d >= 0.0) || !(e >= 0.0) || !(d < 16.0) || !(e < 18.0) || !((StonecutterScreenHandler)this.handler).onButtonClick((PlayerEntity)this.client.player, l)) continue;
                MinecraftClient.getInstance().getSoundManager().play((SoundInstance)PositionedSoundInstance.ui((SoundEvent)SoundEvents.UI_STONECUTTER_SELECT_RECIPE, (float)1.0f));
