...
extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(CyclingButtonWidget.builder(value -> Text.translatable("structure_block.mode." + value.asString()), this.mode).values((List<StructureBlockMode>)MODES_EXCEPT_DATA, (List<StructureBlockMode>)MODES).omitKeyText().build(this.width / 2 - 4 - 150, 185, 50, 20, Text.literal("MODE"), (button, mode) -> {
        ...
        this.ignoreEntitiesButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(!this.structureBlock.shouldIgnoreEntities()).omitKeyText().build(this.width / 2 + 4 + 100, 160, 50, 20, INCLUDE_ENTITIES_TEXT, (button, includeEntities) -> this.structureBlock.setIgnoreEntities(includeEntities == false)));
        this.strictButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.isStrict()).omitKeyText().build(this.width / 2 + 4 + 100, 120, 50, 20, STRICT_TEXT, (button, strict) -> this.structureBlock.setStrict((boolean)strict)));
        this.mirrorButton = this.addDrawableChild(CyclingButtonWidget.builder(BlockMirror::getName, this.mirror).values((BlockMirror[])BlockMirror.values()).omitKeyText().build(this.width / 2 - 20, 185, 40, 20, Text.literal("MIRROR"), (button, mirror) -> this.structureBlock.setMirror((BlockMirror)mirror)));
        this.showAirButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowAir()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_AIR_TEXT, (button, showAir) -> this.structureBlock.setShowAir((boolean)showAir)));
        this.showBoundingBoxButton = this.addDrawableChild(CyclingButtonWidget.onOffBuilder(this.structureBlock.shouldShowBoundingBox()).omitKeyText().build(this.width / 2 + 4 + 100, 80, 50, 20, SHOW_BOUNDING_BOX_TEXT, (button, showBoundingBox) -> this.structureBlock.setShowBoundingBox((boolean)showBoundingBox)));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (structureBlockMode == StructureBlockMode.LOAD || structureBlockMode == StructureBlockMode.SAVE) {
            ...
            context.drawTextWithShadow(this.textRenderer, INCLUDE_ENTITIES_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(INCLUDE_ENTITIES_TEXT), 150, -6250336);
        }
        if (structureBlockMode == StructureBlockMode.SAVE) {
            ...
            context.drawTextWithShadow(this.textRenderer, DETECT_SIZE_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(DETECT_SIZE_TEXT), 110, -6250336);
            context.drawTextWithShadow(this.textRenderer, SHOW_AIR_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(SHOW_AIR_TEXT), 70, -6250336);
        }
        if (structureBlockMode == StructureBlockMode.LOAD) {
            ...
            context.drawTextWithShadow(this.textRenderer, STRICT_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(STRICT_TEXT), 110, -6250336);
            context.drawTextWithShadow(this.textRenderer, SHOW_BOUNDING_BOX_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(SHOW_BOUNDING_BOX_TEXT), 70, -6250336);
        }
        if (structureBlockMode == StructureBlockMode.DATA) {
