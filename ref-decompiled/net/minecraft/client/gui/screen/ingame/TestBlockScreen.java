...
public class TestBlockScreen extends Screen {
   ...
   public void init() {
      ...
      this.addDrawableChild(CyclingButtonWidget.builder(TestBlockMode::getName).values((Collection)MODES).omitKeyText().initially(this.mode).build(this.width / 2 - 4 - 150, 185, 50, 20, TITLE_TEXT, (button, mode) -> {
         this.setMode(mode);
      }));
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
      ...
      if (this.mode != TestBlockMode.START) {
         context.drawTextWithShadow(this.textRenderer, (Text)MESSAGE_TEXT, this.width / 2 - 153, 70, -6250336);
      }

      context.drawTextWithShadow(this.textRenderer, (Text)this.mode.getInfo(), this.width / 2 - 153, 174, -6250336);
   }
   ...
   static {
      TITLE_TEXT = Text.translatable(Blocks.TEST_BLOCK.getTranslationKey());
      MESSAGE_TEXT = Text.translatable("test_block.message");
   }
}
