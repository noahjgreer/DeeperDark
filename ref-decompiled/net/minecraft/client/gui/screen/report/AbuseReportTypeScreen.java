...
public class AbuseReportTypeScreen extends Screen {
   ...
   protected void init() {
      ...
      this.layout.add(new TextWidget(this.title, this.textRenderer), (Positioner)this.layout.copyPositioner().marginBottom(6));
      this.layout.add((new MultilineTextWidget(MESSAGE_TEXT, this.textRenderer)).setCentered(true), (Positioner)this.layout.copyPositioner().marginBottom(6));
      ButtonWidget buttonWidget = (ButtonWidget)this.layout.add(ButtonWidget.builder(CHAT_TYPE_TEXT, (button) -> {
         this.client.setScreen(new ChatReportScreen(this.parent, this.context, this.selectedPlayer.getUuid()));
      }).build());
      ...
      this.layout.add(ButtonWidget.builder(SKIN_TYPE_TEXT, (button) -> {
         this.client.setScreen(new SkinReportScreen(this.parent, this.context, this.selectedPlayer.getUuid(), this.selectedPlayer.getSkinSupplier()));
      }).build());
      this.layout.add(ButtonWidget.builder(NAME_TYPE_TEXT, (button) -> {
         this.client.setScreen(new UsernameReportScreen(this.parent, this.context, this.selectedPlayer.getUuid(), this.selectedPlayer.getName()));
      }).build());
