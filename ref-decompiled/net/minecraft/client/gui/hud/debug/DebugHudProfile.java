...
public class DebugHudProfile {
    ...
    public void readProfileFile() {
        try {
            ...
            Dynamic dynamic = new Dynamic((DynamicOps)JsonOps.INSTANCE, (Object)StrictJsonParser.parse((String)FileUtils.readFileToString((File)this.file, (Charset)StandardCharsets.UTF_8)));
    ...
    public void setProfileType(DebugProfileType type) {
        this.type = type;
        Map map = (Map)DebugHudEntries.PROFILES.get(type);
    ...
    public boolean toggleVisibility(Identifier entryId) {
        ...
        DebugHudEntryVisibility debugHudEntryVisibility2 = debugHudEntryVisibility = (DebugHudEntryVisibility)this.visibilityMap.get(entryId);
        int n = 0;
    ...
    public void updateVisibleEntries() {
        ...
        for (Map.Entry entry : this.visibilityMap.entrySet()) {
            DebugHudEntry debugHudEntry;
            if (entry.getValue() != DebugHudEntryVisibility.ALWAYS_ON && (!this.f3Enabled || entry.getValue() != DebugHudEntryVisibility.IN_OVERLAY) || (debugHudEntry = DebugHudEntries.get((Identifier)((Identifier)entry.getKey()))) == null || !debugHudEntry.canShow(bl)) continue;
