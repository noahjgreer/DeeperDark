...
public class PieChart {
    ...
    public void render(DrawContext context) {
        ...
        String string = ProfileResult.getHumanReadableName((String)profilerTiming.name);
        Object string2 = "";
    ...
    public void select(int index) {
        ...
        } else if (--index < list.size() && !"unspecified".equals(((ProfilerTiming)list.get((int)index)).name)) {
            if (!this.currentPath.isEmpty()) {
                this.currentPath = this.currentPath + "\u001e";
            }
            this.currentPath = this.currentPath + ((ProfilerTiming)list.get((int)index)).name;
        }
    }
}
