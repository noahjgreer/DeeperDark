...
extends DebugChart {
    ...
    protected void renderThresholds(DrawContext context, int x, int width, int height) {
        float f = (float)TimeHelper.SECOND_IN_MILLIS / ((Float)this.millisPerTickSupplier.get()).floatValue();
        this.drawBorderedText(context, String.format(Locale.ROOT, "%.1f TPS", Float.valueOf(f)), x + 1, height - 60 + 1);
    }
    ...
    protected String format(double value) {
        return String.format(Locale.ROOT, "%d ms", (int)Math.round(TickChart.toMillisecondsPerTick((double)value)));
    }

    protected int getHeight(double value) {
        return (int)Math.round(TickChart.toMillisecondsPerTick((double)value) * 60.0 / (double)((Float)this.millisPerTickSupplier.get()).floatValue());
    }

    protected int getColor(long value) {
        float f = ((Float)this.millisPerTickSupplier.get()).floatValue();
        return this.getColor(TickChart.toMillisecondsPerTick((double)value), (double)f, -16711936, (double)f * 1.125, -256, (double)f * 1.25, -65536);
    }
