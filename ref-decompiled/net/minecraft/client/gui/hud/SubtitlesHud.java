...
public class SubtitlesHud implements SoundInstanceListener {
   ...
   private final List audibleEntries = new ArrayList();
   ...
   public void render(DrawContext context) {
      ...
      if (this.enabled) {
         ...
         while(var7.hasNext()) {
            SubtitleEntry subtitleEntry = (SubtitleEntry)var7.next();
         ...
         if (!this.audibleEntries.isEmpty()) {
            ...
            while(iterator.hasNext()) {
               subtitleEntry2 = (SubtitleEntry)iterator.next();
            ...
            iterator = this.audibleEntries.iterator();
   ...
   public void onSoundPlayed(SoundInstance sound, WeightedSoundSet soundSet, float range) {
      if (soundSet.getSubtitle() != null) {
         ...
         if (!this.entries.isEmpty()) {
            ...
            while(var5.hasNext()) {
               SubtitleEntry subtitleEntry = (SubtitleEntry)var5.next();
   ...
   static class SubtitleEntry {
      ...
      public SoundEntry getNearestSound(Vec3d pos) {
         if (this.sounds.isEmpty()) {
            ...
            return this.sounds.size() == 1 ? (SoundEntry)this.sounds.getFirst() : (SoundEntry)this.sounds.stream().min(Comparator.comparingDouble((soundPos) -> {
               return soundPos.location().distanceTo(pos);
            })).orElse((Object)null);
         }
      }
      ...
      public void reset(Vec3d pos) {
         this.sounds.removeIf((sound) -> {
            return pos.equals(sound.location());
         });
         this.sounds.add(new SoundEntry(pos, Util.getMeasuringTimeMs()));
      }
      ...
      public void removeExpired(double expiry) {
         ...
         this.sounds.removeIf((sound) -> {
            return (double)(l - sound.time()) > expiry;
         });
      }
