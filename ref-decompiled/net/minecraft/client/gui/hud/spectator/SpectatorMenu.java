...
public class SpectatorMenu {
   ...
   public SpectatorMenuCommand getCommand(int slot) {
      ...
      if (this.page > 0 && slot == 0) {
         ...
         return i >= 0 && i < this.currentGroup.getCommands().size() ? (SpectatorMenuCommand)MoreObjects.firstNonNull((SpectatorMenuCommand)this.currentGroup.getCommands().get(i), BLANK_COMMAND) : BLANK_COMMAND;
      }
   }
   ...
   static class CloseSpectatorMenuCommand implements SpectatorMenuCommand {
      ...
      public void renderIcon(DrawContext context, float brightness, float alpha) {
         context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SpectatorMenu.CLOSE_TEXTURE, 0, 0, 16, 16, ColorHelper.fromFloats(alpha, brightness, brightness, brightness));
      }
   ...
   private static class ChangePageSpectatorMenuCommand implements SpectatorMenuCommand {
      ...
      public void renderIcon(DrawContext context, float brightness, float alpha) {
         ...
         if (this.direction < 0) {
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SpectatorMenu.SCROLL_LEFT_TEXTURE, 0, 0, 16, 16, i);
         } else {
            context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, SpectatorMenu.SCROLL_RIGHT_TEXTURE, 0, 0, 16, 16, i);
         }
