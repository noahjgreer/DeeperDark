 ...
 *  net.minecraft.text.HoverEvent$ShowEntity
 *  net.minecraft.text.HoverEvent$ShowItem
...
public class DrawContext {
    ...
    public void fillGradient(int startX, int startY, int endX, int endY, int colorStart, int colorEnd) {
        this.fill(RenderPipelines.GUI, TextureSetup.empty(), startX, startY, endX, endY, colorStart, Integer.valueOf(colorEnd));
    }
    ...
    private void fill(RenderPipeline pipeline, TextureSetup textureSetup, int x1, int y1, int x2, int y2, int color, @Nullable Integer color2) {
        this.state.addSimpleElement((SimpleGuiElementRenderState)new ColoredQuadGuiElementRenderState(pipeline, textureSetup, (Matrix3x2fc)new Matrix3x2f((Matrix3x2fc)this.matrices), x1, y1, x2, y2, color, color2 != null ? color2 : color, this.scissorStack.peekLast()));
    }
    ...
    public void drawText(TextRenderer textRenderer, @Nullable String text, int x, int y, int color, boolean shadow) {
        ...
        this.drawText(textRenderer, Language.getInstance().reorder(StringVisitable.plain((String)text)), x, y, color, shadow);
    }
    ...
    public void drawText(TextRenderer textRenderer, OrderedText text, int x, int y, int color, boolean shadow) {
        if (ColorHelper.getAlpha((int)color) == 0) {
            return;
        }
    ...
    public void drawTextWithBackground(TextRenderer textRenderer, Text text, int x, int y, int width, int color) {
        ...
        if (i != 0) {
            int j = 2;
            Objects.requireNonNull(textRenderer);
            this.fill(x - 2, y - 2, x + width + 2, y + 9 + 2, ColorHelper.mix((int)i, (int)color));
        }
        this.drawText(textRenderer, text, x, y, color, true);
    }
    ...
    public void drawGuiTexture(RenderPipeline pipeline, Identifier sprite, int x, int y, int width, int height, float alpha) {
        this.drawGuiTexture(pipeline, sprite, x, y, width, height, ColorHelper.getWhite((float)alpha));
    }
    ...
    public void drawGuiTexture(RenderPipeline pipeline, Identifier sprite, int x, int y, int width, int height, int color) {
        ...
        Scaling scaling2 = scaling = DrawContext.getScaling((Sprite)sprite2);
    ...
    public void drawGuiTexture(RenderPipeline pipeline, Identifier sprite, int textureWidth, int textureHeight, int u, int v, int x, int y, int width, int height, int color) {
        Sprite sprite2 = this.spriteAtlasTexture.getSprite(sprite);
        Scaling scaling = DrawContext.getScaling((Sprite)sprite2);
    ...
    private void drawTexturedQuad(RenderPipeline pipeline, GpuTextureView texture, GpuSampler sampler, int x1, int y1, int x2, int y2, float u1, float v1, float u2, float v2, int color) {
        this.state.addSimpleElement((SimpleGuiElementRenderState)new TexturedQuadGuiElementRenderState(pipeline, TextureSetup.of((GpuTextureView)texture, (GpuSampler)sampler), new Matrix3x2f((Matrix3x2fc)this.matrices), x1, y1, x2, y2, u1, v1, u2, v2, color, this.scissorStack.peekLast()));
    }
    ...
    private void drawTiledTexturedQuad(RenderPipeline pipeline, GpuTextureView texture, GpuSampler sampler, int tileWidth, int tileHeight, int x0, int y0, int x1, int y1, float u0, float v0, float u1, float v1, int color) {
        this.state.addSimpleElement((SimpleGuiElementRenderState)new TiledTexturedQuadGuiElementRenderState(pipeline, TextureSetup.of((GpuTextureView)texture, (GpuSampler)sampler), new Matrix3x2f((Matrix3x2fc)this.matrices), tileWidth, tileHeight, x0, y0, x1, y1, u0, v0, u1, v1, color, this.scissorStack.peekLast()));
    }

    public void drawItem(ItemStack item, int x, int y) {
        this.drawItem((LivingEntity)this.client.player, (World)this.client.world, item, x, y, 0);
    }

    public void drawItem(ItemStack stack, int x, int y, int seed) {
        this.drawItem((LivingEntity)this.client.player, (World)this.client.world, stack, x, y, seed);
    }
    ...
    public void drawItemWithoutEntity(ItemStack stack, int x, int y, int seed) {
        this.drawItem(null, (World)this.client.world, stack, x, y, seed);
    }
    ...
    private void drawItem(@Nullable LivingEntity entity, @Nullable World world, ItemStack stack, int x, int y, int seed) {
        ...
        this.client.getItemModelManager().clearAndUpdate((ItemRenderState)keyedItemRenderState, stack, ItemDisplayContext.GUI, world, (HeldItemContext)entity, seed);
        try {
    ...
    public void drawItemTooltip(TextRenderer textRenderer, ItemStack stack, int x, int y) {
        this.drawTooltip(textRenderer, Screen.getTooltipFromItem((MinecraftClient)this.client, (ItemStack)stack), stack.getTooltipData(), x, y, (Identifier)stack.get(DataComponentTypes.TOOLTIP_STYLE));
    }
    ...
    public void drawTooltip(TextRenderer textRenderer, List<Text> text, Optional<TooltipData> data, int x, int y, @Nullable Identifier texture) {
        ...
        data.ifPresent(datax -> list.add(list.isEmpty() ? 0 : 1, TooltipComponent.of((TooltipData)datax)));
    ...
    public void drawTooltipImmediately(TextRenderer textRenderer, List<TooltipComponent> components, int x, int y, TooltipPositioner positioner, @Nullable Identifier texture) {
        ...
        int o = vector2ic.y();
        this.matrices.pushMatrix();
        TooltipBackgroundRenderer.render((DrawContext)this, (int)n, (int)o, (int)l, (int)m, (Identifier)texture);
        int p = o;
    ...
    private void drawItemBar(ItemStack stack, int x, int y) {
        if (stack.isItemBarVisible()) {
            ...
            this.fill(RenderPipelines.GUI, i, j, i + stack.getItemBarStep(), j + 1, ColorHelper.fullAlpha((int)stack.getItemBarColor()));
        }
    }
    ...
    private void drawCooldownProgress(ItemStack stack, int x, int y) {
        ...
        if (f > 0.0f) {
            int i = y + MathHelper.floor((float)(16.0f * (1.0f - f)));
            int j = i + MathHelper.ceil((float)(16.0f * f));
    ...
    public void drawHoverEvent(TextRenderer textRenderer, @Nullable Style style, int mouseX, int mouseY) {
        ...
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{HoverEvent.ShowItem.class, HoverEvent.ShowEntity.class, HoverEvent.ShowText.class}, (Object)hoverEvent2, n)) {
            case 0: {
                ...
                try {
                    ItemStack itemStack;
                    itemStack2 = itemStack = showItem.item();
                }
                catch (Throwable throwable) {
            ...
            case 2: {
                ...
                {
                    Text text;
                    text2 = text = showText.value();
                }
                this.drawOrderedTooltip(textRenderer, textRenderer.wrapLines((StringVisitable)text2, Math.max(this.getScaledWindowWidth() / 2, 200)), mouseX, mouseY);
                return;
            }
        }
    }
    ...
    public void drawMap(MapRenderState mapState) {
        ...
        for (MapRenderState.Decoration decoration : mapState.decorations) {
            ...
            float f = textRenderer.getWidth((StringVisitable)decoration.name);
            float f2 = 25.0f / f;
            Objects.requireNonNull(textRenderer);
            float g = MathHelper.clamp((float)f2, (float)0.0f, (float)(6.0f / 9.0f));
            this.matrices.pushMatrix();
    ...
    public void addEntity(EntityRenderState entityState, float scale, Vector3f translation, Quaternionf rotation, @Nullable Quaternionf overrideCameraAngle, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new EntityGuiElementRenderState(entityState, translation, rotation, overrideCameraAngle, x1, y1, x2, y2, scale, this.scissorStack.peekLast()));
    }
    ...
    public void addPlayerSkin(PlayerEntityModel playerModel, Identifier texture, float scale, float xRotation, float yRotation, float yPivot, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new PlayerSkinGuiElementRenderState(playerModel, texture, xRotation, yRotation, yPivot, x1, y1, x2, y2, scale, this.scissorStack.peekLast()));
    }
    ...
    public void addBookModel(BookModel bookModel, Identifier texture, float scale, float open, float flip, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new BookModelGuiElementRenderState(bookModel, texture, open, flip, x1, y1, x2, y2, scale, this.scissorStack.peekLast()));
    }
    ...
    public void addBannerResult(BannerFlagBlockModel bannerModel, DyeColor baseColor, BannerPatternsComponent resultBannerPatterns, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new BannerResultGuiElementRenderState(bannerModel, baseColor, resultBannerPatterns, x1, y1, x2, y2, this.scissorStack.peekLast()));
    }

    public void addSign(Model.SinglePartModel model, float scale, WoodType woodType, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new SignGuiElementRenderState(model, woodType, x1, y1, x2, y2, scale, this.scissorStack.peekLast()));
    }

    public void addProfilerChart(List<ProfilerTiming> chartData, int x1, int y1, int x2, int y2) {
        this.state.addSpecialElement((SpecialGuiElementRenderState)new ProfilerChartGuiElementRenderState(chartData, x1, y1, x2, y2, this.scissorStack.peekLast()));
    }
    ...
    public DrawnTextConsumer getHoverListener(ClickableWidget widget, HoverType hoverType) {
        return new TextConsumerImpl(this, this.getTransformationForCurrentState(widget.getAlpha()), hoverType, null);
    }
    ...
    public DrawnTextConsumer getTextConsumer(HoverType hoverType, @Nullable Consumer<Style> styleCallback) {
        return new TextConsumerImpl(this, this.getTransformationForCurrentState(1.0f), hoverType, styleCallback);
    }
