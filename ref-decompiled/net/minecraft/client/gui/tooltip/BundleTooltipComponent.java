...
implements TooltipComponent {
    private static final Identifier BUNDLE_PROGRESS_BAR_BORDER_TEXTURE = Identifier.ofVanilla((String)"container/bundle/bundle_progressbar_border");
    private static final Identifier BUNDLE_PROGRESS_BAR_FILL_TEXTURE = Identifier.ofVanilla((String)"container/bundle/bundle_progressbar_fill");
    private static final Identifier BUNDLE_PROGRESS_BAR_FULL_TEXTURE = Identifier.ofVanilla((String)"container/bundle/bundle_progressbar_full");
    private static final Identifier BUNDLE_SLOT_HIGHLIGHT_BACK_TEXTURE = Identifier.ofVanilla((String)"container/bundle/slot_highlight_back");
    private static final Identifier BUNDLE_SLOT_HIGHLIGHT_FRONT_TEXTURE = Identifier.ofVanilla((String)"container/bundle/slot_highlight_front");
    private static final Identifier BUNDLE_SLOT_BACKGROUND_TEXTURE = Identifier.ofVanilla((String)"container/bundle/slot_background");
    ...
    public int getHeight(TextRenderer textRenderer) {
        return this.bundleContents.isEmpty() ? BundleTooltipComponent.getHeightOfEmpty((TextRenderer)textRenderer) : this.getHeightOfNonEmpty();
    }
    ...
    private static int getHeightOfEmpty(TextRenderer textRenderer) {
        return BundleTooltipComponent.getDescriptionHeight((TextRenderer)textRenderer) + 13 + 8;
    }
    ...
    private int getRows() {
        return MathHelper.ceilDiv((int)this.getNumVisibleSlots(), (int)4);
    }
    ...
    private void drawEmptyTooltip(TextRenderer textRenderer, int x, int y, int width, int height, DrawContext context) {
        BundleTooltipComponent.drawEmptyDescription((int)(x + this.getXMargin(width)), (int)y, (TextRenderer)textRenderer, (DrawContext)context);
        this.drawProgressBar(x + this.getXMargin(width), y + BundleTooltipComponent.getDescriptionHeight((TextRenderer)textRenderer) + 4, textRenderer, context);
    }
    ...
    private void drawNonEmptyTooltip(TextRenderer textRenderer, int x, int y, int width, int height, DrawContext context) {
        ...
        List list = this.firstStacksInContents(this.bundleContents.getNumberOfStacksShown());
        ...
        for (int l = 1; l <= this.getRows(); ++l) {
            for (int m = 1; m <= 4; ++m) {
                int n = i - m * 24;
                int o = j - l * 24;
                if (BundleTooltipComponent.shouldDrawExtraItemsCount((boolean)bl, (int)m, (int)l)) {
                    BundleTooltipComponent.drawExtraItemsCount((int)n, (int)o, (int)this.numContentItemsAfter(list), (TextRenderer)textRenderer, (DrawContext)context);
                    continue;
                }
                if (!BundleTooltipComponent.shouldDrawItem((List)list, (int)k)) continue;
    ...
    private void drawSelectedItemTooltip(TextRenderer textRenderer, DrawContext drawContext, int x, int y, int width) {
        if (this.bundleContents.hasSelectedStack()) {
            ...
            int j = x + width / 2 - 12;
            TooltipComponent tooltipComponent = TooltipComponent.of((OrderedText)text.asOrderedText());
            drawContext.drawTooltipImmediately(textRenderer, List.of(tooltipComponent), j - i / 2, y - 15, HoveredTooltipPositioner.INSTANCE, (Identifier)itemStack.get(DataComponentTypes.TOOLTIP_STYLE));
        }
    }
    ...
    private static void drawEmptyDescription(int x, int y, TextRenderer textRenderer, DrawContext drawContext) {
        drawContext.drawWrappedTextWithShadow(textRenderer, (StringVisitable)BUNDLE_EMPTY_DESCRIPTION, x, y, 96, -5592406);
    }

    private static int getDescriptionHeight(TextRenderer textRenderer) {
        int n = textRenderer.wrapLines((StringVisitable)BUNDLE_EMPTY_DESCRIPTION, 96).size();
