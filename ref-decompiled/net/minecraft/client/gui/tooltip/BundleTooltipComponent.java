...
public class BundleTooltipComponent implements TooltipComponent {
   ...
   private List firstStacksInContents(int numberOfStacksShown) {
   ...
   private static boolean shouldDrawItem(List items, int itemIndex) {
      return items.size() >= itemIndex;
   }

   private int numContentItemsAfter(List items) {
      return this.bundleContents.stream().skip((long)items.size()).mapToInt(ItemStack::getCount).sum();
   }

   private void drawItem(int index, int x, int y, List stacks, int seed, TextRenderer textRenderer, DrawContext drawContext) {
   ...
   private static void drawExtraItemsCount(int x, int y, int numExtra, TextRenderer textRenderer, DrawContext drawContext) {
      drawContext.drawCenteredTextWithShadow(textRenderer, (String)("+" + numExtra), x + 12, y + 10, -1);
   }
   ...
   private void drawProgressBar(int x, int y, TextRenderer textRenderer, DrawContext drawContext) {
      ...
      if (text != null) {
         drawContext.drawCenteredTextWithShadow(textRenderer, (Text)text, x + 48, y + 3, -1);
      }
