...
extends NoRenderParticle {
    ...
    public FireworksSparkParticle.FireworkParticle(ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ, ParticleManager particleManager, List<FireworkExplosionComponent> fireworkExplosions) {
        super(world, x, y, z);
        ...
        for (FireworkExplosionComponent fireworkExplosionComponent : fireworkExplosions) {
            if (!fireworkExplosionComponent.hasTwinkle()) continue;
            this.flicker = true;
            this.maxAge += 15;
            break;
        }
    ...
    public void tick() {
        ...
        if (this.age == 0) {
            ...
            if (this.explosions.size() >= 3) {
                ...
                for (FireworkExplosionComponent fireworkExplosionComponent : this.explosions) {
                    if (fireworkExplosionComponent.shape() != FireworkExplosionComponent.Type.LARGE_BALL) continue;
                    bl2 = true;
                    break;
                }
            }
        ...
        if (this.age % 2 == 0 && this.age / 2 < this.explosions.size()) {
            ...
            this.particleManager.addParticle(TintedParticleEffect.create(ParticleTypes.FLASH, j), this.x, this.y, this.z, 0.0, 0.0, 0.0);
        }
        ++this.age;
    ...
    private boolean isFar() {
        ...
        return minecraftClient.gameRenderer.getCamera().getCameraPos().squaredDistanceTo(this.x, this.y, this.z) >= 256.0;
    }
