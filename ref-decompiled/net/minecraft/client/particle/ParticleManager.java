...
public class ParticleManager {
    ...
    private <T extends ParticleEffect> @Nullable Particle createParticle(T parameters, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
        ParticleFactory particleFactory = (ParticleFactory)this.spriteManager.getParticleFactories().get(Registries.PARTICLE_TYPE.getRawId((Object)parameters.getType()));
    ...
    public void tick() {
        ...
        if (!this.newParticles.isEmpty()) {
            Particle particle2;
            while ((particle2 = (Particle)this.newParticles.poll()) != null) {
                this.particles.computeIfAbsent(particle2.textureSheet(), arg_0 -> this.createParticleRenderer(arg_0)).add(particle2);
            }
        }
    }
