...
public class Keyboard {
    ...
    private boolean processF3(KeyInput key) {
        ...
        if (gameOptions.debugProfilingKey.matchesKey(key)) {
            if (this.client.toggleDebugProfiler(this::debugLog)) {
                this.debugLog(Text.translatable("debug.profiling.start", 10, gameOptions.debugModifierKey.getBoundKeyLocalizedText(), gameOptions.debugProfilingKey.getBoundKeyLocalizedText()));
            }
            bl = true;
        }
            ...
            case ENTITY: {
                ...
                if (queryServer) {
                    this.client.player.networkHandler.getDataQueryHandler().queryEntityNbt(entity.getId(), nbt -> {
                        this.copyEntity(identifier, entity.getEntityPos(), (NbtCompound)nbt);
    ...
    private void onKey(long window, @KeyInput.KeyAction int action, KeyInput input) {
        ...
        boolean bl4 = bl3 = !gameOptions.debugCrashKey.isUnbound() && InputUtil.isKeyPressed(this.client.getWindow(), gameOptions.debugCrashKey.boundKey.getCode());
        ...
        if (!(action != 1 || this.client.currentScreen instanceof KeybindsScreen && ((KeybindsScreen)screen2).lastKeyCodeUpdateTime > Util.getMeasuringTimeMs() - 20L)) {
            if (gameOptions.fullscreenKey.matchesKey(input)) {
                ...
                gameOptions.getFullscreen().setValue(bl42);
                gameOptions.write();
        ...
        if (action != 0) {
            ...
            if (bl43) {
                if (input.hasCtrlOrCmd() && input.key() == 66 && this.client.getNarratorManager().isActive() && gameOptions.getNarratorHotkey().getValue().booleanValue()) {
