...
extends AbstractEndPortalBlockEntityRenderer<EndGatewayBlockEntity, EndGatewayBlockEntityRenderState> {
    ...
    @Override
    public void updateRenderState(EndGatewayBlockEntity endGatewayBlockEntity, EndGatewayBlockEntityRenderState endGatewayBlockEntityRenderState, float f, Vec3d vec3d,  @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlayCommand) {
        ...
        if (endGatewayBlockEntity.isRecentlyGenerated() || endGatewayBlockEntity.needsCooldownBeforeTeleporting() && world != null) {
            ...
            endGatewayBlockEntityRenderState.beamSpan = MathHelper.floor((double)endGatewayBlockEntityRenderState.beamHeight * d);
    ...
    public void render(EndGatewayBlockEntityRenderState endGatewayBlockEntityRenderState, MatrixStack matrixStack, OrderedRenderCommandQueue orderedRenderCommandQueue, CameraRenderState cameraRenderState) {
        if (endGatewayBlockEntityRenderState.beamSpan > 0) {
            BeaconBlockEntityRenderer.renderBeam(matrixStack, orderedRenderCommandQueue, BEAM_TEXTURE, endGatewayBlockEntityRenderState.beamHeight, endGatewayBlockEntityRenderState.beamRotationDegrees, -endGatewayBlockEntityRenderState.beamSpan, endGatewayBlockEntityRenderState.beamSpan * 2, endGatewayBlockEntityRenderState.beamColor, 0.15f, 0.175f);
        }
