...
implements BlockEntityRenderer<T, S> {
    public static final Identifier SKY_TEXTURE = Identifier.ofVanilla((String)"textures/environment/end_sky.png");
    public static final Identifier PORTAL_TEXTURE = Identifier.ofVanilla((String)"textures/entity/end_portal.png");

    public void updateRenderState(T endPortalBlockEntity, S endPortalBlockEntityRenderState, float f, Vec3d vec3d, // Could not load outer class - annotation placement on inner may be incorrect
     @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlayCommand) {
        super.updateRenderState(endPortalBlockEntity, endPortalBlockEntityRenderState, f, vec3d, crumblingOverlayCommand);
        ((EndPortalBlockEntityRenderState)endPortalBlockEntityRenderState).sides.clear();
        for (Direction direction : Direction.values()) {
            if (!endPortalBlockEntity.shouldDrawSide(direction)) continue;
