...
public class BlockEntityRendererFactories {
   private static final Map FACTORIES = Maps.newHashMap();

   public static void register(BlockEntityType type, BlockEntityRendererFactory factory) {
      FACTORIES.put(type, factory);
   }

   public static Map reload(BlockEntityRendererFactory.Context args) {
      ImmutableMap.Builder builder = ImmutableMap.builder();
      FACTORIES.forEach((type, factory) -> {
         try {
            builder.put(type, factory.create(args));
         } catch (Exception var5) {
            throw new IllegalStateException("Failed to create model for " + String.valueOf(Registries.BLOCK_ENTITY_TYPE.getId(type)), var5);
         }
      });
