...
public class BeaconBlockEntityRenderer implements BlockEntityRenderer {
   ...
   public void render(BlockEntity entity, float tickProgress, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, Vec3d cameraPos) {
      ...
      List list = ((BeamEmitter)entity).getBeamSegments();
      int i = 0;
   ...
   public static void renderBeam(MatrixStack matrices, VertexConsumerProvider vertexConsumers, Identifier textureId, float tickProgress, float heightScale, long worldTime, int yOffset, int maxY, int color, float innerRadius, float outerRadius) {
      ...
      renderBeamLayer(matrices, vertexConsumers.getBuffer(RenderLayer.getBeaconBeam(textureId, true)), ColorHelper.withAlpha(32, color), yOffset, i, j, k, outerRadius, m, n, outerRadius, outerRadius, outerRadius, 0.0F, 1.0F, u, t);
      matrices.pop();
   }
   ...
   public boolean isInRenderDistance(BlockEntity blockEntity, Vec3d pos) {
      return Vec3d.ofCenter(blockEntity.getPos()).multiply(1.0, 0.0, 1.0).isInRange(pos.multiply(1.0, 0.0, 1.0), (double)this.getRenderDistance());
   }
}
