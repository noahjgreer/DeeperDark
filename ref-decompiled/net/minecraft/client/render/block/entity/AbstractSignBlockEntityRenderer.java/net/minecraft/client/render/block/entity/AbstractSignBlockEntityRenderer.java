...
implements BlockEntityRenderer<SignBlockEntity, SignBlockEntityRenderState> {
    ...
    protected void renderSign(MatrixStack matrices, int lightmapCoords, WoodType woodType, Model.SinglePartModel model,  @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlay, OrderedRenderCommandQueue queue) {
        ...
        queue.submitModel(model, Unit.INSTANCE, matrices, renderLayer, lightmapCoords, OverlayTexture.DEFAULT_UV, -1, this.spriteHolder.getSprite(spriteIdentifier), 0, crumblingOverlay);
        matrices.pop();
    }
    ...
    private void renderText(SignBlockEntityRenderState renderState, MatrixStack matrices, OrderedRenderCommandQueue queue, boolean front) {
        ...
        int i = AbstractSignBlockEntityRenderer.getTextColor(signText);
        int j = 4 * renderState.textLineHeight / 2;
        OrderedText[] orderedTexts = signText.getOrderedMessages(renderState.filterText, textx -> {
            List<OrderedText> list = this.textRenderer.wrapLines((StringVisitable)textx, signBlockEntityRenderState.maxTextWidth);
    ...
    @Override
    public void updateRenderState(SignBlockEntity signBlockEntity, SignBlockEntityRenderState signBlockEntityRenderState, float f, Vec3d vec3d,  @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlayCommand) {
        ...
        signBlockEntityRenderState.renderTextOutline = AbstractSignBlockEntityRenderer.shouldRenderTextOutline(signBlockEntity.getPos());
    }
