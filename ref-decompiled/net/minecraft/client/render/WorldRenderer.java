 ...
 *  net.minecraft.client.render.WorldRenderer$Gizmos
...
AutoCloseable {
    private static final Identifier TRANSPARENCY = Identifier.ofVanilla((String)"transparency");
    private static final Identifier ENTITY_OUTLINE = Identifier.ofVanilla((String)"entity_outline");
    ...
    private @Nullable PostEffectProcessor getTransparencyPostEffectProcessor() {
        ...
        if (postEffectProcessor == null) {
            this.client.options.getImprovedTransparency().setValue((Object)false);
    ...
    public void reload() {
        ...
        BlockRenderLayers.setCutoutLeaves((boolean)((Boolean)this.client.options.getCutoutLeaves().getValue()));
        LeavesBlock.setCutoutLeaves((boolean)((Boolean)this.client.options.getCutoutLeaves().getValue()));
        this.viewDistance = this.client.options.getClampedViewDistance();
        if (this.chunks != null) {
            this.chunks.clear();
        }
        this.chunkBuilder.cancelAllTasks();
        this.chunks = new BuiltChunkStorage(this.chunkBuilder, (World)this.world, this.client.options.getClampedViewDistance(), this);
        this.chunkRenderingDataPreparer.setStorage(this.chunks);
        this.clear();
        Camera camera = this.client.gameRenderer.getCamera();
        this.chunks.updateCameraPosition(ChunkSectionPos.from((Position)camera.getCameraPos()));
    }
    ...
    private void updateCamera(Camera camera, Frustum frustum, boolean spectator) {
        ...
        int i = ChunkSectionPos.getSectionCoord((double)vec3d.getX());
        int j = ChunkSectionPos.getSectionCoord((double)vec3d.getY());
        int k = ChunkSectionPos.getSectionCoord((double)vec3d.getZ());
        ...
        if (this.capturedFrustum == null) {
            ...
            this.chunkRenderingDataPreparer.updateSectionOcclusionGraph(bl, camera, frustum, (List)this.builtChunks, this.world.getChunkManager().getActiveSections());
            profiler.pop();
            ...
            if (this.chunkRenderingDataPreparer.updateFrustum() || g != this.lastCameraPitch || h != this.lastCameraYaw) {
                ...
                this.applyFrustum(WorldRenderer.offsetFrustum((Frustum)frustum));
                profiler.pop();
    ...
    private void applyFrustum(Frustum frustum) {
        ...
        this.clear();
        this.chunkRenderingDataPreparer.collectChunks(frustum, (List)this.builtChunks, (List)this.nearbyChunks);
    }
    ...
    public void render(ObjectAllocator allocator, RenderTickCounter tickCounter, boolean renderBlockOutline, Camera camera, Matrix4f positionMatrix, Matrix4f basicProjectionMatrix, Matrix4f projectionMatrix, GpuBufferSlice fogBuffer, Vector4f fogColor, boolean renderSky) {
        ...
        this.weatherRendering.buildPrecipitationPieces((World)this.world, this.ticks, f, vec3d, this.worldRenderState.weatherRenderState);
        profiler.swap("sky");
        this.skyRendering.updateRenderState(this.world, f, camera, this.worldRenderState.skyRenderState);
        profiler.swap("border");
        this.worldBorderRendering.updateRenderState(this.world.getWorldBorder(), f, vec3d, (double)(this.client.options.getClampedViewDistance() * 16), this.worldRenderState.worldBorderRenderState);
        profiler.pop();
        profiler.swap("debug");
        ...
        this.framebufferSet.mainFramebuffer = frameGraphBuilder.createObjectNode("main", (Object)this.client.getFramebuffer());
        ...
        if (postEffectProcessor != null) {
            this.framebufferSet.translucentFramebuffer = frameGraphBuilder.createResourceHandle("translucent", (ClosableFactory)simpleFramebufferFactory);
            this.framebufferSet.itemEntityFramebuffer = frameGraphBuilder.createResourceHandle("item_entity", (ClosableFactory)simpleFramebufferFactory);
            this.framebufferSet.particlesFramebuffer = frameGraphBuilder.createResourceHandle("particles", (ClosableFactory)simpleFramebufferFactory);
            this.framebufferSet.weatherFramebuffer = frameGraphBuilder.createResourceHandle("weather", (ClosableFactory)simpleFramebufferFactory);
            this.framebufferSet.cloudsFramebuffer = frameGraphBuilder.createResourceHandle("clouds", (ClosableFactory)simpleFramebufferFactory);
        }
        if (this.entityOutlineFramebuffer != null) {
            this.framebufferSet.entityOutlineFramebuffer = frameGraphBuilder.createObjectNode("entity_outline", (Object)this.entityOutlineFramebuffer);
        }
        ...
        framePass.setRenderer(() -> {
            ...
            RenderSystem.getDevice().createCommandEncoder().clearColorAndDepthTextures(framebuffer.getColorAttachment(), ColorHelper.fromFloats((float)0.0f, (float)vector4f.x, (float)vector4f.y, (float)vector4f.z), framebuffer.getDepthAttachment(), 1.0);
        });
        if (renderSky) {
        ...
        if (this.worldRenderState.hasOutline && postEffectProcessor2 != null) {
            postEffectProcessor2.render(frameGraphBuilder, i, j, (PostEffectProcessor.FramebufferSet)this.framebufferSet);
        }
        ...
        if (cloudRenderMode != CloudRenderMode.OFF && ColorHelper.getAlpha((int)(k = ((Integer)camera.getEnvironmentAttributeInterpolator().get(EnvironmentAttributes.CLOUD_COLOR_VISUAL, f)).intValue())) > 0) {
            float g = ((Float)camera.getEnvironmentAttributeInterpolator().get(EnvironmentAttributes.CLOUD_HEIGHT_VISUAL, f)).floatValue();
        ...
        if (postEffectProcessor != null) {
            postEffectProcessor.render(frameGraphBuilder, i, j, (PostEffectProcessor.FramebufferSet)this.framebufferSet);
        }
        ...
        frameGraphBuilder.run(allocator, (FrameGraphBuilder.Profiler)new /* Unavailable Anonymous Inner Class!! */);
    ...
    private void renderMain(FrameGraphBuilder frameGraphBuilder, Frustum frustum, Matrix4f posMatrix, GpuBufferSlice fogBuffer, boolean renderBlockOutline, WorldRenderState state, RenderTickCounter tickCounter, Profiler profiler) {
        ...
        framePass.setRenderer(() -> {
            ...
            this.pushEntityRenders(matrixStack, state, (OrderedRenderCommandQueue)this.entityRenderCommandQueue);
            ...
            profiler.pop();
            this.collectGizmos();
            this.gizmos.standardPrimitives().draw(matrixStack, (VertexConsumerProvider)immediate, worldRenderState.cameraRenderState, posMatrix);
            immediate.drawCurrentLayer();
            this.checkEmpty(matrixStack);
    ...
    private void renderWeather(FrameGraphBuilder frameGraphBuilder, GpuBufferSlice gpuBufferSlice) {
        ...
        framePass.setRenderer(() -> {
            ...
            this.weatherRendering.renderPrecipitation((VertexConsumerProvider)immediate, cameraRenderState.pos, this.worldRenderState.weatherRenderState);
            this.worldBorderRendering.render(this.worldRenderState.worldBorderRenderState, cameraRenderState.pos, (double)i, (double)f);
            immediate.draw();
        });
    }
    ...
    private void renderLateDebug(FrameGraphBuilder frameGraphBuilder, CameraRenderState cameraRenderState, GpuBufferSlice fogBuffer, Matrix4f matrix4f) {
        ...
        framePass.setRenderer(() -> {
            ...
            if (!this.gizmos.alwaysOnTopPrimitives().isEmpty()) {
                ...
                this.gizmos.alwaysOnTopPrimitives().draw(matrixStack, (VertexConsumerProvider)immediate, cameraRenderState, matrix4f);
                immediate.drawCurrentLayer();
            }
    ...
    private void fillEntityRenderStates(Camera camera, Frustum frustum, RenderTickCounter tickCounter, WorldRenderState renderStates) {
        ...
        Entity.setRenderDistanceMultiplier((double)(MathHelper.clamp((double)((double)this.client.options.getClampedViewDistance() / 8.0), (double)1.0, (double)2.5) * (Double)this.client.options.getEntityDistanceScaling().getValue()));
        for (Entity entity : this.world.getEntities()) {
            BlockPos blockPos;
            if (!this.entityRenderManager.shouldRender(entity, frustum, d, e, f) && !entity.hasPassengerDeep((Entity)this.client.player) || !this.world.isOutOfHeightLimit((blockPos = entity.getBlockPos()).getY()) && !this.isRenderingReady(blockPos) || entity == camera.getFocusedEntity() && !camera.isThirdPerson() && (!(camera.getFocusedEntity() instanceof LivingEntity) || !((LivingEntity)camera.getFocusedEntity()).isSleeping()) || entity instanceof ClientPlayerEntity && camera.getFocusedEntity() != entity) continue;
            if (entity.age == 0) {
    ...
    private void fillBlockEntityRenderStates(Camera camera, float tickProgress, WorldRenderState renderStates) {
        ...
        MatrixStack matrixStack = new MatrixStack();
        for (ChunkBuilder.BuiltChunk builtChunk : this.builtChunks) {
            List list = builtChunk.getCurrentRenderData().getBlockEntities();
    ...
    private void renderBlockEntities(MatrixStack matrices, WorldRenderState renderStates, OrderedRenderCommandQueueImpl queue) {
        ...
        for (BlockEntityRenderState blockEntityRenderState : renderStates.blockEntityRenderStates) {
            ...
            this.blockEntityRenderManager.render(blockEntityRenderState, matrices, (OrderedRenderCommandQueue)queue, renderStates.cameraRenderState);
            matrices.pop();
        }
    }
    ...
    private void renderBlockDamage(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, WorldRenderState renderStates) {
        ...
        for (BreakingBlockRenderState breakingBlockRenderState : renderStates.breakingBlockRenderStates) {
            ...
            OverlayVertexConsumer vertexConsumer = new OverlayVertexConsumer(immediate.getBuffer((RenderLayer)ModelBaker.BLOCK_DESTRUCTION_RENDER_LAYERS.get(breakingBlockRenderState.breakProgress)), entry, 1.0f);
            this.client.getBlockRenderManager().renderDamage(breakingBlockRenderState.blockState, blockPos, (BlockRenderView)breakingBlockRenderState, matrices, (VertexConsumer)vertexConsumer);
            matrices.pop();
        }
    }
    ...
    private void fillEntityOutlineRenderStates(Camera camera, WorldRenderState renderStates) {
        ...
        if (!blockState.isAir() && this.world.getWorldBorder().contains(blockPos)) {
            boolean bl = BlockRenderLayers.getBlockLayer((BlockState)blockState).isTranslucent();
            boolean bl2 = (Boolean)this.client.options.getHighContrastBlockOutline().getValue();
            ShapeContext shapeContext = ShapeContext.of((Entity)camera.getFocusedEntity());
            VoxelShape voxelShape = blockState.getOutlineShape((BlockView)this.world, blockPos, shapeContext);
            if (SharedConstants.SHAPES) {
                VoxelShape voxelShape2 = blockState.getCollisionShape((BlockView)this.world, blockPos, shapeContext);
                VoxelShape voxelShape3 = blockState.getCullingShape();
                VoxelShape voxelShape4 = blockState.getRaycastShape((BlockView)this.world, blockPos);
    ...
    private void renderTargetBlockOutline(VertexConsumerProvider.Immediate immediate, MatrixStack matrices, boolean renderBlockOutline, WorldRenderState renderStates) {
        ...
        int i = outlineRenderState.highContrast() ? -11010079 : ColorHelper.toAlpha((int)102);
    ...
    private void translucencySort(Vec3d cameraPos) {
        ...
        BlockPos blockPos = BlockPos.ofFloored((Position)cameraPos);
        boolean bl = !blockPos.equals((Object)this.lastTranslucencySortCameraPos);
    ...
    private SectionRenderState renderBlockLayers(Matrix4fc matrix, double cameraX, double cameraY, double cameraZ) {
        ...
        while (objectListIterator.hasNext()) {
            ...
            for (BlockRenderLayer blockRenderLayer2 : BlockRenderLayer.values()) {
                ...
                int n = m;
                ((List)enumMap.get(blockRenderLayer2)).add(new RenderPass.RenderObject(0, buffers.getVertexBuffer(), gpuBuffer, indexType, 0, buffers.getIndexCount(), (gpuBufferSlices, uniformUploader) -> uniformUploader.upload("ChunkSection", gpuBufferSlices[n])));
            }
        }
    ...
    public void tick(Camera camera) {
        ...
        this.weatherRendering.addParticlesAndSound(this.world, camera, this.ticks, (ParticlesMode)this.client.options.getParticles().getValue(), ((Integer)this.client.options.getWeatherRadius().getValue()).intValue());
    ...
    private void updateChunks(Camera camera) {
        ...
        long l = MathHelper.floor((double)((Double)this.client.options.getChunkFade().getValue() * 1000.0));
        for (ChunkBuilder.BuiltChunk builtChunk : this.builtChunks) {
            if (!builtChunk.needsRebuild() || builtChunk.getCurrentRenderData() == ChunkRenderData.HIDDEN && !builtChunk.shouldBuild()) continue;
            BlockPos blockPos2 = ChunkSectionPos.from((long)builtChunk.getSectionPos()).getCenterPos();
            double d = blockPos2.getSquaredDistance((Vec3i)blockPos);
            boolean bl = d < 768.0;
            boolean bl2 = false;
    ...
    private void drawBlockOutline(MatrixStack matrices, VertexConsumer vertexConsumer, double x, double y, double z, OutlineRenderState state, int color, float lineWidth) {
        ...
        if (SharedConstants.SHAPES) {
            VertexRendering.drawOutline((MatrixStack)matrices, (VertexConsumer)vertexConsumer, (VoxelShape)state.shape(), (double)((double)blockPos.getX() - x), (double)((double)blockPos.getY() - y), (double)((double)blockPos.getZ() - z), (int)ColorHelper.fromFloats((float)1.0f, (float)1.0f, (float)1.0f, (float)1.0f), (float)lineWidth);
            if (state.collisionShape() != null) {
                VertexRendering.drawOutline((MatrixStack)matrices, (VertexConsumer)vertexConsumer, (VoxelShape)state.collisionShape(), (double)((double)blockPos.getX() - x), (double)((double)blockPos.getY() - y), (double)((double)blockPos.getZ() - z), (int)ColorHelper.fromFloats((float)0.4f, (float)0.0f, (float)0.0f, (float)0.0f), (float)lineWidth);
            }
            if (state.occlusionShape() != null) {
                VertexRendering.drawOutline((MatrixStack)matrices, (VertexConsumer)vertexConsumer, (VoxelShape)state.occlusionShape(), (double)((double)blockPos.getX() - x), (double)((double)blockPos.getY() - y), (double)((double)blockPos.getZ() - z), (int)ColorHelper.fromFloats((float)0.4f, (float)0.0f, (float)1.0f, (float)0.0f), (float)lineWidth);
            }
            if (state.interactionShape() != null) {
                VertexRendering.drawOutline((MatrixStack)matrices, (VertexConsumer)vertexConsumer, (VoxelShape)state.interactionShape(), (double)((double)blockPos.getX() - x), (double)((double)blockPos.getY() - y), (double)((double)blockPos.getZ() - z), (int)ColorHelper.fromFloats((float)0.4f, (float)0.0f, (float)0.0f, (float)1.0f), (float)lineWidth);
            }
        } else {
            VertexRendering.drawOutline((MatrixStack)matrices, (VertexConsumer)vertexConsumer, (VoxelShape)state.shape(), (double)((double)blockPos.getX() - x), (double)((double)blockPos.getY() - y), (double)((double)blockPos.getZ() - z), (int)color, (float)lineWidth);
        }
    }
        ...
        for (int i = pos.getZ() - 1; i <= pos.getZ() + 1; ++i) {
            for (int j = pos.getX() - 1; j <= pos.getX() + 1; ++j) {
                for (int k = pos.getY() - 1; k <= pos.getY() + 1; ++k) {
                    this.scheduleChunkRender(ChunkSectionPos.getSectionCoord((int)j), ChunkSectionPos.getSectionCoord((int)k), ChunkSectionPos.getSectionCoord((int)i), important);
                }
            }
        }
    }
        ...
        for (int i = minZ - 1; i <= maxZ + 1; ++i) {
            for (int j = minX - 1; j <= maxX + 1; ++j) {
                for (int k = minY - 1; k <= maxY + 1; ++k) {
                    this.scheduleChunkRender(ChunkSectionPos.getSectionCoord((int)j), ChunkSectionPos.getSectionCoord((int)k), ChunkSectionPos.getSectionCoord((int)i));
                }
            }
        }
    }
    ...
    public static int getLightmapCoordinates(BlockRenderView world, BlockPos pos) {
        return WorldRenderer.getLightmapCoordinates((BrightnessGetter)BrightnessGetter.DEFAULT, (BlockRenderView)world, (BlockState)world.getBlockState(pos), (BlockPos)pos);
    }
    ...
    public static int getLightmapCoordinates(BrightnessGetter brightnessGetter, BlockRenderView world, BlockState state, BlockPos pos) {
        int k;
        if (state.hasEmissiveLighting((BlockView)world, pos)) {
            return 0xF000F0;
        }
        int i = brightnessGetter.packedBrightness(world, pos);
        int j = LightmapTextureManager.getBlockLightCoordinates((int)i);
        if (j < (k = state.getLuminance())) {
            int l = LightmapTextureManager.getSkyLightCoordinates((int)i);
            return LightmapTextureManager.pack((int)k, (int)l);
        }
        return i;
    }
    ...
    private void collectGizmos() {
        ...
        for (GizmoCollectorImpl.Entry entry : this.gizmoCollector.extractGizmos()) {
            entry.getGizmo().draw((GizmoDrawer)(entry.ignoresOcclusion() ? gizmoDrawerImpl2 : gizmoDrawerImpl), entry.getOpacity(l));
        }
