...
public class ChunkOcclusionDataBuilder {
    ...
    private static final int[] EDGE_POINTS = (int[])Util.make((Object)new int[1352], edgePoints -> {
        boolean i = false;
        int j = 15;
        int k = 0;
        for (int l = 0; l < 16; ++l) {
            for (int m = 0; m < 16; ++m) {
                for (int n = 0; n < 16; ++n) {
                    ...
                    edgePoints[k++] = ChunkOcclusionDataBuilder.pack((int)l, (int)m, (int)n);
                }
            }
        }
    });
    ...
    public void markClosed(BlockPos pos) {
        this.closed.set(ChunkOcclusionDataBuilder.pack((BlockPos)pos), true);
        --this.openCount;
    }

    private static int pack(BlockPos pos) {
        return ChunkOcclusionDataBuilder.pack((int)(pos.getX() & 0xF), (int)(pos.getY() & 0xF), (int)(pos.getZ() & 0xF));
    }
    ...
    private int offset(int pos, Direction direction) {
        switch (1.field_4480[direction.ordinal()]) {
            case 1: {
                if ((pos >> 8 & 0xF) == 0) {
                    return -1;
                }
