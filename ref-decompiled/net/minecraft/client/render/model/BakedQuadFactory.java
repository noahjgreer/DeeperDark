...
public class BakedQuadFactory {
    ...
    static ModelElementFace.UV setDefaultUV(Vector3fc from, Vector3fc to, Direction facing) {
        return switch (1.field_56929[facing.ordinal()]) {
            default -> throw new MatchException(null, null);
            case 1 -> new ModelElementFace.UV(from.x(), 16.0f - to.z(), to.x(), 16.0f - from.z());
            case 2 -> new ModelElementFace.UV(from.x(), from.z(), to.x(), to.z());
            case 3 -> new ModelElementFace.UV(16.0f - to.x(), 16.0f - to.y(), 16.0f - from.x(), 16.0f - from.y());
            case 4 -> new ModelElementFace.UV(from.x(), 16.0f - to.y(), to.x(), 16.0f - from.y());
            case 5 -> new ModelElementFace.UV(from.z(), 16.0f - to.y(), to.z(), 16.0f - from.y());
            case 6 -> new ModelElementFace.UV(16.0f - to.z(), 16.0f - to.y(), 16.0f - from.z(), 16.0f - from.y());
        };
    }
    ...
    public static BakedQuad bake(Baker.Vec3fInterner interner, Vector3fc from, Vector3fc to, ModelElementFace face, Sprite sprite, Direction direction, ModelBakeSettings settings, @Nullable ModelElementRotation rotation, boolean shade, int lightEmission) {
        ...
        if (uV == null) {
            uV = BakedQuadFactory.setDefaultUV((Vector3fc)from, (Vector3fc)to, (Direction)direction);
        }
        Matrix4fc matrix4fc = settings.reverse(direction);
        Vector3fc[] vector3fcs = new Vector3fc[4];
        long[] ls = new long[4];
        CubeFace cubeFace = CubeFace.getFace((Direction)direction);
        for (int i = 0; i < 4; ++i) {
            BakedQuadFactory.packVertexData((int)i, (CubeFace)cubeFace, (ModelElementFace.UV)uV, (AxisRotation)face.rotation(), (Matrix4fc)matrix4fc, (Vector3fc)from, (Vector3fc)to, (Sprite)sprite, (AffineTransformation)settings.getRotation(), (ModelElementRotation)rotation, (Vector3fc[])vector3fcs, (long[])ls, (Baker.Vec3fInterner)interner);
        }
        Direction direction2 = BakedQuadFactory.decodeDirection((Vector3fc[])vector3fcs);
        if (rotation == null && direction2 != null) {
            BakedQuadFactory.encodeDirection((Vector3fc[])vector3fcs, (long[])ls, (Direction)direction2);
        }
    ...
    private static void packVertexData(int corner, CubeFace cubeFace, ModelElementFace.UV uv, AxisRotation axisRotation, Matrix4fc matrix, Vector3fc from, Vector3fc to, Sprite sprite, AffineTransformation affineTransformation, @Nullable ModelElementRotation rotation, Vector3fc[] positions, long[] packedUvs, Baker.Vec3fInterner interner) {
        ...
        if (rotation != null) {
            BakedQuadFactory.transformVertex((Vector3f)vector3f, (Vector3fc)rotation.origin(), (Matrix4fc)rotation.transform());
        }
        if (affineTransformation != AffineTransformation.identity()) {
            BakedQuadFactory.transformVertex((Vector3f)vector3f, (Vector3fc)CENTER, (Matrix4fc)affineTransformation.getMatrix());
        }
        float f = ModelElementFace.getUValue((ModelElementFace.UV)uv, (AxisRotation)axisRotation, (int)corner);
        float g = ModelElementFace.getVValue((ModelElementFace.UV)uv, (AxisRotation)axisRotation, (int)corner);
        if (MatrixUtil.isIdentity((Matrix4fc)matrix)) {
            h = f;
            i = g;
        } else {
            Vector3f vector3f2 = matrix.transformPosition(new Vector3f(BakedQuadFactory.setCenterBack((float)f), BakedQuadFactory.setCenterBack((float)g), 0.0f));
            h = BakedQuadFactory.setCenterForward((float)vector3f2.x);
            i = BakedQuadFactory.setCenterForward((float)vector3f2.y);
        }
        positions[corner] = interner.intern((Vector3fc)vector3f);
        packedUvs[corner] = Vector2f.toLong((float)sprite.getFrameU(h), (float)sprite.getFrameV(i));
    }
    ...
    private static @Nullable Direction decodeDirection(Vector3fc[] vecs) {
        ...
        return BakedQuadFactory.getDirection((Vector3f)vector3f);
    }
    ...
    private static void encodeDirection(Vector3fc[] vector3fcs, long[] ls, Direction direction) {
        ...
        for (int p = 0; p < 4; ++p) {
            ...
            int r = BakedQuadFactory.method_76655((Vector3fc[])vector3fcs, (int)p, (float)n, (float)(o = corner.ySide().get(f, g, h, i, j, k)), (float)(q = corner.zSide().get(f, g, h, i, j, k)));
            if (r == -1) {
                throw new IllegalStateException("Can't find vertex to swap");
            }
            if (r == p) continue;
            BakedQuadFactory.method_76656((Vector3fc[])vector3fcs, (int)r, (int)p);
            BakedQuadFactory.method_76654((long[])ls, (int)r, (int)p);
        }
    }
