...
public interface MultipartModelCondition {
    public static final Codec<MultipartModelCondition> CODEC = Codec.recursive((String)"condition", group -> {
        Codec codec = Codec.simpleMap((Codec)MultipartModelCombinedCondition.LogicalOperator.CODEC, (Codec)group.listOf(), (Keyable)StringIdentifiable.toKeyable((StringIdentifiable[])MultipartModelCombinedCondition.LogicalOperator.values())).codec().comapFlatMap(map -> {
            if (map.size() != 1) {
        ...
        return Codec.either((Codec)codec, (Codec)SimpleMultipartModelSelector.CODEC).flatComapMap(either -> (MultipartModelCondition)either.map(condition -> condition, selector -> selector), condition -> {
