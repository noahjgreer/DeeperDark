...
public class FallingBlockCommandRenderer {
    ...
    public void render(BatchingRenderCommandQueue queue, VertexConsumerProvider.Immediate vertexConsumers, BlockRenderManager blockRenderManager, OutlineVertexConsumerProvider outlineVertexConsumers) {
        for (OrderedRenderCommandQueueImpl.MovingBlockCommand movingBlockCommand : queue.getMovingBlockCommands()) {
            ...
            List<BlockModelPart> list = blockRenderManager.getModel(blockState).getParts(Random.create(blockState.getRenderingSeed(movingBlockRenderState.fallingBlockPos)));
            MatrixStack matrixStack = new MatrixStack();
            matrixStack.multiplyPositionMatrix((Matrix4fc)movingBlockCommand.matricesEntry());
            blockRenderManager.getModelRenderer().render(movingBlockRenderState, list, blockState, movingBlockRenderState.entityBlockPos, matrixStack, vertexConsumers.getBuffer(BlockRenderLayers.getMovingBlockLayer(blockState)), false, OverlayTexture.DEFAULT_UV);
        }
        for (OrderedRenderCommandQueueImpl.BlockCommand blockCommand : queue.getBlockCommands()) {
            ...
            if (blockCommand.outlineColor() != 0) {
                ...
                blockRenderManager.renderBlockAsEntity(blockCommand.state(), this.matrices, outlineVertexConsumers, blockCommand.lightCoords(), blockCommand.overlayCoords());
            }
            this.matrices.pop();
        }
        for (OrderedRenderCommandQueueImpl.BlockStateModelCommand blockStateModelCommand : queue.getBlockStateModelCommands()) {
            BlockModelRenderer.render(blockStateModelCommand.matricesEntry(), vertexConsumers.getBuffer(blockStateModelCommand.renderLayer()), blockStateModelCommand.model(), blockStateModelCommand.r(), blockStateModelCommand.g(), blockStateModelCommand.b(), blockStateModelCommand.lightCoords(), blockStateModelCommand.overlayCoords());
            if (blockStateModelCommand.outlineColor() == 0) continue;
            outlineVertexConsumers.setColor(blockStateModelCommand.outlineColor());
            BlockModelRenderer.render(blockStateModelCommand.matricesEntry(), outlineVertexConsumers.getBuffer(blockStateModelCommand.renderLayer()), blockStateModelCommand.model(), blockStateModelCommand.r(), blockStateModelCommand.g(), blockStateModelCommand.b(), blockStateModelCommand.lightCoords(), blockStateModelCommand.overlayCoords());
        }
    }
}
