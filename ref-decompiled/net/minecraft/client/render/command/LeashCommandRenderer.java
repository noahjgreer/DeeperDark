...
public class LeashCommandRenderer {
    ...
    public void render(BatchingRenderCommandQueue queue, VertexConsumerProvider.Immediate vertexConsumers) {
        for (OrderedRenderCommandQueueImpl.LeashCommand leashCommand : queue.getLeashCommands()) {
            LeashCommandRenderer.render((Matrix4f)leashCommand.matricesEntry(), (VertexConsumerProvider)vertexConsumers, (EntityRenderState.LeashData)leashCommand.leashState());
        }
    }
    ...
    private static void render(Matrix4f matrix, VertexConsumerProvider vertexConsumers, EntityRenderState.LeashData data) {
        ...
        float i = MathHelper.inverseSqrt((float)(f * f + h * h)) * 0.05f / 2.0f;
        float j = h * i;
        float k = f * i;
        ...
        for (l = 0; l <= 24; ++l) {
            LeashCommandRenderer.render((VertexConsumer)vertexConsumer, (Matrix4f)matrix, (float)f, (float)g, (float)h, (float)0.05f, (float)j, (float)k, (int)l, (boolean)false, (EntityRenderState.LeashData)data);
        }
        for (l = 24; l >= 0; --l) {
            LeashCommandRenderer.render((VertexConsumer)vertexConsumer, (Matrix4f)matrix, (float)f, (float)g, (float)h, (float)0.0f, (float)j, (float)k, (int)l, (boolean)true, (EntityRenderState.LeashData)data);
        }
    }
    ...
    private static void render(VertexConsumer vertexConsumer, Matrix4f matrix, float offsetX, float offsetY, float offsetZ, float yOffset, float sideOffset, float perpendicularOffset, int segmentIndex, boolean backside, EntityRenderState.LeashData data) {
        ...
        int i = (int)MathHelper.lerp((float)f, (float)data.leashedEntityBlockLight, (float)data.leashHolderBlockLight);
        int j = (int)MathHelper.lerp((float)f, (float)data.leashedEntitySkyLight, (float)data.leashHolderSkyLight);
        int k = LightmapTextureManager.pack((int)i, (int)j);
