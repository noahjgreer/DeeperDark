...
public class WardenEntityRenderer extends MobEntityRenderer {
   ...
   public WardenEntityRenderer(EntityRendererFactory.Context context) {
      ...
      this.addFeature(new EmissiveFeatureRenderer(this, BIOLUMINESCENT_LAYER_TEXTURE, (state, tickProgress) -> {
         return 1.0F;
      }, WardenEntityModel::getHeadAndLimbs, RenderLayer::getEntityTranslucentEmissive, false));
      this.addFeature(new EmissiveFeatureRenderer(this, PULSATING_SPOTS_1_TEXTURE, (state, tickProgress) -> {
         return Math.max(0.0F, MathHelper.cos(tickProgress * 0.045F) * 0.25F);
      }, WardenEntityModel::getBodyHeadAndLimbs, RenderLayer::getEntityTranslucentEmissive, false));
      this.addFeature(new EmissiveFeatureRenderer(this, PULSATING_SPOTS_2_TEXTURE, (state, tickProgress) -> {
         return Math.max(0.0F, MathHelper.cos(tickProgress * 0.045F + 3.1415927F) * 0.25F);
      }, WardenEntityModel::getBodyHeadAndLimbs, RenderLayer::getEntityTranslucentEmissive, false));
      this.addFeature(new EmissiveFeatureRenderer(this, TEXTURE, (state, tickProgress) -> {
         return state.tendrilAlpha;
      }, WardenEntityModel::getTendrils, RenderLayer::getEntityTranslucentEmissive, false));
      this.addFeature(new EmissiveFeatureRenderer(this, HEART_TEXTURE, (state, tickProgress) -> {
         return state.heartAlpha;
      }, WardenEntityModel::getBody, RenderLayer::getEntityTranslucentEmissive, false));
   }
