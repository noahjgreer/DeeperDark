...
public class EndermiteEntityModel extends EntityModel {
   ...
   public static TexturedModelData getTexturedModelData() {
      ...
      for(int i = 0; i < 4; ++i) {
         modelPartData.addChild(getSegmentName(i), ModelPartBuilder.create().uv(SEGMENT_UVS[i][0], SEGMENT_UVS[i][1]).cuboid((float)SEGMENT_DIMENSIONS[i][0] * -0.5F, 0.0F, (float)SEGMENT_DIMENSIONS[i][2] * -0.5F, (float)SEGMENT_DIMENSIONS[i][0], (float)SEGMENT_DIMENSIONS[i][1], (float)SEGMENT_DIMENSIONS[i][2]), ModelTransform.origin(0.0F, (float)(24 - SEGMENT_DIMENSIONS[i][1]), f));
         if (i < 3) {
   ...
   public void setAngles(EntityRenderState state) {
      ...
      for(int i = 0; i < this.bodySegments.length; ++i) {
         this.bodySegments[i].yaw = MathHelper.cos(state.age * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.01F * (float)(1 + Math.abs(i - 2));
         this.bodySegments[i].originX = MathHelper.sin(state.age * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.1F * (float)Math.abs(i - 2);
      }
