...
extends EntityModel<EntityRenderState> {
    ...
    public void setAngles(EntityRenderState entityRenderState) {
        ...
        for (int i = 0; i < this.bodySegments.length; ++i) {
            this.bodySegments[i].yaw = MathHelper.cos(entityRenderState.age * 0.9f + (float)i * 0.15f * (float)Math.PI) * (float)Math.PI * 0.01f * (float)(1 + Math.abs(i - 2));
            this.bodySegments[i].originX = MathHelper.sin(entityRenderState.age * 0.9f + (float)i * 0.15f * (float)Math.PI) * (float)Math.PI * 0.1f * (float)Math.abs(i - 2);
        }
    }
}
