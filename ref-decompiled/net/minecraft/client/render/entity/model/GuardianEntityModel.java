...
public class GuardianEntityModel extends EntityModel {
   ...
   public void setAngles(GuardianEntityRenderState guardianEntityRenderState) {
      ...
      this.head.yaw = guardianEntityRenderState.relativeHeadYaw * 0.017453292F;
      ...
      if (guardianEntityRenderState.lookAtPos != null && guardianEntityRenderState.rotationVec != null) {
         ...
         if (d > 0.0) {
            this.eye.originY = 0.0F;
         } else {
            this.eye.originY = 1.0F;
         }
      ...
      this.tail[0].yaw = MathHelper.sin(g) * 3.1415927F * 0.05F;
      this.tail[1].yaw = MathHelper.sin(g) * 3.1415927F * 0.1F;
      this.tail[2].yaw = MathHelper.sin(g) * 3.1415927F * 0.15F;
   }

   private void updateSpikeExtensions(float animationProgress, float extension) {
      for(int i = 0; i < 12; ++i) {
         this.spikes[i].originX = getSpikePivotX(i, animationProgress, extension);
         this.spikes[i].originY = getSpikePivotY(i, animationProgress, extension);
         this.spikes[i].originZ = getSpikePivotZ(i, animationProgress, extension);
      }
