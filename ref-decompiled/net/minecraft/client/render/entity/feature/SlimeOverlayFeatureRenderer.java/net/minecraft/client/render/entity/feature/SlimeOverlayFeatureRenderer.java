...
extends FeatureRenderer<SlimeEntityRenderState, SlimeEntityModel> {
    ...
    public void render(MatrixStack matrixStack, OrderedRenderCommandQueue orderedRenderCommandQueue, int i, SlimeEntityRenderState slimeEntityRenderState, float f, float g) {
        ...
        if (bl) {
            orderedRenderCommandQueue.getBatchingQueue(1).submitModel(this.model, slimeEntityRenderState, matrixStack, RenderLayers.outlineNoCull(SlimeEntityRenderer.TEXTURE), i, j, -1, null, slimeEntityRenderState.outlineColor, null);
        } else {
            orderedRenderCommandQueue.getBatchingQueue(1).submitModel(this.model, slimeEntityRenderState, matrixStack, RenderLayers.entityTranslucent(SlimeEntityRenderer.TEXTURE), i, j, -1, null, slimeEntityRenderState.outlineColor, null);
        }
    }
}
