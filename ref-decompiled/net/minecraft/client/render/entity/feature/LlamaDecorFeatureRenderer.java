...
extends FeatureRenderer<LlamaEntityRenderState, LlamaEntityModel> {
    ...
    public void render(MatrixStack matrixStack, OrderedRenderCommandQueue orderedRenderCommandQueue, int i, LlamaEntityRenderState llamaEntityRenderState, float f, float g) {
        ...
        if (equippableComponent != null && equippableComponent.assetId().isPresent()) {
            this.render(matrixStack, orderedRenderCommandQueue, llamaEntityRenderState, itemStack, (RegistryKey)equippableComponent.assetId().get(), i);
    ...
    private void render(MatrixStack matrices, OrderedRenderCommandQueue queue, LlamaEntityRenderState state, ItemStack stack, RegistryKey<EquipmentAsset> assetKey, int light) {
        LlamaEntityModel llamaEntityModel = state.baby ? this.babyModel : this.model;
        this.equipmentRenderer.render(EquipmentModel.LayerType.LLAMA_BODY, assetKey, (Model)llamaEntityModel, (Object)state, stack, matrices, queue, light, state.outlineColor);
    }
}
