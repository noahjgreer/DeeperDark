...
public class OctreeDebugRenderer {
   ...
   public void render(MatrixStack matrices, Frustum frustum, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      octree.visit((node, skipVisibilityCheck, depth, bl) -> {
         this.renderNode(node, matrices, vertexConsumers, cameraX, cameraY, cameraZ, depth, skipVisibilityCheck, mutableInt, bl);
      }, frustum, 32);
   }
   ...
   private void renderNode(Octree.Node node, MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ, int depth, boolean skipVisibilityCheck, MutableInt id, boolean bl) {
      ...
      long m = l + 5L;
      VertexRendering.drawBox(matrices, vertexConsumer, box.contract(0.1 * (double)depth).offset(-cameraX, -cameraY, -cameraZ), getColorComponent(m, 0.3F), getColorComponent(m, 0.8F), getColorComponent(m, 0.5F), skipVisibilityCheck ? 0.4F : 1.0F);
   }
