...
public class SupportingBlockDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      if (playerEntity != null && playerEntity.supportingBlockPos.isPresent()) {
         this.renderBlockHighlights(matrices, vertexConsumers, cameraX, cameraY, cameraZ, playerEntity, () -> {
            return 0.0;
         }, 1.0F, 0.0F, 0.0F);
      }

      Iterator var12 = this.entities.iterator();

      while(var12.hasNext()) {
         Entity entity2 = (Entity)var12.next();
         if (entity2 != playerEntity) {
            this.renderBlockHighlights(matrices, vertexConsumers, cameraX, cameraY, cameraZ, entity2, () -> {
               return this.getAdditionalDilation(entity2);
            }, 0.0F, 1.0F, 0.0F);
         }
      }
   ...
   private void renderBlockHighlight(BlockPos pos, MatrixStack matrices, double cameraX, double cameraY, double cameraZ, VertexConsumerProvider vertexConsumers, double dilation, float red, float green, float blue) {
      ...
      DebugRenderer.drawVoxelShapeOutlines(matrices, vertexConsumers.getBuffer(RenderLayer.getLines()), this.client.world.getBlockState(pos).getCollisionShape(this.client.world, pos, ShapeContext.absent()).offset((Vec3i)pos), -cameraX, -cameraY, -cameraZ, red, green, blue, 1.0F, false);
   }
}
