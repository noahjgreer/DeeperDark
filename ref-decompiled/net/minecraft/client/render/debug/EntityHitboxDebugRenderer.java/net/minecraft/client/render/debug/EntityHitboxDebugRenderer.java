...
implements DebugRenderer.Renderer {
    ...
    public void render(double cameraX, double cameraY, double cameraZ, DebugDataStore store, Frustum frustum, float tickProgress) {
        ...
        for (Entity entity : this.client.world.getEntities()) {
            ...
            GizmoDrawing.text("Missing Server Entity", entity.getLerpedPos(tickProgress).add(0.0, entity.getBoundingBox().getLengthY() + 1.5, 0.0), TextGizmo.Style.left(-65536));
        }
    }
    ...
    private void drawHitbox(Entity entity, float tickProgress, boolean inLocalServer) {
        ...
        if (entity2 != null) {
            ...
            GizmoDrawing.box(new Box(vec3d4.x - (double)f, vec3d4.y, vec3d4.z - (double)f, vec3d4.x + (double)f, vec3d4.y + 0.0625, vec3d4.z + (double)f), DrawStyle.stroked(-256));
        }
