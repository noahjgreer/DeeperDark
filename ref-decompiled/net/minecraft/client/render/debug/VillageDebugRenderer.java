...
public class VillageDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void setFreeTicketCount(BlockPos pos, int freeTicketCount) {
      PointOfInterest pointOfInterest = (PointOfInterest)this.pointsOfInterest.get(pos);
   ...
   private void drawGhostPointOfInterest(MatrixStack matrices, VertexConsumerProvider vertexConsumers, BlockPos pos, List brains) {
      float f = 0.05F;
      DebugRenderer.drawBox(matrices, vertexConsumers, pos, 0.05F, 0.2F, 0.2F, 1.0F, 0.3F);
      drawString(matrices, vertexConsumers, "" + String.valueOf(brains), (BlockPos)pos, 0, -256);
   ...
   private void drawPointOfInterestInfo(MatrixStack matrices, VertexConsumerProvider vertexConsumers, PointOfInterest pointOfInterest) {
      ...
      if (set.size() < 4) {
         drawString(matrices, vertexConsumers, "Owners: " + String.valueOf(set), (PointOfInterest)pointOfInterest, i, -256);
      } else {
      ...
      if (set2.size() < 4) {
         drawString(matrices, vertexConsumers, "Candidates: " + String.valueOf(set2), (PointOfInterest)pointOfInterest, i, -23296);
      } else {
      ...
      ++i;
      drawString(matrices, vertexConsumers, pointOfInterest.type, (PointOfInterest)pointOfInterest, i, -1);
   }
   ...
   private Collection getBrainsContainingPointOfInterest(BlockPos pointOfInterest) {
      ...
      }).map(DebugBrainCustomPayload.Brain::uuid).collect(Collectors.toSet());
   }
   ...
   private Map getGhostPointsOfInterest() {
      ...
      while(var2.hasNext()) {
         DebugBrainCustomPayload.Brain brain = (DebugBrainCustomPayload.Brain)var2.next();
         Iterator var4 = Iterables.concat(brain.pois(), brain.potentialPois()).iterator();

         while(var4.hasNext()) {
            BlockPos blockPos = (BlockPos)var4.next();
            if (!this.pointsOfInterest.containsKey(blockPos)) {
               ((List)map.computeIfAbsent(blockPos, (pos) -> {
