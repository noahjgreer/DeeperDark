...
public class RenderLayer {
    ...
    private RenderLayer(String name, RenderSetup renderSetup) {
        ...
        this.affectedOutline = renderSetup.outlineMode == RenderSetup.OutlineMode.AFFECTS_OUTLINE ? renderSetup.textures.values().stream().findFirst().map(texture -> (RenderLayer)RenderLayers.OUTLINE.apply(texture.location(), renderSetup.pipeline.isCull())) : Optional.empty();
    }
    ...
    public void draw(BuiltBuffer buffer) {
        ...
        Consumer consumer = this.renderSetup.layeringTransform.getTransform();
        if (consumer != null) {
        ...
        Map map = this.renderSetup.resolveTextures();
        try (BuiltBuffer builtBuffer = buffer;){
            ...
            if (buffer.getSortedBuffer() == null) {
                RenderSystem.ShapeIndexBuffer shapeIndexBuffer = RenderSystem.getSequentialBuffer((VertexFormat.DrawMode)buffer.getDrawParameters().mode());
