...
public class HeldItemRenderer {
    ...
    public void renderItem(float tickProgress, MatrixStack matrices, OrderedRenderCommandQueue orderedRenderCommandQueue, ClientPlayerEntity player, int light) {
        ...
        if (handRenderType.renderMainHand) {
            ...
            k = this.itemModelManager.getSwapAnimationScale(this.mainHand) * (1.0f - MathHelper.lerp(tickProgress, this.lastEquipProgressMainHand, this.equipProgressMainHand));
        ...
        if (handRenderType.renderOffHand) {
            ...
            k = this.itemModelManager.getSwapAnimationScale(this.offHand) * (1.0f - MathHelper.lerp(tickProgress, this.lastEquipProgressOffHand, this.equipProgressOffHand));
            ...
            if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
                ...
                switch (useAction) {
                    ...
                    case SPEAR: {
                        ...
                        float f = (float)item.getMaxUseTime(player) - ((float)player.getItemUseTimeLeft() - tickProgress + 1.0f);
                        Lancing.method_75396(player.getTimeSinceLastKineticAttack(tickProgress), matrices, f, arm, item);
                        break;
                    }
                }
    ...
    private boolean shouldSkipHandAnimationOnSwap(ItemStack from, ItemStack to) {
        if (ItemStack.shouldSkipHandAnimationOnSwap(from, to, ComponentType::skipsHandAnimation)) {
            return true;
        }
    ...
    extends Enum<HandRenderType> {
        public static final /* enum */ HandRenderType RENDER_BOTH_HANDS = new HandRenderType(true, true);
        public static final /* enum */ HandRenderType RENDER_MAIN_HAND_ONLY = new HandRenderType(true, false);
        public static final /* enum */ HandRenderType RENDER_OFF_HAND_ONLY = new HandRenderType(false, true);
        final boolean renderMainHand;
        final boolean renderOffHand;
        private static final /* synthetic */ HandRenderType[] field_28389;
        ...
        private static /* synthetic */ HandRenderType[] method_36915() {
