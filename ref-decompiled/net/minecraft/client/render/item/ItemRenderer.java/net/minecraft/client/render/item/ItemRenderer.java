...
public class ItemRenderer {
    ...
    private static VertexConsumer getSpecialItemGlintConsumer(VertexConsumerProvider consumers, RenderLayer layer, MatrixStack.Entry matrix) {
        return VertexConsumers.union((VertexConsumer)new OverlayVertexConsumer(consumers.getBuffer(ItemRenderer.useTransparentGlint(layer) ? RenderLayers.glintTranslucent() : RenderLayers.glint()), matrix, 0.0078125f), consumers.getBuffer(layer));
    }
    ...
    public static VertexConsumer getItemGlintConsumer(VertexConsumerProvider vertexConsumers, RenderLayer layer, boolean solid, boolean glint) {
        if (glint) {
            if (ItemRenderer.useTransparentGlint(layer)) {
                return VertexConsumers.union(vertexConsumers.getBuffer(RenderLayers.glintTranslucent()), vertexConsumers.getBuffer(layer));
            }
            return VertexConsumers.union(vertexConsumers.getBuffer(solid ? RenderLayers.glint() : RenderLayers.entityGlint()), vertexConsumers.getBuffer(layer));
        }
