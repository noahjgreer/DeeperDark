...
public class VertexRendering {
   ...
   public static void drawVector(MatrixStack matrices, VertexConsumer vertexConsumers, Vector3f offset, Vec3d vec, int argb) {
      ...
      vertexConsumers.vertex(entry, (float)((double)offset.x() + vec.x), (float)((double)offset.y() + vec.y), (float)((double)offset.z() + vec.z)).color(argb).normal(entry, (float)vec.x, (float)vec.y, (float)vec.z);
   }
}
