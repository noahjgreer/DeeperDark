...
implements AutoCloseable {
    ...
    public void update(float tickProgress) {
        ...
        float g = camera.getEnvironmentAttributeInterpolator().get(EnvironmentAttributes.SKY_LIGHT_FACTOR_VISUAL, tickProgress).floatValue();
        ...
        h = this.client.options.getDarknessEffectScale().getValue().floatValue();
        ...
        try (GpuBuffer.MappedView mappedView = commandEncoder.mapBuffer(this.buffer.getBlocking(), false, true);){
            Std140Builder.intoBuffer(mappedView.data()).putFloat(f).putFloat(g).putFloat(n).putFloat(m).putFloat(k).putFloat(this.renderer.getSkyDarkness(tickProgress)).putFloat(Math.max(0.0f, o - j)).putVec3((Vector3fc)ColorHelper.toRgbVector(i)).putVec3((Vector3fc)vector3f);
        }
