...
public class InputUtil {
   ...
   public static Key fromTranslationKey(String translationKey) {
      if (InputUtil.Key.KEYS.containsKey(translationKey)) {
         return (Key)InputUtil.Key.KEYS.get(translationKey);
      } else {
         Type[] var1 = InputUtil.Type.values();
   ...
   public static enum Type {
      ...
      MOUSE("key.mouse", (buttonCode, translationKey) -> {
         return Language.getInstance().hasTranslation(translationKey) ? Text.translatable(translationKey) : Text.translatable("key.mouse", buttonCode + 1);
      });
      ...
      final String name;
      final BiFunction textTranslator;
      ...
      private Type(final String name, final BiFunction textTranslator) {
         this.name = name;
         this.textTranslator = textTranslator;
      }

      public Key createFromCode(int code) {
         return (Key)this.map.computeIfAbsent(code, (codex) -> {
            int i = codex;
            if (this == MOUSE) {
               i = codex + 1;
            }
            ...
            return new Key(string, this, codex);
         });
      }
   ...
   public static final class Key {
      ...
      private final Lazy localizedText;
      static final Map KEYS = Maps.newHashMap();

      Key(String translationKey, Type type, int code) {
         this.translationKey = translationKey;
         this.type = type;
         this.code = code;
         this.localizedText = new Lazy(() -> {
            return (Text)type.textTranslator.apply(code, translationKey);
         });
