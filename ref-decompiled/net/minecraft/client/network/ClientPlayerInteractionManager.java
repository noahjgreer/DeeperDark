...
public class ClientPlayerInteractionManager {
    ...
    public boolean breakBlock(BlockPos pos) {
        if (this.client.player.isBlockBreakingRestricted((World)this.client.world, pos, this.gameMode)) {
            return false;
        }
        ClientWorld world = this.client.world;
        BlockState blockState = world.getBlockState(pos);
        if (!this.client.player.getMainHandStack().canMine(blockState, (World)world, pos, (PlayerEntity)this.client.player)) {
            return false;
        }
        ...
        block.onBreak((World)world, pos, blockState, (PlayerEntity)this.client.player);
        FluidState fluidState = world.getFluidState(pos);
        boolean bl = world.setBlockState(pos, fluidState.getBlockState(), 11);
        if (bl) {
            block.onBroken((WorldAccess)world, pos, blockState);
        }
        if (SharedConstants.BLOCK_BREAK) {
            LOGGER.error("client broke {} {} -> {}", new Object[]{pos, blockState, world.getBlockState(pos)});
        }
        return bl;
    }
    ...
    public boolean attackBlock(BlockPos pos, Direction direction) {
        ...
        if (this.client.player.getAbilities().creativeMode) {
            ...
            if (SharedConstants.BLOCK_BREAK) {
                LOGGER.info("Creative start {} {}", (Object)pos, (Object)blockState);
            }
            ...
            if (this.breakingBlock) {
                ...
                this.networkHandler.sendPacket((Packet)new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, this.currentBreakingPos, direction));
            }
            BlockState blockState = this.client.world.getBlockState(pos);
            this.client.getTutorialManager().onBlockBreaking(this.client.world, pos, blockState, 0.0f);
            if (SharedConstants.BLOCK_BREAK) {
                LOGGER.info("Start break {} {}", (Object)pos, (Object)blockState);
            }
            this.sendSequencedPacket(this.client.world, sequence -> {
                boolean bl;
                boolean bl2 = bl = !blockState.isAir();
                if (bl && this.currentBreakingProgress == 0.0f) {
                    blockState.onBlockBreakStart((World)this.client.world, pos, (PlayerEntity)this.client.player);
                }
                if (bl && blockState.calcBlockBreakingDelta((PlayerEntity)this.client.player, (BlockView)this.client.player.getEntityWorld(), pos) >= 1.0f) {
                    this.breakBlock(pos);
                } else {
    ...
    public void cancelBlockBreaking() {
        if (this.breakingBlock) {
            ...
            if (SharedConstants.BLOCK_BREAK) {
                LOGGER.info("Stop dest {} {}", (Object)this.currentBreakingPos, (Object)blockState);
            }
            this.networkHandler.sendPacket((Packet)new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.ABORT_DESTROY_BLOCK, this.currentBreakingPos, Direction.DOWN));
            this.breakingBlock = false;
            this.currentBreakingProgress = 0.0f;
            this.client.world.setBlockBreakingInfo(this.client.player.getId(), this.currentBreakingPos, -1);
            this.client.player.resetTicksSince();
        }
    }
    ...
    public boolean updateBlockBreakingProgress(BlockPos pos, Direction direction) {
        ...
        if (this.client.player.getAbilities().creativeMode && this.client.world.getWorldBorder().contains(pos)) {
            ...
            this.client.getTutorialManager().onBlockBreaking(this.client.world, pos, blockState, 1.0f);
            if (SharedConstants.BLOCK_BREAK) {
                LOGGER.info("Creative cont {} {}", (Object)pos, (Object)blockState);
            }
            this.sendSequencedPacket(this.client.world, sequence -> {
                this.breakBlock(pos);
                return new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, pos, direction, sequence);
        ...
        if (this.isCurrentlyBreaking(pos)) {
            ...
            this.currentBreakingProgress += blockState.calcBlockBreakingDelta((PlayerEntity)this.client.player, (BlockView)this.client.player.getEntityWorld(), pos);
            if (this.blockBreakingSoundCooldown % 4.0f == 0.0f) {
                BlockSoundGroup blockSoundGroup = blockState.getSoundGroup();
                this.client.getSoundManager().play((SoundInstance)new PositionedSoundInstance(blockSoundGroup.getHitSound(), SoundCategory.BLOCKS, (blockSoundGroup.getVolume() + 1.0f) / 8.0f, blockSoundGroup.getPitch() * 0.5f, SoundInstance.createRandom(), pos));
            }
            this.blockBreakingSoundCooldown += 1.0f;
            this.client.getTutorialManager().onBlockBreaking(this.client.world, pos, blockState, MathHelper.clamp((float)this.currentBreakingProgress, (float)0.0f, (float)1.0f));
            if (this.currentBreakingProgress >= 1.0f) {
                this.breakingBlock = false;
                if (SharedConstants.BLOCK_BREAK) {
                    LOGGER.info("Finished breaking {} {}", (Object)pos, (Object)blockState);
                }
                this.sendSequencedPacket(this.client.world, sequence -> {
                    this.breakBlock(pos);
    ...
    private void sendSequencedPacket(ClientWorld world, SequencedPacketCreator packetCreator) {
        try (PendingUpdateManager pendingUpdateManager = world.getPendingUpdateManager().incrementSequence();){
            ...
            Packet packet = packetCreator.predict(i);
    ...
    private boolean isCurrentlyBreaking(BlockPos pos) {
        ...
        return pos.equals((Object)this.currentBreakingPos) && ItemStack.areItemsAndComponentsEqual((ItemStack)itemStack, (ItemStack)this.selectedStack);
    }
    ...
    private void syncSelectedSlot() {
        ...
        if (i != this.lastSelectedSlot) {
            ...
            this.networkHandler.sendPacket((Packet)new UpdateSelectedSlotC2SPacket(this.lastSelectedSlot));
        }
    }
    ...
    public ActionResult interactBlock(ClientPlayerEntity player, Hand hand, BlockHitResult hitResult) {
        ...
        this.sendSequencedPacket(this.client.world, sequence -> {
            mutableObject.setValue((Object)this.interactBlockInternal(player, hand, hitResult));
            return new PlayerInteractBlockC2SPacket(hand, hitResult, sequence);
        });
        return (ActionResult)mutableObject.get();
    }
    ...
    private ActionResult interactBlockInternal(ClientPlayerEntity player, Hand hand, BlockHitResult hitResult) {
        ...
        if (!bl2) {
            ...
            ActionResult actionResult = blockState.onUseWithItem(player.getStackInHand(hand), (World)this.client.world, (PlayerEntity)player, hand, hitResult);
            if (actionResult.isAccepted()) {
                return actionResult;
            }
            if (actionResult instanceof ActionResult.PassToDefaultBlockAction && hand == Hand.MAIN_HAND && (actionResult2 = blockState.onUse((World)this.client.world, (PlayerEntity)player, hitResult)).isAccepted()) {
        ...
        ItemUsageContext itemUsageContext = new ItemUsageContext((PlayerEntity)player, hand, hitResult);
    ...
    public ActionResult interactItem(PlayerEntity player, Hand hand) {
        ...
        this.sendSequencedPacket(this.client.world, sequence -> {
            ...
            if (player.getItemCooldownManager().isCoolingDown(itemStack)) {
                mutableObject.setValue((Object)ActionResult.PASS);
                return playerInteractItemC2SPacket;
            }
            ActionResult actionResult = itemStack.use((World)this.client.world, player, hand);
    ...
    public void attackEntity(PlayerEntity player, Entity target) {
        this.syncSelectedSlot();
        this.networkHandler.sendPacket((Packet)PlayerInteractEntityC2SPacket.attack((Entity)target, (boolean)player.isSneaking()));
        if (this.gameMode != GameMode.SPECTATOR) {
    ...
    public ActionResult interactEntity(PlayerEntity player, Entity entity, Hand hand) {
        this.syncSelectedSlot();
        this.networkHandler.sendPacket((Packet)PlayerInteractEntityC2SPacket.interact((Entity)entity, (boolean)player.isSneaking(), (Hand)hand));
        if (this.gameMode == GameMode.SPECTATOR) {
            return ActionResult.PASS;
        }
    ...
    public ActionResult interactEntityAtLocation(PlayerEntity player, Entity entity, EntityHitResult hitResult, Hand hand) {
        ...
        this.networkHandler.sendPacket((Packet)PlayerInteractEntityC2SPacket.interactAt((Entity)entity, (boolean)player.isSneaking(), (Hand)hand, (Vec3d)vec3d));
        if (this.gameMode == GameMode.SPECTATOR) {
            return ActionResult.PASS;
        }
    ...
    public void clickSlot(int syncId, int slotId, int button, SlotActionType actionType, PlayerEntity player) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            int2ObjectMap.put(j, (Object)ItemStackHash.fromItemStack((ItemStack)itemStack2, (ComponentChangesHash.ComponentHasher)this.networkHandler.getComponentHasher()));
        }
        ItemStackHash itemStackHash = ItemStackHash.fromItemStack((ItemStack)screenHandler.getCursorStack(), (ComponentChangesHash.ComponentHasher)this.networkHandler.getComponentHasher());
        this.networkHandler.sendPacket((Packet)new ClickSlotC2SPacket(syncId, screenHandler.getRevision(), Shorts.checkedCast((long)slotId), SignedBytes.checkedCast((long)button), actionType, (Int2ObjectMap)int2ObjectMap, itemStackHash));
    }

    public void clickRecipe(int syncId, NetworkRecipeId recipeId, boolean craftAll) {
        this.networkHandler.sendPacket((Packet)new CraftRequestC2SPacket(syncId, recipeId, craftAll));
    }

    public void clickButton(int syncId, int buttonId) {
        this.networkHandler.sendPacket((Packet)new ButtonClickC2SPacket(syncId, buttonId));
    }
    ...
    public void clickCreativeStack(ItemStack stack, int slotId) {
        if (this.client.player.isInCreativeMode() && this.networkHandler.hasFeature(stack.getItem().getRequiredFeatures())) {
            this.networkHandler.sendPacket((Packet)new CreativeInventoryActionC2SPacket(slotId, stack));
        }
    }
    ...
    public void dropCreativeStack(ItemStack stack) {
        ...
        if (this.client.player.isInCreativeMode() && !bl && !stack.isEmpty() && this.networkHandler.hasFeature(stack.getItem().getRequiredFeatures())) {
            this.networkHandler.sendPacket((Packet)new CreativeInventoryActionC2SPacket(-1, stack));
    ...
    public void stopUsingItem(PlayerEntity player) {
        this.syncSelectedSlot();
        this.networkHandler.sendPacket((Packet)new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, Direction.DOWN));
        player.stopUsingItem();
    }

    public void attackWithPiercingWeapon(PiercingWeaponComponent piercingWeaponComponent) {
        this.syncSelectedSlot();
        this.networkHandler.sendPacket((Packet)new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STAB, BlockPos.ORIGIN, Direction.DOWN));
        this.client.player.beforePlayerAttack();
        this.client.player.useAttackEnchantmentEffects();
        piercingWeaponComponent.playSound((Entity)this.client.player);
    }
    ...
    public void pickItemFromBlock(BlockPos pos, boolean includeData) {
        this.networkHandler.sendPacket((Packet)new PickItemFromBlockC2SPacket(pos, includeData));
    }

    public void pickItemFromEntity(Entity entity, boolean includeData) {
        this.networkHandler.sendPacket((Packet)new PickItemFromEntityC2SPacket(entity.getId(), includeData));
    }

    public void slotChangedState(int slot, int screenHandlerId, boolean newState) {
        this.networkHandler.sendPacket((Packet)new SlotChangedStateC2SPacket(slot, screenHandlerId, newState));
    }
}
