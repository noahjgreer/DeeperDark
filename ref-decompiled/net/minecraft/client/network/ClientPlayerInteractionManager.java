      ...
      if (this.blockBreakingCooldown > 0) {
         ...
         if (this.client.player.getAbilities().creativeMode && this.client.world.getWorldBorder().contains(pos)) {
            ...
            if (blockState.isAir()) {
               ...
               this.currentBreakingProgress += blockState.calcBlockBreakingDelta(this.client.player, this.client.player.getWorld(), pos);
      ...
      if (syncId != screenHandler.syncId) {
         ...
         for(int j = 0; j < i; ++j) {
            ...
            if (!ItemStack.areEqual(itemStack, itemStack2)) {
               int2ObjectMap.put(j, ItemStackHash.fromItemStack(itemStack2, this.networkHandler.getComponentHasher()));
            }
         }
         ...
         this.networkHandler.sendPacket(new ClickSlotC2SPacket(syncId, screenHandler.getRevision(), Shorts.checkedCast((long)slotId), SignedBytes.checkedCast((long)button), actionType, int2ObjectMap, itemStackHash));
      }
   }
