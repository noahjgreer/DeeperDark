...
public class MessageHandler {
    private static final Text VALIDATION_ERROR_TEXT = Text.translatable((String)"chat.validation_error").formatted(new Formatting[]{Formatting.RED, Formatting.ITALIC});
    ...
    public void processDelayedMessages() {
        ...
        if (this.chatDelay == 0L) {
            ...
            while ((processableMessage = (ProcessableMessage)this.delayedMessages.poll()) != null && !processableMessage.accept()) {
            }
        }
    }
    ...
    public boolean removeDelayedMessage(MessageSignatureData signature) {
        return this.delayedMessages.removeIf(message -> signature.equals((Object)message.signature()));
    }
    ...
    private MessageTrustStatus getStatus(SignedMessage message, Text decorated, Instant receptionTimestamp) {
        ...
        return MessageTrustStatus.getStatus((SignedMessage)message, (Text)decorated, (Instant)receptionTimestamp);
    }
    ...
    private void addToChatLog(SignedMessage message, GameProfile gameProfile, MessageTrustStatus messageTrustStatus) {
        ...
        chatLog.add((ChatLogEntry)ReceivedMessage.of((GameProfile)gameProfile, (SignedMessage)message, (MessageTrustStatus)messageTrustStatus));
    }

    private void addToChatLog(Text message, Instant timestamp) {
        ChatLog chatLog = this.client.getAbuseReportContext().getChatLog();
        chatLog.add((ChatLogEntry)ReceivedMessage.of((Text)message, (Instant)timestamp));
    }

    public void onGameMessage(Text message, boolean overlay) {
        if (((Boolean)this.client.options.getHideMatchedNames().getValue()).booleanValue() && this.client.shouldBlockMessages(this.extractSender(message))) {
            return;
        }
        if (overlay) {
    ...
    private UUID extractSender(Text text) {
        String string = TextVisitFactory.removeFormattingCodes((StringVisitable)text);
