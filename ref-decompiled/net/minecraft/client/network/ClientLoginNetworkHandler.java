...
public class ClientLoginNetworkHandler implements ClientLoginPacketListener {
   ...
   private final Map serverCookies;
   private final boolean hasCookies;
   private final AtomicReference state;

   public ClientLoginNetworkHandler(ClientConnection connection, MinecraftClient client, @Nullable ServerInfo serverInfo, @Nullable Screen parentScreen, boolean newWorld, @Nullable Duration worldLoadTime, Consumer statusConsumer, @Nullable CookieStorage cookieStorage) {
      this.state = new AtomicReference(ClientLoginNetworkHandler.State.CONNECTING);
   ...
   private void switchTo(State state) {
      State state2 = (State)this.state.updateAndGet((currentState) -> {
         if (!state.prevStates.contains(currentState)) {
            ...
            throw new IllegalStateException("Tried to switch to " + var10002 + " from " + String.valueOf(currentState) + ", but expected one of " + String.valueOf(state.prevStates));
         } else {
            return state;
         }
      });
   ...
   private void setupEncryption(LoginKeyC2SPacket keyPacket, Cipher decryptionCipher, Cipher encryptionCipher) {
      ...
      this.connection.send(keyPacket, PacketCallbacks.always(() -> {
         this.connection.setupEncryption(decryptionCipher, encryptionCipher);
      }));
   }
   ...
   public void onQueryRequest(LoginQueryRequestS2CPacket packet) {
      ...
      this.connection.send(new LoginQueryResponseC2SPacket(packet.queryId(), (LoginQueryResponsePayload)null));
   }
   ...
   static enum State {
      ...
      final Text name;
      final Set prevStates;

      private State(final Text name, final Set prevStates) {
         this.name = name;
         this.prevStates = prevStates;
      }

      // $FF: synthetic method
      private static State[] method_53874() {
