...
public class WorldEventHandler {
   ...
   private final Map playingSongs = new HashMap();
   ...
   public void processWorldEvent(int eventId, BlockPos pos, int data) {
      ...
      switch (eventId) {
         ...
         case 1009:
            ...
            break;
         case 1010:
            this.world.getRegistryManager().getOrThrow(RegistryKeys.JUKEBOX_SONG).getEntry(data).ifPresent((song) -> {
               this.playJukeboxSong(song, pos);
            });
            break;
         case 1011:
         ...
         case 1502:
            ...
            for(m = 0; m < 5; ++m) {
               g = (double)pos.getX() + random.nextDouble() * 0.6 + 0.2;
               n = (double)pos.getY() + random.nextDouble() * 0.6 + 0.2;
               o = (double)pos.getZ() + random.nextDouble() * 0.6 + 0.2;
         ...
         case 1503:
            ...
            for(m = 0; m < 16; ++m) {
               g = (double)pos.getX() + (5.0 + random.nextDouble() * 6.0) / 16.0;
               n = (double)pos.getY() + 0.8125;
               o = (double)pos.getZ() + (5.0 + random.nextDouble() * 6.0) / 16.0;
               this.world.addParticleClient(ParticleTypes.SMOKE, g, n, o, 0.0, 0.0, 0.0);
            }
         ...
         case 2007:
            ...
            for(int j = 0; j < 8; ++j) {
               this.worldRenderer.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.SPLASH_POTION)), vec3d.x, vec3d.y, vec3d.z, random.nextGaussian() * 0.15, random.nextDouble() * 0.2, random.nextGaussian() * 0.15);
            }

            float h = (float)(data >> 16 & 255) / 255.0F;
            float k = (float)(data >> 8 & 255) / 255.0F;
            l = (float)(data >> 0 & 255) / 255.0F;
            ParticleEffect particleEffect = eventId == 2007 ? ParticleTypes.INSTANT_EFFECT : ParticleTypes.EFFECT;

            for(m = 0; m < 100; ++m) {
               g = random.nextDouble() * 4.0;
               n = random.nextDouble() * Math.PI * 2.0;
               o = Math.cos(n) * g;
               p = 0.01 + random.nextDouble() * 0.5;
               double q = Math.sin(n) * g;
               Particle particle = this.worldRenderer.spawnParticle(particleEffect, particleEffect.getType().shouldAlwaysSpawn(), vec3d.x + o * 0.1, vec3d.y + 0.3, vec3d.z + q * 0.1, o, p, q);
               if (particle != null) {
         ...
         case 2003:
            ...
            for(int i = 0; i < 8; ++i) {
               this.worldRenderer.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.ENDER_EYE)), d, e, f, random.nextGaussian() * 0.15, random.nextDouble() * 0.2, random.nextGaussian() * 0.15);
            }

            for(g = 0.0; g < 6.283185307179586; g += 0.15707963267948966) {
         ...
         case 2004:
            for(s = 0; s < 20; ++s) {
               t = (double)pos.getX() + 0.5 + (random.nextDouble() - 0.5) * 2.0;
               u = (double)pos.getY() + 0.5 + (random.nextDouble() - 0.5) * 2.0;
               v = (double)pos.getZ() + 0.5 + (random.nextDouble() - 0.5) * 2.0;
         ...
         case 2006:
            for(m = 0; m < 200; ++m) {
               ab = random.nextFloat() * 4.0F;
               float ag = random.nextFloat() * 6.2831855F;
               n = (double)(MathHelper.cos(ag) * ab);
               o = 0.01 + random.nextDouble() * 0.5;
               p = (double)(MathHelper.sin(ag) * ab);
               Particle particle2 = this.worldRenderer.spawnParticle(ParticleTypes.DRAGON_BREATH, false, (double)pos.getX() + n * 0.1, (double)pos.getY() + 0.3, (double)pos.getZ() + p * 0.1, n, o, p);
               if (particle2 != null) {
                  particle2.move(ab);
               }
            }
         ...
         case 2013:
            ...
            break;
         case 3000:
            this.world.addParticleClient(ParticleTypes.EXPLOSION_EMITTER, true, true, (double)pos.getX() + 0.5, (double)pos.getY() + 0.5, (double)pos.getZ() + 0.5, 0.0, 0.0, 0.0);
         ...
         case 3006:
            ...
            if (s > 0) {
               if (random.nextFloat() < 0.3F + (float)s * 0.1F) {
                  l = 0.15F + 0.02F * (float)s * (float)s * random.nextFloat();
               ...
               if (b == 0) {
                  ...
                  for(int var14 = 0; var14 < var13; ++var14) {
                     ...
                     float y = direction == Direction.DOWN ? 3.1415927F : 0.0F;
                     p = direction.getAxis() == Direction.Axis.Y ? 0.65 : 0.57;
                     ParticleUtil.spawnParticles(this.world, pos, new SculkChargeParticleEffect(y), intProvider, direction, supplier, p);
                  }

                  return;
               } else {
                  Iterator var50 = MultifaceBlock.flagToDirections(b).iterator();

                  while(var50.hasNext()) {
                     Direction direction2 = (Direction)var50.next();
                     z = direction2 == Direction.UP ? 3.1415927F : 0.0F;
                     o = 0.35;
               ...
               for(int ac = 0; ac < aa; ++ac) {
                  ...
                  z = 2.0F * random.nextFloat() - 1.0F;
                  float ae = 2.0F * random.nextFloat() - 1.0F;
                  this.world.addParticleClient(ParticleTypes.SCULK_CHARGE_POP, (double)pos.getX() + 0.5 + (double)(ad * x), (double)pos.getY() + 0.5 + (double)(z * x), (double)pos.getZ() + 0.5 + (double)(ae * x), (double)(ad * 0.07F), (double)(z * 0.07F), (double)(ae * 0.07F));
               }
         ...
         case 3008:
            ...
            if (var31 instanceof BrushableBlock brushableBlock) {
            ...
            break;
         case 3009:
            ParticleUtil.spawnParticle(this.world, pos, (ParticleEffect)ParticleTypes.EGG_CRACK, (IntProvider)UniformIntProvider.create(3, 6));
            break;
         case 3011:
         ...
         case 3014:
            ...
            break;
         case 3015:
            BlockEntity var30 = this.world.getBlockEntity(pos);
            if (var30 instanceof VaultBlockEntity vaultBlockEntity) {
         ...
         case 3018:
            for(s = 0; s < 10; ++s) {
               t = random.nextGaussian() * 0.02;
               u = random.nextGaussian() * 0.02;
               v = random.nextGaussian() * 0.02;
   ...
   private void playJukeboxSong(RegistryEntry song, BlockPos jukeboxPos) {
