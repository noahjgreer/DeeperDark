...
public class PeriodicNotificationManager extends SinglePreparationResourceReloader implements AutoCloseable {
   ...
   protected Map prepare(ResourceManager resourceManager, Profiler profiler) {
      try {
         ...
         Map var4;
         try {
            var4 = (Map)CODEC.parse(JsonOps.INSTANCE, StrictJsonParser.parse((Reader)reader)).result().orElseThrow();
   ...
   private long getPeriod(List entries, long minDelay) {
      return entries.stream().mapToLong((entry) -> {
         ...
         return new IllegalStateException("Empty notifications from: " + String.valueOf(this.id));
      });
   }
      ...
      public void run() {
         ...
         while(var5.hasNext()) {
            ...
            if (l >= entry.delay) {
               ...
               if (n != o) {
                  this.client.execute(() -> {
                     SystemToast.add(MinecraftClient.getInstance().getToastManager(), SystemToast.Type.PERIODIC_NOTIFICATION, Text.translatable(entry.title, n), Text.translatable(entry.message, n));
                  });
                  return;
               }
            }
