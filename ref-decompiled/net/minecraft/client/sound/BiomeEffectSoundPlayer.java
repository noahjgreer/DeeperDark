...
public class BiomeEffectSoundPlayer implements ClientPlayerTickable {
   ...
   public void tick() {
      ...
      if (biome != this.activeBiome) {
         ...
         biome.getLoopSound().ifPresent((sound) -> {
            this.soundLoops.compute(biome, (soundx, loop) -> {
               if (loop == null) {
      ...
      this.moodSound.ifPresent((sound) -> {
         ...
         if (j > 0) {
            this.moodPercentage -= (float)j / 15.0F * 0.001F;
         } else {
            this.moodPercentage -= (float)(world.getLightLevel(LightType.BLOCK, blockPos) - 1) / (float)sound.getCultivationTicks();
         }
         ...
         if (this.moodPercentage >= 1.0F) {
            ...
            PositionedSoundInstance positionedSoundInstance = PositionedSoundInstance.ambient((SoundEvent)sound.getSound().value(), this.random, this.player.getX() + g / l * m, this.player.getEyeY() + h / l * m, this.player.getZ() + k / l * m);
