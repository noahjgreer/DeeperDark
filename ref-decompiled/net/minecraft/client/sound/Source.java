...
public class Source {
    ...
    static @Nullable Source create() {
        ...
        if (AlUtil.checkErrors((String)"Allocate new source")) {
            return null;
        }
    ...
    public void close() {
        if (this.playing.compareAndSet(true, false)) {
            ...
            AlUtil.checkErrors((String)"Stop");
            if (this.stream != null) {
                try {
                    this.stream.close();
                }
            ...
            AlUtil.checkErrors((String)"Cleanup");
        }
    }
    ...
    public void setStream(AudioStream stream) {
        ...
        this.bufferSize = Source.getBufferSize((AudioFormat)audioFormat, (int)1);
        this.read(4);
    }
    ...
    private int removeProcessedBuffers() {
        ...
        if (i > 0) {
            ...
            AlUtil.checkErrors((String)"Unqueue buffers");
            AL10.alDeleteBuffers((int[])is);
            AlUtil.checkErrors((String)"Remove processed buffers");
        }
        return i;
    }
}
