...
public class Source {
   ...
   public void setBuffer(StaticSound sound) {
      sound.getStreamBufferPointer().ifPresent((pointer) -> {
         AL10.alSourcei(this.pointer, 4105, pointer);
      });
   }
      ...
      if (this.stream != null) {
         try {
            for(int i = 0; i < count; ++i) {
               ...
               if (byteBuffer != null) {
                  (new StaticSound(byteBuffer, this.stream.getFormat())).takeStreamBufferPointer().ifPresent((pointer) -> {
                     AL10.alSourceQueueBuffers(this.pointer, new int[]{pointer});
                  });
               }
            }
