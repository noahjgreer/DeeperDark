...
public class MapTextureManager implements AutoCloseable {
   private final Int2ObjectMap texturesByMapId = new Int2ObjectOpenHashMap();
   ...
   public void clear() {
      ...
      while(var1.hasNext()) {
         MapTexture mapTexture = (MapTexture)var1.next();
         mapTexture.close();
      }
   ...
   private class MapTexture implements AutoCloseable {
      ...
      MapTexture(final MapTextureManager mapTextureManager, final int id, final MapState state) {
         this.state = state;
         this.texture = new NativeImageBackedTexture(() -> {
            return "Map " + id;
         }, 128, 128, true);
         this.textureId = Identifier.ofVanilla("map/" + id);
         mapTextureManager.textureManager.registerTexture(this.textureId, (AbstractTexture)this.texture);
      }
