...
public class NativeImageBackedTexture extends AbstractTexture implements DynamicTexture {
   ...
   public NativeImageBackedTexture(Supplier nameSupplier, NativeImage image) {
      this.image = image;
   ...
   public NativeImageBackedTexture(Supplier nameSupplier, int width, int height, boolean useStb) {
      this.image = new NativeImage(width, height, useStb);
      this.createTexture(nameSupplier);
   }

   private void createTexture(Supplier nameSupplier) {
      GpuDevice gpuDevice = RenderSystem.getDevice();
      this.glTexture = gpuDevice.createTexture((Supplier)nameSupplier, 5, TextureFormat.RGBA8, this.image.getWidth(), this.image.getHeight(), 1, 1);
   ...
   private void createTexture(String name) {
      GpuDevice gpuDevice = RenderSystem.getDevice();
      this.glTexture = gpuDevice.createTexture((String)name, 5, TextureFormat.RGBA8, this.image.getWidth(), this.image.getHeight(), 1, 1);
      this.glTexture.setTextureFilter(FilterMode.NEAREST, false);
      this.glTextureView = gpuDevice.createTextureView(this.glTexture);
   }
