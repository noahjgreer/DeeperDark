...
public class MipmapHelper {
    ...
    private static void adjustAlphaForTargetCoverage(NativeImage image, float targetCoverage, float alphaThreshold, float cutoffBias) {
        ...
        for (m = 0; m < 5; ++m) {
            float n = MipmapHelper.getOpacityCoverage(image, alphaThreshold, h);
    ...
    public static NativeImage[] getMipmapLevelsImages(Identifier id, NativeImage[] mipmapLevelImages, int mipmapLevels, MipmapStrategy strategy, float cutoffBias) {
        ...
        float g = bl ? MipmapHelper.getOpacityCoverage(mipmapLevelImages[0], f, 1.0f) : 0.0f;
        for (int i = 1; i <= mipmapLevels; ++i) {
            if (i < mipmapLevelImages.length) {
                ...
                for (int l = 0; l < j; ++l) {
                    for (int m = 0; m < k; ++m) {
                        ...
                        int r = strategy == MipmapStrategy.DARK_CUTOUT ? MipmapHelper.blendDarkenedCutout(n, o, p, q) : ColorHelper.interpolate(n, o, p, q);
            ...
            if (!bl) continue;
            MipmapHelper.adjustAlphaForTargetCoverage(nativeImages[i], g, f, cutoffBias);
        }
        return nativeImages;
    }
    ...
    private static int blendDarkenedCutout(int nw, int ne, int sw, int se) {
        ...
        return ColorHelper.getArgb(ColorHelper.linearToSrgb(f /= 4.0f), ColorHelper.linearToSrgb(g /= 4.0f), ColorHelper.linearToSrgb(h /= 4.0f), ColorHelper.linearToSrgb(i /= 4.0f));
    }
}
