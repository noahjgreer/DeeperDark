...
public class WorldGenSettingsDisallowOldCustomWorldsFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("WorldGenSettingsDisallowOldCustomWorldsFix_" + this.getOutputSchema().getVersionKey(), type, (worldGenSettingsTyped) -> {
         return worldGenSettingsTyped.updateTyped(opticFinder, (dimensionsTyped) -> {
            dimensionsTyped.write().map((dimensionsDynamic) -> {
               return dimensionsDynamic.getMapValues().map((dimensions) -> {
