...
public class CauldronRenameFix extends DataFix {
   ...
   private static Dynamic rename(Dynamic cauldronDynamic) {
      ...
      if (optional.equals(Optional.of("minecraft:cauldron"))) {
         ...
         return dynamic.get("level").asString("0").equals("0") ? cauldronDynamic.remove("Properties") : cauldronDynamic.set("Name", cauldronDynamic.createString("minecraft:water_cauldron"));
      } else {
   ...
   protected TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("cauldron_rename_fix", this.getInputSchema().getType(TypeReferences.BLOCK_STATE), (typed) -> {
         return typed.update(DSL.remainderFinder(), CauldronRenameFix::rename);
      });
   }
}
