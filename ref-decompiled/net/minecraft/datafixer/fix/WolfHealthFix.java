...
public class WolfHealthFix extends ChoiceFix {
   ...
   protected Typed transform(Typed inputTyped) {
      return inputTyped.update(DSL.remainderFinder(), (wolfDynamic) -> {
         ...
         wolfDynamic = wolfDynamic.update("Attributes", (attributesDynamic) -> {
            return attributesDynamic.createList(attributesDynamic.asStream().map((attributeDynamic) -> {
               return "minecraft:generic.max_health".equals(IdentifierNormalizingSchema.normalize(attributeDynamic.get("Name").asString(""))) ? attributeDynamic.update("Base", (baseDynamic) -> {
         ...
         if (mutableBoolean.isTrue()) {
            wolfDynamic = wolfDynamic.update("Health", (healthDynamic) -> {
               return healthDynamic.createFloat(healthDynamic.asFloat(0.0F) * 2.0F);
            });
         }
