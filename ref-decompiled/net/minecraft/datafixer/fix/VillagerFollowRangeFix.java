...
public class VillagerFollowRangeFix extends ChoiceFix {
   ...
   private static Dynamic fix(Dynamic villagerDynamic) {
      return villagerDynamic.update("Attributes", (attributesDynamic) -> {
         return villagerDynamic.createList(attributesDynamic.asStream().map((attributeDynamic) -> {
            return attributeDynamic.get("Name").asString("").equals("generic.follow_range") && attributeDynamic.get("Base").asDouble(0.0) == 16.0 ? attributeDynamic.set("Base", attributeDynamic.createDouble(48.0)) : attributeDynamic;
         }));
      });
   }
}
