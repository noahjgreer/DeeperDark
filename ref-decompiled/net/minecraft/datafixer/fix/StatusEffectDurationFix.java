...
public class StatusEffectDurationFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("EffectDurationEntity", schema.getType(TypeReferences.ENTITY), (entityTyped) -> {
         return entityTyped.update(DSL.remainderFinder(), this::fixEntityStatusEffects);
      }), new TypeRewriteRule[]{this.fixTypeEverywhereTyped("EffectDurationPlayer", schema.getType(TypeReferences.PLAYER), (playerTyped) -> {
         return playerTyped.update(DSL.remainderFinder(), this::fixEntityStatusEffects);
      }), this.fixTypeEverywhereTyped("EffectDurationItem", type, (itemStackTyped) -> {
         if (itemStackTyped.getOptional(opticFinder).filter((pair) -> {
            return POTION_ITEM_IDS.contains(pair.getSecond());
         }).isPresent()) {
            Optional optional = itemStackTyped.getOptionalTyped(opticFinder2);
