...
public class EntityCustomNameToTextFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("EntityCustomNameToComponentFix", type, type2, (typed) -> {
         return method_66064(typed, type2, opticFinder, opticFinder2, type3);
      });
   }

   private static Typed method_66064(Typed typed, Type type, OpticFinder opticFinder, OpticFinder opticFinder2, Type type2) {
      Optional optional = typed.getOptional(opticFinder2);
      if (optional.isEmpty()) {
         return FixUtil.withType(type, typed);
      } else if (((String)optional.get()).isEmpty()) {
         return Util.apply(typed, type, (dynamicx) -> {
            return dynamicx.remove("CustomName");
         });
      } else {
   ...
   private static Dynamic method_66066(DynamicOps dynamicOps, String string, String string2) {
