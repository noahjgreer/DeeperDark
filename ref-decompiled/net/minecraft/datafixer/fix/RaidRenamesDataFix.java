...
public class RaidRenamesDataFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("RaidRenamesDataFix", this.getInputSchema().getType(TypeReferences.SAVED_DATA_RAIDS), (typed) -> {
         return typed.update(DSL.remainderFinder(), (dynamic) -> {
            return dynamic.update("data", RaidRenamesDataFix::fixRaidsData);
         });
      });
   }

   private static Dynamic fixRaidsData(Dynamic dynamic) {
      return dynamic.renameAndFixField("Raids", "raids", (raidsDynamic) -> {
         return raidsDynamic.createList(raidsDynamic.asStream().map(RaidRenamesDataFix::fixRaid));
