   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("Handle ticks saved in the wrong chunk", type, (chunkTyped) -> {
         ...
         return chunkTyped.update(DSL.remainderFinder(), (chunkTag) -> {
            ...
            Optional optional2x = chunkTag.get("fluid_ticks").get().result();
            chunkTag = putNeighborTicks(chunkTag, i, j, optional2, "neighbor_block_ticks");
            chunkTag = putNeighborTicks(chunkTag, i, j, optional2x, "neighbor_fluid_ticks");
            return chunkTag;
         });
      });
   }

   private static Dynamic putNeighborTicks(Dynamic chunkTag, int chunkX, int chunkZ, Optional ticks, String upgradeDataKey) {
      if (ticks.isPresent()) {
         List list = ((Dynamic)ticks.get()).asStream().filter((tickDynamic) -> {
