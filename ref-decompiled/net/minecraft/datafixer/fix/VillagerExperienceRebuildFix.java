...
public class VillagerExperienceRebuildFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("Villager level and xp rebuild", this.getInputSchema().getType(TypeReferences.ENTITY), (entityTyped) -> {
         return entityTyped.updateTyped(opticFinder, type, (villagerTyped) -> {
            ...
            if (i == 0 || i == 1) {
               int j = (Integer)villagerTyped.getOptionalTyped(opticFinder2).flatMap((offersTyped) -> {
                  return offersTyped.getOptionalTyped(opticFinder3);
               }).map((recipesTyped) -> {
                  return recipesTyped.getAllTyped(opticFinder4).size();
               }).orElse(0);
   ...
   private static Typed fixLevel(Typed villagerTyped, int level) {
      return villagerTyped.update(DSL.remainderFinder(), (villager) -> {
         return villager.update("VillagerData", (villagerDataDynamic) -> {
            return villagerDataDynamic.set("level", villagerDataDynamic.createInt(level));
         });
      });
   }
   ...
   private static Typed fixExperience(Typed villagerTyped, int level) {
      ...
      return villagerTyped.update(DSL.remainderFinder(), (villager) -> {
         return villager.set("Xp", villager.createInt(i));
      });
   }
}
