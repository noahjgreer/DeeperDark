...
public class ItemIdFix extends DataFix {
   private static final Int2ObjectMap NUMERICAL_ID_TO_STRING_ID_MAP = (Int2ObjectMap)DataFixUtils.make(new Int2ObjectOpenHashMap(), (map) -> {
   ...
   public TypeRewriteRule makeRule() {
      Type type = DSL.or(DSL.intType(), DSL.named(TypeReferences.ITEM_NAME.typeName(), IdentifierNormalizingSchema.getIdentifierType()));
      Type type2 = DSL.named(TypeReferences.ITEM_NAME.typeName(), IdentifierNormalizingSchema.getIdentifierType());
      OpticFinder opticFinder = DSL.fieldFinder("id", type);
      return this.fixTypeEverywhereTyped("ItemIdFix", this.getInputSchema().getType(TypeReferences.ITEM_STACK), this.getOutputSchema().getType(TypeReferences.ITEM_STACK), (itemStackTyped) -> {
         return itemStackTyped.update(opticFinder, type2, (id) -> {
            return (Pair)id.map((ordinal) -> {
               return Pair.of(TypeReferences.ITEM_NAME.typeName(), fromId(ordinal));
            }, (named) -> {
               return named;
            });
         });
