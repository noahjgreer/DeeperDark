...
public class EntityBlockStateFix extends DataFix {
   private static final Map BLOCK_NAME_TO_ID = (Map)DataFixUtils.make(Maps.newHashMap(), (map) -> {
   ...
   public TypeRewriteRule makeRule() {
      ...
      Function function = (minecart) -> {
         return this.mergeIdAndData(minecart, "DisplayTile", "DisplayData", "DisplayState");
      };
      Function function2 = (arrow) -> {
         return this.mergeIdAndData(arrow, "inTile", "inData", "inBlockState");
      };
      Type type = DSL.and(DSL.optional(DSL.field("inTile", DSL.named(TypeReferences.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), IdentifierNormalizingSchema.getIdentifierType())))), DSL.remainderType());
      Function function3 = (projectile) -> {
         return projectile.update(type.finder(), DSL.remainderType(), Pair::getSecond);
      };
      return this.fixTypeEverywhereTyped("EntityBlockStateFix", schema.getType(TypeReferences.ENTITY), schema2.getType(TypeReferences.ENTITY), (entity) -> {
         ...
         entity = this.useFunction(entity, "minecraft:enderman", (enderman) -> {
            return this.mergeIdAndData(enderman, "carried", "carriedData", "carriedBlockState");
         });
         ...
         entity = this.useFunction(entity, "minecraft:spawner_minecart", function);
         return entity;
      });
   }

   private Typed fixFallingBlock(Typed fallingBlock) {
      Type type = DSL.optional(DSL.field("Block", DSL.named(TypeReferences.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), IdentifierNormalizingSchema.getIdentifierType()))));
      Type type2 = DSL.optional(DSL.field("BlockState", DSL.named(TypeReferences.BLOCK_STATE.typeName(), DSL.remainderType())));
      Dynamic dynamic = (Dynamic)fallingBlock.get(DSL.remainderFinder());
      return fallingBlock.update(type.finder(), type2, (state) -> {
         int i = (Integer)state.map((pair) -> {
            return (Integer)((Either)pair.getSecond()).map((id) -> {
               return id;
            }, EntityBlockStateFix::getNumericalBlockId);
         }, (unit) -> {
   ...
   private Typed mergeIdAndData(Typed entity, String oldIdKey, String oldDataKey, String newStateKey) {
      Type type = DSL.field(oldIdKey, DSL.named(TypeReferences.BLOCK_NAME.typeName(), DSL.or(DSL.intType(), IdentifierNormalizingSchema.getIdentifierType())));
      Type type2 = DSL.field(newStateKey, DSL.named(TypeReferences.BLOCK_STATE.typeName(), DSL.remainderType()));
      Dynamic dynamic = (Dynamic)entity.getOrCreate(DSL.remainderFinder());
      return entity.update(type.finder(), type2, (state) -> {
         int i = (Integer)((Either)state.getSecond()).map((id) -> {
            return id;
         }, EntityBlockStateFix::getNumericalBlockId);
   ...
   private Typed useFunction(Typed entity, String entityId, Function function) {
