...
public class OptionsKeyLwjgl3Fix extends DataFix {
   ...
   private static final Int2ObjectMap NUMERICAL_KEY_IDS_TO_KEY_NAMES = (Int2ObjectMap)DataFixUtils.make(new Int2ObjectOpenHashMap(), (map) -> {
   ...
   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("OptionsKeyLwjgl3Fix", this.getInputSchema().getType(TypeReferences.OPTIONS), (optionsTyped) -> {
         return optionsTyped.update(DSL.remainderFinder(), (optionsDynamic) -> {
            return (Dynamic)optionsDynamic.getMapValues().map((optionsMap) -> {
               return optionsDynamic.createMap((Map)optionsMap.entrySet().stream().map((entry) -> {
                  if (((Dynamic)entry.getKey()).asString("").startsWith("key_")) {
                     ...
                     if (i < 0) {
                        ...
                        String string2 = (String)NUMERICAL_KEY_IDS_TO_KEY_NAMES.getOrDefault(i, "key.unknown");
