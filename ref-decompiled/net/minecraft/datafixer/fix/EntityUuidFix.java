...
public class EntityUuidFix extends AbstractUuidFix {
   ...
   private static final Set RIDEABLE_TAMEABLES = Sets.newHashSet();
   private static final Set TAMEABLE_PETS = Sets.newHashSet();
   private static final Set BREEDABLES = Sets.newHashSet();
   private static final Set LEASHABLES = Sets.newHashSet();
   private static final Set OTHER_LIVINGS = Sets.newHashSet();
   ...
   protected TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), (typed) -> {
         ...
         Iterator var2;
         String string;
         for(var2 = RIDEABLE_TAMEABLES.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateTameable)) {
            string = (String)var2.next();
         }

         for(var2 = TAMEABLE_PETS.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateTameable)) {
            string = (String)var2.next();
         }

         for(var2 = BREEDABLES.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateBreedable)) {
            string = (String)var2.next();
         }

         for(var2 = LEASHABLES.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateLeashable)) {
            string = (String)var2.next();
         }

         for(var2 = OTHER_LIVINGS.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateLiving)) {
            string = (String)var2.next();
         }

         for(var2 = PROJECTILES.iterator(); var2.hasNext(); typed = this.updateTyped(typed, string, EntityUuidFix::updateProjectile)) {
            string = (String)var2.next();
         }
   ...
   private static Dynamic updateFox(Dynamic dynamic) {
      Optional optional = dynamic.get("TrustedUUIDs").result().map((dynamic2) -> {
