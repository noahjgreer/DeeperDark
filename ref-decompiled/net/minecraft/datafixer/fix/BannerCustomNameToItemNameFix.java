...
public class BannerCustomNameToItemNameFix extends DataFix {
   ...
   private Typed fix(Typed typed, OpticFinder opticFinder, OpticFinder opticFinder2) {
      Optional optional = typed.getOptionalTyped(opticFinder2).flatMap((typedx) -> {
         return typedx.getOptional(opticFinder).map(Pair::getSecond);
      });
      boolean bl = optional.flatMap(TextFixes::getTranslate).filter((name) -> {
         return name.equals("block.minecraft.ominous_banner");
      }).isPresent();
      return bl ? Util.apply(typed, typed.getType(), (dynamic) -> {
