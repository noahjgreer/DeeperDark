...
public class RemoveGolemGossipFix extends ChoiceFix {
   ...
   private static Dynamic updateGossipsList(Dynamic villagerData) {
      return villagerData.update("Gossips", (gossipsDynamic) -> {
         return villagerData.createList(gossipsDynamic.asStream().filter((gossipDynamic) -> {
            return !gossipDynamic.get("Type").asString("").equals("golem");
         }));
      });
   }
}
