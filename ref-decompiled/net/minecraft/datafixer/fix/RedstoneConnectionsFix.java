...
public class RedstoneConnectionsFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("RedstoneConnectionsFix", schema.getType(TypeReferences.BLOCK_STATE), (blockStateTyped) -> {
         return blockStateTyped.update(DSL.remainderFinder(), this::updateBlockState);
      });
   }

   private Dynamic updateBlockState(Dynamic blockStateDynamic) {
      ...
      return !bl ? blockStateDynamic : blockStateDynamic.update("Properties", (propertiesDynamic) -> {
         ...
         return propertiesDynamic.update("east", (eastDynamic) -> {
            return eastDynamic.createString(string5);
         }).update("west", (westDynamic) -> {
            return westDynamic.createString(string6);
         }).update("north", (northDynamic) -> {
            return northDynamic.createString(string7);
         }).update("south", (southDynamic) -> {
            return southDynamic.createString(string8);
         });
      });
   }
