...
public class BannerPatternFormatFix extends ChoiceFix {
   ...
   private static Dynamic replacePatterns(Dynamic dynamic) {
      return dynamic.renameAndFixField("Patterns", "patterns", (dynamicx) -> {
         return dynamicx.createList(dynamicx.asStream().map(BannerPatternFormatFix::replacePatternAndColor));
      });
   }

   private static Dynamic replacePatternAndColor(Dynamic dynamic) {
      dynamic = dynamic.renameAndFixField("Pattern", "pattern", (dynamicx) -> {
         DataResult var10000 = dynamicx.asString().map((string) -> {
            return (String)OLD_TO_NEW_PATTERNS.getOrDefault(string, string);
         });
         Objects.requireNonNull(dynamicx);
         return (Dynamic)DataFixUtils.orElse(var10000.map(dynamicx::createString).result(), dynamicx);
      });
      dynamic = dynamic.set("color", dynamic.createString(FixUtil.getColorName(dynamic.get("Color").asInt(0))));
