...
public class InlineBlockPosFormatFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      List list = List.of(this.getEntityFinder("minecraft:item_frame"), this.getEntityFinder("minecraft:glow_item_frame"), this.getEntityFinder("minecraft:painting"), this.getEntityFinder("minecraft:leash_knot"));
      return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("InlineBlockPosFormatFix - player", this.getInputSchema().getType(TypeReferences.PLAYER), (playerTyped) -> {
         return playerTyped.update(DSL.remainderFinder(), this::fixPlayerFields);
      }), this.fixTypeEverywhereTyped("InlineBlockPosFormatFix - entity", this.getInputSchema().getType(TypeReferences.ENTITY), (entityTyped) -> {
         entityTyped = entityTyped.update(DSL.remainderFinder(), this::fixSleeping).updateTyped(opticFinder, (vexTyped) -> {
            return vexTyped.update(DSL.remainderFinder(), this::fixVexFields);
         }).updateTyped(opticFinder2, (phantomTyped) -> {
            return phantomTyped.update(DSL.remainderFinder(), this::fixPhantomFields);
         }).updateTyped(opticFinder3, (turtleTyped) -> {
            return turtleTyped.update(DSL.remainderFinder(), this::fixTurtleFields);
         });

         OpticFinder opticFinder4;
         for(Iterator var6 = list.iterator(); var6.hasNext(); entityTyped = entityTyped.updateTyped(opticFinder4, (decorationTyped) -> {
            return decorationTyped.update(DSL.remainderFinder(), this::fixDecorationFields);
         })) {
            opticFinder4 = (OpticFinder)var6.next();
         }
   ...
   private Dynamic fixPlayerFields(Dynamic dynamic) {
      ...
      Optional optional4 = dynamic.get("enteredNetherPosition").result();
