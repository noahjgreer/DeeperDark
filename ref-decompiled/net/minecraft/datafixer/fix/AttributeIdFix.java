...
public class AttributeIdFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(this.fixTypeEverywhereTyped("AttributeIdFix (ItemStack)", type, (typed) -> {
         return typed.updateTyped(opticFinder, (typedx) -> {
            return typedx.update(DSL.remainderFinder(), AttributeIdFix::fixItemStack);
         });
      }), new TypeRewriteRule[]{this.fixTypeEverywhereTyped("AttributeIdFix (Entity)", this.getInputSchema().getType(TypeReferences.ENTITY), AttributeIdFix::fixEntity), this.fixTypeEverywhereTyped("AttributeIdFix (Player)", this.getInputSchema().getType(TypeReferences.PLAYER), AttributeIdFix::fixEntity)});
   }

   private static Stream method_60683(Stream stream) {
      return fixModifiers(stream);
   }

   private static Stream fixModifiers(Stream dynamicStream) {
      ...
      dynamicStream.forEach((modifierDynamic) -> {
         ...
         if (string2 != null) {
            ...
            String string4 = "minecraft:" + var10000;
   ...
   private static Dynamic renameOtherFields(Dynamic attributeDynamic) {
      return attributeDynamic.renameField("UUID", "uuid").renameField("Name", "name").renameField("Amount", "amount").renameAndFixField("Operation", "operation", (operationDynamic) -> {
         String var10001;
   ...
   private static Dynamic fixItemStack(Dynamic stackDataDynamic) {
      return stackDataDynamic.update("minecraft:attribute_modifiers", (attributeModifiersDynamic) -> {
         return attributeModifiersDynamic.update("modifiers", (modifiersDynamic) -> {
            Optional var10000 = modifiersDynamic.asStreamOpt().result().map(AttributeIdFix::method_60683);
            Objects.requireNonNull(modifiersDynamic);
            return (Dynamic)DataFixUtils.orElse(var10000.map(modifiersDynamic::createList), modifiersDynamic);
         });
      });
   }

   private static Dynamic fixAttribute(Dynamic attributeDynamic) {
      return attributeDynamic.renameField("Name", "id").renameField("Base", "base").renameAndFixField("Modifiers", "modifiers", (modifiersDynamic) -> {
         Optional var10000 = modifiersDynamic.asStreamOpt().result().map((stream) -> {
            return stream.map(AttributeIdFix::renameOtherFields);
         }).map(AttributeIdFix::method_60683);
         Objects.requireNonNull(attributeDynamic);
         return (Dynamic)DataFixUtils.orElse(var10000.map(attributeDynamic::createList), modifiersDynamic);
      });
   }

   private static Typed fixEntity(Typed entityTyped) {
      return entityTyped.update(DSL.remainderFinder(), (remainder) -> {
         return remainder.renameAndFixField("Attributes", "attributes", (attributesDynamic) -> {
            Optional var10000 = attributesDynamic.asStreamOpt().result().map((stream) -> {
               return stream.map(AttributeIdFix::fixAttribute);
            });
   ...
   public static UUID getUuidFromIntArray(int[] uuidArray) {
      return uuidArray.length != 4 ? null : new UUID((long)uuidArray[0] << 32 | (long)uuidArray[1] & 4294967295L, (long)uuidArray[2] << 32 | (long)uuidArray[3] & 4294967295L);
   }
}
