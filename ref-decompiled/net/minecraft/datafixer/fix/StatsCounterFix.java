...
public class StatsCounterFix extends DataFix {
   ...
   private static Stat rename(String old) {
      if (REMOVED_STATS.contains(old)) {
         ...
         if (string != null) {
            return new Stat("minecraft:custom", string);
         } else {
            ...
            if (i < 0) {
               ...
               if ("stat.mineBlock".equals(string2)) {
                  string3 = getBlock(old.substring(i + 1).replace('.', ':'));
                  return new Stat("minecraft:mined", string3);
               } else {
                  string3 = (String)RENAMED_ITEM_STATS.get(string2);
                  String string4;
                  String string5;
                  String string6;
                  if (string3 != null) {
                     string4 = old.substring(i + 1).replace('.', ':');
                     string5 = getItem(string4);
                     string6 = string5 == null ? string4 : string5;
                     return new Stat(string3, string6);
                  } else {
                     string4 = (String)RENAMED_ENTITY_STATS.get(string2);
                     if (string4 != null) {
                        string5 = old.substring(i + 1).replace('.', ':');
                        string6 = (String)RENAMED_ENTITIES.getOrDefault(string5, string5);
   ...
   private TypeRewriteRule makeSecondRoundRule() {
      ...
      return this.fixTypeEverywhereTyped("ObjectiveStatFix", type, type2, (objectiveTyped) -> {
         ...
         Dynamic dynamic2 = dynamic.update("CriteriaName", (criteriaNameDynamic) -> {
            Optional var10000 = criteriaNameDynamic.asString().result().map((criteriaName) -> {
               if (SKIPPED_STATS.contains(criteriaName)) {
                  return criteriaName;
               } else {
                  Stat stat = rename(criteriaName);
                  if (stat == null) {
                     return "dummy";
                  } else {
                     String var10000 = Schema1451v6.toDotSeparated(stat.type);
                     return var10000 + ":" + Schema1451v6.toDotSeparated(stat.typeKey);
                  }
               }
            });
            Objects.requireNonNull(criteriaNameDynamic);
            return (Dynamic)DataFixUtils.orElse(var10000.map(criteriaNameDynamic::createString), criteriaNameDynamic);
         });
   ...
   private static record Stat(String type, String typeKey) {
      final String type;
      final String typeKey;

      Stat(String string, String string2) {
         this.type = string;
