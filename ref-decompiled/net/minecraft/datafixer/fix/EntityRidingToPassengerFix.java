...
public class EntityRidingToPassengerFix extends DataFix {
   ...
   private TypeRewriteRule fixEntityTree(Schema inputSchema, Schema outputSchema, Type inputEntityTreeType, Type outputEntityTreeType, Type inputEntityType) {
      Type type = DSL.named(TypeReferences.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Riding", inputEntityTreeType)), inputEntityType));
      Type type2 = DSL.named(TypeReferences.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Passengers", DSL.list(outputEntityTreeType))), inputEntityType));
