...
public class OminousBannerRarityFix extends DataFix {
   ...
   private Typed fixNameAndRarity(Typed typed, OpticFinder opticFinder, OpticFinder opticFinder2, OpticFinder opticFinder3) {
      return typed.updateTyped(opticFinder, (typedx) -> {
         boolean bl = typedx.getOptionalTyped(opticFinder2).flatMap((typed) -> {
            return typed.getOptional(opticFinder3);
         }).map(Pair::getSecond).flatMap(TextFixes::getTranslate).filter((string) -> {
            return string.equals("block.minecraft.ominous_banner");
         }).isPresent();
         return bl ? typedx.updateTyped(opticFinder2, (typed) -> {
            return typed.set(opticFinder3, Pair.of(TypeReferences.TEXT_COMPONENT.typeName(), TextFixes.translate("block.minecraft.ominous_banner")));
