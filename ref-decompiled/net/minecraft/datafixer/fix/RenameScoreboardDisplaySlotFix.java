...
public class RenameScoreboardDisplaySlotFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("Scoreboard DisplaySlot rename", type, (scoreboardTyped) -> {
         return scoreboardTyped.updateTyped(opticFinder, (scoreboardDataTyped) -> {
            return scoreboardDataTyped.update(DSL.remainderFinder(), (scoreboardDataDynamic) -> {
               return scoreboardDataDynamic.update("DisplaySlots", (displaySlotsDynamic) -> {
                  return displaySlotsDynamic.updateMapValues((entry) -> {
                     return entry.mapFirst((key) -> {
                        Optional var10000 = key.asString().result().map(RenameScoreboardDisplaySlotFix::getUpdatedName);
