...
extends DataFix {
    ...
    public TypeRewriteRule makeRule() {
        return this.fixTypeEverywhereTyped("LevelFlatGeneratorInfoFix", this.getInputSchema().getType(TypeReferences.LEVEL), levelTyped -> levelTyped.update(DSL.remainderFinder(), this::fixGeneratorOptions));
    }

    private Dynamic<?> fixGeneratorOptions(Dynamic<?> levelDynamic) {
        if (levelDynamic.get("generatorName").asString("").equalsIgnoreCase("flat")) {
            return levelDynamic.update(GENERATOR_OPTIONS_KEY, generatorOptionsDynamic -> (Dynamic)DataFixUtils.orElse((Optional)generatorOptionsDynamic.asString().map(this::fixFlatGeneratorOptions).map(arg_0 -> ((Dynamic)generatorOptionsDynamic).createString(arg_0)).result(), (Object)generatorOptionsDynamic));
        }
        return levelDynamic;
    }
