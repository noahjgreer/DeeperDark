...
extends DataFix {
    ...
    private Typed<?> fixEntities(Typed<?> entityTyped) {
        ...
        entityTyped = this.update(entityTyped, "minecraft:trident", this::moveFlatOwnerMostLeastToArray);
        return entityTyped;
    }
    ...
    private Dynamic<?> insertOwnerUuidArray(Dynamic<?> entityDynamic, long most, long least) {
        ...
        if (most != 0L && least != 0L) {
            return entityDynamic.set("OwnerUUID", entityDynamic.createIntList(Arrays.stream(EntityProjectileOwnerFix.makeUuidArray(most, least))));
        }
    ...
    private Typed<?> update(Typed<?> entityTyped, String matchId, Function<Dynamic<?>, Dynamic<?>> fixer) {
        ...
        return entityTyped.updateTyped(DSL.namedChoice((String)matchId, (Type)type), type2, typed -> typed.update(DSL.remainderFinder(), fixer));
    }
}
