...
public class TrialSpawnerConfigInRegistryFix extends ChoiceFix {
   ...
   public Dynamic fix(Dynamic nbt) {
      ...
      if (optional.isEmpty()) {
         ...
         if (optional2.isEmpty()) {
            ...
            Identifier identifier = (Identifier)TrialSpawnerConfigInRegistryFix.Replacements.REPLACEMENTS.get(Pair.of((Dynamic)optional.get(), (Dynamic)optional2.get()));
            return identifier == null ? nbt : nbt.set("normal_config", nbt.createString(identifier.withSuffixedPath("/normal").toString())).set("ominous_config", nbt.createString(identifier.withSuffixedPath("/ominous").toString()));
         }
      }
   }
   ...
   private static final class Replacements {
      public static final Map REPLACEMENTS = new HashMap();
      ...
      private static void register(Identifier id, String normal, String ominous) {
         try {
            ...
            Dynamic dynamic = toDynamic(nbtCompound);
            ...
            throw new IllegalStateException("Failed to parse NBT for " + String.valueOf(id), var8);
         }
      }

      private static Dynamic toDynamic(NbtCompound nbt) {
