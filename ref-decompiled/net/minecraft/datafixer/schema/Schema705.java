...
public class Schema705 extends IdentifierNormalizingSchema {
   ...
   public void registerTypes(Schema schema, Map entityTypes, Map blockEntityTypes) {
      ...
      schema.registerType(true, TypeReferences.ENTITY, () -> {
         return DSL.and(TypeReferences.ENTITY_EQUIPMENT.in(schema), DSL.optionalFields("CustomName", DSL.constType(DSL.string()), DSL.taggedChoiceLazy("id", getIdentifierType(), entityTypes)));
      });
      schema.registerType(true, TypeReferences.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", TypeReferences.ITEM_NAME.in(schema), "tag", Schema99.method_66195(schema)), field_5746, HookFunction.IDENTITY);
      });
   }
}
