...
public class Schema135 extends Schema {
   ...
   public void registerTypes(Schema schema, Map entityTypes, Map blockEntityTypes) {
      ...
      schema.registerType(false, TypeReferences.PLAYER, () -> {
         return DSL.optionalFields("RootVehicle", DSL.optionalFields("Entity", TypeReferences.ENTITY_TREE.in(schema)), "ender_pearls", DSL.list(TypeReferences.ENTITY_TREE.in(schema)), "Inventory", DSL.list(TypeReferences.ITEM_STACK.in(schema)), "EnderItems", DSL.list(TypeReferences.ITEM_STACK.in(schema)));
      });
