...
public class Schema704 extends Schema {
   protected static final Map BLOCK_RENAMES = (Map)DataFixUtils.make(() -> {
   ...
   public Type getChoiceType(DSL.TypeReference type, String choiceName) {
      return Objects.equals(type.typeName(), TypeReferences.BLOCK_ENTITY.typeName()) ? super.getChoiceType(type, IdentifierNormalizingSchema.normalize(choiceName)) : super.getChoiceType(type, choiceName);
   }

   public Map registerBlockEntities(Schema schema) {
      Map map = super.registerBlockEntities(schema);
      BlockEntityIdFix.RENAMED_BLOCK_ENTITIES.forEach((string, string2) -> {
         map.put(string2, (Supplier)Objects.requireNonNull((Supplier)map.remove(string), () -> {
            return "Didn't find " + string + " in schema";
         }));
      });
      return map;
   }
   ...
   public void registerTypes(Schema schema, Map entityTypes, Map blockEntityTypes) {
      ...
      schema.registerType(true, TypeReferences.BLOCK_ENTITY, () -> {
         return DSL.optionalFields("components", TypeReferences.DATA_COMPONENTS.in(schema), DSL.taggedChoiceLazy("id", IdentifierNormalizingSchema.getIdentifierType(), blockEntityTypes));
      });
      schema.registerType(true, TypeReferences.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", TypeReferences.ITEM_NAME.in(schema), "tag", Schema99.method_66195(schema)), field_5745, HookFunction.IDENTITY);
      });
   }
}
