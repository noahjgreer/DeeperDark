...
public class TradeOffer {
   public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
      ...
      }), TradedItem.CODEC.lenientOptionalFieldOf("buyB").forGetter((tradeOffer) -> {
         return tradeOffer.secondBuyItem;
      }), ItemStack.CODEC.fieldOf("sell").forGetter((tradeOffer) -> {
         return tradeOffer.sellItem;
      }), Codec.INT.lenientOptionalFieldOf("uses", 0).forGetter((tradeOffer) -> {
         return tradeOffer.uses;
      }), Codec.INT.lenientOptionalFieldOf("maxUses", 4).forGetter((tradeOffer) -> {
         return tradeOffer.maxUses;
      }), Codec.BOOL.lenientOptionalFieldOf("rewardExp", true).forGetter((tradeOffer) -> {
         return tradeOffer.rewardingPlayerExperience;
      }), Codec.INT.lenientOptionalFieldOf("specialPrice", 0).forGetter((tradeOffer) -> {
         return tradeOffer.specialPrice;
      }), Codec.INT.lenientOptionalFieldOf("demand", 0).forGetter((tradeOffer) -> {
         return tradeOffer.demandBonus;
      }), Codec.FLOAT.lenientOptionalFieldOf("priceMultiplier", 0.0F).forGetter((tradeOffer) -> {
         return tradeOffer.priceMultiplier;
      }), Codec.INT.lenientOptionalFieldOf("xp", 1).forGetter((tradeOffer) -> {
         return tradeOffer.merchantExperience;
      })).apply(instance, TradeOffer::new);
   });
   public static final PacketCodec PACKET_CODEC = PacketCodec.ofStatic(TradeOffer::write, TradeOffer::read);
   private final TradedItem firstBuyItem;
   private final Optional secondBuyItem;
   ...
   private TradeOffer(TradedItem firstBuyItem, Optional secondBuyItem, ItemStack sellItem, int uses, int maxUses, boolean rewardingPlayerExperience, int specialPrice, int demandBonus, float priceMultiplier, int merchantExperience) {
   ...
   public TradeOffer(TradedItem firstBuyItem, Optional secondBuyItem, ItemStack sellItem, int maxUses, int merchantExperience, float priceMultiplier) {
      this(firstBuyItem, secondBuyItem, sellItem, 0, maxUses, merchantExperience, priceMultiplier);
   }

   public TradeOffer(TradedItem firstBuyItem, Optional secondBuyItem, ItemStack sellItem, int uses, int maxUses, int merchantExperience, float priceMultiplier) {
      this(firstBuyItem, secondBuyItem, sellItem, uses, maxUses, merchantExperience, priceMultiplier, 0);
   }

   public TradeOffer(TradedItem firstBuyItem, Optional secondBuyItem, ItemStack sellItem, int uses, int maxUses, int merchantExperience, float priceMultiplier, int demandBonus) {
   ...
   public boolean matchesBuyItems(ItemStack stack, ItemStack buyItem) {
      if (this.firstBuyItem.matches(stack) && stack.getCount() >= this.getFirstBuyItemCount(this.firstBuyItem)) {
         if (!this.secondBuyItem.isPresent()) {
            ...
            return ((TradedItem)this.secondBuyItem.get()).matches(buyItem) && buyItem.getCount() >= ((TradedItem)this.secondBuyItem.get()).count();
         }
      } else {
         return false;
      }
   ...
   public static TradeOffer read(RegistryByteBuf buf) {
      TradedItem tradedItem = (TradedItem)TradedItem.PACKET_CODEC.decode(buf);
