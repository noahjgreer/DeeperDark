...
public class ReloadableRegistries {
    ...
    public static CompletableFuture<ReloadResult> reload(CombinedDynamicRegistries<ServerDynamicRegistryType> dynamicRegistries, List<Registry.PendingTagLoad<?>> pendingTagLoads, ResourceManager resourceManager, Executor prepareExecutor) {
        List<RegistryWrapper.Impl<?>> list = TagGroupLoader.collectRegistries(dynamicRegistries.getPrecedingRegistryManagers(ServerDynamicRegistryType.RELOADABLE), pendingTagLoads);
        ...
        List<CompletableFuture> list2 = LootDataType.stream().map(type -> ReloadableRegistries.prepare(type, registryOps, resourceManager, prepareExecutor)).toList();
        CompletableFuture completableFuture = Util.combineSafe(list2);
        return completableFuture.thenApplyAsync(registries -> ReloadableRegistries.toResult(dynamicRegistries, wrapperLookup, registries), prepareExecutor);
    }
    ...
    private static <T> CompletableFuture<MutableRegistry<?>> prepare(LootDataType<T> type, RegistryOps<JsonElement> ops, ResourceManager resourceManager, Executor prepareExecutor) {
        return CompletableFuture.supplyAsync(() -> {
            SimpleRegistry mutableRegistry = new SimpleRegistry(type.registryKey(), Lifecycle.experimental());
            ...
            map.forEach((id, value) -> mutableRegistry.add(RegistryKey.of(type.registryKey(), id), value, DEFAULT_REGISTRY_ENTRY_INFO));
    ...
    private static ReloadResult toResult(CombinedDynamicRegistries<ServerDynamicRegistryType> dynamicRegistries, RegistryWrapper.WrapperLookup nonReloadables, List<MutableRegistry<?>> registries) {
        CombinedDynamicRegistries<ServerDynamicRegistryType> combinedDynamicRegistries = ReloadableRegistries.with(dynamicRegistries, registries);
        RegistryWrapper.WrapperLookup wrapperLookup = ReloadableRegistries.concat(nonReloadables, combinedDynamicRegistries.get(ServerDynamicRegistryType.RELOADABLE));
        ReloadableRegistries.validate(wrapperLookup);
    ...
    private static void validate(RegistryWrapper.WrapperLookup registries) {
        ...
        LootDataType.stream().forEach(type -> ReloadableRegistries.validateLootData(lootTableReporter, type, registries));
        impl.apply((id, error) -> LOGGER.warn("Found loot table element validation problem in {}: {}", id, (Object)error.getMessage()));
    }
    ...
    private static <T> void validateLootData(LootTableReporter reporter, LootDataType<T> lootDataType, RegistryWrapper.WrapperLookup registries) {
        RegistryEntryLookup registryWrapper = registries.getOrThrow(lootDataType.registryKey());
