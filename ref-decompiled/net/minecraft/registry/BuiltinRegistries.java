...
public class BuiltinRegistries {
   ...
   public static void validate(RegistryEntryLookup placedFeatureLookup, RegistryWrapper biomeLookup) {
      biomeLookup.streamEntries().forEach((biome) -> {
         Identifier identifier = biome.registryKey().getValue();
         List list = ((Biome)biome.value()).getGenerationSettings().getFeatures();
         list.stream().flatMap(RegistryEntryList::stream).forEach((placedFeature) -> {
            placedFeature.getKeyOrValue().ifLeft((key) -> {
               RegistryEntry.Reference reference = placedFeatureLookup.getOrThrow(key);
               if (!hasBiomePlacementModifier((PlacedFeature)reference.value())) {
                  String var10000 = String.valueOf(key.getValue());
                  Util.logErrorOrPause("Placed feature " + var10000 + " in biome " + String.valueOf(identifier) + " is missing BiomeFilter.biome()");
               }

            }).ifRight((value) -> {
               if (!hasBiomePlacementModifier(value)) {
                  Util.logErrorOrPause("Placed inline feature in biome " + String.valueOf(biome) + " is missing BiomeFilter.biome()");
               }
   ...
   static {
      REGISTRY_BUILDER = (new RegistryBuilder()).addRegistry(RegistryKeys.DIMENSION_TYPE, DimensionTypeRegistrar::bootstrap).addRegistry(RegistryKeys.CONFIGURED_CARVER, ConfiguredCarvers::bootstrap).addRegistry(RegistryKeys.CONFIGURED_FEATURE, ConfiguredFeatures::bootstrap).addRegistry(RegistryKeys.PLACED_FEATURE, PlacedFeatures::bootstrap).addRegistry(RegistryKeys.STRUCTURE, Structures::bootstrap).addRegistry(RegistryKeys.STRUCTURE_SET, StructureSets::bootstrap).addRegistry(RegistryKeys.PROCESSOR_LIST, StructureProcessorLists::bootstrap).addRegistry(RegistryKeys.TEMPLATE_POOL, StructurePools::bootstrap).addRegistry(RegistryKeys.BIOME, BuiltinBiomes::bootstrap).addRegistry(RegistryKeys.MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST, MultiNoiseBiomeSourceParameterLists::bootstrap).addRegistry(RegistryKeys.NOISE_PARAMETERS, BuiltinNoiseParameters::bootstrap).addRegistry(RegistryKeys.DENSITY_FUNCTION, DensityFunctions::bootstrap).addRegistry(RegistryKeys.CHUNK_GENERATOR_SETTINGS, ChunkGeneratorSettings::bootstrap).addRegistry(RegistryKeys.WORLD_PRESET, WorldPresets::bootstrap).addRegistry(RegistryKeys.FLAT_LEVEL_GENERATOR_PRESET, FlatLevelGeneratorPresets::bootstrap).addRegistry(RegistryKeys.MESSAGE_TYPE, MessageType::bootstrap).addRegistry(RegistryKeys.TRIM_PATTERN, ArmorTrimPatterns::bootstrap).addRegistry(RegistryKeys.TRIM_MATERIAL, ArmorTrimMaterials::bootstrap).addRegistry(RegistryKeys.TRIAL_SPAWNER, TrialSpawnerConfigs::bootstrap).addRegistry(RegistryKeys.WOLF_VARIANT, WolfVariants::bootstrap).addRegistry(RegistryKeys.WOLF_SOUND_VARIANT, WolfSoundVariants::bootstrap).addRegistry(RegistryKeys.PAINTING_VARIANT, PaintingVariants::bootstrap).addRegistry(RegistryKeys.DAMAGE_TYPE, DamageTypes::bootstrap).addRegistry(RegistryKeys.BANNER_PATTERN, BannerPatterns::bootstrap).addRegistry(RegistryKeys.ENCHANTMENT, Enchantments::bootstrap).addRegistry(RegistryKeys.ENCHANTMENT_PROVIDER, EnchantmentProviders::bootstrap).addRegistry(RegistryKeys.JUKEBOX_SONG, JukeboxSongs::bootstrap).addRegistry(RegistryKeys.INSTRUMENT, Instruments::bootstrap).addRegistry(RegistryKeys.PIG_VARIANT, PigVariants::bootstrap).addRegistry(RegistryKeys.COW_VARIANT, CowVariants::bootstrap).addRegistry(RegistryKeys.CHICKEN_VARIANT, ChickenVariants::bootstrap).addRegistry(RegistryKeys.TEST_ENVIRONMENT, TestEnvironments::bootstrap).addRegistry(RegistryKeys.TEST_INSTANCE, TestInstances::bootstrap).addRegistry(RegistryKeys.FROG_VARIANT, FrogVariants::bootstrap).addRegistry(RegistryKeys.CAT_VARIANT, CatVariants::bootstrap).addRegistry(RegistryKeys.DIALOG, Dialogs::bootstrap);
   }
}
