...
public class CraftingDecoratedPotRecipe extends SpecialCraftingRecipe {
   ...
   public boolean matches(CraftingRecipeInput craftingRecipeInput, World world) {
      if (craftingRecipeInput.getWidth() == 3 && craftingRecipeInput.getHeight() == 3 && craftingRecipeInput.getStackCount() == 4) {
         return getBack(craftingRecipeInput).isIn(ItemTags.DECORATED_POT_INGREDIENTS) && getLeft(craftingRecipeInput).isIn(ItemTags.DECORATED_POT_INGREDIENTS) && getRight(craftingRecipeInput).isIn(ItemTags.DECORATED_POT_INGREDIENTS) && getFront(craftingRecipeInput).isIn(ItemTags.DECORATED_POT_INGREDIENTS);
      } else {
         return false;
      }
   }
   ...
   public ItemStack craft(CraftingRecipeInput craftingRecipeInput, RegistryWrapper.WrapperLookup wrapperLookup) {
      Sherds sherds = new Sherds(getBack(craftingRecipeInput).getItem(), getLeft(craftingRecipeInput).getItem(), getRight(craftingRecipeInput).getItem(), getFront(craftingRecipeInput).getItem());
