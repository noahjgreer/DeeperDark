...
public class PreparedRecipes {
    public static final PreparedRecipes EMPTY = new PreparedRecipes((Multimap<RecipeType<?>, RecipeEntry<?>>)ImmutableMultimap.of(), Map.of());
    ...
    public static PreparedRecipes of(Iterable<RecipeEntry<?>> recipes) {
        ...
        return new PreparedRecipes((Multimap<RecipeType<?>, RecipeEntry<?>>)builder.build(), (Map<RegistryKey<Recipe<?>>, RecipeEntry<?>>)builder2.build());
    }
    ...
    public <I extends RecipeInput, T extends Recipe<I>> Stream<RecipeEntry<T>> find(RecipeType<T> type, I input, World world) {
        ...
        return this.getAll(type).stream().filter(entry -> entry.value().matches((RecipeInput)input, world));
    }
}
