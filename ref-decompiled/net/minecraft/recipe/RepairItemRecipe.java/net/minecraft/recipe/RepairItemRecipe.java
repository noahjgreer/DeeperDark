...
extends SpecialCraftingRecipe {
    ...
    private static @Nullable Pair<ItemStack, ItemStack> findPair(CraftingRecipeInput craftingRecipeInput) {
        ...
        for (int i = 0; i < craftingRecipeInput.size(); ++i) {
            ...
            return RepairItemRecipe.canCombineStacks(itemStack, itemStack2) ? Pair.of((Object)itemStack, (Object)itemStack2) : null;
        }
        return null;
    }
    ...
    public boolean matches(CraftingRecipeInput craftingRecipeInput, World world) {
        return RepairItemRecipe.findPair(craftingRecipeInput) != null;
    }

    @Override
    public ItemStack craft(CraftingRecipeInput craftingRecipeInput, RegistryWrapper.WrapperLookup wrapperLookup) {
        Pair<ItemStack, ItemStack> pair = RepairItemRecipe.findPair(craftingRecipeInput);
        if (pair == null) {
        ...
        EnchantmentHelper.apply(itemStack3, builder -> wrapperLookup.getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().filter(enchantment -> enchantment.isIn(EnchantmentTags.CURSE)).forEach(enchantment -> {
            int i = Math.max(itemEnchantmentsComponent.getLevel((RegistryEntry<Enchantment>)enchantment), itemEnchantmentsComponent2.getLevel((RegistryEntry<Enchantment>)enchantment));
            if (i > 0) {
