...
public class RepairItemRecipe extends SpecialCraftingRecipe {
   ...
   @Nullable
   private static Pair findPair(CraftingRecipeInput craftingRecipeInput) {
   ...
   public ItemStack craft(CraftingRecipeInput craftingRecipeInput, RegistryWrapper.WrapperLookup wrapperLookup) {
      Pair pair = findPair(craftingRecipeInput);
      if (pair == null) {
         ...
         EnchantmentHelper.apply(itemStack3, (builder) -> {
            wrapperLookup.getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().filter((enchantment) -> {
               return enchantment.isIn(EnchantmentTags.CURSE);
            }).forEach((enchantment) -> {
               int i = Math.max(itemEnchantmentsComponent.getLevel(enchantment), itemEnchantmentsComponent2.getLevel(enchantment));
               if (i > 0) {
                  builder.add(enchantment, i);
               }
