...
public class BrewingRecipeRegistry {
   ...
   private final List potionTypes;
   private final List potionRecipes;
   private final List itemRecipes;

   BrewingRecipeRegistry(List potionTypes, List potionRecipes, List itemRecipes) {
   ...
   private boolean isPotionType(ItemStack stack) {
      Iterator var2 = this.potionTypes.iterator();
   ...
   public boolean isItemRecipeIngredient(ItemStack stack) {
      Iterator var2 = this.itemRecipes.iterator();
   ...
   public boolean isPotionRecipeIngredient(ItemStack stack) {
      Iterator var2 = this.potionRecipes.iterator();
   ...
   public boolean isBrewable(RegistryEntry potion) {
      Iterator var2 = this.potionRecipes.iterator();
      ...
      do {
         ...
         recipe = (Recipe)var2.next();
      } while(!recipe.to.matches(potion));
   ...
   public boolean hasItemRecipe(ItemStack input, ItemStack ingredient) {
      Iterator var3 = this.itemRecipes.iterator();
      ...
      do {
         ...
         recipe = (Recipe)var3.next();
      } while(!input.itemMatches(recipe.from) || !recipe.ingredient.test(ingredient));

      return true;
   }

   public boolean hasPotionRecipe(ItemStack input, ItemStack ingredient) {
      Optional optional = ((PotionContentsComponent)input.getOrDefault(DataComponentTypes.POTION_CONTENTS, PotionContentsComponent.DEFAULT)).potion();
      if (optional.isEmpty()) {
         return false;
      } else {
         Iterator var4 = this.potionRecipes.iterator();
         ...
         do {
            ...
            recipe = (Recipe)var4.next();
         } while(!recipe.from.matches((RegistryEntry)optional.get()) || !recipe.ingredient.test(ingredient));
   ...
   public ItemStack craft(ItemStack ingredient, ItemStack input) {
      if (input.isEmpty()) {
         ...
         if (optional.isEmpty()) {
            ...
            Iterator var4 = this.itemRecipes.iterator();

            Recipe recipe;
            do {
               if (!var4.hasNext()) {
                  var4 = this.potionRecipes.iterator();
            ...
            return PotionContentsComponent.createStack((Item)recipe.to.value(), (RegistryEntry)optional.get());
         }
      }
   }
   ...
   private static record Recipe(RegistryEntry from, Ingredient ingredient, RegistryEntry to) {
      final RegistryEntry from;
      final Ingredient ingredient;
      final RegistryEntry to;

      Recipe(RegistryEntry registryEntry, Ingredient ingredient, RegistryEntry registryEntry2) {
         this.from = registryEntry;
         this.ingredient = ingredient;
         this.to = registryEntry2;
      }

      public RegistryEntry from() {
         return this.from;
      }
      ...
      public RegistryEntry to() {
         return this.to;
      }
   }

   public static class Builder implements FabricBrewingRecipeRegistryBuilder {
      private final List potionTypes = new ArrayList();
      private final List potionRecipes = new ArrayList();
      private final List itemRecipes = new ArrayList();
      ...
      public void registerItemRecipe(Item input, Item ingredient, Item output) {
         if (input.isEnabled(this.enabledFeatures) && ingredient.isEnabled(this.enabledFeatures) && output.isEnabled(this.enabledFeatures)) {
            assertPotion(input);
            assertPotion(output);
            this.itemRecipes.add(new Recipe(input.getRegistryEntry(), Ingredient.ofItem(ingredient), output.getRegistryEntry()));
         }
      }
      ...
      public void registerPotionRecipe(RegistryEntry input, Item ingredient, RegistryEntry output) {
         if (((Potion)input.value()).isEnabled(this.enabledFeatures) && ingredient.isEnabled(this.enabledFeatures) && ((Potion)output.value()).isEnabled(this.enabledFeatures)) {
            this.potionRecipes.add(new Recipe(input, Ingredient.ofItem(ingredient), output));
         }

      }

      public void registerRecipes(Item ingredient, RegistryEntry potion) {
