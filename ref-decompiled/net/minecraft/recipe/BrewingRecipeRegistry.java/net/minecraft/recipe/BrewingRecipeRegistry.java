...
public class BrewingRecipeRegistry {
    ...
    public boolean isBrewable(RegistryEntry<Potion> potion) {
        for (Recipe<Potion> recipe : this.potionRecipes) {
            if (!recipe.to.matches(potion)) continue;
            return true;
        }
        return false;
    }
    ...
    public static BrewingRecipeRegistry create(FeatureSet enabledFeatures) {
        ...
        BrewingRecipeRegistry.registerDefaults(builder);
    ...
    static final class Recipe<T>
    extends Record {
        final RegistryEntry<T> from;
        final Ingredient ingredient;
        final RegistryEntry<T> to;

        Recipe(RegistryEntry<T> from, Ingredient ingredient, RegistryEntry<T> to) {
            this.from = from;
        ...
        public final String toString() {
            return ObjectMethods.bootstrap("toString", new MethodHandle[]{Recipe.class, "from;ingredient;to", "from", "ingredient", "to"}, this);
        }

        @Override
        public final int hashCode() {
            return (int)ObjectMethods.bootstrap("hashCode", new MethodHandle[]{Recipe.class, "from;ingredient;to", "from", "ingredient", "to"}, this);
        }

        @Override
        public final boolean equals(Object object) {
            return (boolean)ObjectMethods.bootstrap("equals", new MethodHandle[]{Recipe.class, "from;ingredient;to", "from", "ingredient", "to"}, this, object);
        }
    ...
    implements FabricBrewingRecipeRegistryBuilder {
        ...
        public void registerItemRecipe(Item input, Item ingredient, Item output) {
            ...
            this.itemRecipes.add(new Recipe<Item>(input.getRegistryEntry(), Ingredient.ofItem(ingredient), output.getRegistryEntry()));
        }
        ...
        public void registerPotionRecipe(RegistryEntry<Potion> input, Item ingredient, RegistryEntry<Potion> output) {
            if (input.value().isEnabled(this.enabledFeatures) && ingredient.isEnabled(this.enabledFeatures) && output.value().isEnabled(this.enabledFeatures)) {
                this.potionRecipes.add(new Recipe<Potion>(input, Ingredient.ofItem(ingredient), output));
            }
        }
