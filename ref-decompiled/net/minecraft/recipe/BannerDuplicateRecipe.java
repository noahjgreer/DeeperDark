...
public class BannerDuplicateRecipe extends SpecialCraftingRecipe {
      ...
      if (craftingRecipeInput.getStackCount() != 2) {
         ...
         for(int i = 0; i < craftingRecipeInput.size(); ++i) {
            ...
            if (!itemStack.isEmpty()) {
               ...
               int j = ((BannerPatternsComponent)itemStack.getOrDefault(DataComponentTypes.BANNER_PATTERNS, BannerPatternsComponent.DEFAULT)).layers().size();
               if (j > 6) {
                  return false;
               }
   ...
   public DefaultedList getRecipeRemainders(CraftingRecipeInput input) {
      ...
      for(int i = 0; i < defaultedList.size(); ++i) {
         ...
         if (!itemStack.isEmpty()) {
            ...
            } else if (!((BannerPatternsComponent)itemStack.getOrDefault(DataComponentTypes.BANNER_PATTERNS, BannerPatternsComponent.DEFAULT)).layers().isEmpty()) {
