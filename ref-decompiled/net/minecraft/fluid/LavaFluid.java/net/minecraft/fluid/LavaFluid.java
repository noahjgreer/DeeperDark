...
extends FlowableFluid {
    ...
    public void onRandomTick(ServerWorld world, BlockPos pos, FluidState state, Random random) {
        ...
        if (i > 0) {
            ...
            for (int j = 0; j < i; ++j) {
                if (!world.isPosLoaded(blockPos = blockPos.add(random.nextInt(3) - 1, 1, random.nextInt(3) - 1))) {
                    return;
                }
    ...
    public int getMaxFlowDistance(WorldView world) {
        return LavaFluid.shouldLavaFlowFaster(world) ? 4 : 2;
    }
    ...
    public int getLevelDecreasePerBlock(WorldView world) {
        return LavaFluid.shouldLavaFlowFaster(world) ? 1 : 2;
    }
    ...
    public int getTickRate(WorldView world) {
        return LavaFluid.shouldLavaFlowFaster(world) ? 10 : 30;
    }
