...
public static final class PlayerPublicKey.PublicKeyData
extends Record {
    private final Instant expiresAt;
    final PublicKey key;
    private final byte[] keySignature;
    private static final int KEY_SIGNATURE_MAX_SIZE = 4096;
    public static final Codec<PlayerPublicKey.PublicKeyData> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codecs.INSTANT.fieldOf("expires_at").forGetter(PlayerPublicKey.PublicKeyData::expiresAt), (App)NetworkEncryptionUtils.RSA_PUBLIC_KEY_CODEC.fieldOf("key").forGetter(PlayerPublicKey.PublicKeyData::key), (App)Codecs.BASE_64.fieldOf("signature_v2").forGetter(PlayerPublicKey.PublicKeyData::keySignature)).apply((Applicative)instance, PlayerPublicKey.PublicKeyData::new));

    public PlayerPublicKey.PublicKeyData(PacketByteBuf buf) {
        this(buf.readInstant(), buf.readPublicKey(), buf.readByteArray(4096));
    }

    public PlayerPublicKey.PublicKeyData(Instant expiresAt, PublicKey key, byte[] keySignature) {
    ...
    public boolean equals(Object o) {
        if (o instanceof PlayerPublicKey.PublicKeyData) {
    ...
    public final String toString() {
        return ObjectMethods.bootstrap("toString", new MethodHandle[]{PlayerPublicKey.PublicKeyData.class, "expiresAt;key;keySignature", "expiresAt", "key", "keySignature"}, this);
    }

    @Override
    public final int hashCode() {
        return (int)ObjectMethods.bootstrap("hashCode", new MethodHandle[]{PlayerPublicKey.PublicKeyData.class, "expiresAt;key;keySignature", "expiresAt", "key", "keySignature"}, this);
    }
