...
public record CustomPayloadC2SPacket(CustomPayload payload) implements Packet<ServerCommonPacketListener>
{
    ...
    public static final PacketCodec<PacketByteBuf, CustomPayloadC2SPacket> CODEC = CustomPayload.createCodec((Identifier id) -> UnknownCustomPayload.createCodec(id, Short.MAX_VALUE), Util.make(Lists.newArrayList((Object[])new CustomPayload.Type[]{new CustomPayload.Type<PacketByteBuf, BrandCustomPayload>(BrandCustomPayload.ID, BrandCustomPayload.CODEC)}), types -> {})).xmap(CustomPayloadC2SPacket::new, CustomPayloadC2SPacket::payload);
