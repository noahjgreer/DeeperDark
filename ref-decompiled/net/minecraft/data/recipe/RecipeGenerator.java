...
public abstract class RecipeGenerator {
   ...
   private final RegistryEntryLookup itemLookup;
   ...
   public void generateFamilies(FeatureSet enabledFeatures) {
      BlockFamilies.getFamilies().filter(BlockFamily::shouldGenerateRecipes).forEach((family) -> {
         this.generateFamily(family, enabledFeatures);
      });
   }
   ...
   public void offerShapelessRecipe(ItemConvertible output, ItemConvertible input, @Nullable String group, int outputCount) {
      this.createShapeless(RecipeCategory.MISC, output, outputCount).input(input).group(group).criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter, convertBetween(output, input));
   }

   public void offerSmelting(List inputs, RecipeCategory category, ItemConvertible output, float experience, int cookingTime, String group) {
   ...
   public void offerBlasting(List inputs, RecipeCategory category, ItemConvertible output, float experience, int cookingTime, String group) {
   ...
   public final void offerMultipleOptions(RecipeSerializer serializer, AbstractCookingRecipe.RecipeFactory recipeFactory, List inputs, RecipeCategory category, ItemConvertible output, float experience, int cookingTime, String group, String suffix) {
      Iterator var10 = inputs.iterator();

      while(var10.hasNext()) {
         ItemConvertible itemConvertible = (ItemConvertible)var10.next();
         CookingRecipeJsonBuilder.create(Ingredient.ofItem(itemConvertible), category, output, experience, cookingTime, serializer, recipeFactory).group(group).criterion(hasItem(itemConvertible), this.conditionsFromItem(itemConvertible)).offerTo(this.exporter, getItemPath(output) + suffix + "_" + getItemPath(itemConvertible));
      }

   }

   public void offerNetheriteUpgradeRecipe(Item input, RecipeCategory category, Item result) {
      SmithingTransformRecipeJsonBuilder.create(Ingredient.ofItem(Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE), Ingredient.ofItem(input), this.ingredientFromTag(ItemTags.NETHERITE_TOOL_MATERIALS), category, result).criterion("has_netherite_ingot", this.conditionsFromTag(ItemTags.NETHERITE_TOOL_MATERIALS)).offerTo(this.exporter, getItemPath(result) + "_smithing");
   }

   public void offerSmithingTrimRecipe(Item input, RegistryKey pattern, RegistryKey recipeKey) {
      RegistryEntry.Reference reference = this.registries.getOrThrow(RegistryKeys.TRIM_PATTERN).getOrThrow(pattern);
   ...
   public void offer2x2CompactingRecipe(RecipeCategory category, ItemConvertible output, ItemConvertible input) {
      this.createShaped(category, output, 1).input('#', input).pattern("##").pattern("##").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public void offerCompactingRecipe(RecipeCategory category, ItemConvertible output, ItemConvertible input, String criterionName) {
      this.createShapeless(category, output).input((ItemConvertible)input, 9).criterion(criterionName, this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public void offerPlanksRecipe2(ItemConvertible output, TagKey logTag, int count) {
      this.createShapeless(RecipeCategory.BUILDING_BLOCKS, output, count).input(logTag).group("planks").criterion("has_log", this.conditionsFromTag(logTag)).offerTo(this.exporter);
   }

   public void offerPlanksRecipe(ItemConvertible output, TagKey logTag, int count) {
      this.createShapeless(RecipeCategory.BUILDING_BLOCKS, output, count).input(logTag).group("planks").criterion("has_logs", this.conditionsFromTag(logTag)).offerTo(this.exporter);
   }

   public void offerBarkBlockRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.BUILDING_BLOCKS, output, 3).input('#', input).pattern("##").pattern("##").group("bark").criterion("has_log", this.conditionsFromItem(input)).offerTo(this.exporter);
   }

   public void offerBoatRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.TRANSPORTATION, output).input('#', input).pattern("# #").pattern("###").group("boat").criterion("in_water", requireEnteringFluid(Blocks.WATER)).offerTo(this.exporter);
   }

   public void offerChestBoatRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShapeless(RecipeCategory.TRANSPORTATION, output).input((ItemConvertible)Blocks.CHEST).input(input).group("chest_boat").criterion("has_boat", this.conditionsFromTag(ItemTags.BOATS)).offerTo(this.exporter);
   }
   ...
   public final CraftingRecipeJsonBuilder createFenceRecipe(ItemConvertible output, Ingredient input) {
      ...
      return this.createShaped(RecipeCategory.DECORATIONS, output, i).input('W', input).input('#', (ItemConvertible)item).pattern("W#W").pattern("W#W");
   }

   public final CraftingRecipeJsonBuilder createFenceGateRecipe(ItemConvertible output, Ingredient input) {
      return this.createShaped(RecipeCategory.REDSTONE, output).input('#', (ItemConvertible)Items.STICK).input('W', input).pattern("#W#").pattern("#W#");
   }

   public void offerPressurePlateRecipe(ItemConvertible output, ItemConvertible input) {
      this.createPressurePlateRecipe(RecipeCategory.REDSTONE, output, Ingredient.ofItem(input)).criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public final CraftingRecipeJsonBuilder createSignRecipe(ItemConvertible output, Ingredient input) {
      return this.createShaped(RecipeCategory.DECORATIONS, output, 3).group("sign").input('#', input).input('X', (ItemConvertible)Items.STICK).pattern("###").pattern("###").pattern(" X ");
   }

   public void offerHangingSignRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.DECORATIONS, output, 6).group("hanging_sign").input('#', input).input('X', (ItemConvertible)Items.CHAIN).pattern("X X").pattern("###").pattern("###").criterion("has_stripped_logs", this.conditionsFromItem(input)).offerTo(this.exporter);
   }

   public void offerDyeableRecipes(List dyes, List dyeables, String group, RecipeCategory category) {
      this.offerDyeablesRecipes(dyes, dyeables, (Item)null, group, category);
   }

   public void offerDyeablesRecipes(List dyes, List dyeables, @Nullable Item undyed, String group, RecipeCategory category) {
      for(int i = 0; i < dyes.size(); ++i) {
         Item item = (Item)dyes.get(i);
         Item item2 = (Item)dyeables.get(i);
         Stream stream = dyeables.stream().filter((itemx) -> {
            return !itemx.equals(item2);
         });
         if (undyed != null) {
            stream = Stream.concat(stream, Stream.of(undyed));
         }

         this.createShapeless(category, (ItemConvertible)item2).input((ItemConvertible)item).input(Ingredient.ofItems(stream)).group(group).criterion("has_needed_dye", this.conditionsFromItem(item)).offerTo(this.exporter, "dye_" + getItemPath(item2));
      }

   }

   public void offerCarpetRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.DECORATIONS, output, 3).input('#', input).pattern("##").group("carpet").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }

   public void offerBedRecipe(ItemConvertible output, ItemConvertible inputWool) {
      this.createShaped(RecipeCategory.DECORATIONS, output).input('#', inputWool).input('X', ItemTags.PLANKS).pattern("###").pattern("XXX").group("bed").criterion(hasItem(inputWool), this.conditionsFromItem(inputWool)).offerTo(this.exporter);
   }

   public void offerBannerRecipe(ItemConvertible output, ItemConvertible inputWool) {
      this.createShaped(RecipeCategory.DECORATIONS, output).input('#', inputWool).input('|', (ItemConvertible)Items.STICK).pattern("###").pattern("###").pattern(" | ").group("banner").criterion(hasItem(inputWool), this.conditionsFromItem(inputWool)).offerTo(this.exporter);
   }

   public void offerStainedGlassDyeingRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.BUILDING_BLOCKS, output, 8).input('#', (ItemConvertible)Blocks.GLASS).input('X', input).pattern("###").pattern("#X#").pattern("###").group("stained_glass").criterion("has_glass", this.conditionsFromItem(Blocks.GLASS)).offerTo(this.exporter);
   }

   public void offerDriedGhast(ItemConvertible output) {
      this.createShaped(RecipeCategory.BUILDING_BLOCKS, output, 1).input('#', (ItemConvertible)Items.GHAST_TEAR).input('X', (ItemConvertible)Items.SOUL_SAND).pattern("###").pattern("#X#").pattern("###").group("dry_ghast").criterion(hasItem(Items.GHAST_TEAR), this.conditionsFromItem(Items.GHAST_TEAR)).offerTo(this.exporter);
   }

   public void offerHarness(ItemConvertible output, ItemConvertible wool) {
      this.createShaped(RecipeCategory.COMBAT, output).input('#', wool).input('G', (ItemConvertible)Items.GLASS).input('L', (ItemConvertible)Items.LEATHER).pattern("LLL").pattern("G#G").group("harness").criterion("has_dried_ghast", this.conditionsFromItem(Blocks.DRIED_GHAST)).offerTo(this.exporter);
   }

   public void offerStainedGlassPaneRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.DECORATIONS, output, 16).input('#', input).pattern("###").pattern("###").group("stained_glass_pane").criterion("has_glass", this.conditionsFromItem(input)).offerTo(this.exporter);
   }

   public void offerStainedGlassPaneDyeingRecipe(ItemConvertible output, ItemConvertible inputDye) {
      this.createShaped(RecipeCategory.DECORATIONS, output, 8).input('#', (ItemConvertible)Blocks.GLASS_PANE).input('$', inputDye).pattern("###").pattern("#$#").pattern("###").group("stained_glass_pane").criterion("has_glass_pane", this.conditionsFromItem(Blocks.GLASS_PANE)).criterion(hasItem(inputDye), this.conditionsFromItem(inputDye)).offerTo(this.exporter, convertBetween(output, Blocks.GLASS_PANE));
   }

   public void offerTerracottaDyeingRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShaped(RecipeCategory.BUILDING_BLOCKS, output, 8).input('#', (ItemConvertible)Blocks.TERRACOTTA).input('X', input).pattern("###").pattern("#X#").pattern("###").group("stained_terracotta").criterion("has_terracotta", this.conditionsFromItem(Blocks.TERRACOTTA)).offerTo(this.exporter);
   }

   public void offerConcretePowderDyeingRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShapeless(RecipeCategory.BUILDING_BLOCKS, output, 8).input(input).input((ItemConvertible)Blocks.SAND, 4).input((ItemConvertible)Blocks.GRAVEL, 4).group("concrete_powder").criterion("has_sand", this.conditionsFromItem(Blocks.SAND)).criterion("has_gravel", this.conditionsFromItem(Blocks.GRAVEL)).offerTo(this.exporter);
   }

   public void offerCandleDyeingRecipe(ItemConvertible output, ItemConvertible input) {
      this.createShapeless(RecipeCategory.DECORATIONS, output).input((ItemConvertible)Blocks.CANDLE).input(input).group("dyed_candle").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public void offerMosaicRecipe(RecipeCategory category, ItemConvertible output, ItemConvertible input) {
      this.createShaped(category, output).input('#', input).pattern("#").pattern("#").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public void offerStonecuttingRecipe(RecipeCategory category, ItemConvertible output, ItemConvertible input, int count) {
      StonecuttingRecipeJsonBuilder var10000 = StonecuttingRecipeJsonBuilder.createStonecutting(Ingredient.ofItem(input), category, output, count).criterion(hasItem(input), this.conditionsFromItem(input));
      RecipeExporter var10001 = this.exporter;
      String var10002 = convertBetween(output, input);
      var10000.offerTo(var10001, var10002 + "_stonecutting");
   }
   ...
   public void offerReversibleCompactingRecipes(RecipeCategory reverseCategory, ItemConvertible baseItem, RecipeCategory compactingCategory, ItemConvertible compactItem) {
      this.offerReversibleCompactingRecipes(reverseCategory, baseItem, compactingCategory, compactItem, getRecipeName(compactItem), (String)null, getRecipeName(baseItem), (String)null);
   }
   ...
   public void offerReversibleCompactingRecipesWithCompactingRecipeGroup(RecipeCategory reverseCategory, ItemConvertible baseItem, RecipeCategory compactingCategory, ItemConvertible compactItem, String compactingId, String compactingGroup) {
      this.offerReversibleCompactingRecipes(reverseCategory, baseItem, compactingCategory, compactItem, compactingId, compactingGroup, getRecipeName(baseItem), (String)null);
   }
   ...
   public void offerReversibleCompactingRecipesWithReverseRecipeGroup(RecipeCategory reverseCategory, ItemConvertible baseItem, RecipeCategory compactingCategory, ItemConvertible compactItem, String reverseId, String reverseGroup) {
      this.offerReversibleCompactingRecipes(reverseCategory, baseItem, compactingCategory, compactItem, getRecipeName(compactItem), (String)null, reverseId, reverseGroup);
   }
   ...
   public final void offerReversibleCompactingRecipes(RecipeCategory reverseCategory, ItemConvertible baseItem, RecipeCategory compactingCategory, ItemConvertible compactItem, String compactingId, @Nullable String compactingGroup, String reverseId, @Nullable String reverseGroup) {
      this.createShapeless(reverseCategory, baseItem, 9).input(compactItem).group(reverseGroup).criterion(hasItem(compactItem), this.conditionsFromItem(compactItem)).offerTo(this.exporter, RegistryKey.of(RegistryKeys.RECIPE, Identifier.of(reverseId)));
      this.createShaped(compactingCategory, compactItem).input('#', baseItem).pattern("###").pattern("###").pattern("###").group(compactingGroup).criterion(hasItem(baseItem), this.conditionsFromItem(baseItem)).offerTo(this.exporter, RegistryKey.of(RegistryKeys.RECIPE, Identifier.of(compactingId)));
   }

   public void offerSmithingTemplateCopyingRecipe(ItemConvertible template, ItemConvertible resource) {
      this.createShaped(RecipeCategory.MISC, template, 2).input('#', (ItemConvertible)Items.DIAMOND).input('C', resource).input('S', template).pattern("#S#").pattern("#C#").pattern("###").criterion(hasItem(template), this.conditionsFromItem(template)).offerTo(this.exporter);
   }
   ...
   public void generateCookingRecipes(String cooker, RecipeSerializer serializer, AbstractCookingRecipe.RecipeFactory recipeFactory, int cookingTime) {
   ...
   public final void offerFoodCookingRecipe(String cooker, RecipeSerializer serializer, AbstractCookingRecipe.RecipeFactory recipeFactory, int cookingTime, ItemConvertible input, ItemConvertible output, float experience) {
      CookingRecipeJsonBuilder var10000 = CookingRecipeJsonBuilder.create(Ingredient.ofItem(input), RecipeCategory.FOOD, output, experience, cookingTime, serializer, recipeFactory).criterion(hasItem(input), this.conditionsFromItem(input));
      RecipeExporter var10001 = this.exporter;
      String var10002 = getItemPath(output);
      var10000.offerTo(var10001, var10002 + "_from_" + cooker);
   }

   public void offerWaxingRecipes(FeatureSet enabledFeatures) {
      ((BiMap)HoneycombItem.UNWAXED_TO_WAXED_BLOCKS.get()).forEach((unwaxed, waxed) -> {
         if (waxed.getRequiredFeatures().isSubsetOf(enabledFeatures)) {
            this.createShapeless(RecipeCategory.BUILDING_BLOCKS, (ItemConvertible)waxed).input((ItemConvertible)unwaxed).input((ItemConvertible)Items.HONEYCOMB).group(getItemPath(waxed)).criterion(hasItem(unwaxed), this.conditionsFromItem(unwaxed)).offerTo(this.exporter, convertBetween(waxed, Items.HONEYCOMB));
         }
      });
   }

   public void offerGrateRecipe(Block output, Block input) {
      this.createShaped(RecipeCategory.BUILDING_BLOCKS, output, 4).input('M', (ItemConvertible)input).pattern(" M ").pattern("M M").pattern(" M ").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }

   public void offerBulbRecipe(Block output, Block input) {
      this.createShaped(RecipeCategory.REDSTONE, output, 4).input('C', (ItemConvertible)input).input('R', (ItemConvertible)Items.REDSTONE).input('B', (ItemConvertible)Items.BLAZE_ROD).pattern(" C ").pattern("CBC").pattern(" R ").criterion(hasItem(input), this.conditionsFromItem(input)).offerTo(this.exporter);
   }
   ...
   public void offerSuspiciousStewRecipe(Item input, SuspiciousStewIngredient stewIngredient) {
      ...
      ShapelessRecipeJsonBuilder var10000 = this.createShapeless(RecipeCategory.FOOD, itemStack).input((ItemConvertible)Items.BOWL).input((ItemConvertible)Items.BROWN_MUSHROOM).input((ItemConvertible)Items.RED_MUSHROOM).input((ItemConvertible)input).group("suspicious_stew").criterion(hasItem(input), this.conditionsFromItem(input));
      RecipeExporter var10001 = this.exporter;
      String var10002 = getItemPath(itemStack.getItem());
      var10000.offerTo(var10001, var10002 + "_from_" + getItemPath(input));
   }
   ...
   public void generateFamily(BlockFamily family, FeatureSet enabledFeatures) {
      family.getVariants().forEach((variant, block) -> {
         if (block.getRequiredFeatures().isSubsetOf(enabledFeatures)) {
            BlockFamilyRecipeFactory blockFamilyRecipeFactory = (BlockFamilyRecipeFactory)VARIANT_FACTORIES.get(variant);
            ...
            if (blockFamilyRecipeFactory != null) {
               ...
               family.getGroup().ifPresent((group) -> {
                  craftingRecipeJsonBuilder.group(group + (variant == BlockFamily.Variant.CUT ? "" : "_" + variant.getName()));
               });
               craftingRecipeJsonBuilder.criterion((String)family.getUnlockCriterionName().orElseGet(() -> {
   ...
   public static AdvancementCriterion requireEnteringFluid(Block block) {
   ...
   public final AdvancementCriterion conditionsFromItem(NumberRange.IntRange count, ItemConvertible item) {
   ...
   public AdvancementCriterion conditionsFromTag(TagKey tag) {
      return conditionsFromPredicates(ItemPredicate.Builder.create().tag(this.itemLookup, tag));
   }

   public static AdvancementCriterion conditionsFromPredicates(ItemPredicate.Builder... predicates) {
      return conditionsFromItemPredicates((ItemPredicate[])Arrays.stream(predicates).map(ItemPredicate.Builder::build).toArray((i) -> {
         return new ItemPredicate[i];
      }));
   }

   public static AdvancementCriterion conditionsFromItemPredicates(ItemPredicate... predicates) {
   ...
   public Ingredient ingredientFromTag(TagKey tag) {
   ...
   static {
      VARIANT_FACTORIES = ImmutableMap.builder().put(BlockFamily.Variant.BUTTON, (generator, output, input) -> {
         return generator.createButtonRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.CHISELED, (generator, output, input) -> {
         return generator.createChiseledBlockRecipe(RecipeCategory.BUILDING_BLOCKS, output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.CUT, (generator, output, input) -> {
         return generator.createCutCopperRecipe(RecipeCategory.BUILDING_BLOCKS, output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.DOOR, (generator, output, input) -> {
         return generator.createDoorRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.CUSTOM_FENCE, (generator, output, input) -> {
         return generator.createFenceRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.FENCE, (generator, output, input) -> {
         return generator.createFenceRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.CUSTOM_FENCE_GATE, (generator, output, input) -> {
         return generator.createFenceGateRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.FENCE_GATE, (generator, output, input) -> {
         return generator.createFenceGateRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.SIGN, (generator, output, input) -> {
         return generator.createSignRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.SLAB, (generator, output, input) -> {
         return generator.createSlabRecipe(RecipeCategory.BUILDING_BLOCKS, output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.STAIRS, (generator, output, input) -> {
         return generator.createStairsRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.PRESSURE_PLATE, (generator, output, input) -> {
         return generator.createPressurePlateRecipe(RecipeCategory.REDSTONE, output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.POLISHED, (generator, output, input) -> {
         return generator.createCondensingRecipe(RecipeCategory.BUILDING_BLOCKS, output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.TRAPDOOR, (generator, output, input) -> {
         return generator.createTrapdoorRecipe(output, Ingredient.ofItem(input));
      }).put(BlockFamily.Variant.WALL, (generator, output, input) -> {
         return generator.getWallRecipe(RecipeCategory.DECORATIONS, output, Ingredient.ofItem(input));
      }).build();
   }
   ...
   public abstract static class RecipeProvider implements DataProvider {
      ...
      protected RecipeProvider(DataOutput output, CompletableFuture registriesFuture) {
         this.output = output;
      ...
      public CompletableFuture run(DataWriter writer) {
         return this.registriesFuture.thenCompose((registries) -> {
            ...
            RecipeExporter recipeExporter = new RecipeExporter(this) {
               public void accept(RegistryKey key, Recipe recipe, @Nullable AdvancementEntry advancement) {
                  if (!set.add(key)) {
               ...
               private void addRecipe(RegistryKey key, Recipe recipe) {
                  list.add(DataProvider.writeCodecToPath(writer, (RegistryWrapper.WrapperLookup)registries, Recipe.CODEC, recipe, pathResolver.resolveJson(key.getValue())));
               }

               private void addRecipeAdvancement(AdvancementEntry advancementEntry) {
                  list.add(DataProvider.writeCodecToPath(writer, (RegistryWrapper.WrapperLookup)registries, Advancement.CODEC, advancementEntry.value(), pathResolver2.resolveJson(advancementEntry.id())));
               }
            };
