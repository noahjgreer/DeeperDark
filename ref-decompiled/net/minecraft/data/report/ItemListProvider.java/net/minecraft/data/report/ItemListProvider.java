    ...
    public CompletableFuture<?> run(DataWriter writer) {
        ...
        return this.registriesFuture.thenCompose(registries -> {
            ...
            registries.getOrThrow(RegistryKeys.ITEM).streamEntries().forEach(entry -> {
                ...
                jsonObject2.add("components", (JsonElement)ComponentMap.CODEC.encodeStart((DynamicOps)registryOps, (Object)((Item)entry.value()).getComponents()).getOrThrow(components -> new IllegalStateException("Failed to encode components: " + components)));
                jsonObject.add(entry.getIdAsString(), (JsonElement)jsonObject2);
            });
