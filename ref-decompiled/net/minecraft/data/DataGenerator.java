...
public class DataGenerator {
   ...
   public void run() throws IOException {
      ...
      this.runningProviders.forEach((name, provider) -> {
         if (!this.ignoreCache && !dataCache.isVersionDifferent(name)) {
            LOGGER.debug("Generator {} already run for version {}", name, this.gameVersion.name());
         } else {
   ...
   public class Pack {
      ...
      public DataProvider addProvider(DataProvider.Factory factory) {
         DataProvider dataProvider = factory.create(this.output);
         String var10000 = this.packName;
         String string = var10000 + "/" + dataProvider.getName();
