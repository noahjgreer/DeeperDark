...
public enum GameMode implements StringIdentifiable {
   ...
   public static final StringIdentifiable.EnumCodec CODEC = StringIdentifiable.createCodec(GameMode::values);
   private static final IntFunction INDEX_MAPPER = ValueLists.createIndexToValueFunction(GameMode::getIndex, values(), (ValueLists.OutOfBoundsHandling)ValueLists.OutOfBoundsHandling.ZERO);
   public static final PacketCodec PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, GameMode::getIndex);
   /** @deprecated */
   @Deprecated
   public static final Codec INDEX_CODEC = Codec.INT.xmap(GameMode::byIndex, GameMode::getIndex);
   ...
   public static boolean isValid(int index) {
      return Arrays.stream(values()).anyMatch((gameMode) -> {
         return gameMode.index == index;
      });
   }
