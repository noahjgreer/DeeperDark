...
public final class LevelInfo {
   ...
   public static LevelInfo fromDynamic(Dynamic dynamic, DataConfiguration dataConfiguration) {
      ...
      return new LevelInfo(dynamic.get("LevelName").asString(""), gameMode, dynamic.get("hardcore").asBoolean(false), (Difficulty)dynamic.get("Difficulty").asNumber().map((difficulty) -> {
         return Difficulty.byId(difficulty.byteValue());
      }).result().orElse(Difficulty.NORMAL), dynamic.get("allowCommands").asBoolean(gameMode == GameMode.CREATIVE), new GameRules(dataConfiguration.enabledFeatures(), dynamic.get("GameRules")), dataConfiguration);
   }
