...
public class NetherPortal {
   ...
   public static Optional getNewPortal(WorldAccess world, BlockPos pos, Direction.Axis firstCheckedAxis) {
      return getOrEmpty(world, pos, (areaHelper) -> {
         return areaHelper.isValid() && areaHelper.foundPortalBlocks == 0;
      }, firstCheckedAxis);
   }

   public static Optional getOrEmpty(WorldAccess world, BlockPos pos, Predicate validator, Direction.Axis firstCheckedAxis) {
      Optional optional = Optional.of(getOnAxis(world, pos, firstCheckedAxis)).filter(validator);
   ...
   public static Vec3d findOpenPosition(Vec3d fallback, ServerWorld world, Entity entity, EntityDimensions dimensions) {
      if (!(dimensions.width() > 4.0F) && !(dimensions.height() > 4.0F)) {
         ...
         Optional optional = world.findClosestCollision(entity, voxelShape, vec3d, (double)dimensions.width(), (double)dimensions.height(), (double)dimensions.width());
