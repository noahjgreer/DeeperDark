...
public class ChunkCache implements CollisionView {
   ...
   public ChunkCache(World world, BlockPos minPos, BlockPos maxPos) {
      ...
      this.plainsEntryGetter = Suppliers.memoize(() -> {
         return world.getRegistryManager().getOrThrow(RegistryKeys.BIOME).getOrThrow(BiomeKeys.PLAINS);
      });
   ...
   private Chunk getChunk(int chunkX, int chunkZ) {
      ...
      if (i >= 0 && i < this.chunks.length && j >= 0 && j < this.chunks[i].length) {
         ...
         return (Chunk)(chunk != null ? chunk : new EmptyChunk(this.world, new ChunkPos(chunkX, chunkZ), (RegistryEntry)this.plainsEntryGetter.get()));
      } else {
