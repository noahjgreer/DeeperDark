...
public class SurfaceBuilder {
   ...
   public void buildSurface(NoiseConfig noiseConfig, BiomeAccess biomeAccess, Registry biomeRegistry, boolean useLegacyRandom, HeightContext heightContext, final Chunk chunk, ChunkNoiseSampler chunkNoiseSampler, MaterialRules.MaterialRule materialRule) {
      ...
      for(int k = 0; k < 16; ++k) {
         for(int l = 0; l < 16; ++l) {
            ...
            RegistryEntry registryEntry = biomeAccess.getBiome(mutable2.set(m, useLegacyRandom ? 0 : o, n));
   ...
   public Optional applyMaterialRule(MaterialRules.MaterialRule rule, CarverContext context, Function posToBiome, Chunk chunk, ChunkNoiseSampler chunkNoiseSampler, BlockPos pos, boolean hasFluid) {
      MaterialRules.MaterialRuleContext materialRuleContext = new MaterialRules.MaterialRuleContext(this, context.getNoiseConfig(), chunk, chunkNoiseSampler, posToBiome, context.getRegistryManager().getOrThrow(RegistryKeys.BIOME), context);
   ...
   private void placeIceberg(int minY, Biome biome, BlockColumn column, BlockPos.Mutable mutablePos, int x, int z, int surfaceY) {
      ...
      if (!(e <= 1.8)) {
         ...
         for(int o = Math.max(surfaceY, (int)i + 1); o >= minY; --o) {
            if (column.getState(o).isAir() && o < (int)k && random.nextDouble() > 0.01 || column.getState(o).isOf(Blocks.WATER) && o > (int)j && o < this.seaLevel && j != 0.0 && random.nextDouble() > 0.15) {
               if (n <= l && o > m) {
