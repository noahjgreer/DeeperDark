...
public interface BlockPredicate extends BiPredicate {
   Codec BASE_CODEC = Registries.BLOCK_PREDICATE_TYPE.getCodec().dispatch(BlockPredicate::getType, BlockPredicateType::codec);
   ...
   static BlockPredicate allOf(List predicates) {
   ...
   static BlockPredicate anyOf(List predicates) {
   ...
   static BlockPredicate matchingBlocks(Vec3i offset, List blocks) {
      return new MatchingBlocksBlockPredicate(offset, RegistryEntryList.of((Function)(Block::getRegistryEntry), (Collection)blocks));
   }

   static BlockPredicate matchingBlocks(List blocks) {
   ...
   static BlockPredicate matchingBlockTag(Vec3i offset, TagKey tag) {
      return new MatchingBlockTagPredicate(offset, tag);
   }

   static BlockPredicate matchingBlockTag(TagKey offset) {
      return matchingBlockTag(Vec3i.ZERO, offset);
   }

   static BlockPredicate matchingFluids(Vec3i offset, List fluids) {
      return new MatchingFluidsBlockPredicate(offset, RegistryEntryList.of((Function)(Fluid::getRegistryEntry), (Collection)fluids));
   }
