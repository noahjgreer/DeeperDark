...
public class DualNoiseBlockStateProvider extends NoiseBlockStateProvider {
   public static final MapCodec DUAL_CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(Range.createRangedCodec(Codec.INT, 1, 64).fieldOf("variety").forGetter((dualNoiseBlockStateProvider) -> {
         return dualNoiseBlockStateProvider.variety;
      }), DoublePerlinNoiseSampler.NoiseParameters.CODEC.fieldOf("slow_noise").forGetter((dualNoiseBlockStateProvider) -> {
         return dualNoiseBlockStateProvider.slowNoiseParameters;
      }), Codecs.POSITIVE_FLOAT.fieldOf("slow_scale").forGetter((dualNoiseBlockStateProvider) -> {
         return dualNoiseBlockStateProvider.slowScale;
