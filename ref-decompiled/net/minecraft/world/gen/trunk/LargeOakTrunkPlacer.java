...
public class LargeOakTrunkPlacer extends TrunkPlacer {
   ...
   public List generate(TestableWorld world, BiConsumer replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
      ...
      while(var37.hasNext()) {
         BranchPosition branchPosition = (BranchPosition)var37.next();
   ...
   private boolean makeOrCheckBranch(TestableWorld world, BiConsumer replacer, Random random, BlockPos startPos, BlockPos branchPos, boolean make, TreeFeatureConfig config) {
      if (!make && Objects.equals(startPos, branchPos)) {
         ...
         for(int j = 0; j <= i; ++j) {
            ...
            if (make) {
               this.getAndSetState(world, replacer, random, blockPos2, config, (state) -> {
                  return (BlockState)state.withIfExists(PillarBlock.AXIS, this.getLogAxis(startPos, blockPos2));
               });
   ...
   private void makeBranches(TestableWorld world, BiConsumer replacer, Random random, int treeHeight, BlockPos startPos, List branchPositions, TreeFeatureConfig config) {
      Iterator var8 = branchPositions.iterator();

      while(var8.hasNext()) {
         BranchPosition branchPosition = (BranchPosition)var8.next();
