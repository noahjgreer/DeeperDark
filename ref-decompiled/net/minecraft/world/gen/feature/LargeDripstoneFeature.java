...
public class LargeDripstoneFeature extends Feature {
   ...
   public boolean generate(FeatureContext context) {
      ...
      LargeDripstoneFeatureConfig largeDripstoneFeatureConfig = (LargeDripstoneFeatureConfig)context.getConfig();
      Random random = context.getRandom();
      if (!DripstoneHelper.canGenerate(structureWorldAccess, blockPos)) {
         return false;
      } else {
         Optional optional = CaveSurface.create(structureWorldAccess, blockPos, largeDripstoneFeatureConfig.floorToCeilingSearchRange, DripstoneHelper::canGenerate, DripstoneHelper::canReplaceOrLava);
         if (!optional.isEmpty() && optional.get() instanceof CaveSurface.Bounded) {
            ...
            if (bounded.getHeight() < 4) {
               ...
               if (dripstoneGenerator.generateWind(largeDripstoneFeatureConfig) && dripstoneGenerator2.generateWind(largeDripstoneFeatureConfig)) {
                  windModifier = new WindModifier(blockPos.getY(), random, largeDripstoneFeatureConfig.windSpeed);
               } else {
