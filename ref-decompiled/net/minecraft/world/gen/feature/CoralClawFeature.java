...
public class CoralClawFeature extends CoralFeature {
   ...
   protected boolean generateCoral(WorldAccess world, Random random, BlockPos pos, BlockState state) {
      if (!this.generateCoralPiece(world, random, pos, state)) {
         ...
         List list = Util.copyShuffled(Stream.of(direction, direction.rotateYClockwise(), direction.rotateYCounterclockwise()), random);
         ...
         while(var9.hasNext()) {
            ...
            if (direction2 == direction) {
               ...
               direction3 = (Direction)Util.getRandom((Object[])directions, random);
