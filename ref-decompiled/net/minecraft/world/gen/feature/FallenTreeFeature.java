...
public class FallenTreeFeature extends Feature {
   ...
   public FallenTreeFeature(Codec codec) {
      super(codec);
   }

   public boolean generate(FeatureContext context) {
      this.generate((FallenTreeFeatureConfig)context.getConfig(), context.getOrigin(), context.getWorld(), context.getRandom());
      return true;
   }
   ...
   private BlockPos setBlockStateAndGetPos(FallenTreeFeatureConfig config, StructureWorldAccess world, Random random, BlockPos.Mutable pos, Function stateFunction) {
      world.setBlockState(pos, (BlockState)stateFunction.apply(config.trunkProvider.get(random, pos)), 3);
      this.markBlocksAboveForPostProcessing(world, pos);
      return pos.toImmutable();
   }

   private void applyDecorators(StructureWorldAccess world, Random random, Set positions, List decorators) {
   ...
   private BiConsumer createStatePlacer(StructureWorldAccess world) {
      return (pos, state) -> {
         world.setBlockState(pos, state, 19);
      };
   }

   private static Function createAxisApplier(Direction direction) {
      return (state) -> {
