...
public class RootSystemFeature extends Feature {
   ...
   public boolean generate(FeatureContext context) {
      ...
      if (!structureWorldAccess.getBlockState(blockPos).isAir()) {
         ...
         RootSystemFeatureConfig rootSystemFeatureConfig = (RootSystemFeatureConfig)context.getConfig();
   ...
   private static boolean generateTreeAndRoots(StructureWorldAccess world, ChunkGenerator generator, RootSystemFeatureConfig config, Random random, BlockPos.Mutable mutablePos, BlockPos pos) {
      for(int i = 0; i < config.maxRootColumnHeight; ++i) {
         ...
         if (config.predicate.test(world, mutablePos) && hasSpaceForTree(world, config, mutablePos)) {
            ...
            if (((PlacedFeature)config.feature.value()).generateUnregistered(world, generator, random, mutablePos)) {
   ...
   private static void generateRoots(StructureWorldAccess world, RootSystemFeatureConfig config, Random random, int x, int z, BlockPos.Mutable mutablePos) {
      ...
      for(int j = 0; j < config.rootPlacementAttempts; ++j) {
         mutablePos.set((Vec3i)mutablePos, random.nextInt(i) - random.nextInt(i), 0, random.nextInt(i) - random.nextInt(i));
