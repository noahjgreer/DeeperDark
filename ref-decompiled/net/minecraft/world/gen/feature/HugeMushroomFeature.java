...
public abstract class HugeMushroomFeature extends Feature {
      ...
      if (i >= world.getBottomY() + 1 && i + height + 1 <= world.getTopYInclusive()) {
         ...
         if (!isSoil(blockState) && !blockState.isIn(BlockTags.MUSHROOM_GROW_BLOCK)) {
            ...
            for(int j = 0; j <= height; ++j) {
               ...
               for(int l = -k; l <= k; ++l) {
                  for(int m = -k; m <= k; ++m) {
                     BlockState blockState2 = world.getBlockState(mutablePos.set((Vec3i)pos, l, j, m));
   ...
   public boolean generate(FeatureContext context) {
      ...
      HugeMushroomFeatureConfig hugeMushroomFeatureConfig = (HugeMushroomFeatureConfig)context.getConfig();
