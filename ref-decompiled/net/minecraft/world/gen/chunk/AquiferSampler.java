            ...
            if (fluidLevel.getBlockState(j).isOf(Blocks.LAVA)) {
               ...
               if (d <= 0.0) {
                  ...
                  if (density + e > 0.0) {
                     ...
                     double g;
                     if (f > 0.0) {
                        g = d * f * this.calculateDensity(pos, mutableDouble, fluidLevel2, fluidLevel5);
                        if (density + g > 0.0) {
                           this.needsFluidTick = false;
                           return null;
                        }
                     ...
                     if (!bl && !bl2 && !bl3) {
                        this.needsFluidTick = f >= NEEDS_FLUID_TICK_DISTANCE_THRESHOLD && maxDistance(o, r) >= NEEDS_FLUID_TICK_DISTANCE_THRESHOLD && !fluidLevel2.equals(this.getWaterLevel(v));
                     } else {
                        this.needsFluidTick = true;
                     }
