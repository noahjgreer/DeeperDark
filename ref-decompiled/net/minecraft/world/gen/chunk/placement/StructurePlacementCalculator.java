...
public class StructurePlacementCalculator {
   ...
   private static boolean hasValidBiome(StructureSet structureSet, BiomeSource biomeSource) {
      Stream stream = structureSet.structures().stream().flatMap((structure) -> {
         Structure structure2 = (Structure)structure.structure().value();
   ...
   private void calculate() {
      ...
      this.getStructureSets().forEach((structureSet) -> {
         ...
         boolean bl = false;
         Iterator var5 = structureSet2.structures().iterator();

         while(var5.hasNext()) {
            StructureSet.WeightedEntry weightedEntry = (StructureSet.WeightedEntry)var5.next();
            Structure structure = (Structure)weightedEntry.structure().value();
            Stream var10000 = structure.getValidBiomes().stream();
            Objects.requireNonNull(set);
            if (var10000.anyMatch(set::contains)) {
               ((List)this.structuresToPlacements.computeIfAbsent(structure, (structurex) -> {
         ...
         if (bl) {
            ...
            if (structurePlacement instanceof ConcentricRingsStructurePlacement) {
               ...
               this.concentricPlacementsToPositions.put(concentricRingsStructurePlacement, this.calculateConcentricsRingPlacementPos(structureSet, concentricRingsStructurePlacement));
            }
         }
   ...
   private CompletableFuture calculateConcentricsRingPlacementPos(RegistryEntry structureSetEntry, ConcentricRingsStructurePlacement placement) {
      if (placement.getCount() == 0) {
         ...
         RegistryEntryList registryEntryList = placement.getPreferredBiomes();
         ...
         for(int n = 0; n < j; ++n) {
            ...
            list.add(CompletableFuture.supplyAsync(() -> {
               ...
               Pair pair = var10000.locateBiome(var10001, 0, var10003, 112, registryEntryList::contains, random2, this.noiseConfig.getMultiNoiseSampler());
               if (pair != null) {
