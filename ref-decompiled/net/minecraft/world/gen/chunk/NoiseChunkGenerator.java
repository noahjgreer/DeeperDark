...
public final class NoiseChunkGenerator extends ChunkGenerator {
   ...
   public void carve(ChunkRegion chunkRegion, long seed, NoiseConfig noiseConfig, BiomeAccess biomeAccess, StructureAccessor structureAccessor, Chunk chunk) {
      ...
      CarverContext carverContext = new CarverContext(this, chunkRegion.getRegistryManager(), chunk.getHeightLimitView(), chunkNoiseSampler, noiseConfig, ((ChunkGeneratorSettings)this.settings.value()).surfaceRule());
      ...
      for(int j = -8; j <= 8; ++j) {
         for(int k = -8; k <= 8; ++k) {
            ...
            Iterable iterable = generationSettings.getCarversForStep();
            int l = 0;

            for(Iterator var23 = iterable.iterator(); var23.hasNext(); ++l) {
               RegistryEntry registryEntry = (RegistryEntry)var23.next();
   ...
   public void populateEntities(ChunkRegion region) {
      if (!((ChunkGeneratorSettings)this.settings.value()).mobGenerationDisabled()) {
         ...
         RegistryEntry registryEntry = region.getBiome(chunkPos.getStartPos().withY(region.getTopYInclusive()));
         ChunkRandom chunkRandom = new ChunkRandom(new CheckedRandom(RandomSeed.getSeed()));
