...
public class Heightmap {
   ...
   static final Predicate SUFFOCATES = AbstractBlock.AbstractBlockState::blocksMovement;
   ...
   public void setTo(Chunk chunk, Type type, long[] values) {
      ...
      if (ls.length == values.length) {
         ...
         String var10001 = String.valueOf(chunk.getPos());
         var10000.warn("Ignoring heightmap data for chunk " + var10001 + ", size does not match; expected: " + ls.length + ", got: " + values.length);
   ...
   public static enum Type implements StringIdentifiable {
      ...
      MOTION_BLOCKING(4, "MOTION_BLOCKING", Heightmap.Purpose.CLIENT, (state) -> {
         return state.blocksMovement() || !state.getFluidState().isEmpty();
      }),
      MOTION_BLOCKING_NO_LEAVES(5, "MOTION_BLOCKING_NO_LEAVES", Heightmap.Purpose.CLIENT, (state) -> {
         return (state.blocksMovement() || !state.getFluidState().isEmpty()) && !(state.getBlock() instanceof LeavesBlock);
      });

      public static final Codec CODEC = StringIdentifiable.createCodec(Type::values);
      private static final IntFunction INDEX_MAPPER = ValueLists.createIndexToValueFunction((type) -> {
         return type.index;
      }, values(), (ValueLists.OutOfBoundsHandling)ValueLists.OutOfBoundsHandling.ZERO);
      public static final PacketCodec PACKET_CODEC = PacketCodecs.indexed(INDEX_MAPPER, (type) -> {
         return type.index;
      });
      ...
      private Type(final int index, final String id, final Purpose purpose, final Predicate blockPredicate) {
         this.index = index;
         this.id = id;
