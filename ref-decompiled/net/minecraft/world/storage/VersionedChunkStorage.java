...
public class VersionedChunkStorage implements AutoCloseable {
      ...
      if (i == SharedConstants.getGameVersion().dataVersion().id()) {
         ...
         try {
            if (i < 1493) {
               ...
               if ((Boolean)nbt.getCompound("Level").flatMap((level) -> {
                  return level.getBoolean("hasLegacyStructureData");
               }).orElse(false)) {
            ...
            crashReportSection.add("Data version", (Object)i);
   ...
   public static void saveContextToNbt(NbtCompound nbt, RegistryKey worldKey, Optional generatorCodecKey) {
      ...
      generatorCodecKey.ifPresent((key) -> {
         nbtCompound.putString("generator", key.getValue().toString());
      });
   ...
   public static int getDataVersion(NbtCompound nbt) {
      return NbtHelper.getDataVersion((NbtCompound)nbt, -1);
   }
