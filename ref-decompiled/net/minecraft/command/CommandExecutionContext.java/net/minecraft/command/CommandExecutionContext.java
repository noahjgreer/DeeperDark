...
implements AutoCloseable {
    ...
    public static <T extends AbstractServerCommandSource<T>> void enqueueProcedureCall(CommandExecutionContext<T> context, Procedure<T> procedure, T source, ReturnValueConsumer returnValueConsumer) {
        context.enqueueCommand(new CommandQueueEntry<T>(CommandExecutionContext.frame(context, returnValueConsumer), new CommandFunctionAction<T>(procedure, source.getReturnValueConsumer(), false).bind(source)));
    }

    public static <T extends AbstractServerCommandSource<T>> void enqueueCommand(CommandExecutionContext<T> context, String command, ContextChain<T> contextChain, T source, ReturnValueConsumer returnValueConsumer) {
        context.enqueueCommand(new CommandQueueEntry<T>(CommandExecutionContext.frame(context, returnValueConsumer), new SingleCommandAction.SingleSource<T>(command, contextChain, source)));
    }
    ...
    public void run() {
        ...
        while (true) {
            if (this.commandsRemaining <= 0) {
                LOGGER.info("Command execution stopped due to limit (executed {} commands)", (Object)this.maxCommandChainLength);
                break;
            }
