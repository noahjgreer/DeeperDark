...
extends PermissionSource {
    public static final CharMatcher SUGGESTION_MATCH_PREFIX = CharMatcher.anyOf((CharSequence)"._/");
    ...
    default public void suggestIdentifiers(RegistryWrapper<?> registry, SuggestedIdType suggestedIdType, SuggestionsBuilder builder) {
        if (suggestedIdType.canSuggestTags()) {
            CommandSource.suggestIdentifiers(registry.streamTagKeys().map(TagKey::id), builder, "#");
        }
        if (suggestedIdType.canSuggestElements()) {
            CommandSource.suggestIdentifiers(registry.streamKeys().map(RegistryKey::getValue), builder);
        }
    }

    public static <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder, RegistryKey<? extends Registry<?>> registryRef, SuggestedIdType suggestedIdType) {
    ...
    public static CompletableFuture<Suggestions> suggestIdentifiers(Iterable<Identifier> candidates, SuggestionsBuilder builder, String prefix) {
        ...
        CommandSource.forEachMatching(candidates, string, prefix, id -> id, id -> builder.suggest(prefix + String.valueOf(id)));
        return builder.buildFuture();
    }
    ...
    public static boolean shouldSuggest(String remaining, String candidate) {
        ...
        while (!candidate.startsWith(remaining, i)) {
            int j = SUGGESTION_MATCH_PREFIX.indexIn((CharSequence)candidate, i);
            if (j < 0) {
                return false;
            }
            i = j + 1;
