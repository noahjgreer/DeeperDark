...
static class ItemPredicateParsing.SubPredicateParsingRule<T, C, P>
extends IdentifiableParsingRule<ItemPredicateParsing.Callbacks<T, C, P>, P> {
    ItemPredicateParsing.SubPredicateParsingRule(ParsingRuleEntry<StringReader, Identifier> idParsingRule, ItemPredicateParsing.Callbacks<T, C, P> callbacks) {
        super(idParsingRule, callbacks);
    }

    @Override
    protected P parse(ImmutableStringReader reader, Identifier id) throws Exception {
        return ((ItemPredicateParsing.Callbacks)this.callbacks).subPredicateCheck(reader, id);
    }

    @Override
    public Stream<Identifier> possibleIds() {
        return ((ItemPredicateParsing.Callbacks)this.callbacks).streamSubPredicateIds();
    }
}
