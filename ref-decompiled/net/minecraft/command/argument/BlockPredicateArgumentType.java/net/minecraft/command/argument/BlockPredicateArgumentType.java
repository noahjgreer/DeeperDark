...
implements ArgumentType<BlockPredicate> {
    ...
    public static BlockPredicate parse(RegistryWrapper<Block> registryWrapper, StringReader reader) throws CommandSyntaxException {
        return (BlockPredicate)BlockArgumentParser.blockOrTag(registryWrapper, reader, true).map(result -> new StatePredicate(result.blockState(), result.properties().keySet(), result.nbt()), result -> new TagPredicate(result.tag(), result.vagueProperties(), result.nbt()));
    }
