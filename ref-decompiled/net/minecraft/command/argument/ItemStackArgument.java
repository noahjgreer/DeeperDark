...
public class ItemStackArgument {
   private static final Dynamic2CommandExceptionType OVERSTACKED_EXCEPTION = new Dynamic2CommandExceptionType((item, maxCount) -> {
      return Text.stringifiedTranslatable("arguments.item.overstacked", item, maxCount);
   });
   private final RegistryEntry item;
   private final ComponentChanges components;

   public ItemStackArgument(RegistryEntry item, ComponentChanges components) {
      this.item = item;
   ...
   private String componentsAsString(RegistryWrapper.WrapperLookup registries) {
      DynamicOps dynamicOps = registries.getOps(NbtOps.INSTANCE);
      return (String)this.components.entrySet().stream().flatMap((entry) -> {
         ...
         if (identifier == null) {
            ...
            if (optional.isPresent()) {
               ...
               return component.encode(dynamicOps).result().stream().map((value) -> {
                  String var10000 = identifier.toString();
                  return var10000 + "=" + String.valueOf(value);
               });
            } else {
