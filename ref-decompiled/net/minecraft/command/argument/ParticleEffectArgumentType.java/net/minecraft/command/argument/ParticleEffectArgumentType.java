...
implements ArgumentType<ParticleEffect> {
    ...
    public static ParticleEffect readParameters(StringReader reader, RegistryWrapper.WrapperLookup registries) throws CommandSyntaxException {
        ParticleType<?> particleType = ParticleEffectArgumentType.getType(reader, registries.getOrThrow(RegistryKeys.PARTICLE_TYPE));
        return ParticleEffectArgumentType.readParameters(SNBT_READER, reader, particleType, registries);
    }
    ...
    private static ParticleType<?> getType(StringReader reader, RegistryWrapper<ParticleType<?>> registryWrapper) throws CommandSyntaxException {
        ...
        return registryWrapper.getOptional(registryKey).orElseThrow(() -> UNKNOWN_PARTICLE_EXCEPTION.createWithContext((ImmutableStringReader)reader, (Object)identifier)).value();
    }
    ...
    private static <T extends ParticleEffect, O> T readParameters(StringNbtReader<O> snbtReader, StringReader reader, ParticleType<T> particleType, RegistryWrapper.WrapperLookup registries) throws CommandSyntaxException {
        RegistryOps<O> registryOps = registries.getOps(snbtReader.getOps());
        Object object = reader.canRead() && reader.peek() == '{' ? snbtReader.readAsArgument(reader) : registryOps.emptyMap();
        return (T)((ParticleEffect)particleType.getCodec().codec().parse(registryOps, object).getOrThrow(arg_0 -> ((DynamicCommandExceptionType)INVALID_OPTIONS_EXCEPTION).create(arg_0)));
    }

    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        RegistryEntryLookup impl = this.registries.getOrThrow(RegistryKeys.PARTICLE_TYPE);
