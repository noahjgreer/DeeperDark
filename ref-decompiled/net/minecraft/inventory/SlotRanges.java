...
public class SlotRanges {
   private static final List SLOT_RANGES = (List)Util.make(new ArrayList(), (list) -> {
   ...
   public static final Codec CODEC = StringIdentifiable.createBasicCodec(() -> {
      return (SlotRange[])SLOT_RANGES.toArray(new SlotRange[0]);
   });
   private static final Function FROM_NAME;
   ...
   private static void createAndAdd(List list, String name, int slotId) {
      list.add(create(name, slotId));
   }

   private static void createAndAdd(List list, String baseName, int firstSlotId, int slotCount) {
      ...
      list.add(create(baseName + "*", (IntList)intList));
   }

   private static void createAndAdd(List list, String name, int... slots) {
   ...
   static {
      FROM_NAME = StringIdentifiable.createMapper((SlotRange[])SLOT_RANGES.toArray(new SlotRange[0]), (name) -> {
         return name;
      });
   }
}
