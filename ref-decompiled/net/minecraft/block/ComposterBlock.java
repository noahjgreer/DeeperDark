...
public class ComposterBlock extends Block implements InventoryProvider {
   public static final MapCodec CODEC = createCodec(ComposterBlock::new);
   ...
   private static final VoxelShape[] COLLISION_SHAPES_BY_LEVEL;
   ...
   protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      return COLLISION_SHAPES_BY_LEVEL[(Integer)state.get(LEVEL)];
   }
   ...
   public static BlockState emptyFullComposter(Entity user, BlockState state, World world, BlockPos pos) {
      ...
      world.playSound((Entity)null, pos, SoundEvents.BLOCK_COMPOSTER_EMPTY, SoundCategory.BLOCKS, 1.0F, 1.0F);
      return blockState;
   }
   ...
   static BlockState emptyComposter(@Nullable Entity user, BlockState state, WorldAccess world, BlockPos pos) {
      ...
      world.emitGameEvent((RegistryEntry)GameEvent.BLOCK_CHANGE, (BlockPos)pos, (GameEvent.Emitter)GameEvent.Emitter.of(user, blockState));
      return blockState;
   }
   ...
   protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
      if ((Integer)state.get(LEVEL) == 7) {
         ...
         world.playSound((Entity)null, pos, SoundEvents.BLOCK_COMPOSTER_READY, SoundCategory.BLOCKS, 1.0F, 1.0F);
      }
   ...
   static {
      ...
      COLLISION_SHAPES_BY_LEVEL = (VoxelShape[])Util.make(() -> {
         VoxelShape[] voxelShapes = Block.createShapeArray(8, (level) -> {
            return VoxelShapes.combineAndSimplify(VoxelShapes.fullCube(), Block.createColumnShape(12.0, (double)Math.clamp((long)(1 + level * 2), 2, 16), 16.0), BooleanBiFunction.ONLY_FIRST);
         });
   ...
   private static class ComposterInventory extends SimpleInventory implements SidedInventory {
      ...
      public void markDirty() {
         ...
         if (!itemStack.isEmpty()) {
            this.dirty = true;
            BlockState blockState = ComposterBlock.addToComposter((Entity)null, this.state, this.world, this.pos, itemStack);
