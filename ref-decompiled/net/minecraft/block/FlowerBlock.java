...
implements SuspiciousStewIngredient {
    ...
    public static final MapCodec<FlowerBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)STEW_EFFECT_CODEC.forGetter(FlowerBlock::getStewEffects), (App)FlowerBlock.createSettingsCodec()).apply((Applicative)instance, FlowerBlock::new));
    private static final VoxelShape SHAPE = Block.createColumnShape((double)6.0, (double)0.0, (double)10.0);
    ...
    public FlowerBlock(RegistryEntry<StatusEffect> stewEffect, float effectLengthInSeconds, AbstractBlock.Settings settings) {
        this(FlowerBlock.createStewEffectList(stewEffect, (float)effectLengthInSeconds), settings);
    }
    ...
    protected static SuspiciousStewEffectsComponent createStewEffectList(RegistryEntry<StatusEffect> effect, float effectLengthInSeconds) {
        return new SuspiciousStewEffectsComponent(List.of(new SuspiciousStewEffectsComponent.StewEffect(effect, MathHelper.floor((float)(effectLengthInSeconds * 20.0f)))));
    }
