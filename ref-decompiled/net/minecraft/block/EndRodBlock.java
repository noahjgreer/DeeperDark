...
public class EndRodBlock extends RodBlock {
   public static final MapCodec CODEC = createCodec(EndRodBlock::new);
   ...
   public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
      ...
      if (random.nextInt(5) == 0) {
         world.addParticleClient(ParticleTypes.END_ROD, d + (double)direction.getOffsetX() * g, e + (double)direction.getOffsetY() * g, f + (double)direction.getOffsetZ() * g, random.nextGaussian() * 0.005, random.nextGaussian() * 0.005, random.nextGaussian() * 0.005);
      }
