...
extends AbstractFireBlock {
    ...
    public static final Map<Direction, BooleanProperty> DIRECTION_PROPERTIES = (Map)ConnectingBlock.FACING_PROPERTIES.entrySet().stream().filter(entry -> entry.getKey() != Direction.DOWN).collect(Util.toMap());
    ...
    public FireBlock(AbstractBlock.Settings settings) {
        super(settings, 1.0f);
        this.setDefaultState((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)((BlockState)this.stateManager.getDefaultState()).with((Property)AGE, (Comparable)Integer.valueOf(0))).with((Property)NORTH, (Comparable)Boolean.valueOf(false))).with((Property)EAST, (Comparable)Boolean.valueOf(false))).with((Property)SOUTH, (Comparable)Boolean.valueOf(false))).with((Property)WEST, (Comparable)Boolean.valueOf(false))).with((Property)UP, (Comparable)Boolean.valueOf(false)));
    ...
    private Function<BlockState, VoxelShape> createShapeFunction() {
        ...
        return this.createShapeFunction(state -> {
            ...
            for (Map.Entry entry : DIRECTION_PROPERTIES.entrySet()) {
    ...
    protected BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        if (this.canPlaceAt(state, world, pos)) {
            return this.getStateWithAge(world, pos, ((Integer)state.get((Property)AGE)).intValue());
        }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        return this.getStateForPosition((BlockView)ctx.getWorld(), ctx.getBlockPos());
    }
    ...
    protected BlockState getStateForPosition(BlockView world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            blockState2 = (BlockState)blockState2.with((Property)booleanProperty, (Comparable)Boolean.valueOf(this.isFlammable(world.getBlockState(pos.offset(direction)))));
        }
        return blockState2;
    }
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        BlockPos blockPos = pos.down();
        return world.getBlockState(blockPos).isSideSolidFullSquare((BlockView)world, blockPos, Direction.UP) || this.areBlocksAroundFlammable((BlockView)world, pos);
    }

    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        boolean bl2;
        world.scheduleBlockTick(pos, (Block)this, FireBlock.getFireTickDelay((Random)world.random));
        ...
        if (!bl && world.isRaining() && this.isRainingAround((World)world, pos) && random.nextFloat() < 0.2f + (float)i * 0.03f) {
            world.removeBlock(pos, false);
            return;
        }
        int j = Math.min(15, i + random.nextInt(3) / 2);
        if (i != j) {
            state = (BlockState)state.with((Property)AGE, (Comparable)Integer.valueOf(j));
            world.setBlockState(pos, state, 260);
        }
        if (!bl) {
            if (!this.areBlocksAroundFlammable((BlockView)world, pos)) {
                BlockPos blockPos = pos.down();
        ...
        int k = (bl2 = ((Boolean)world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.INCREASED_FIRE_BURNOUT_GAMEPLAY, pos)).booleanValue()) ? -50 : 0;
        this.trySpreadingFire((World)world, pos.east(), 300 + k, random, i);
        this.trySpreadingFire((World)world, pos.west(), 300 + k, random, i);
        this.trySpreadingFire((World)world, pos.down(), 250 + k, random, i);
        this.trySpreadingFire((World)world, pos.up(), 250 + k, random, i);
        this.trySpreadingFire((World)world, pos.north(), 300 + k, random, i);
        this.trySpreadingFire((World)world, pos.south(), 300 + k, random, i);
        ...
        for (int l = -1; l <= 1; ++l) {
            for (int m = -1; m <= 1; ++m) {
                for (int n = -1; n <= 4; ++n) {
                    ...
                    int p = this.getBurnChance((WorldView)world, (BlockPos)mutable);
                    if (p <= 0) continue;
                    int q = (p + 40 + world.getDifficulty().getId() * 7) / (i + 30);
                    if (bl2) {
                        q /= 2;
                    }
                    if (q <= 0 || random.nextInt(o) > q || world.isRaining() && this.isRainingAround((World)world, (BlockPos)mutable)) continue;
                    int r = Math.min(15, i + random.nextInt(5) / 4);
                    world.setBlockState((BlockPos)mutable, this.getStateWithAge((WorldView)world, (BlockPos)mutable, r), 3);
                }
            }
        }
    }
    ...
    private void trySpreadingFire(World world, BlockPos pos, int spreadFactor, Random random, int currentAge) {
        ...
        if (random.nextInt(spreadFactor) < i) {
            ...
            if (random.nextInt(currentAge + 10) < 5 && !world.hasRain(pos)) {
                ...
                world.setBlockState(pos, this.getStateWithAge((WorldView)world, pos, j), 3);
            } else {
                world.removeBlock(pos, false);
            }
    ...
    private BlockState getStateWithAge(WorldView world, BlockPos pos, int age) {
        BlockState blockState = FireBlock.getState((BlockView)world, (BlockPos)pos);
        if (blockState.isOf(Blocks.FIRE)) {
            return (BlockState)blockState.with((Property)AGE, (Comparable)Integer.valueOf(age));
        }
        return blockState;
    }
