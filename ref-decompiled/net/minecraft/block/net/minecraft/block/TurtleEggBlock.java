...
extends Block {
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world, pos) && TurtleEggBlock.isSandBelow(world, pos)) {
    ...
    private boolean shouldHatchProgress(World world, BlockPos pos) {
        float f = world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.TURTLE_EGG_HATCH_CHANCE_GAMEPLAY, pos).floatValue();
    ...
    private boolean breaksEgg(ServerWorld world, Entity entity) {
        ...
        if (entity instanceof LivingEntity) {
            return entity instanceof PlayerEntity || world.getGameRules().getValue(GameRules.DO_MOB_GRIEFING) != false;
        }
        return false;
    }
}
