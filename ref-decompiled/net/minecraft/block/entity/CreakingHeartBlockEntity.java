...
extends BlockEntity {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, CreakingHeartBlockEntity blockEntity) {
        ...
        BlockState blockState = CreakingHeartBlockEntity.getBlockState((World)world, (BlockState)state, (BlockPos)pos, (CreakingHeartBlockEntity)blockEntity);
        if (blockState != state) {
            world.setBlockState(pos, blockState, 3);
            if (blockState.get((Property)CreakingHeartBlock.ACTIVE) == CreakingHeartState.UPROOTED) {
                return;
            }
        }
        if (blockEntity.creakingPuppet != null) {
            ...
            if (optional.isPresent()) {
                ...
                if ((Boolean)world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.CREAKING_ACTIVE_GAMEPLAY, pos) == false && !creakingEntity.isPersistent() || blockEntity.getDistanceToPuppet() > 34.0 || creakingEntity.isStuckWithPlayer()) {
                    blockEntity.killPuppet(null);
                }
            }
            return;
        }
        if (blockState.get((Property)CreakingHeartBlock.ACTIVE) != CreakingHeartState.AWAKE) {
            return;
        }
        ...
        if (playerEntity != null && (creakingEntity = CreakingHeartBlockEntity.spawnCreakingPuppet((ServerWorld)serverWorld, (CreakingHeartBlockEntity)blockEntity)) != null) {
    ...
    private static BlockState getBlockState(World world, BlockState state, BlockPos pos, CreakingHeartBlockEntity creakingHeart) {
        if (!CreakingHeartBlock.shouldBeEnabled((BlockState)state, (WorldView)world, (BlockPos)pos) && creakingHeart.creakingPuppet == null) {
            return (BlockState)state.with((Property)CreakingHeartBlock.ACTIVE, (Comparable)CreakingHeartState.UPROOTED);
        }
        CreakingHeartState creakingHeartState = (Boolean)world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.CREAKING_ACTIVE_GAMEPLAY, pos) != false ? CreakingHeartState.AWAKE : CreakingHeartState.DORMANT;
        return (BlockState)state.with((Property)CreakingHeartBlock.ACTIVE, (Comparable)creakingHeartState);
    }

    private double getDistanceToPuppet() {
        return this.getCreakingPuppet().map(creaking -> Math.sqrt(creaking.squaredDistanceTo(Vec3d.ofBottomCenter((Vec3i)this.getPos())))).orElse(0.0);
    }
    ...
    private static @Nullable CreakingEntity spawnCreakingPuppet(ServerWorld world, CreakingHeartBlockEntity blockEntity) {
        ...
        Optional optional = LargeEntitySpawnHelper.trySpawnAt((EntityType)EntityType.CREAKING, (SpawnReason)SpawnReason.SPAWNER, (ServerWorld)world, (BlockPos)blockPos, (int)5, (int)16, (int)8, (LargeEntitySpawnHelper.Requirements)LargeEntitySpawnHelper.Requirements.CREAKING, (boolean)true);
        if (optional.isEmpty()) {
            return null;
        }
        CreakingEntity creakingEntity = (CreakingEntity)optional.get();
        world.emitGameEvent((Entity)creakingEntity, (RegistryEntry)GameEvent.ENTITY_PLACE, creakingEntity.getEntityPos());
        world.sendEntityStatus((Entity)creakingEntity, (byte)60);
    ...
    public void onPuppetDamage() {
        ...
        if (this.getCachedState().get((Property)CreakingHeartBlock.ACTIVE) == CreakingHeartState.AWAKE) {
            ...
            for (int j = 0; j < i; ++j) {
                this.findResinGenerationPos(serverWorld).ifPresent(pos -> {
                    ...
                    this.world.emitGameEvent((RegistryEntry)GameEvent.BLOCK_PLACE, pos, GameEvent.Emitter.of((BlockState)this.getCachedState()));
                });
            }
        }
    ...
    private Optional<BlockPos> findResinGenerationPos(ServerWorld world) {
        ...
        BlockPos.iterateRecursively((BlockPos)this.pos, (int)2, (int)64, (pos, consumer) -> {
            for (Direction direction : Util.copyShuffled((Object[])Direction.values(), (Random)serverWorld.random)) {
    ...
    private void spawnTrailParticles(ServerWorld world, int count, boolean towardsPuppet) {
        ...
        for (double d = 0.0; d < (double)count; d += 1.0) {
            ...
            Vec3d vec3d2 = Vec3d.of((Vec3i)this.getPos()).add(random.nextDouble(), random.nextDouble(), random.nextDouble());
            if (towardsPuppet) {
                Vec3d vec3d3 = vec3d;
                vec3d = vec3d2;
                vec3d2 = vec3d3;
            ...
            world.spawnParticles((ParticleEffect)trailParticleEffect, true, true, vec3d.x, vec3d.y, vec3d.z, 1, 0.0, 0.0, 0.0, 0.0);
        }
    }
    ...
    protected void readData(ReadView view) {
        super.readData(view);
        view.read("creaking", Uuids.INT_STREAM_CODEC).ifPresentOrElse(arg_0 -> this.setCreakingPuppetFromUuid(arg_0), () -> this.clearCreakingPuppet());
    }

    protected void writeData(WriteView view) {
        super.writeData(view);
        if (this.creakingPuppet != null) {
            view.put("creaking", Uuids.INT_STREAM_CODEC, (Object)((UUID)this.creakingPuppet.map(Entity::getUuid, uuid -> uuid)));
        }
    }
    ...
    private static /* synthetic */ BlockPos.IterationState method_65169(ServerWorld serverWorld, Mutable mutable, BlockPos pos) {
        ...
        for (Direction direction : Util.copyShuffled((Object[])Direction.values(), (Random)serverWorld.random)) {
            ...
            if (blockState.isAir()) {
                ...
                blockState = (BlockState)Blocks.RESIN_CLUMP.getDefaultState().with((Property)MultifaceBlock.WATERLOGGED, (Comparable)Boolean.valueOf(true));
            }
            if (!blockState.isOf(Blocks.RESIN_CLUMP) || MultifaceBlock.hasDirection((BlockState)blockState, (Direction)direction2)) continue;
            serverWorld.setBlockState(blockPos, (BlockState)blockState.with((Property)MultifaceBlock.getProperty((Direction)direction2), (Comparable)Boolean.valueOf(true)), 3);
