...
public class CampfireBlockEntity extends BlockEntity implements Clearable {
   ...
   public static void litServerTick(ServerWorld world, BlockPos pos, BlockState state, CampfireBlockEntity blockEntity, ServerRecipeManager.MatchGetter recipeMatchGetter) {
      boolean bl = false;
      ...
      for(int i = 0; i < blockEntity.itemsBeingCooked.size(); ++i) {
         ...
         if (!itemStack.isEmpty()) {
            ...
            if (blockEntity.cookingTimes[i] >= blockEntity.cookingTotalTimes[i]) {
               ...
               ItemStack itemStack2 = (ItemStack)recipeMatchGetter.getFirstMatch(singleStackRecipeInput, world).map((recipe) -> {
                  return ((CampfireCookingRecipe)recipe.value()).craft(singleStackRecipeInput, world.getRegistryManager());
               }).orElse(itemStack);
               if (itemStack2.isItemEnabled(world.getEnabledFeatures())) {
                  ItemScatterer.spawn(world, (double)pos.getX(), (double)pos.getY(), (double)pos.getZ(), (ItemStack)itemStack2);
   ...
   public static void clientTick(World world, BlockPos pos, BlockState state, CampfireBlockEntity campfire) {
      ...
      i = ((Direction)state.get(CampfireBlock.FACING)).getHorizontalQuarterTurns();
   ...
   protected void readData(ReadView view) {
      ...
      view.getOptionalIntArray("CookingTimes").ifPresentOrElse((is) -> {
         System.arraycopy(is, 0, this.cookingTimes, 0, Math.min(this.cookingTotalTimes.length, is.length));
      }, () -> {
         Arrays.fill(this.cookingTimes, 0);
      });
      view.getOptionalIntArray("CookingTotalTimes").ifPresentOrElse((is) -> {
   ...
   public boolean addItem(ServerWorld world, @Nullable LivingEntity entity, ItemStack stack) {
      for(int i = 0; i < this.itemsBeingCooked.size(); ++i) {
         ...
         if (itemStack.isEmpty()) {
            Optional optional = world.getRecipeManager().getFirstMatch(RecipeType.CAMPFIRE_COOKING, new SingleStackRecipeInput(stack), world);
            if (optional.isEmpty()) {
               return false;
            }
