...
extends BlockEntity {
    ...
    public boolean brush(long worldTime, ServerWorld world, LivingEntity brusher, Direction hitDirection, ItemStack brush) {
        ...
        if (i != j) {
            ...
            BlockState blockState2 = (BlockState)blockState.with((Property)Properties.DUSTED, (Comparable)Integer.valueOf(j));
    ...
    private void generateItem(ServerWorld world, LivingEntity brusher, ItemStack brush) {
        ...
        LootWorldContext lootWorldContext = new LootWorldContext.Builder(world).add(LootContextParameters.ORIGIN, (Object)Vec3d.ofCenter((Vec3i)this.pos)).luck(brusher.getLuck()).add(LootContextParameters.THIS_ENTITY, (Object)brusher).add(LootContextParameters.TOOL, (Object)brush).build(LootContextTypes.ARCHAEOLOGY);
    ...
    private void finishBrushing(ServerWorld world, LivingEntity brusher, ItemStack brush) {
        ...
        BlockState blockState = this.getCachedState();
        world.syncWorldEvent(3008, this.getPos(), Block.getRawIdFromState((BlockState)blockState));
    ...
    private void spawnItem(ServerWorld world, LivingEntity brusher, ItemStack brush) {
        ...
        if (!this.item.isEmpty()) {
            ...
            ItemEntity itemEntity = new ItemEntity((World)world, g, h, i, this.item.split(world.random.nextInt(21) + 10));
    ...
    public void scheduledTick(ServerWorld world) {
        if (this.brushesCount != 0 && world.getTime() >= this.nextDustTime) {
            ...
            int j = this.getDustedLevel();
            if (i != j) {
                world.setBlockState(this.getPos(), (BlockState)this.getCachedState().with((Property)Properties.DUSTED, (Comparable)Integer.valueOf(j)), 3);
            }
            int k = 4;
    ...
    private boolean writeLootTableToData(WriteView view) {
        ...
        view.put(LOOT_TABLE_NBT_KEY, LootTable.TABLE_KEY, (Object)this.lootTable);
    ...
    public NbtCompound toInitialChunkDataNbt(RegistryWrapper.WrapperLookup registries) {
        ...
        nbtCompound.putNullable(HIT_DIRECTION_NBT_KEY, Direction.INDEX_CODEC, (Object)this.hitDirection);
        if (!this.item.isEmpty()) {
            RegistryOps registryOps = registries.getOps((DynamicOps)NbtOps.INSTANCE);
            nbtCompound.put(ITEM_NBT_KEY, ItemStack.CODEC, (DynamicOps)registryOps, (Object)this.item);
        }
        return nbtCompound;
    }
    ...
    protected void writeData(WriteView view) {
        ...
        if (!this.writeLootTableToData(view) && !this.item.isEmpty()) {
            view.put(ITEM_NBT_KEY, ItemStack.CODEC, (Object)this.item);
        }
    }
