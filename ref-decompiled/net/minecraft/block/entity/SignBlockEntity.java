...
extends BlockEntity {
    ...
    public boolean isPlayerFacingFront(PlayerEntity player) {
        ...
        if (block instanceof AbstractSignBlock) {
            ...
            return MathHelper.angleBetween((float)f, (float)(g = (float)(MathHelper.atan2((double)e, (double)d) * 57.2957763671875) - 90.0f)) <= 90.0f;
        }
        return false;
    }
    ...
    protected void writeData(WriteView view) {
        ...
        view.put("front_text", SignText.CODEC, (Object)this.frontText);
        view.put("back_text", SignText.CODEC, (Object)this.backText);
        view.putBoolean("is_waxed", this.waxed);
    }

    protected void readData(ReadView view) {
        super.readData(view);
        this.frontText = view.read("front_text", SignText.CODEC).map(arg_0 -> this.parseLines(arg_0)).orElseGet(SignText::new);
        this.backText = view.read("back_text", SignText.CODEC).map(arg_0 -> this.parseLines(arg_0)).orElseGet(SignText::new);
    ...
    private Text parseLine(Text text) {
        ...
        if (world instanceof ServerWorld) {
            ...
            try {
                return Texts.parse((ServerCommandSource)SignBlockEntity.createCommandSource(null, (ServerWorld)serverWorld, (BlockPos)this.pos), (Text)text, null, (int)0);
            }
    ...
    private SignText getTextWithMessages(PlayerEntity player, List<FilteredMessage> messages, SignText text) {
        for (int i = 0; i < messages.size(); ++i) {
            ...
            text = player.shouldFilterText() ? text.withMessage(i, (Text)Text.literal((String)filteredMessage.getString()).setStyle(style)) : text.withMessage(i, (Text)Text.literal((String)filteredMessage.raw()).setStyle(style), (Text)Text.literal((String)filteredMessage.getString()).setStyle(style));
        }
        return text;
    }
        ...
        block5: for (Text text : this.getText(front).getMessages(player.shouldFilterText())) {
            ...
            switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{ClickEvent.RunCommand.class, ClickEvent.ShowDialog.class, ClickEvent.Custom.class}, (Object)clickEvent2, n)) {
                case 0: {
                    ...
                    world.getServer().getCommandManager().parseAndExecute(SignBlockEntity.createCommandSource((PlayerEntity)player, (ServerWorld)world, (BlockPos)pos), runCommand.command());
                    bl = true;
                    continue block5;
                }
                case 1: {
    ...
    private static ServerCommandSource createCommandSource(@Nullable PlayerEntity player, ServerWorld world, BlockPos pos) {
        ...
        MutableText text = player == null ? Text.literal((String)"Sign") : player.getDisplayName();
        return new ServerCommandSource(CommandOutput.DUMMY, Vec3d.ofCenter((Vec3i)pos), Vec2f.ZERO, world, (PermissionPredicate)LeveledPermissionPredicate.GAMEMASTERS, string, (Text)text, world.getServer(), (Entity)player);
    }
