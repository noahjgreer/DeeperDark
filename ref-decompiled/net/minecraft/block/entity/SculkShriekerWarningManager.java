...
public class SculkShriekerWarningManager {
   public static final Codec CODEC = RecordCodecBuilder.create((instance) -> {
      return instance.group(Codecs.NON_NEGATIVE_INT.fieldOf("ticks_since_last_warning").orElse(0).forGetter((manager) -> {
         return manager.ticksSinceLastWarning;
      }), Codecs.NON_NEGATIVE_INT.fieldOf("warning_level").orElse(0).forGetter((manager) -> {
         return manager.warningLevel;
      }), Codecs.NON_NEGATIVE_INT.fieldOf("cooldown_ticks").orElse(0).forGetter((manager) -> {
            ...
            if (optional.isPresent()) {
               ...
               list.forEach((nearbyPlayer) -> {
                  nearbyPlayer.getSculkShriekerWarningManager().ifPresent((warningManager) -> {
                     warningManager.copy(sculkShriekerWarningManager);
                  });
               });
   ...
   private static List getPlayersInRange(ServerWorld world, BlockPos pos) {
      ...
      return world.getPlayers((player) -> {
         return !player.isSpectator() && player.getPos().isInRange(vec3d, 16.0) && player.isAlive();
      });
   }
