...
public class SnifferEggBlock extends Block {
   public static final MapCodec CODEC = createCodec(SnifferEggBlock::new);
   ...
   public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
      if (!this.isReadyToHatch(state)) {
         world.playSound((Entity)null, pos, SoundEvents.BLOCK_SNIFFER_EGG_CRACK, SoundCategory.BLOCKS, 0.7F, 0.9F + random.nextFloat() * 0.2F);
         world.setBlockState(pos, (BlockState)state.with(HATCH, this.getHatchStage(state) + 1), 2);
      } else {
         world.playSound((Entity)null, pos, SoundEvents.BLOCK_SNIFFER_EGG_HATCH, SoundCategory.BLOCKS, 0.7F, 0.9F + random.nextFloat() * 0.2F);
         world.breakBlock(pos, false);
         SnifferEntity snifferEntity = (SnifferEntity)EntityType.SNIFFER.create(world, SpawnReason.BREEDING);
   ...
   static {
      ...
      SHAPE = Block.createColumnShape(14.0, 12.0, 0.0, 16.0);
   }
}
