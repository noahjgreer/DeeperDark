...
public class FenceGateBlock extends HorizontalFacingBlock {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      return instance.group(WoodType.CODEC.fieldOf("wood_type").forGetter((block) -> {
         return block.type;
   ...
   private static final Map REGULAR_OUTLINE_SHAPES;
   private static final Map IN_WALL_OUTLINE_SHAPES;
   private static final Map CLOSED_COLLISION_SHAPES;
   private static final Map CLOSED_SIDES_SHAPES;
   private static final Map REGULAR_CULLING_SHAPES;
   private static final Map IN_WALL_CULLING_SHAPES;
   ...
   protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      ...
      return (VoxelShape)((Boolean)state.get(IN_WALL) ? IN_WALL_OUTLINE_SHAPES : REGULAR_OUTLINE_SHAPES).get(axis);
   }
   ...
   protected VoxelShape getSidesShape(BlockState state, BlockView world, BlockPos pos) {
      Direction.Axis axis = ((Direction)state.get(FACING)).getAxis();
      return (Boolean)state.get(OPEN) ? VoxelShapes.empty() : (VoxelShape)CLOSED_SIDES_SHAPES.get(axis);
   }

   protected VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      Direction.Axis axis = ((Direction)state.get(FACING)).getAxis();
      return (Boolean)state.get(OPEN) ? VoxelShapes.empty() : (VoxelShape)CLOSED_COLLISION_SHAPES.get(axis);
   }

   protected VoxelShape getCullingShape(BlockState state) {
      Direction.Axis axis = ((Direction)state.get(FACING)).getAxis();
      return (VoxelShape)((Boolean)state.get(IN_WALL) ? IN_WALL_CULLING_SHAPES : REGULAR_CULLING_SHAPES).get(axis);
   }
   ...
   protected void onExploded(BlockState state, ServerWorld world, BlockPos pos, Explosion explosion, BiConsumer stackMerger) {
      if (explosion.canTriggerBlocks() && !(Boolean)state.get(POWERED)) {
         ...
         world.playSound((Entity)null, pos, bl ? this.type.fenceGateClose() : this.type.fenceGateOpen(), SoundCategory.BLOCKS, 1.0F, world.getRandom().nextFloat() * 0.1F + 0.9F);
      ...
      if (!world.isClient) {
         ...
         if ((Boolean)state.get(POWERED) != bl) {
            ...
            if ((Boolean)state.get(OPEN) != bl) {
               world.playSound((Entity)null, pos, bl ? this.type.fenceGateOpen() : this.type.fenceGateClose(), SoundCategory.BLOCKS, 1.0F, world.getRandom().nextFloat() * 0.1F + 0.9F);
   ...
   static {
      ...
      REGULAR_OUTLINE_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createCuboidShape(16.0, 16.0, 4.0));
      IN_WALL_OUTLINE_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_OUTLINE_SHAPES, (shape) -> {
         return VoxelShapes.combineAndSimplify(shape, Block.createColumnShape(16.0, 13.0, 16.0), BooleanBiFunction.ONLY_FIRST);
      }));
      CLOSED_COLLISION_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16.0, 4.0, 0.0, 24.0));
      CLOSED_SIDES_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16.0, 4.0, 5.0, 24.0));
      REGULAR_CULLING_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(VoxelShapes.union(Block.createCuboidShape(0.0, 5.0, 7.0, 2.0, 16.0, 9.0), Block.createCuboidShape(14.0, 5.0, 7.0, 16.0, 16.0, 9.0)));
      IN_WALL_CULLING_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_CULLING_SHAPES, (shape) -> {
         return shape.offset(0.0, -0.1875, 0.0).simplify();
      }));
   }
}
