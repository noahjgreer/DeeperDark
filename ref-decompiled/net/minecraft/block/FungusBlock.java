...
implements Fertilizable {
    public static final MapCodec<FungusBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)RegistryKey.createCodec((RegistryKey)RegistryKeys.CONFIGURED_FEATURE).fieldOf("feature").forGetter(block -> block.featureKey), (App)Registries.BLOCK.getCodec().fieldOf("grows_on").forGetter(block -> block.nylium), (App)FungusBlock.createSettingsCodec()).apply((Applicative)instance, FungusBlock::new));
    private static final double GROW_CHANCE = 0.4;
    private static final VoxelShape SHAPE = Block.createColumnShape((double)8.0, (double)0.0, (double)9.0);
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        this.getFeatureEntry((WorldView)world).ifPresent(featureEntry -> ((ConfiguredFeature)featureEntry.value()).generate((StructureWorldAccess)world, world.getChunkManager().getChunkGenerator(), random, pos));
    }
}
