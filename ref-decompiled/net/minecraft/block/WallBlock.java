...
public class WallBlock extends Block implements Waterloggable {
   public static final MapCodec CODEC = createCodec(WallBlock::new);
   ...
   private final Function outlineShapeFunction;
   private final Function collisionShapeFunction;
   private static final VoxelShape POST_SHAPE_FOR_TALL_TEST;
   private static final Map WALL_SHAPES_FOR_TALL_TEST_BY_DIRECTION;
   ...
   private Function createShapeFunction(float tallHeight, float lowHeight) {
      VoxelShape voxelShape = Block.createColumnShape(8.0, 0.0, (double)tallHeight);
      int i = true;
      Map map = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(6.0, 0.0, (double)lowHeight, 0.0, 11.0));
      Map map2 = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(6.0, 0.0, (double)tallHeight, 0.0, 11.0));
      return this.createShapeFunction((state) -> {
         VoxelShape voxelShape2 = (Boolean)state.get(UP) ? voxelShape : VoxelShapes.empty();

         VoxelShape var10001;
         for(Iterator var5 = WALL_SHAPE_PROPERTIES_BY_DIRECTION.entrySet().iterator(); var5.hasNext(); voxelShape2 = VoxelShapes.union(voxelShape2, var10001)) {
            Map.Entry entry = (Map.Entry)var5.next();
            switch ((WallShape)state.get((Property)entry.getValue())) {
               case NONE:
                  var10001 = VoxelShapes.empty();
                  break;
               case LOW:
                  var10001 = (VoxelShape)map.get(entry.getKey());
                  break;
               case TALL:
                  var10001 = (VoxelShape)map2.get(entry.getKey());
                  break;
               default:
   ...
   private boolean shouldHavePost(BlockState state, BlockState aboveState, VoxelShape aboveShape) {
      ...
      if (bl) {
         ...
         WallShape wallShape = (WallShape)state.get(NORTH_WALL_SHAPE);
         WallShape wallShape2 = (WallShape)state.get(SOUTH_WALL_SHAPE);
         WallShape wallShape3 = (WallShape)state.get(EAST_WALL_SHAPE);
         WallShape wallShape4 = (WallShape)state.get(WEST_WALL_SHAPE);
   ...
   protected BlockState rotate(BlockState state, BlockRotation rotation) {
      switch (rotation) {
         case CLOCKWISE_180:
            return (BlockState)((BlockState)((BlockState)((BlockState)state.with(NORTH_WALL_SHAPE, (WallShape)state.get(SOUTH_WALL_SHAPE))).with(EAST_WALL_SHAPE, (WallShape)state.get(WEST_WALL_SHAPE))).with(SOUTH_WALL_SHAPE, (WallShape)state.get(NORTH_WALL_SHAPE))).with(WEST_WALL_SHAPE, (WallShape)state.get(EAST_WALL_SHAPE));
         case COUNTERCLOCKWISE_90:
            return (BlockState)((BlockState)((BlockState)((BlockState)state.with(NORTH_WALL_SHAPE, (WallShape)state.get(EAST_WALL_SHAPE))).with(EAST_WALL_SHAPE, (WallShape)state.get(SOUTH_WALL_SHAPE))).with(SOUTH_WALL_SHAPE, (WallShape)state.get(WEST_WALL_SHAPE))).with(WEST_WALL_SHAPE, (WallShape)state.get(NORTH_WALL_SHAPE));
         case CLOCKWISE_90:
            return (BlockState)((BlockState)((BlockState)((BlockState)state.with(NORTH_WALL_SHAPE, (WallShape)state.get(WEST_WALL_SHAPE))).with(EAST_WALL_SHAPE, (WallShape)state.get(NORTH_WALL_SHAPE))).with(SOUTH_WALL_SHAPE, (WallShape)state.get(EAST_WALL_SHAPE))).with(WEST_WALL_SHAPE, (WallShape)state.get(SOUTH_WALL_SHAPE));
         default:
            return state;
      }
   }
   ...
   protected BlockState mirror(BlockState state, BlockMirror mirror) {
      switch (mirror) {
         case LEFT_RIGHT:
            return (BlockState)((BlockState)state.with(NORTH_WALL_SHAPE, (WallShape)state.get(SOUTH_WALL_SHAPE))).with(SOUTH_WALL_SHAPE, (WallShape)state.get(NORTH_WALL_SHAPE));
         case FRONT_BACK:
            return (BlockState)((BlockState)state.with(EAST_WALL_SHAPE, (WallShape)state.get(WEST_WALL_SHAPE))).with(WEST_WALL_SHAPE, (WallShape)state.get(EAST_WALL_SHAPE));
         default:
   ...
   static {
      UP = Properties.UP;
      EAST_WALL_SHAPE = Properties.EAST_WALL_SHAPE;
      NORTH_WALL_SHAPE = Properties.NORTH_WALL_SHAPE;
      SOUTH_WALL_SHAPE = Properties.SOUTH_WALL_SHAPE;
      WEST_WALL_SHAPE = Properties.WEST_WALL_SHAPE;
      ...
      POST_SHAPE_FOR_TALL_TEST = Block.createColumnShape(2.0, 0.0, 16.0);
      WALL_SHAPES_FOR_TALL_TEST_BY_DIRECTION = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(2.0, 16.0, 0.0, 9.0));
   }
}
