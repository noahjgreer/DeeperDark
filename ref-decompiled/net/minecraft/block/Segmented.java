...
public interface Segmented {
   ...
   default Function createShapeFunction(EnumProperty directionProperty, IntProperty segmentAmountProperty) {
      Map map = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidShape(0.0, 0.0, 0.0, 8.0, this.getHeight(), 8.0));
      return (state) -> {
   ...
   default BlockState getPlacementState(ItemPlacementContext context, Block block, IntProperty amountProperty, EnumProperty directionProperty) {
      BlockState blockState = context.getWorld().getBlockState(context.getBlockPos());
      return blockState.isOf(block) ? (BlockState)blockState.with(amountProperty, Math.min(4, (Integer)blockState.get(amountProperty) + 1)) : (BlockState)block.getDefaultState().with(directionProperty, context.getHorizontalPlayerFacing().getOpposite());
   }
}
