...
public class RedstoneWireBlock extends Block {
   public static final MapCodec CODEC = createCodec(RedstoneWireBlock::new);
   public static final EnumProperty WIRE_CONNECTION_NORTH;
   public static final EnumProperty WIRE_CONNECTION_EAST;
   public static final EnumProperty WIRE_CONNECTION_SOUTH;
   public static final EnumProperty WIRE_CONNECTION_WEST;
   public static final IntProperty POWER;
   public static final Map DIRECTION_TO_WIRE_CONNECTION_PROPERTY;
   ...
   private Function createShapeFunction() {
      ...
      Map map = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(10.0, 0.0, 1.0, 0.0, 8.0));
      Map map2 = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(10.0, 16.0, 0.0, 1.0));
      return this.createShapeFunction((state) -> {
         VoxelShape voxelShape2 = voxelShape;

         VoxelShape var10000;
         for(Iterator var5 = DIRECTION_TO_WIRE_CONNECTION_PROPERTY.entrySet().iterator(); var5.hasNext(); voxelShape2 = var10000) {
            Map.Entry entry = (Map.Entry)var5.next();
            switch ((WireConnection)state.get((Property)entry.getValue())) {
               case UP:
                  var10000 = VoxelShapes.union(voxelShape2, (VoxelShape)map.get(entry.getKey()), (VoxelShape)map2.get(entry.getKey()));
                  break;
               case SIDE:
                  var10000 = VoxelShapes.union(voxelShape2, (VoxelShape)map.get(entry.getKey()));
                  break;
               case NONE:
                  var10000 = voxelShape2;
                  break;
               default:
   ...
   protected void prepare(BlockState state, WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
      ...
      while(var7.hasNext()) {
         ...
         if (wireConnection != WireConnection.NONE && !world.getBlockState(mutable.set(pos, (Direction)direction)).isOf(this)) {
            ...
            mutable.set(pos, (Direction)direction).move(Direction.UP);
   ...
   protected void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
      if (!oldState.isOf(state.getBlock()) && !world.isClient) {
         this.update(world, pos, state, (WireOrientation)null, true);
   ...
   protected void onStateReplaced(BlockState state, ServerWorld world, BlockPos pos, boolean moved) {
      if (!moved) {
         ...
         this.update(world, pos, state, (WireOrientation)null, false);
         this.updateOffsetNeighbors(world, pos);
      }
   }
   ...
   private void updateForNewState(World world, BlockPos pos, BlockState oldState, BlockState newState) {
      WireOrientation wireOrientation = OrientationHelper.getEmissionOrientation(world, (Direction)null, Direction.UP);
      Iterator var6 = Direction.Type.HORIZONTAL.iterator();
   ...
   static {
      ...
      DIRECTION_TO_WIRE_CONNECTION_PROPERTY = ImmutableMap.copyOf(Maps.newEnumMap(Map.of(Direction.NORTH, WIRE_CONNECTION_NORTH, Direction.EAST, WIRE_CONNECTION_EAST, Direction.SOUTH, WIRE_CONNECTION_SOUTH, Direction.WEST, WIRE_CONNECTION_WEST)));
      COLORS = (int[])Util.make(new int[16], (colors) -> {
         for(int i = 0; i <= 15; ++i) {
