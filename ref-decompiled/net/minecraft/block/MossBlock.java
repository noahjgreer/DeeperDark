...
public class MossBlock extends Block implements Fertilizable {
   public static final MapCodec CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      ...
      }), createSettingsCodec()).apply(instance, MossBlock::new);
   });
   ...
   public MossBlock(RegistryKey feature, AbstractBlock.Settings settings) {
      super(settings);
   ...
   public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
      world.getRegistryManager().getOptional(RegistryKeys.CONFIGURED_FEATURE).flatMap((registry) -> {
         return registry.getOptional(this.feature);
      }).ifPresent((entry) -> {
