...
implements DispenserBehavior {
    ...
    private void addStackOrSpawn(BlockPointer pointer, ItemStack stack) {
        ...
        Direction direction = pointer.state().get(DispenserBlock.FACING);
        ItemDispenserBehavior.spawnItem(pointer.world(), itemStack, 6, direction, DispenserBlock.getOutputLocation(pointer));
        ItemDispenserBehavior.syncDispensesEvent(pointer);
        ItemDispenserBehavior.syncActivatesEvent(pointer, direction);
    }
}
