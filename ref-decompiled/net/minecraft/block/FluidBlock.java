...
implements FluidDrainable {
    ...
    public static final MapCodec<FluidBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)FLUID_CODEC.fieldOf("fluid").forGetter(block -> block.fluid), (App)FluidBlock.createSettingsCodec()).apply((Applicative)instance, FluidBlock::new));
    ...
    public static final VoxelShape COLLISION_SHAPE = Block.createColumnShape((double)16.0, (double)0.0, (double)8.0);
    ...
    protected VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        ...
        if (context.isAbove(COLLISION_SHAPE, pos, true) && (Integer)state.get((Property)LEVEL) == 0 && context.canWalkOnFluid(world.getFluidState(pos.up()), state.getFluidState())) {
    ...
    protected boolean isSideInvisible(BlockState state, BlockState stateFrom, Direction direction) {
        return stateFrom.getFluidState().getFluid().matchesType((Fluid)this.fluid);
    }
    ...
    protected void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (this.receiveNeighborFluids(world, pos, state)) {
            world.scheduleFluidTick(pos, state.getFluidState().getFluid(), this.fluid.getTickRate((WorldView)world));
        }
    }
        ...
        if (this.fluid.isIn(FluidTags.LAVA)) {
            ...
            for (Direction direction : FLOW_DIRECTIONS) {
                ...
                if (world.getFluidState(blockPos).isIn(FluidTags.WATER)) {
                    ...
                    this.playExtinguishSound((WorldAccess)world, pos);
                    return false;
                }
    ...
    public ItemStack tryDrainFluid(@Nullable LivingEntity drainer, WorldAccess world, BlockPos pos, BlockState state) {
        if ((Integer)state.get((Property)LEVEL) == 0) {
            world.setBlockState(pos, Blocks.AIR.getDefaultState(), 11);
            return new ItemStack((ItemConvertible)this.fluid.getBucketItem());
        }
