...
public class FrogspawnBlock extends Block {
   public static final MapCodec CODEC = createCodec(FrogspawnBlock::new);
   ...
   private void hatch(ServerWorld world, BlockPos pos, Random random) {
      this.breakWithoutDrop(world, pos);
      world.playSound((Entity)null, pos, SoundEvents.BLOCK_FROGSPAWN_HATCH, SoundCategory.BLOCKS, 1.0F, 1.0F);
   ...
   private void spawnTadpoles(ServerWorld world, BlockPos pos, Random random) {
      ...
      for(int j = 1; j <= i; ++j) {
         TadpoleEntity tadpoleEntity = (TadpoleEntity)EntityType.TADPOLE.create(world, SpawnReason.BREEDING);
   ...
   private double getSpawnOffset(Random random) {
      ...
      return MathHelper.clamp(random.nextDouble(), 0.20000000298023224, 0.7999999970197678);
   }
