...
public abstract class AbstractPressurePlateBlock extends Block {
   ...
   private void updatePlateState(@Nullable Entity entity, World world, BlockPos pos, BlockState state, int output) {
      ...
      if (!bl2 && bl) {
         world.playSound((Entity)null, pos, this.blockSetType.pressurePlateClickOff(), SoundCategory.BLOCKS);
         world.emitGameEvent(entity, GameEvent.BLOCK_DEACTIVATE, pos);
      } else if (bl2 && !bl) {
         world.playSound((Entity)null, pos, this.blockSetType.pressurePlateClickOn(), SoundCategory.BLOCKS);
   ...
   protected static int getEntityCount(World world, Box box, Class entityClass) {
