...
implements NbtElementVisitor {
    ...
    public void visitByte(NbtByte element) {
        this.result.append(Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(BYTE_TYPE_SUFFIX);
    }

    @Override
    public void visitShort(NbtShort element) {
        this.result.append(Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(SHORT_TYPE_SUFFIX);
    }
    ...
    public void visitLong(NbtLong element) {
        this.result.append(Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(LONG_TYPE_SUFFIX);
    }

    @Override
    public void visitFloat(NbtFloat element) {
        this.result.append(Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(FLOAT_TYPE_SUFFIX);
    }

    @Override
    public void visitDouble(NbtDouble element) {
        this.result.append(Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(DOUBLE_TYPE_SUFFIX);
    }
    ...
    public void visitByteArray(NbtByteArray element) {
        ...
        for (int i = 0; i < bs.length && i < 128; ++i) {
            ...
            this.result.append(SPACE).append(mutableText).append(ARRAY_BYTE_TYPE_SUFFIX);
    ...
    public void visitLongArray(NbtLongArray element) {
        ...
        for (int i = 0; i < ls.length && i < 128; ++i) {
            MutableText text = Text.literal(String.valueOf(ls[i])).formatted(NUMBER_COLOR);
            this.result.append(SPACE).append(text).append(LONG_TYPE_SUFFIX);
    ...
    private static boolean shouldIndent(NbtList list) {
        ...
        for (NbtElement nbtElement : list) {
            if (nbtElement instanceof AbstractNbtNumber) continue;
            return true;
        }
        return false;
    }
    ...
    public void visitList(NbtList element) {
        ...
        if (!NbtTextFormatter.shouldIndent(element)) {
            this.result.append(SQUARE_OPEN_BRACKET);
            for (int i = 0; i < element.size(); ++i) {
                if (i != 0) {
                    this.result.append(ENTRY_SEPARATOR_WITH_SPACE);
                }
                this.formatSubElement(element.method_10534(i), false);
            }
            this.result.append(SQUARE_CLOSE_BRACKET);
            return;
        }
        ...
        String string = Strings.repeat((String)this.prefix, (int)(this.indentationLevel + 1));
        for (int j = 0; j < element.size() && j < 128; ++j) {
            this.result.append(string);
            this.formatSubElement(element.method_10534(j), true);
        ...
        if (!this.prefix.isEmpty()) {
            this.result.append(NEW_LINE + Strings.repeat((String)this.prefix, (int)this.indentationLevel));
        }
        this.result.append(SQUARE_CLOSE_BRACKET);
    }
