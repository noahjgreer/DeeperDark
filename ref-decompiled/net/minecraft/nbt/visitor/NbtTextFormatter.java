...
public class NbtTextFormatter implements NbtElementVisitor {
   ...
   public void visitByte(NbtByte element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(BYTE_TYPE_SUFFIX);
   }

   public void visitShort(NbtShort element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(SHORT_TYPE_SUFFIX);
   }

   public void visitInt(NbtInt element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR));
   }

   public void visitLong(NbtLong element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(LONG_TYPE_SUFFIX);
   }

   public void visitFloat(NbtFloat element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(FLOAT_TYPE_SUFFIX);
   }

   public void visitDouble(NbtDouble element) {
      this.result.append((Text)Text.literal(String.valueOf(element.value())).formatted(NUMBER_COLOR)).append(DOUBLE_TYPE_SUFFIX);
   }
   ...
   private static boolean shouldIndent(NbtList list) {
      if (list.size() >= 8) {
         ...
         do {
            ...
            nbtElement = (NbtElement)var1.next();
         } while(nbtElement instanceof AbstractNbtNumber);
