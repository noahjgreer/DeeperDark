...
public class ParticleUtil {
    public static void spawnParticle(World world, BlockPos pos, ParticleEffect effect, IntProvider count) {
        for (Direction direction : Direction.values()) {
            ParticleUtil.spawnParticles(world, pos, effect, count, direction, () -> ParticleUtil.getRandomVelocity(world.random), 0.55);
        }
    }
    ...
    public static void spawnParticlesAround(WorldAccess world, BlockPos pos, int count, double horizontalOffset, double verticalOffset, boolean force, ParticleEffect effect) {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (!force && world.getBlockState(BlockPos.ofFloored(h, j, k).down()).isAir()) continue;
    ...
    public static void spawnSmashAttackParticles(WorldAccess world, BlockPos pos, int count) {
        ...
        while ((float)i < (float)count / 1.5f) {
            d = vec3d.x + 3.5 * Math.cos(i) + world.getRandom().nextGaussian() / 2.0;
            e = vec3d.y;
            f = vec3d.z + 3.5 * Math.sin(i) + world.getRandom().nextGaussian() / 2.0;
            g = world.getRandom().nextGaussian() * (double)0.05f;
            h = world.getRandom().nextGaussian() * (double)0.05f;
            j = world.getRandom().nextGaussian() * (double)0.05f;
            world.addParticleClient(blockStateParticleEffect, d, e, f, g, h, j);
            ++i;
        }
    }
