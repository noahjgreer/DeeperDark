...
public class ParticleUtil {
   public static void spawnParticle(World world, BlockPos pos, ParticleEffect effect, IntProvider count) {
      ...
      for(int var6 = 0; var6 < var5; ++var6) {
         ...
         spawnParticles(world, pos, effect, count, direction, () -> {
   ...
   public static void spawnSmashAttackParticles(WorldAccess world, BlockPos pos, int count) {
      ...
      double g;
      double h;
      for(i = 0; (float)i < (float)count / 3.0F; ++i) {
         d = vec3d.x + world.getRandom().nextGaussian() / 2.0;
         e = vec3d.y;
         f = vec3d.z + world.getRandom().nextGaussian() / 2.0;
         g = world.getRandom().nextGaussian() * 0.20000000298023224;
         h = world.getRandom().nextGaussian() * 0.20000000298023224;
         j = world.getRandom().nextGaussian() * 0.20000000298023224;
         world.addParticleClient(blockStateParticleEffect, d, e, f, g, h, j);
      }

      for(i = 0; (float)i < (float)count / 1.5F; ++i) {
         d = vec3d.x + 3.5 * Math.cos((double)i) + world.getRandom().nextGaussian() / 2.0;
         e = vec3d.y;
         f = vec3d.z + 3.5 * Math.sin((double)i) + world.getRandom().nextGaussian() / 2.0;
         g = world.getRandom().nextGaussian() * 0.05000000074505806;
         h = world.getRandom().nextGaussian() * 0.05000000074505806;
         j = world.getRandom().nextGaussian() * 0.05000000074505806;
         world.addParticleClient(blockStateParticleEffect, d, e, f, g, h, j);
      }
