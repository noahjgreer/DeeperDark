...
public class ChatSoundManager {
    ...
    public static void playSendMessageSound(ServerPlayerEntity sourcePlayer) {
    ...
    public static void playDeathMessageSound(ServerPlayerEntity sourcePlayer) {
        DeeperDarkConfig.ChatSoundProfile profile = getProfileForPlayer(sourcePlayer);
        if (profile == null) return;
        playGlobalSound(sourcePlayer, profile.deathMessageSound, profile.pitch, profile.pitchDeviance);
    }

    public static void playJoinMessageSound(ServerPlayerEntity sourcePlayer) {
        DeeperDarkConfig.ChatSoundProfile profile = getProfileForPlayer(sourcePlayer);
        if (profile == null) return;
        playGlobalSound(sourcePlayer, profile.joinMessageSound, profile.pitch, profile.pitchDeviance);
    }

    private static DeeperDarkConfig.ChatSoundProfile getProfileForPlayer(ServerPlayerEntity player) {
    ...
    private static void playGlobalSound(ServerPlayerEntity sourcePlayer, String rawSoundId, double basePitch, double pitchDeviance) {
